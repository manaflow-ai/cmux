...
struct CLICodexHookTimeoutRegressionTests {
    ...
    @Test func codexInstalledHookStaysInertWithoutCmuxContext() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("cmux-codex-hook-no-context-\(UUID().uuidString)", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        let fakeCLI = root.appendingPathComponent("cmux", isDirectory: false)
        let capturedArgs = root.appendingPathComponent("hook-args.txt", isDirectory: false)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
        ...
        // Codex hooks are installed globally; a Codex session outside cmux has
        // neither CMUX_SURFACE_ID nor CMUX_WORKSPACE_ID and must not dispatch.
        let run = runCodexHookProcess(
            executablePath: "/bin/sh",
            arguments: ["-c", command],
            environment: [
            ...
            standardInput: #"{"session_id":"codex-session","prompt":"outside cmux"}"#,
            timeout: 2
        )
