...
fn forged_or_stale_revocation_updates_are_rejected() {
    ...
    let signer = GrantSigner::new("authority".into(), &key).unwrap();
    let mut update = RevocationUpdate {
        key_id: String::new(),
        team_id: "team".into(),
        sequence: 1,
        policy_revision: 1,
    ...
    parts[1].replace_range(..1, replacement);
    let mut keys = AuthorityKeys::default();
    keys.insert("authority".into(), key.verifying_key());
    assert!(keys
        .admit_revocation(&parts.join("."), "team", 1001)
        .is_err());
