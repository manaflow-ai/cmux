...
- The control binary also passes a live local `/readyz` and `/healthz` check against the PlanetScale staging branch with staging Stack credentials and the staged authority seed; the process is terminated after the probe.
