...
extension CMUXCLI {
    ...
    static var vmRunUsage: String {
        """
        Usage: cmux vm run [--sync] [--pull <remote-path>] [--machine <id>] [--new] [--size <8g|16g|32g>] [--timeout <seconds>] -- <command...>
    ...
    func runVMRunCommand(rest: [String], client: SocketClient, jsonOutput: Bool) throws {
        ...
        if let sizeOption {
            guard let parsed = Self.parseCloudVMSize(sizeOption) else {
                throw CLIError(message: "vm run: unknown size '\(sizeOption)'. Sizes: 8g, 16g, 32g (or memory in MB).")
            }
            memoryMb = parsed
        }
...
extension CMUXCLI {
    static var vmRouteUsage: String {
        """
        Usage: cmux vm route [--cwd <dir>] [--new] [--provision] [--size <8g|16g|32g>] [--json]
    ...
    func runVMRouteCommand(rest: [String], client: SocketClient, jsonOutput: Bool) throws {
        ...
        if let sizeOption {
            guard let parsed = Self.parseCloudVMSize(sizeOption) else {
                throw CLIError(message: "vm route: unknown size '\(sizeOption)'. Sizes: 8g, 16g, 32g (or memory in MB).")
            }
            memoryMb = parsed
        }
    ...
    func runVMAgentCommand(rest: [String], client: SocketClient, jsonOutput: Bool) throws {
        ...
        if let sizeOption {
            guard let parsed = Self.parseCloudVMSize(sizeOption) else {
                throw CLIError(message: "vm agent: unknown size '\(sizeOption)'. Sizes: 8g, 16g, 32g (or memory in MB).")
            }
            memoryMb = parsed
        }
        let workDirectory = cwdOption.map { URL(fileURLWithPath: $0).standardizedFileURL.path }
