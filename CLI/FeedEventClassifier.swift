...
struct FeedEventClassifier {
    ...
    /// - Returns: The resolved wire event and its Feed/native-attention flags.
    ...
    private static func sourceRaisesNativeApprovalPrompts(_ source: String) -> Bool {
        guard let integration = BuiltInAgentIntegration(feedSourceName: source) else {
            return false
        }
        return feedEventSemanticRegistry[integration]?
            .values.contains(.nativeApprovalPrompt) == true
    }
    ...
    private static func feedEventSemantics(
        ...
        case .codex:
            semantics.merge([
                // Permission requests inherit `.nativeApprovalPrompt` from
                // Codex's native-reviewer policy. These are its additional
                // snake_case lifecycle spellings.
    ...
    /// Shared event spellings every built-in must support. Permission-request
    /// and raw tool-start behavior are explicitly selected by the
    /// integration's required approval mechanism.
    private static func builtInFeedEventSemantics(
        ...
        let semantics: (
            ...
            permissionRequest: FeedEventSemantic
        ) = switch approvalDetection {
        case .dedicatedPermissionEvent:
            (.toolStart, .toolStart, .approvalRequest)
        ...
        case .nativeApprovalReviewer:
            (.toolStart, .toolStart, .nativeApprovalPrompt)
        }
        return [
            "PreToolUse": semantics.toolStart,
            "preToolUse": semantics.toolStart,
            "beforeShellExecution": semantics.shellStart,
            "PermissionRequest": semantics.permissionRequest,
            "permissionRequest": semantics.permissionRequest,
            "permission_request": semantics.permissionRequest,
            "PostToolUse": .toolEnd,
            "postToolUse": .toolEnd,
