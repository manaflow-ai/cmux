...
extension CMUXCLI {
    ...
    private func editorMonacoOptions() -> [String: Any] {
        for path in diffViewerDefaultSettingsPaths() {
            ...
            let curated = Self.curateEditorMonacoOptions(section)
            if !curated.isEmpty {
                return curated
            }
        }
    ...
    /// Pure function (no I/O) mapping a cmux.json `editor` object to a Monaco
    /// `IStandaloneEditorConstructionOptions` subset. Each key is type- and
    /// enum-validated; invalid or unknown keys are dropped. Exposed
    /// `static`/`internal` so it can be unit-tested without touching disk.
    static func curateEditorMonacoOptions(_ section: [String: Any]) -> [String: Any] {
        var out: [String: Any] = [:]
