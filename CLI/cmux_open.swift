...
extension CMUXCLI {
    private enum DiffViewerLimits {
        ...
        static let initialResponseChunkBytes = 16 * 1024
    ...
    private static let diffViewerHTTPServerProtocolVersion = 6
    private static let diffViewerHTTPServerHealthResponse = Data("ok cmux-diff-viewer-server-v6\n".utf8)
    ...
    private func diffViewerShouldStreamRemotePatch() -> Bool {
        guard let value = ProcessInfo.processInfo.environment["CMUX_DIFF_VIEWER_STREAM_REMOTE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased(),
              !value.isEmpty else {
            return true
            ...
            if let remoteURLString = file.remoteURL {
                ...
                if !file.filePath.isEmpty {
                    ...
                    guard fileURL.path.hasPrefix(rootPath + "/") else {
                        throw CLIError(message: "Diff viewer remote cache file is outside the viewer directory")
                    }
    ...
    private func sendDiffViewerHTTPFile(
        ...
        if let remoteURLString = file.remoteURL,
           let remoteURL = URL(string: remoteURLString),
           diffViewerHTTPIsAllowedRemotePatchURL(remoteURL) {
            if let cacheFileURL = diffViewerHTTPRemotePatchCacheFileURL(file),
               diffViewerHTTPFileIsReadableRegularFile(cacheFileURL) {
                var cachedFile = file
                cachedFile.remoteURL = nil
                cachedFile.filePath = cacheFileURL.path
                try sendDiffViewerHTTPFile(
                    cachedFile,
                    fileDescriptor: fd,
                    port: port,
            ...
            try sendDiffViewerHTTPRemotePatch(
                remoteURL,
                cacheFileURL: diffViewerHTTPRemotePatchCacheFileURL(file),
                fileDescriptor: fd,
                port: port,
                omitBody: omitBody
            )
    ...
    private func diffViewerHTTPRemotePatchCacheFileURL(_ file: DiffViewerAllowedFile) -> URL? {
        guard !file.filePath.isEmpty else { return nil }
        return URL(fileURLWithPath: file.filePath, isDirectory: false)
    }

    private func diffViewerHTTPFileIsReadableRegularFile(_ fileURL: URL) -> Bool {
        var info = stat()
        return stat(fileURL.path, &info) == 0 &&
    ...
    private func sendDiffViewerHTTPRemotePatch(
        ...
        var cacheTemporaryURL: URL?
        var cacheHandle: FileHandle?
        ...
        if let cacheFileURL {
            ...
            let temporaryURL = cacheFileURL.deletingLastPathComponent().appendingPathComponent(
                ".\(cacheFileURL.lastPathComponent).\(UUID().uuidString).tmp",
                isDirectory: false
            )
            FileManager.default.createFile(atPath: temporaryURL.path, contents: nil)
            cacheHandle = try? FileHandle(forWritingTo: temporaryURL)
            cacheTemporaryURL = cacheHandle == nil ? nil : temporaryURL
        }

        func discardRemotePatchCache() {
            cacheFailed = true
            try? cacheHandle?.close()
            cacheHandle = nil
            if let cacheTemporaryURL {
                try? FileManager.default.removeItem(at: cacheTemporaryURL)
            }
            cacheTemporaryURL = nil
        }

        func appendRemotePatchCache(_ data: Data) {
            guard !cacheFailed, !data.isEmpty, let cacheHandle else { return }
            do {
                try cacheHandle.write(contentsOf: data)
            } catch {
                discardRemotePatchCache()
            }
        }

        func commitRemotePatchCacheIfComplete() {
            guard !cacheFailed,
                  let cacheFileURL,
                  let temporaryURL = cacheTemporaryURL else {
                discardRemotePatchCache()
                return
            }
            do {
                try cacheHandle?.close()
                cacheHandle = nil
                if rename(temporaryURL.path, cacheFileURL.path) != 0 {
                    let code = Int(errno)
        ...
        let firstChunk = try handle.read(upToCount: DiffViewerLimits.initialResponseChunkBytes) ?? Data()
        if firstChunk.isEmpty {
            ...
            commitRemotePatchCacheIfComplete()
            try sendDiffViewerHTTPHeader(
                fileDescriptor: fd,
                status: 200,
                reason: "OK",
            ...
            try sendDiffViewerHTTPFinalChunk(fileDescriptor: fd)
            return
        }
        ...
        appendRemotePatchCache(firstChunk)
        try sendDiffViewerHTTPChunk(firstChunk, fileDescriptor: fd)
        ...
        while true {
            ...
            appendRemotePatchCache(data)
            try sendDiffViewerHTTPChunk(data, fileDescriptor: fd)
        }
        process.waitUntilExit()
    ...
    private func sendDiffViewerHTTPChunk(_ data: Data, fileDescriptor fd: Int32) throws {
        guard !data.isEmpty else { return }
        try sendAllDiffViewerHTTPData(Data("\(String(data.count, radix: 16))\r\n".utf8), fileDescriptor: fd)
        try sendAllDiffViewerHTTPData(data, fileDescriptor: fd)
        try sendAllDiffViewerHTTPData(Data("\r\n".utf8), fileDescriptor: fd)
    }

    private func sendDiffViewerHTTPFinalChunk(fileDescriptor fd: Int32) throws {
        try sendAllDiffViewerHTTPData(Data("0\r\n\r\n".utf8), fileDescriptor: fd)
    }
    ...
    private func writeDiffViewerHTML(
        ...
        let assetPayloadLiteral = try jsonScriptLiteral([
        ...
        let appModuleHref = htmlEscaped(assets.appModuleURL)
        ...
        <head>
          ...
          <meta name="viewport" content="width=device-width, initial-scale=1">
          <title>\(escapedTitle)</title>
          <link rel="modulepreload" href="\(appModuleHref)">
          ...
          <style>
            ...
            * {
            ...
            #jump-select {
              ...
              border: 1px solid transparent;
              border-radius: 6px;
              background: color-mix(in lab, var(--cmux-diff-fg) 7%, transparent);
              color: inherit;
              font: inherit;
              cursor: pointer;
              overflow: hidden;
            ...
            #file-list {
              ...
              --trees-status-renamed-override: light-dark(#007aff, #007aff);
            ...
            #status[data-error="true"] {
              ...
              top: 0;
              z-index: 20;
              background: color-mix(in lab, var(--cmux-diff-bg) 96%, var(--cmux-diff-fg));
              border-bottom: 1px solid var(--cmux-diff-border);
              color: light-dark(#b42318, #ff8a80);
            }
          </style>
        ...
        <body data-files-hidden="false">
          <div id="app">
            <header id="toolbar">
              ...
              <div class="toolbar-middle">
                <button id="jump-select" type="button" aria-label="\(jumpToFileLabel)" title="\(jumpToFileLabel)" hidden>\(jumpToFileLabel)</button>
              </div>
          ...
          <script id="cmux-diff-viewer-assets" type="application/json">\(assetPayloadLiteral)</script>
          <script id="cmux-diff-viewer-payload" type="application/json">\(payloadLiteral)</script>
    ...
    private func ensureDiffViewerAssets(nextTo viewerURL: URL) throws -> DiffViewerAssets {
        ...
        guard assetPaths.contains("cmux-diff-viewer-app.mjs"),
              assetPaths.contains("diffs.mjs"),
        ...
        return DiffViewerAssets(
            appModuleURL: "./assets/\(assetDirectoryName)/cmux-diff-viewer-app.mjs",
