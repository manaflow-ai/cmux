...
extension CMUXCLI {
    ...
    private func reusableShellStartupCommand(
        ...
        let wrapper = [
            ...
            "cmux_b64_decode=-d; base64 -d </dev/null >/dev/null 2>&1 || cmux_b64_decode=-D; (printf %s \(encodedLiteral) | base64 \"$cmux_b64_decode\" 2>/dev/null) > \"$cmux_tmp\" || exit 1",
            "chmod 700 \"$cmux_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$cmux_tmp\"",
            "cmux_status=$?",
            "trap - EXIT HUP INT TERM",
            "cmux_cleanup",
            "unset cmux_tmp cmux_status cmux_b64_decode",
