...
extension CMUXCLI {
    ...
    func sshAskpassExecShellScript(passwordCredential: String) -> String {
        ...
        return [
            ...
            "set cmux_interactive_stdin [expr {[catch {fconfigure stdin -mode} _cmux_stdin_mode] == 0}]",
            "log_user 0",
            "spawn {*}$argv",
            ...
            "  global cmux_interactive_stdin expect_out",
            "  set timeout 8",
            "  if {$cmux_interactive_stdin} {",
            "    expect {",
            "      -re \"(?i)(password:|permission denied)\" { cmux_rejected_password }",
            "      -re \".\" {",
