...
extension CMUXCLI {
    ...
    static func agentHookCaptureTimeShell() -> String {
        #"{ export LC_ALL=C; capture_file=`/usr/bin/mktemp "${TMPDIR:-/tmp}/cmux-agent-hook-time.XXXXXX" 2>/dev/null || true`; captured_at=; if [ -n "$capture_file" ] && [ -f "$capture_file" ]; then captured_at=`/usr/bin/stat -f %Fm "$capture_file" 2>/dev/null || true`; /bin/unlink "$capture_file" 2>/dev/null || true; fi; if [ -n "$captured_at" ]; then formatted_at=`printf "%.6f" "$captured_at" 2>/dev/null || true`; if [ -n "$formatted_at" ]; then printf "%s" "$formatted_at"; exit 0; fi; fi; date_bin="${CMUX_AGENT_HOOK_DATE_BIN:-/bin/date}"; epoch=`"$date_bin" +%s 2>/dev/null || printf 946684800`; printf "%s.%06d" "$epoch" 0; }"#
    }
