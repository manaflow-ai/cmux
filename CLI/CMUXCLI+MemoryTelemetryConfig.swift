...
extension CMUXCLI {
    func memoryTelemetryDatabaseURL() throws -> URL {
        ...
        if let override = env["CMUX_MEMORY_TELEMETRY_DB_PATH"] {
            ...
            guard !trimmed.isEmpty else {
                throw CLIError(message: "CMUX_MEMORY_TELEMETRY_DB_PATH must not be empty")
            }
            ...
            if FileManager.default.fileExists(atPath: url.path, isDirectory: &isDirectory),
               isDirectory.boolValue {
                throw CLIError(message: "CMUX_MEMORY_TELEMETRY_DB_PATH must point to a database file, not a directory: \(url.path)")
            }
            return url
        }
    ...
    func memoryTelemetryRetention() throws -> TimeInterval {
        ...
        guard let value = Double(trimmed), value.isFinite, value > 0 else {
            throw CLIError(message: "CMUX_MEMORY_TELEMETRY_RETENTION_SECONDS must be a positive number of seconds")
        }
        return value
    }
}
