...
struct CMUXCLI {
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "agent":
            ...
            Low-latency CLI surface for coding agents that want Bash-callable JSON
            instead of MCP. Commands reuse cmux's local socket transport and batch
            can run multiple operations through one CLI process/socket connection.

            Commands:
              capture [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>] [--raw]
              send [--workspace <id|ref>] [--surface <id|ref>] [--enter] [--] <text>
              send-key [--workspace <id|ref>] [--surface <id|ref>] [--] <key>
              list-panes [--workspace <id|ref>]
              list-surfaces [--workspace <id|ref>]
              batch [--file <path>] <json>

            Batch JSON:
              [{"op":"list-panes"},{"op":"capture","scrollback":true,"lines":200}]

            Examples:
              cmux agent capture --scrollback --lines 200
              cmux agent send --enter -- "npm test"
              cmux agent batch '[{"op":"list-panes"},{"op":"list-surfaces"}]'
            """
        case "focus-pane":
            return """
    ...
    private func agentFastPathBool(_ raw: Any?, default defaultValue: Bool = false) throws -> Bool {
        ...
        if let value = raw as? String,
           let parsed = parseBoolString(value.trimmingCharacters(in: .whitespacesAndNewlines)) {
            return parsed
        }
    ...
    private func runAgentFastPathBatch(
        ...
        let results = try operations.enumerated().map { index, spec -> [String: Any] in
    ...
    private func runAgentFastPathCommand(
        commandArgs: [String],
        client: SocketClient,
        idFormat: CLIIDFormat,
        windowOverride: String?
        ...
        case "capture", "read", "read-screen":
            ...
            let lines = try parsePositiveInt(linesArg, label: "--lines")
...
"""#
    ...
    private func usage() -> String {
        ...
        Commands:
          ...
          agent <capture|send|send-key|list-panes|list-surfaces|batch> [flags]
