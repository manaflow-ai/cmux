...
struct CMUXCLI {
    ...
    private func runClaudeHook(
        ...
        case "session-start", "active":
            ...
            if let claudePid {
                _ = try? sendV1Command(
                    "set_agent_pid claude_code \(claudePid) --tab=\(workspaceId)\(socketSurfaceOption(surfaceId))",
                    client: client
                )
            }
            print("OK")
        ...
        case "prompt-submit":
            ...
            let mappedSession = parsedInput.sessionId.flatMap { try? sessionStore.lookup(sessionId: $0) }
            guard let workspaceId = resolveOptionalWorkspaceIdForClaudeHook(
                preferred: mappedSession?.workspaceId,
                fallback: workspaceArg,
                client: client
            ) else {
            ...
            let surfaceId = resolveOptionalSurfaceIdForClaudeHook(
                preferred: mappedSession?.surfaceId,
                fallback: surfaceArg,
                workspaceId: workspaceId,
                client: client
            ...
            sendClaudeFeedTelemetry(workspaceId: workspaceId)
            try clearNotifications(client: client, workspaceId: workspaceId, surfaceId: surfaceId)
            try setClaudeStatus(
                client: client,
                workspaceId: workspaceId,
                surfaceId: surfaceId,
        ...
        case "session-end":
            ...
            if let consumedSession {
                ...
                _ = try? clearNotifications(client: client, workspaceId: workspaceId, surfaceId: consumedSession.surfaceId)
            }
            print("OK")
