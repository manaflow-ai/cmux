...
struct CMUXCLI {
    ...
    private func buildSSHStartupScriptBody(
        ...
        scriptLines += [
            ...
            "cmux_ssh_signal_exit() { cmux_ssh_signal_status=\"$1\"; CMUX_SSH_SESSION_ENDED=1; if [ -z \"${CMUX_SSH_CHILD_PID:-}\" ]; then CMUX_SSH_PENDING_SIGNAL=\"$cmux_ssh_signal_status\"; return; fi; trap - EXIT HUP INT TERM; kill -TERM \"$CMUX_SSH_CHILD_PID\" 2>/dev/null || true; exit \"$cmux_ssh_signal_status\"; }",
