...
extension CMUXCLI {
    ...
    static func codexFireAndForgetAgentHookShellCommand(_ command: String, for def: AgentHookDef) -> String {
        ...
        let fallbackCaptureTime = #"{ date_bin="${CMUX_AGENT_HOOK_DATE_BIN:-/bin/date}"; epoch=`"$date_bin" +%s 2>/dev/null || printf 946684800`; lock="${TMPDIR:-/tmp}/cmux-codex-hook-time.lock"; state="${TMPDIR:-/tmp}/cmux-codex-hook-time.state"; tries=0; while ! mkdir "$lock" 2>/dev/null; do tries=$((tries + 1)); if [ "$tries" -ge 100 ]; then rmdir "$lock" 2>/dev/null || true; tries=0; fi; sleep 0.01 2>/dev/null || sleep 1; done; last_epoch=; last_seq=-1; if IFS=' ' read -r last_epoch last_seq 2>/dev/null <"$state"; then :; fi; if [ "$last_epoch" = "$epoch" ] && [ "$last_seq" -ge 0 ] 2>/dev/null; then seq=$((last_seq + 1)); else seq=0; fi; if [ "$seq" -gt 999999 ] 2>/dev/null; then while [ "$epoch" = "$last_epoch" ]; do sleep 0.01 2>/dev/null || sleep 1; epoch=`"$date_bin" +%s 2>/dev/null || printf 946684800`; done; seq=0; fi; printf '%s %s\n' "$epoch" "$seq" >"$state"; rmdir "$lock" 2>/dev/null || true; printf '%s.%06d' "$epoch" "$seq"; }"#
