...
extension CMUXCLI {
    ...
    static func codexFireAndForgetAgentHookShellCommand(_ command: String, for def: AgentHookDef) -> String {
        ...
        let runner = "payload=\"$1\"; shift; timer_stop=\"$payload.timer-stop\"; timer_done=\"$payload.timer-done\"; timer_pid_file=\"$payload.timer-pid\"; rm -f \"$timer_stop\" \"$timer_done\" \"$timer_pid_file\"; ( sleep 30 & timer=\"$!\"; printf \"%s\\n\" \"$timer\" >\"$timer_pid_file\"; while [ ! -e \"$timer_stop\" ]; do timer_state=$(/bin/ps -o state= -p \"$timer\" 2>/dev/null | /usr/bin/tr -d \"[:space:]\"); case \"$timer_state\" in \"\"|Z*) wait \"$timer\" 2>/dev/null || true; printf done >\"$timer_done\"; exit 0;; esac; /bin/sleep 0.05; done; kill \"$timer\" 2>/dev/null || true; wait \"$timer\" 2>/dev/null || true; exit 0 ) & timer_supervisor=\"$!\"; \"$@\" <\"$payload\" >/dev/null 2>&1 & child=\"$!\"; ( while [ ! -s \"$timer_done\" ] && [ ! -e \"$timer_stop\" ]; do /bin/sleep 0.05; done; if [ -s \"$timer_done\" ]; then kill \"$child\" 2>/dev/null || true; fi ) & watchdog=\"$!\"; wait \"$child\" 2>/dev/null || true; : >\"$timer_stop\"; if [ -s \"$timer_pid_file\" ]; then timer_pid=$(/bin/cat \"$timer_pid_file\" 2>/dev/null || true); case \"$timer_pid\" in \"\"|*[!0-9]*) ;; *) kill \"$timer_pid\" 2>/dev/null || true;; esac; fi; kill \"$watchdog\" 2>/dev/null || true; wait \"$watchdog\" 2>/dev/null || true; wait \"$timer_supervisor\" 2>/dev/null || true; rm -f \"$payload\" \"$timer_stop\" \"$timer_done\" \"$timer_pid_file\""
