...
extension CMUXCLI {
    ...
    static func codexFireAndForgetAgentHookShellCommand(_ command: String, for def: AgentHookDef) -> String {
        ...
        return [
            ...
            "if [ -n \"$CMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$cmux_cli\" ]; then payload=\"$(mktemp \"${TMPDIR:-/tmp}/cmux-codex-hook.XXXXXX\" 2>/dev/null || mktemp -t cmux-codex-hook 2>/dev/null)\" || { \(noOp); exit 0; }; cat >\"$payload\" || true; set -- \"$cmux_cli\"; if [ -n \"${CMUX_SOCKET_PATH:-}\" ]; then set -- \"$@\" --socket \"$CMUX_SOCKET_PATH\"; fi; set -- \"$@\" \(routedArguments); if [ -n \"${CMUX_WORKSPACE_ID:-}\" ]; then set -- \"$@\" --workspace \"$CMUX_WORKSPACE_ID\" --surface \"$CMUX_SURFACE_ID\"; fi; CMUX_CODEX_PID=\"$agent_pid\" nohup sh -c '\(runner)' cmux-codex-hook \"$payload\" \"$@\" >/dev/null 2>&1 & echo '{}'; else \(noOp); fi",
        ].joined(separator: "; ")
    }
}
