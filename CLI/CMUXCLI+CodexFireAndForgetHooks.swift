...
extension CMUXCLI {
    ...
    static func codexFireAndForgetAgentHookShellCommand(_ command: String, for def: AgentHookDef) -> String {
        ...
        let fallbackCaptureTime = #"{ date_bin="${CMUX_AGENT_HOOK_DATE_BIN:-/bin/date}"; epoch=`"$date_bin" +%s 2>/dev/null || printf 946684800`; lock="${TMPDIR:-/tmp}/cmux-codex-hook-time.lock"; owner_file="$lock.owner"; owner_started_file="$lock.started"; state="${TMPDIR:-/tmp}/cmux-codex-hook-time.state"; tries=0; unowned_checks=0; while ! mkdir "$lock" 2>/dev/null; do tries=$((tries + 1)); if [ "$tries" -ge 100 ]; then owner=; owner_started=; if IFS= read -r owner 2>/dev/null <"$owner_file"; then :; fi; if IFS= read -r owner_started 2>/dev/null <"$owner_started_file"; then :; fi; now_epoch=`"$date_bin" +%s 2>/dev/null || printf 946684800`; owner_is_stale=0; if [ "$owner_started" -ge 0 ] 2>/dev/null && [ "$now_epoch" -ge "$owner_started" ] 2>/dev/null && [ $((now_epoch - owner_started)) -ge 10 ]; then owner_is_stale=1; fi; if [ "$owner_is_stale" -eq 1 ] || { [ "$owner" -gt 0 ] 2>/dev/null && ! kill -0 "$owner" 2>/dev/null; }; then rm -f "$owner_file" "$owner_started_file" 2>/dev/null || true; rmdir "$lock" 2>/dev/null || true; unowned_checks=0; elif [ "$owner" -gt 0 ] 2>/dev/null && kill -0 "$owner" 2>/dev/null; then unowned_checks=0; else unowned_checks=$((unowned_checks + 1)); if [ "$unowned_checks" -ge 2 ]; then rm -f "$owner_file" "$owner_started_file" 2>/dev/null || true; rmdir "$lock" 2>/dev/null || true; unowned_checks=0; fi; fi; tries=0; fi; sleep 0.01 2>/dev/null || sleep 1; done; owner_started=`"$date_bin" +%s 2>/dev/null || printf 946684800`; printf '%s\n' "$$" >"$owner_file"; printf '%s\n' "$owner_started" >"$owner_started_file"; last_epoch=; last_seq=-1; if IFS=' ' read -r last_epoch last_seq 2>/dev/null <"$state"; then :; fi; if [ "$last_epoch" = "$epoch" ] && [ "$last_seq" -ge 0 ] 2>/dev/null; then seq=$((last_seq + 1)); else seq=0; fi; if [ "$seq" -gt 999999 ] 2>/dev/null; then while [ "$epoch" = "$last_epoch" ]; do sleep 0.01 2>/dev/null || sleep 1; epoch=`"$date_bin" +%s 2>/dev/null || printf 946684800`; done; seq=0; fi; printf '%s %s\n' "$epoch" "$seq" >"$state"; current_owner=; current_owner_started=; if IFS= read -r current_owner 2>/dev/null <"$owner_file"; then :; fi; if IFS= read -r current_owner_started 2>/dev/null <"$owner_started_file"; then :; fi; if [ "$current_owner" = "$$" ] && [ "$current_owner_started" = "$owner_started" ]; then rm -f "$owner_file" "$owner_started_file" 2>/dev/null || true; rmdir "$lock" 2>/dev/null || true; fi; printf '%s.%06d' "$epoch" "$seq"; }"#
