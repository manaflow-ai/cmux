...
incomplete_commit="ffffffffffffffffffffffffffffffffffffffff"
...
# A crashed publish can leave an owned 40-hex directory without its binary.
# It is not a retention candidate and must not block cleanup of valid entries.
make_baseline
mkdir "$test_root/$incomplete_commit"
printf '%s\t%s\n' "$incomplete_commit" 50 >> "$tmp/stat-map"
expect_success
test_dry_run=0
test_confirm=1
expect_success
assert_missing "$active_commit"
assert_missing "$old_commit"
assert_exists "$incomplete_commit"
