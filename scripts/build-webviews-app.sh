...
write_issue_inbox_html() {
  out_dir="$1"
  {
    printf '<!doctype html>\n'
    printf '<html lang="en" data-cmux-webview-kind="issue-inbox" data-codex-window-type="electron" data-window-type="electron" data-codex-os="darwin">\n'
    printf '  <head>\n'
    printf '    <meta charset="UTF-8" />\n'
    printf '    <meta name="viewport" content="width=device-width, initial-scale=1.0" />\n'
    printf '    <title>cmux Issue Inbox</title>\n'
    printf '  </head>\n'
    printf '  <body data-cmux-webview-kind="issue-inbox" data-codex-window-type="electron">\n'
    printf '    <main id="root"></main>\n'
    printf '    <script type="module" src="./main.mjs"></script>\n'
    printf '  </body>\n'
    printf '</html>\n'
  } > "$out_dir/issue-inbox.html"
}
...
normalize_webviews_output() {
  out_dir="$1"
  strip_trailing_line_whitespace "$out_dir/main.mjs" "$out_dir/agent-session.html" "$out_dir/issue-inbox.html"
}
...
if [ "${1:-}" = "--check" ]; then
  ...
  (
    ...
    write_issue_inbox_html "$tmp_dir"
...
write_issue_inbox_html "$OUT_DIR"
