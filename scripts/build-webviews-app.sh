...
write_feed_html() {
  ...
  {
    printf '<!doctype html>\n'
    printf '<html lang="en" data-cmux-webview-kind="feed">\n'
    printf '  <head>\n'
    printf '    <meta charset="UTF-8" />\n'
    printf '    <meta name="viewport" content="width=device-width, initial-scale=1.0" />\n'
    printf '    <title>cmux</title>\n'
    printf '  </head>\n'
    printf '  <body data-cmux-webview-kind="feed">\n'
    printf '    <main id="root"></main>\n'
    printf '    <script id="cmux-feed-config" type="application/json">{}</script>\n'
    printf '    <script type="module" src="./main.mjs"></script>\n'
    printf '  </body>\n'
    printf '</html>\n'
  } > "$out_dir/feed.html"
...
normalize_webviews_output() {
  out_dir="$1"
  strip_trailing_line_whitespace "$out_dir/main.mjs" "$out_dir/agent-session.html" "$out_dir/feed.html"
}
