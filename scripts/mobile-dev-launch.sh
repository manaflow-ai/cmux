...
if [[ -n "$AUTH_CREDENTIALS_FILE" ]]; then
  ...
  cmux_dev_secrets_load --dogfood-only || exit $?
else
...
if [[ "$TARGET" == "simulator" ]]; then
  ...
  SIMCTL_CHILD_CMUX_UITEST_CLEAR_AUTH="${CMUX_UITEST_CLEAR_AUTH:-0}" \
  ...
  DEVICECTL_CHILD_CMUX_UITEST_CLEAR_AUTH="${CMUX_UITEST_CLEAR_AUTH:-0}" \
