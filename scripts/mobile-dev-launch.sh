...
ATTACH_READY_TIMEOUT_SECONDS="${CMUX_ATTACH_READY_TIMEOUT_SECONDS:-30}"
ATTACH_STABILITY_SECONDS="${CMUX_ATTACH_STABILITY_SECONDS:-35}"
...
if [[ -n "$READINESS_CURSOR" ]]; then
  cmux_attach_wait_for_usable_session \
    ...
    "$ATTACH_STABILITY_SECONDS"
  echo "==> stable RPC session established between $BUNDLE_ID and tagged Mac '$TAG'"
fi
