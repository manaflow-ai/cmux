...
CRATE_DIR="${ROOT}/Native/AlacrittyTerminalFFI"
LIB_NAME="libcmux_alacritty_terminal_ffi.dylib"
BUILD_OUTPUT_DIR="${TARGET_BUILD_DIR:-${CRATE_DIR}/target}/cmux-alacritty-ffi"
...
requested_targets="${CMUX_ALACRITTY_FFI_TARGETS:-}"
...
if [ -z "${requested_targets}" ]; then
  requested_archs="${CMUX_ALACRITTY_FFI_ARCHS:-${ARCHS:-}}"
...
for target in ${requested_targets}; do
  ...
  if [ ! -f "${source_lib}" ]; then
    echo "error: expected Alacritty FFI library at ${source_lib}" >&2
    exit 1
  fi
  libs+=("${source_lib}")
done
