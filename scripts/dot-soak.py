#!/usr/bin/env python3
"""dot (Durable Object relay) engaged soak: verify continuous real use over
...
Preconditions (the runner sets these up):
  - tagged Mac (cmux DEV <tag>) running with cmux.dot.enabled, pointed at the
    relay worker via CMUX_PRESENCE_BASE_URL
  - simulator app installed, signed in, paired, foregrounded on a terminal
  - both journals live: /tmp/cmux-dot-journal-mac-<tag>.jsonl and the sim app
    container's Documents/dot-journal.jsonl
...
Usage:
  python3 scripts/dot-soak.py --tag dotx --udid <sim-udid> --bundle-id <ios-bundle>
      [--minutes 60] [--out /tmp/dot-soak-rounds] [--no-input]
...
parser.add_argument("--tag", default="dotx")
parser.add_argument("--udid", required=True)
parser.add_argument("--bundle-id", required=True)
parser.add_argument("--minutes", type=int, default=60)
parser.add_argument("--out", default="/tmp/dot-soak-rounds")
...
mac_journal_path = pathlib.Path(f"/tmp/cmux-dot-journal-mac-{args.tag}.jsonl")
...
def sim_journal_path():
    try:
        container = subprocess.check_output(
            ...
            text=True).strip()
        return pathlib.Path(container) / "Documents" / "dot-journal.jsonl"
