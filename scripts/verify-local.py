...
def run(repo, selected, timeout, stream=sys.stdout):
    ...
    for item in items:
        ...
        if execution["status"] != "passed":
            ...
            print(f"Rerun from this checkout: python3 scripts/verify-local.py --only {item[0]}", file=stream)
    ...
    for phase in ("tests", "static_analysis"):
    ...
    result["outcome"] = {"status": status, "scope": "selected static preflight checks"}
...
def main():
    ...
    parser.add_argument("--only", action="append", choices=[c[0] for c in CHECKS],
