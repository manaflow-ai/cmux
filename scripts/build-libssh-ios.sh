...
case "$sdk" in iphonesimulator|iphoneos) ;; *) echo "unsupported SDK: $sdk" >&2; exit 2 ;; esac
...
python3 - "$output" "$sdk" "$arch" "$deployment_target" "$libssh_sha256" "$mbedtls_sha256" <<'PY'
import hashlib, json, pathlib, sys
out, sdk, arch, deployment, libssh_sha, mbedtls_sha = sys.argv[1:]
files = {}
...
python3 - "$output" "$sdk" "$arch" "$deployment_target" <<'PY'
from pathlib import Path
import json, subprocess, sys
out=Path(sys.argv[1]); sdk,arch,deployment=sys.argv[2:]
...
minimum=("-mios-simulator-version-min=" if sdk=="iphonesimulator" else "-miphoneos-version-min=")+deployment
subprocess.run(["xcrun","--sdk",sdk,"clang","-arch",arch,"-isysroot",sdk_path,minimum,
