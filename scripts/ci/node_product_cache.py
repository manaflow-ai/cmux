...
class Store:
    def __init__(self, root: Path):
        ...
        for child in ("objects", "state", "fills", "leases", "locks", "staging"):
            _mkdir(root / child)
...
def acquire(
    ...
    *,
    ...
    wait_deadline = time.time() + max(0.0, wait)
    ...
    while True:
        ...
        with store.lock(key):
            ...
            if validated:
                ...
                return {
                    ...
                    "waited_seconds": round(elapsed if observed_waiter else 0.0, 6),
            ...
            if not active:
                ...
                return {
                    ...
                    "waited_seconds": round(time.monotonic() - started if observed_waiter else 0.0, 6),
                    "snapshot": _snapshot(store, stats),
                }
            if not observed_waiter:
        ...
        sleeper(min(0.25, max(0.01, wait_deadline - time.time())))
...
def _abort_fill_locked(store: Store, key: str, token: str) -> None:
    fill = _read_json(store.fill(key))
    if token and fill and fill.get("token") == token:
...
def finalize(
    ...
    *,
    ...
    if not restore_succeeded:
        with store.lock(key):
            _abort_fill_locked(store, key, token)
    ...
    *,
    ...
    try:
        ...
        with store.lock(key):
            ...
            _abort_fill_locked(store, key, (_read_json(store.fill(key)) or {}).get("token", ""))
        reclaim_result = reclaim(store, budget, protected_key=key)
