...
build() {
  ...
  case "$series" in
    ...
    # Cache on, but swift-driver compares imported modules and headers by
    # content hash instead of mtime (-enable-incremental-file-hashing).
    cached-hashing|cached-hashing-full)
      extra=(COMPILATION_CACHE_ENABLE_CACHING=YES 'OTHER_SWIFT_FLAGS=$(inherited) -enable-incremental-file-hashing -driver-show-incremental') ;;
...
series_run() {
  ...
  case "$s" in tests-uncached-implicit|cached-short|cached-hashing) short_run "$s"; return ;; esac
  restore
  build "$s" baseline
  build "$s" null
