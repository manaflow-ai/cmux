...
# (diff sidecar, command palette Nucleo FFI, cmux-cua) while xcodebuild compiles
# Swift.
#
...
# nightly build. This script runs the same build scripts with the same Cargo
...
nucleo_pid=$!
run_helper cmux-cua \
  "$ROOT/scripts/build-cmux-cua.sh" --output "$scratch/cmux-cua" --archs "$archs" &
cua_pid=$!
