...
def selected_inventory(
    ...
    """Return the exact built Test Cases selected by this invocation."""
    ...
    for selector in selectors:
        ...
        matches = {
            ...
            if selector_matches(identifier, normalized)
        }
        if matches:
...
def validate_selectors(inventory: set[str], selectors: Iterable[str]) -> list[str]:
    _, missing = selected_inventory(inventory, selectors)
    return missing
