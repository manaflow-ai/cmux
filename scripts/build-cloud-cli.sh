#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CRATE_DIR="${ROOT}/Native/CloudCLI"
BIN_NAME="cmux-cloud"
BUILD_OUTPUT_DIR="${TARGET_BUILD_DIR:-${CRATE_DIR}/target}/cmux-cloud-cli"

if ! command -v cargo >/dev/null 2>&1; then
  echo "error: cargo is required to build ${BIN_NAME}" >&2
  exit 1
fi

rust_target_for_arch() {
  case "$1" in
    arm64|arm64e) echo "aarch64-apple-darwin" ;;
    x86_64) echo "x86_64-apple-darwin" ;;
    *)
      echo "error: unsupported Rust macOS arch $1" >&2
      return 1
      ;;
  esac
}

rust_target_for_triple() {
  case "$1" in
    aarch64-apple-darwin|x86_64-apple-darwin) echo "$1" ;;
    arm64-*|arm64e-*|aarch64-*) echo "aarch64-apple-darwin" ;;
    x86_64-*) echo "x86_64-apple-darwin" ;;
    *)
      echo "error: unsupported Rust macOS target triple $1" >&2
      return 1
      ;;
  esac
}

ensure_rust_target() {
  local target="$1"
  if command -v rustup >/dev/null 2>&1; then
    if rustup target list --installed | grep -qx "$target"; then
      return
    fi
    rustup target add "$target"
  fi
}

requested_targets="${CMUX_CLOUD_CLI_TARGETS:-}"
if [ -z "${requested_targets}" ] && [ -n "${TARGET_TRIPLE:-}" ]; then
  requested_targets="$(rust_target_for_triple "${TARGET_TRIPLE}")"
fi

if [ -z "${requested_targets}" ]; then
  requested_archs="${CMUX_CLOUD_CLI_ARCHS:-${ARCHS:-}}"
  if [ -z "${requested_archs}" ]; then
    case "$(uname -m)" in
      arm64|aarch64) requested_archs="arm64" ;;
      x86_64) requested_archs="x86_64" ;;
      *)
        echo "error: cannot infer Rust macOS target for host arch $(uname -m)" >&2
        exit 1
        ;;
    esac
  fi

  for arch in ${requested_archs}; do
    requested_targets="${requested_targets} $(rust_target_for_arch "$arch")"
  done
fi

mkdir -p "${BUILD_OUTPUT_DIR}"
bins=()
seen_targets=""
for target in ${requested_targets}; do
  case " ${seen_targets} " in
    *" ${target} "*) continue ;;
  esac
  seen_targets="${seen_targets} ${target}"
  ensure_rust_target "$target"
  cargo build --manifest-path "${CRATE_DIR}/Cargo.toml" --release --target "$target"
  source_bin="${CRATE_DIR}/target/${target}/release/${BIN_NAME}"
  if [ ! -x "${source_bin}" ]; then
    echo "error: expected cloud CLI at ${source_bin}" >&2
    exit 1
  fi
  bins+=("${source_bin}")
done

if [ "${#bins[@]}" -eq 0 ]; then
  echo "error: no Rust macOS architectures requested" >&2
  exit 1
fi

OUTPUT_BIN="${BUILD_OUTPUT_DIR}/${BIN_NAME}"
if [ "${#bins[@]}" -eq 1 ]; then
  rsync -a "${bins[0]}" "${OUTPUT_BIN}"
else
  lipo -create -output "${OUTPUT_BIN}" "${bins[@]}"
fi
chmod +x "${OUTPUT_BIN}"

if [ -z "${TARGET_BUILD_DIR:-}" ]; then
  echo "built ${OUTPUT_BIN}"
  exit 0
fi

DEST_DIR="${TARGET_BUILD_DIR}/${UNLOCALIZED_RESOURCES_FOLDER_PATH:-${CONTENTS_FOLDER_PATH:-Contents}/Resources}/bin"
DEST_BIN="${DEST_DIR}/${BIN_NAME}"
mkdir -p "${DEST_DIR}"
rsync -a "${OUTPUT_BIN}" "${DEST_BIN}"
chmod +x "${DEST_BIN}"

if [ "${CODE_SIGNING_ALLOWED:-YES}" != "NO" ] && [ -n "${EXPANDED_CODE_SIGN_IDENTITY:-}" ]; then
  codesign --force --sign "${EXPANDED_CODE_SIGN_IDENTITY}" "${DEST_BIN}" >/dev/null
fi
