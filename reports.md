...
- **Evidence:** Lines 16-21: the `pump(until:)` helper drives `RunLoop.main.run(until: Date().addingTimeInterval(0.02))` in a 3-second deadline loop, relying on an offscreen hosted view invalidating within that window. In headless CI environments (no display server or display-link ticks), the offscreen view may never lay out, causing `pump` to spin the full 3 seconds and exit silently with an empty `recorder.labels`. The subsequent `#expect(recorder.labels.contains(...))` then soft-fails instead of crashing, so the test can produce false failures in headless CI.
- **Why flaky:** An offscreen hosted view in a headless macOS CI process may not receive render-server callbacks. The 3-second deadline is wall-clock only, so on a slow or display-less host the pump exits vacuously without the condition ever being met, and downstream assertions fail non-deterministically.
- **Suggested fix:** Use a `@testable` hook or observer on `LiveEvalEngine`/`EvalRecorder` to drive evaluation imperatively in tests. Alternatively, call `hosting.layoutSubtreeIfNeeded()` after each `store.box(...).value = ...` mutation and assert synchronously, bypassing the run-loop timer entirely.
