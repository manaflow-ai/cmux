...
public struct MobileDiagnosticsEnvironment: Sendable {
    ...
    @MainActor public static func current() -> MobileDiagnosticsEnvironment {
        ...
        #if canImport(UIKit)
        deviceModel = hardwareModelIdentifier() ?? UIDevice.current.model
        osVersion = "\(UIDevice.current.systemName) \(UIDevice.current.systemVersion)"
        #else
        deviceModel = hardwareModelIdentifier() ?? "Mac"
        let os = ProcessInfo.processInfo.operatingSystemVersion
        osVersion = "macOS \(os.majorVersion).\(os.minorVersion).\(os.patchVersion)"
        #endif

        return MobileDiagnosticsEnvironment(
            appName: infoString("CFBundleName", in: bundle),
            appVersion: infoString("CFBundleShortVersionString", in: bundle),
