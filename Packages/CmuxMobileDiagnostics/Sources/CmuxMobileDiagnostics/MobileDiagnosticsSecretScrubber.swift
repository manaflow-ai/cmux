...
public struct MobileDiagnosticsSecretScrubber: Sendable {
    ...
    /// One compiled scrub rule: a regex plus the capture group whose contents
    /// are replaced with ``redactionMarker``.
    private struct Pattern: Sendable {
        let regex: NSRegularExpression
        /// The 1-based capture group holding the secret value to mask. The rest
        /// of the match (a prefix like `Bearer ` or `token=`) is preserved.
        let valueGroup: Int
    }
    ...
    /// Replace the contents of one capture group across every match, scanning
    /// from the end so earlier match ranges stay valid as the string mutates.
    ...
    private static func makePatterns() -> [Pattern] {
        ...
        return raw.compactMap { source, group in
            ...
            return Pattern(regex: regex, valueGroup: group)
        }
    }
}
