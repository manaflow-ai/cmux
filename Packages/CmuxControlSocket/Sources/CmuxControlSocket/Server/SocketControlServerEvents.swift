...
///
/// Threading: closures are invoked synchronously on the server's listener
/// queue — the actor's executor. When the host drives the server through
/// ``SocketControlServer/performSync(_:)``, the calling thread is blocked in
/// `queue.sync` while the closures run, so implementations must never block
/// on that caller (in particular, never hop synchronously to the main
/// thread). They must be safe to call from any thread and must not block.
public struct SocketControlServerEvents: Sendable {
    ...
    /// monitor and accept source. Invoked at the same lifecycle point the
    /// legacy implementation posted `.socketListenerDidStart`, but on the
    /// listener queue (the actor's executor); hosts that need the main
    /// thread must hop asynchronously.
    ...
    /// (validated against the bound identity under actor isolation). The host
