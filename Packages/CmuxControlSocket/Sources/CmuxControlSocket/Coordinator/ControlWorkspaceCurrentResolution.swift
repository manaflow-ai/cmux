...
public enum ControlWorkspaceCurrentResolution: Sendable, Equatable {
    ...
    /// The selected workspace was snapshotted. Carries the owning window id (may
    /// be absent), the selected workspace's id, its index within the list (if
    /// resolvable), and its summary.
    case resolved(
        windowID: UUID?,
        workspaceID: UUID,
        index: Int?,
