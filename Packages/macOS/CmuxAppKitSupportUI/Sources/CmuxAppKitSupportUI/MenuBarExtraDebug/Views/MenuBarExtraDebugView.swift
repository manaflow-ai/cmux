...
/// The Menu Bar Extra Debug panel: live controls for the menu-bar icon's unread
/// badge (preview count override, badge rect, and per-digit text metrics) backed
/// by the shared ``MenuBarIconDebugSettings`` `@AppStorage` keys the production
/// renderer reads.
///
/// The view is byte-faithful to the panel that previously lived in the app target.
/// The one irreducibly app-coupled value is injected:
///
/// - ``refreshMenuBarIcon``: redraws the live menu-bar icon after a tuning change.
///   The app resolves this against its menu-bar controller (the legacy app-side
///   `AppDelegate.shared?.refreshMenuBarExtraForDebug()`), so the app supplies the
///   closure. ``DebugWindowsCoordinator`` mounts this view directly, so no
///   app-target content view is injected.
public struct MenuBarExtraDebugView: View {
    ...
    private let refreshMenuBarIcon: () -> Void

    /// Creates the panel.
    ///
    /// - Parameter refreshMenuBarIcon: Redraws the live menu-bar icon after any
    ///   tuning change. Evaluated on appear and on every control mutation, matching
    ///   the legacy app-side `refreshMenuBarExtraForDebug()`.
    public init(refreshMenuBarIcon: @escaping () -> Void) {
