...
@MainActor
public enum CmuxResizeCursors {
    public static let leftRight = arrowsCursor(directions: [CGVector(dx: 1, dy: 0), CGVector(dx: -1, dy: 0)])
    public static let upDown = arrowsCursor(directions: [CGVector(dx: 0, dy: 1), CGVector(dx: 0, dy: -1)])
    public static let allAxes = arrowsCursor(directions: [
    ...
    public static let diagonalNWSE = arrowsCursor(directions: [
        CGVector(dx: -0.7071, dy: 0.7071), CGVector(dx: 0.7071, dy: -0.7071),
    ])
    /// Northeast–southwest diagonal (top-right/bottom-left corners).
    public static let diagonalNESW = arrowsCursor(directions: [
        CGVector(dx: 0.7071, dy: 0.7071), CGVector(dx: -0.7071, dy: -0.7071),
    ])

    private static func arrowsCursor(directions: [CGVector]) -> NSCursor {
        let side: CGFloat = 24
    ...
    /// 24pt cursor image.
    private static func arrowsPath(center c: NSPoint, directions: [CGVector]) -> NSBezierPath {
