...
extension Notification.Name {
    /// Posted by the app when the persisted file-explorer visual style changes, so
    /// outline views re-read ``FileExplorerStyle/current`` and relayout.
    public static let fileExplorerStyleDidChange = Notification.Name("fileExplorerStyleDidChange")
}

/// The file-explorer outline view's visual style, persisted as the
/// `fileExplorer.style` `UserDefaults` integer (the raw value). Each case carries
/// the production layout metrics, fonts, and AppKit colors the running outline view
/// reads to render rows, icons, selection, and git-status tinting.
public enum FileExplorerStyle: Int, CaseIterable {
    case liquidGlass = 0
    case highDensity = 1
    ...
    /// Human-readable display name shown in the style picker.
    ...
    /// Child indentation per level in points.
    ...
    /// Symbol-icon font weight.
    public var iconWeight: NSFont.Weight {
        switch self {
    ...
    /// Font used for the file/folder name label.
    ...
    /// Horizontal gap between the icon and the name label in points.
    public var iconToTextSpacing: CGFloat {
        switch self {
    ...
    /// Horizontal inset of the selection background from the row edges in points.
    ...
    /// Corner radius of the selection background in points.
    public var selectionRadius: CGFloat {
        switch self {
    ...
    /// Fill color of the selection background.
    ...
    /// Background color drawn on a hovered row.
    ...
    /// Whether the selection is drawn as a border rather than a fill.
    public var usesBorderSelection: Bool {
        self == .terminalStealth
    }

    /// Tint color applied to file icons.
    public var fileIconTint: NSColor {
        switch self {
    ...
    /// Tint color applied to folder icons.
    public var folderIconTint: NSColor {
        switch self {
    ...
    /// Name-label color for a file with the given git status.
    public func gitColor(for status: GitFileStatus) -> NSColor {
        switch self {
        case .liquidGlass:
            switch status {
    ...
    /// The currently persisted style, defaulting to ``highDensity`` when the
    /// `fileExplorer.style` `UserDefaults` key is unset or out of range.
    public static var current: FileExplorerStyle {
...
extension FileExplorerStyle {
    ...
    public var fileExplorerStyleDebugDescription: String {
        switch self {
