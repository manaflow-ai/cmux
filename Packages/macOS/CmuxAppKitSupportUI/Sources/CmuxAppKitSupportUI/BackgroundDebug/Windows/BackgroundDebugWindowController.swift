...
/// Hosts the app's "Background Debug" panel in a floating utility window.
///
...
/// the utility ``NSPanel`` with the fixed `cmux.backgroundDebug` chrome, and
/// decorates the panel through the injected ``WindowDecorating`` seam. The panel's
/// SwiftUI content is irreducibly app-coupled (it drives the live main-window glass
/// tint through the app-target window-chrome composition), so the app target
...
public final class BackgroundDebugWindowController: ReleasingWindowController {
