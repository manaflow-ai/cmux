...
struct SSHForegroundAuthenticationRetryPolicyTests {
    ...
    @Test(arguments: ["/bin/sh", "/bin/zsh"])
    func recoverySweepLeavesLivePublisherToItsCompletionPath(shellPath: String) throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent(
                "cmux-ssh-auth-live-publisher-recovery-\(UUID().uuidString)",
                isDirectory: true
            )
        let groupDirectory = root
            .appendingPathComponent("cmux-ssh-auth-group.live", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        try createSecureGroupDirectory(at: groupDirectory)
        try "999999|888888|Thu_Jan_1_00:00:00_1970\n".write(
            to: groupDirectory.appendingPathComponent("identity"),
        ...
        /bin/sleep 30 &
        cmux_test_publisher=$!
        trap '/bin/kill -KILL "$cmux_test_publisher" >/dev/null 2>&1 || true; wait "$cmux_test_publisher" 2>/dev/null || true' EXIT
        cmux_test_publisher_identity=$(cmux_ssh_auth_identity "$cmux_test_publisher") || exit 99
        printf '%s|%s\n' "$cmux_test_publisher" "$cmux_test_publisher_identity" \
          > "$CMUX_TEST_LIVE_GROUP/publisher" || exit 98
        CMUX_SSH_AUTH_GROUP_DIR=
        export CMUX_SSH_AUTH_GROUP_DIR
        cmux_ssh_auth_recovery_enqueue "$CMUX_TEST_LIVE_GROUP" || exit 97
        cmux_ssh_resume_failed_auth_group_reapers || exit 96
        cmux_ssh_auth_recovery_lock || exit 95
        if cmux_ssh_auth_recovery_queue_has_work_locked; then
          cmux_ssh_auth_recovery_unlock
          exit 94
        fi
        cmux_ssh_auth_recovery_unlock
        test -s "$CMUX_TEST_LIVE_GROUP/identity" || exit 93
        """

        let result = try runShellCommand(
            command,
            environment: [
                "CMUX_TEST_LIVE_GROUP": groupDirectory.path,
                "TMPDIR": root.path,
            ],
            shellPath: shellPath
        )
