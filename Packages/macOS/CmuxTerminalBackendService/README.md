...
Registration, status lookup, and bundle inspection run on the coordinator actor rather than the app's main actor. `stateUpdates()` exposes a sendable newest-value stream for UI presentation. Normal launch never unregisters or re-registers an enabled agent. The explicit `unregister()` API waits for process termination and is reserved for deletion workflows because it terminates backend-owned PTYs. A future executable upgrade still needs a backend handoff protocol before the production gate can be enabled.
