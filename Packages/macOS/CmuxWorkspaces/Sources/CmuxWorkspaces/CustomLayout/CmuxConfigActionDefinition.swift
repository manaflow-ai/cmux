...
/// A named `actions` entry in `cmux.json`: a reusable action template that a
/// surface tab-bar button (or a built-in) can reference by id.
///
/// This is the `Codable`, `Sendable` wire image of one entry under the config's
/// `actions` map. Its ``init(from:)`` infers the action kind (`builtin`,
/// `command`, `agent`, `workspaceCommand`) from the explicit `type` field or, in
/// its absence, from which of `agent`/`builtin`/`command` keys are present, and
/// it normalizes presentation fields (trimmed, blank-as-nil) while enforcing the
/// schema invariants. The shortcut grammar is decoded/encoded through
/// ``StoredShortcut``'s `cmux.json` config form (``CmuxShortcuts``); changing any
/// token here is a wire-format change to every user's `cmux.json`.
public struct CmuxConfigActionDefinition: Codable, Sendable, Hashable {
    /// The typed action this definition runs, or `nil` for a presentation-only
    /// override of another action.
    public var action: CmuxSurfaceTabBarButtonAction?
    /// The display title.
    public var title: String?
    /// The display subtitle (also accepted under the `description` key).
    public var subtitle: String?
    /// Discovery keywords used when searching for the action.
    public var keywords: [String]?
    /// Whether the action appears in the command palette.
    public var palette: Bool?
    /// The keyboard shortcut bound to the action, in `cmux.json` config form.
    public var shortcut: StoredShortcut?
    /// The button/action icon override.
    ...
    private static func encodeShortcut(
        ...
        if let secondStroke = shortcut.second {
            try container.encode(
                [shortcut.first.configString(), secondStroke.configString()],
                forKey: key
            )
        } else {
            try container.encode(shortcut.first.configString(), forKey: key)
        }
    }
}
