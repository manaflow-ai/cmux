...
///    (e.g. command-not-found exiting 127) is logged and falls back to the
...
public struct PreferredEditorService: FileOpening {
    ...
    public func open(_ url: URL, line: Int?, column: Int?) {
        ...
        process.terminationHandler = { @Sendable process in
            ...
            preferredEditorLogger.error(
                "preferred editor command \(commandName, privacy: .public) exited \(process.terminationStatus, privacy: .public); falling back to the OS default handler"
            )
            Task { @MainActor in
                systemOpener.openWithSystemDefault(url)
            }
    ...
    /// The executable's basename only, so logs never carry a full path or a
    /// leading `NAME=value` assignment.
    private nonisolated static func publicCommandName(_ command: String) -> String {
        shellWords(command).first { !$0.contains("=") }
            .map { URL(fileURLWithPath: $0).lastPathComponent } ?? "(empty)"
    }
