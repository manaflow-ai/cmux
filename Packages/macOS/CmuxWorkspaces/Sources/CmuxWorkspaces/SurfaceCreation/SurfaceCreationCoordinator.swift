...
public final class SurfaceCreationCoordinator {
    ...
    /// are protected at spawn time by ``mergedStartupEnvironment(base:remoteEnvironment:)``'s
    /// downstream `protectedKeys` authority, the single authority on which keys are
    /// managed. That protection is an exact Swift-string match, but the env
    /// eventually crosses the Swift→C boundary (`strdup` / Ghostty), where a key is
    /// truncated at its first NUL. A key like `"CMUX_SOCKET_PATH\0x"` would dodge
    /// the exact-match check yet collapse to `CMUX_SOCKET_PATH` in the spawned
    /// shell, so reject any key containing a NUL (and `=`, which is never a valid
    /// env var name) and any value containing a NUL. This is the single choke point
    /// for every entry point (CLI, cmux.json, session restore), so the guard cannot
    /// be bypassed.
    ///
    /// `static` and `nonisolated` so the nonisolated socket workspace-create
    /// parsing path (`v2WorkspaceCreate`) and the `Workspace.init` path can call
    /// this pure helper without a coordinator instance and without hopping to the
    /// main actor.
    ///
    /// - Parameter environment: the raw user-supplied environment.
    /// - Returns: the environment with blank/NUL-bearing/`=`-bearing entries dropped
    ///   and keys whitespace-trimmed.
    public nonisolated static func sanitizedWorkspaceEnvironment(
    ...
    /// ``mergedStartupEnvironment(base:remoteEnvironment:)``'s `protectedKeys`
    /// authority; this only decides precedence among user-supplied values —
    /// explicit per-surface entries (layout `env`, scrollback replay, SSH startup)
    /// win over the workspace set.
    ///
    /// `static` so the `Workspace.init` path can call it before `self` is fully
    /// initialized, with no coordinator instance.
    ///
    /// - Parameters:
    ///   - workspaceEnvironment: the workspace's sanitized environment set.
    ///   - explicit: the per-surface explicit overrides to overlay.
    /// - Returns: `explicit` when `workspaceEnvironment` is empty, otherwise the
    ///   workspace set with the explicit pairs assigned over it.
    public nonisolated static func startupEnvironment(
