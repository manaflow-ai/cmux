    ...
    public func removeWorkspaceAndPromoteGroupAnchor(closedWorkspaceId: UUID) -> [UUID] {
        ...
        for groupId in affectedGroupIds {
            ...
            if let nextAnchor = remainingTabs.first(where: { $0.groupId == groupId }) {
                ...
                promotedAnchorIds.append(nextAnchor.id)
            } else if nextGroups[groupIndex].isPinned {
                // A pinned group is durable even after its last member closes.
                // Keep its stable header identity so dividers and persisted
                // group placement can continue to resolve against the empty
                // group row.
                nextGroups[groupIndex].anchor = .empty(groupId)
            } else {
