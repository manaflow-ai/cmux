...
public enum BrowserWebAuthnBridgeContract {
    public static let handlerName = "cmuxWebAuthn"
    @MainActor public static let contentWorld = WKContentWorld.world(name: "cmux.webauthn")
    ...
    public static let relayScriptSource: String = {
    ...
    public static let scriptSource: String = {
        let requestEventName = BrowserWebAuthnBridgeContract.requestEventName
        let acknowledgeEventName = BrowserWebAuthnBridgeContract.acknowledgeEventName
        let responseEventName = BrowserWebAuthnBridgeContract.responseEventName
        return #"""
...
@MainActor
public final class BrowserWebAuthnCoordinator: NSObject, WKScriptMessageHandlerWithReply {
    private weak var installedWebView: WKWebView?
    private var activeAuthorizationController: ASAuthorizationController?
    private var activeAuthorizationContinuation: CheckedContinuation<[String: Any], any Error>?
    ...
    public func uninstall(from webView: WKWebView) {
    ...
    @MainActor
    public func tearDown(from webView: WKWebView) {
    ...
    public func userContentController(
        ...
        // The async form of the requirement has the same shape on every SDK the
        // package builds with; the reply-handler form's closure attributes differ.
        let reply = await withCheckedContinuation { (continuation: CheckedContinuation<ScriptReply, Never>) in
            handleScriptMessage(message) { reply, errorMessage in
                continuation.resume(returning: ScriptReply(value: reply, errorMessage: errorMessage))
            }
        }
        return (reply.value, reply.errorMessage)
    }

    /// A JSON-shaped reply built on the main actor and handed back to WebKit on the
    /// main actor; it never crosses to another isolation domain.
    private struct ScriptReply: @unchecked Sendable {
        let value: Any?
        let errorMessage: String?
    }

    private func handleScriptMessage(
        ...
        Task { @MainActor in
            do {
                ...
                #if DEBUG
                CMUXDebugLog.logDebugEvent("webauthn.dispatch kind=\(envelope.kind.rawValue) frame=\(message.frameInfo.isMainFrame ? "main" : "sub") url=\(message.frameInfo.securityOrigin.host)")
                #endif
                ...
                case .capabilities:
                    ...
                    #if DEBUG
                    CMUXDebugLog.logDebugEvent("webauthn.capabilities reply=\(capReply)")
                    #endif
                ...
                case .createCredential:
                    ...
                    #if DEBUG
                    CMUXDebugLog.logDebugEvent("webauthn.createCredential reply.ok=\(reply["ok"] ?? "nil") hasCredential=\(reply["credential"] != nil) fallback=\(reply["useWebKitFallback"] ?? "nil")")
                    #endif
                ...
                case .getCredential:
                    ...
                    #if DEBUG
                    CMUXDebugLog.logDebugEvent("webauthn.getCredential reply.ok=\(reply["ok"] ?? "nil") hasCredential=\(reply["credential"] != nil) fallback=\(reply["useWebKitFallback"] ?? "nil")")
                    #endif
                    replyHandler(reply, nil)
                }
            } catch let error as BrowserWebAuthnBridgeError {
                #if DEBUG
                CMUXDebugLog.logDebugEvent("webauthn.error bridge: \(error.replyObject())")
                #endif
                replyHandler(error.replyObject(), nil)
            } catch {
                #if DEBUG
                CMUXDebugLog.logDebugEvent("webauthn.error unknown: \(error.localizedDescription)")
                #endif
...
extension BrowserWebAuthnCoordinator: ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    public func authorizationController(
        ...
        #if DEBUG
        CMUXDebugLog.logDebugEvent("webauthn.asAuth.didComplete credentialType=\(type(of: authorization.credential))")
        #endif
        do {
            finishAuthorization(
                with: .success(
            ...
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.asAuth.didComplete replyMarshalError=\(error)")
            #endif
    ...
    public func authorizationController(
        ...
        CMUXDebugLog.logDebugEvent("webauthn.asAuth.didFail domain=\(nsError.domain) code=\(nsError.code)")
        #endif
    ...
    public func presentationAnchor(for controller: ASAuthorizationController) -> ASPresentationAnchor {
        ...
        #if DEBUG
        CMUXDebugLog.logDebugEvent("webauthn.asAuth.presentationAnchor hasTitle=\(anchor.title.isEmpty ? 0 : 1) isVisible=\(anchor.isVisible) isKey=\(anchor.isKeyWindow)")
        #endif
        return anchor
    }
}
...
private extension BrowserWebAuthnCoordinator {
    ...
    func handleCreateCredential(
        ...
        guard let plan = try buildCreationPlan(request, clientDataContext: clientDataContext) else {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.handleCreate no plan — returning fallback")
            #endif
        ...
        guard !requests.isEmpty else {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.handleCreate authorizationRequests empty — returning fallback")
            #endif
            return fallbackReply()
        }
    ...
    func handleGetCredential(
        ...
        guard let plan = try buildAssertionPlan(request, clientDataContext: clientDataContext) else {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.handleGet no plan — returning fallback")
            #endif
            return fallbackReply()
        }
        ...
        guard !requests.isEmpty else {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.handleGet authorizationRequests empty — returning fallback")
            #endif
            return fallbackReply()
        }
        let presentationWindow = try interactivePresentationWindow(for: message)
    ...
    func authorizationRequests(
        ...
        #if DEBUG
        CMUXDebugLog.logDebugEvent("webauthn.authRequests hasPlatform=\(plan.hasPlatformRequests) hasSecurityKey=\(plan.securityKeyRequests.count > 0) order=\(plan.order)")
        #endif

        if includePlatformRequests {
            let currentState = BrowserPasskeyAuthorizationGate.shared.currentAuthorizationState()
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.authRequests passkeyAuthState=\(currentState.rawValue) callerMayPrompt=\(callerMayPromptForPlatformAuthorization(message))")
            #endif
            if currentState == .notDetermined && !callerMayPromptForPlatformAuthorization(message) {
                #if DEBUG
                CMUXDebugLog.logDebugEvent("webauthn.authRequests skipping platform: cross-origin subframe can't prompt")
                #endif
                includePlatformRequests = false
            } else {
                let authorizationState = await BrowserPasskeyAuthorizationGate.shared.authorizeIfNeeded()
                #if DEBUG
                CMUXDebugLog.logDebugEvent("webauthn.authRequests authorizeIfNeeded result=\(authorizationState.rawValue)")
                #endif
        ...
        #if DEBUG
        CMUXDebugLog.logDebugEvent("webauthn.authRequests finalCount=\(requests.count) includePlatform=\(includePlatformRequests)")
        #endif
        guard !requests.isEmpty else {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.authRequests FAIL: no requests available, throwing notAllowed")
            #endif
        ...
        if plan.needsBluetoothPreparation(includePlatformRequests: includePlatformRequests) {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.authRequests preparing bluetooth")
            #endif
            let btState = await BrowserBluetoothAuthorizationGate.shared.prepareIfNeeded()
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.authRequests bluetooth result=\(btState)")
            #endif
        }
    ...
    func performAuthorization(
        ...
        for (i, req) in requests.enumerated() {
            CMUXDebugLog.logDebugEvent("webauthn.performAuth request[\(i)]=\(type(of: req))")
        }
        #endif
        guard !requests.isEmpty else {
            throw BrowserWebAuthnBridgeError.notSupported("Native passkey support is unavailable.")
        ...
        guard let window = browserInteractiveModalHostWindow(window) else {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.performAuth FAIL: no interactive window")
            #endif
            throw BrowserWebAuthnBridgeError.notAllowed("Passkey access is not available.")
        }
        guard activeAuthorizationContinuation == nil else {
            #if DEBUG
            CMUXDebugLog.logDebugEvent("webauthn.performAuth FAIL: ceremony already in progress")
            #endif
            throw BrowserWebAuthnBridgeError.notAllowed("The passkey request failed.")
        }

        #if DEBUG
        CMUXDebugLog.logDebugEvent("webauthn.performAuth launching ASAuthorizationController")
        #endif
    ...
    func finishAuthorization(with result: sending Result<[String: Any], any Error>) {
        let continuation = activeAuthorizationContinuation
        activeAuthorizationController = nil
        activeAuthorizationContinuation = nil
        activePresentationWindow = nil
    ...
    func buildAssertionPlan(
        ...
        #if DEBUG
        CMUXDebugLog.logDebugEvent("webauthn.buildAssertionPlan rp=\(relyingPartyIdentifier) platform=\(platformRequests.count) securityKey=\(securityKeyRequests.count) allowCredentials=\(allowCredentials.count) mediation=\(request.mediation ?? "(nil)") hybridTransport=\(transportSummary.shouldShowHybridTransport)")
        #endif
        return .init(
            platformRequests: platformRequests,
            securityKeyRequests: securityKeyRequests,
    ...
    func successCredentialReply(from credential: any ASAuthorizationCredential) throws -> [String: Any] {
    ...
    func bridgeError(from error: any Error) -> BrowserWebAuthnBridgeError {
