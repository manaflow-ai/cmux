...
extension BrowserOmnibarPageFocusRepository {
    ...
    static let addressBarFocusRegistryInstaller = """
    (() => {
    ...
    /// `window.__cmuxAddressBarFocusState` (mirrored to the top frame) and records
    /// the element in the in-page focus registry without mutating the DOM. Returns
    /// a `captured:<id>` / `cleared:*` / `error` status string.
    ...
    /// first-responder handoff has already cleared `document.activeElement`. Focus
    /// identity lives in the in-page registry so tracking never mutates the DOM.
    ...
    /// restores its text selection, resolving the element through the in-page
    /// focus registry rather than a DOM attribute selector.
    ///
