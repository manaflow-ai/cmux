...
/// The SF Symbol the debug-only browser dev-tools toolbar button can display.
///
/// Each case's `rawValue` is the SF Symbol name rendered by the button, so the
/// persisted `UserDefaults` string round-trips directly into a symbol. ``title``
/// is the human label shown in the debug picker. The catalog is moved byte-identical
/// out of the app target so the persisted icon selection and ``BrowserConfigTests``
/// assertions keep resolving the same raw values.
public enum BrowserDevToolsIconOption: String, CaseIterable, Identifiable, Sendable {
    ...
    /// The stable identity used by `Identifiable`, equal to the SF Symbol `rawValue`.
    public var id: String { rawValue }

    /// The human-readable label shown in the debug icon picker.
