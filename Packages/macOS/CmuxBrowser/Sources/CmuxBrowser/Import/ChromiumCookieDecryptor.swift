...
/// One Keychain generic-password lookup target (service + account pair) that may
/// hold a Chromium "Safe Storage"/"Storage Key" secret.
struct ChromiumCookieKeychainItem: Hashable {
    let service: String
    let account: String
}

/// Decrypts Chromium `v10`/`v11` AES-128-CBC cookie values using the per-browser
/// "Safe Storage" secret read from the login Keychain.
///
/// The secret is located by probing a ranked set of service/account names
/// derived from the browser's display name, bundle metadata, and known aliases;
/// it is cached after the first successful lookup. `lastFailureReason` records
/// why decryption could not proceed so the importer can surface an accurate
/// skipped-cookies warning.
...
#else
/// Stub decryptor used on platforms without CommonCrypto/Security: it cannot
/// decrypt encrypted cookies and only reports how many were skipped.
final class ChromiumCookieDecryptor {
    init(browser: InstalledBrowserCandidate) {}
