...
public actor ChromiumBrowserSession {
    ...
    var mainLoaderID: String?
    var awaitingNavigationCommit = false
    ...
    ///   - extensionDirectories: Explicitly configured unpacked MV3 directories.
    ...
    private func performStart(generation: UInt64) async throws {
        // Register the shared startup task before suspending on the previous
        // child. Concurrent callers then join the same restart generation.
        ...
        do {
            let extensions = try await extensionStore.prepare(directories: extensionDirectories, profileID: profileID)
            let executable = try await artifactStore.ensureInstalled(
                manifest: extensions.isEmpty ? .production : .extensions
            )
            ...
            let configuration = ChromiumLaunchConfiguration(
                ...
                additionalArguments: extensions.isEmpty ? [] : [
            ...
            try await cdp.loadExtensions(extensions)
    ...
    public func stop() {
        ...
        for child in pendingProcesses.values { requestTermination(child) }
    ...
    func requestTermination(_ child: Process) {
        let identity = ObjectIdentifier(child)
        guard shutdownTasks[identity] == nil else { return }
        child.terminate()
        shutdownTasks[identity] = Task { [weak self, child] in
            do { try await ContinuousClock().sleep(for: .seconds(2)) }
            catch { return }
            await self?.forceTermination(child, identity: identity)
        }
    }

    private func forceTermination(_ child: Process, identity: ObjectIdentifier) {
        guard pendingProcesses[identity] === child, child.isRunning else { return }
        // The retained Process identity prevents signaling a reused PID.
    ...
    func childTerminated(process terminatedProcess: Process, status: Int32) {
        ...
        pendingProcesses.removeValue(forKey: processID)
        shutdownTasks.removeValue(forKey: processID)?.cancel()
