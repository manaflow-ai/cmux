...
actor ChromiumCDPConnection {
    ...
    init(endpoint: URL, session: URLSession, commandTimeout: Duration = .seconds(30)) throws {
        self.commandTimeout = commandTimeout
        transport = try ChromiumCDPWebSocketTransport(endpoint: endpoint, session: session)
    }

    init(transport: any ChromiumCDPTransport, commandTimeout: Duration = .seconds(30)) {
        self.commandTimeout = commandTimeout
    ...
    func attachToPageTarget() async throws {
        ...
        guard case .object(let targetPayload) = targets,
              case .array(let targetInfos)? = targetPayload["targetInfos"] else {
            throw CDPError.malformedMessage
        }
        let targetID: String
        if let existing = targetInfos.compactMap(Self.pageTargetID).first {
            targetID = existing
        } else {
            let created = try await sendBrowserCommand(
                method: "Target.createTarget", parameters: .object(["url": .string("about:blank")])
            )
            guard case .object(let payload) = created, let id = payload["targetId"]?.stringValue else {
                throw CDPError.malformedMessage
            }
            targetID = id
        }
    ...
    /// Chrome validates the full manifest and reports a precise load error.
    func loadExtensions(_ directories: [URL]) async throws {
        for directory in directories {
            do {
                _ = try await sendBrowserCommand(
                    method: "Extensions.loadUnpacked",
                    parameters: .object(["path": .string(directory.path)])
                )
            } catch {
                throw ChromiumExtensionError(path: directory.path, reason: error.localizedDescription)
            }
        }
    }
    ...
    func events() -> AsyncStream<CDPEvent> {
        let id = UUID()
        return AsyncStream(bufferingPolicy: .bufferingOldest(128)) { continuation in
            guard !isClosed else {
                continuation.finish()
                return
            }
    ...
    private func send(
        method: String, parameters: CDPValue?, targetSessionID: String?
    ) async throws -> CDPValue {
        try await withThrowingTaskGroup(of: CDPValue.self) { group in
            group.addTask { try await self.sendPending(method: method, parameters: parameters, targetSessionID: targetSessionID) }
            group.addTask { [commandTimeout] in
                try await ContinuousClock().sleep(for: commandTimeout)
                throw CDPError.commandFailed(ChromiumBrowserDiagnostic.commandTimedOut.message)
            }
    ...
    private func sendPending(
        ...
        guard pending.count < 128 else { throw CDPError.commandFailed(ChromiumBrowserDiagnostic.commandQueueFull.message) }
