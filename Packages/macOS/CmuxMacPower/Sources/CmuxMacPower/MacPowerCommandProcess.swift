...
private let macPowerSigkillGraceSeconds: Double = 0.2
private let macPowerCommandTimerQueue = DispatchQueue(label: "dev.cmux.mac-power-command-runner.timer")
...
func macPowerRunProcess(
    ...
    return await withCheckedContinuation { (continuation: CheckedContinuation<MacPowerRunResult, Never>) in
        ...
        let timer = DispatchSource.makeTimerSource(queue: macPowerCommandTimerQueue)
        ...
        timer.setEventHandler {
            ...
            Task {
                if await state.claim(failure) {
                    ...
                    macPowerScheduleSigkill(pid: processID)
                }
            }
        }
        timer.resume()
...
private func macPowerScheduleSigkill(pid: Int32) {
    let timer = DispatchSource.makeTimerSource(queue: macPowerCommandTimerQueue)
    timer.schedule(deadline: .now() + macPowerSigkillGraceSeconds)
    timer.setEventHandler {
