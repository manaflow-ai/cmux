...
public actor CloudHubPortForwarder {
    ...
    public typealias ForwardFactory = @Sendable (CloudPortForwardTarget, any CloudHubDialing) throws -> CloudLoopbackPortForward
    ...
    public func forward(machineID: String, to target: CloudPortForwardTarget) async throws -> CloudLoopbackPortForward {
    ...
    public func localPort(machineID: String, port: Int) async -> UInt16? {
        await forwards[Key(machineID: machineID, port: port)]?.localPort
    }

    public var count: Int { forwards.count }

    public func close(machineID: String, port: Int) async {
        await close(Key(machineID: machineID, port: port))
    }

    /// Closes every forward on a machine (it left the fleet or was deleted).
    public func close(machineID: String) async {
    ...
    public func close(machineIDs: Set<String>) async {
