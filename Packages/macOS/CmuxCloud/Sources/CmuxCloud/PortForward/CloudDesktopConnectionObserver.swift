...
@MainActor
public final class CloudDesktopConnectionObserver: NSObject, WKScriptMessageHandler {
    public static let name = "cmuxCloudDesktopConnection"
    public static let contentWorld = WKContentWorld.world(name: "cmux.cloud.desktop-connection")
    ...
    public init(webView: WKWebView, onConnecting: (@MainActor (URL) -> Void)?, onChange: @escaping @MainActor (URL, Bool) -> Void) {
        self.webView = webView
        self.onChange = onChange
        self.onConnecting = onConnecting
    }

    public static func install(on webView: WKWebView, onConnecting: (@MainActor (URL) -> Void)? = nil, onChange: @escaping @MainActor (URL, Bool) -> Void) {
