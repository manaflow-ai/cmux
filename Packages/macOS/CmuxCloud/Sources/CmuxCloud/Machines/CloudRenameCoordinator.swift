...
@MainActor
public final class CloudRenameCoordinator {
    public init() {}
    ...
    public struct Key: Hashable, Sendable {
        ...
        public static func workspace(machine: SurfaceMachineID, id: String) -> Self {
            Self(machine: machine, scope: .workspace, remoteID: id)
        }

        public static func tab(machine: SurfaceMachineID, id: String) -> Self {
    ...
    public var onPendingNamesChanged: @MainActor (SurfaceMachineID) -> Void = { _ in }

    public func pendingName(for key: Key) -> String? {
        pendingNames[key]?.value
    }

    /// Every intent still awaiting its RPC, newest name per identity.
    public var allPendingNames: [Key: String] {
    ...
    public func waitForPendingRenames(on machine: SurfaceMachineID) async throws {
