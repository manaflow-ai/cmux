...
/// `Divider()`s, informational `Text`/`Label` rows, `.disabled(true)`, and
/// `.keyboardShortcut` hints. Container kinds flatten into their children;
/// kinds with no menu representation (shapes, images, gradients) fall back to
/// a text-only row when they carry text, and are skipped otherwise.
@MainActor
enum RenderNodeMenuBuilder {
    /// Builds a menu whose items mirror `nodes`, dispatching button actions
    /// through `dispatch`.
    static func menu(for nodes: [RenderNode], dispatch: SidebarActionDispatch) -> NSMenu {
        let menu = NSMenu()
        ...
        append(nodes, to: menu, dispatch: dispatch)
        return menu
    }

    private static func append(_ nodes: [RenderNode], to menu: NSMenu, dispatch: SidebarActionDispatch) {
        for node in nodes {
            switch node.kind {
            case .divider:
                menu.addItem(.separator())
            case .menu:
                menu.addItem(submenuItem(for: node, dispatch: dispatch))
            case .button:
                menu.addItem(actionItem(for: node, dispatch: dispatch))
            case .text, .label:
            ...
            case .vstack, .hstack, .zstack, .lazyVStack, .lazyHStack, .group,
                ...
                append(node.children, to: menu, dispatch: dispatch)
            case .section:
            ...
            default:
                ...
                if node.action != nil || !plainText(of: node).isEmpty {
                    menu.addItem(actionItem(for: node, dispatch: dispatch))
                }
            }
        }
    ...
    /// A `Menu("…") { … }` node as an item with a recursive submenu.
    private static func submenuItem(for node: RenderNode, dispatch: SidebarActionDispatch) -> NSMenuItem {
        let title = node.text ?? ""
        let item = NSMenuItem(title: title, action: nil, keyEquivalent: "")
        item.isEnabled = !isDisabled(node)
        let submenu = NSMenu(title: title)
        submenu.autoenablesItems = false
        append(node.children, to: submenu, dispatch: dispatch)
        item.submenu = submenu
        return item
    }

    /// A leaf item: title from the node (or its rich label's text), enabled
    /// when it carries an action and isn't `.disabled(true)`, firing the
    /// node's ``ButtonAction`` through `dispatch`.
    private static func actionItem(for node: RenderNode, dispatch: SidebarActionDispatch) -> NSMenuItem {
        let title = node.text?.isEmpty == false ? node.text! : plainText(of: node.children)
        let item = NSMenuItem(title: title, action: nil, keyEquivalent: "")
        if let symbol = firstSystemImage(of: node) {
            item.image = NSImage(systemSymbolName: symbol, accessibilityDescription: nil)
        }
        applyKeyEquivalent(from: node, to: item)
        if let action = node.action, !isDisabled(node) {
    ...
    private static func isDisabled(_ node: RenderNode) -> Bool {
    ...
    private static func applyKeyEquivalent(from node: RenderNode, to item: NSMenuItem) {
        guard let modifier = node.modifiers.first(where: { $0.name == "keyboardShortcut" }),
              let key = nsKeyEquivalent(modifier.firstValue) else { return }
        item.keyEquivalent = key
        item.keyEquivalentModifierMask = nsModifierFlags(modifier.value("modifiers"))
    }

    /// Resolves a key token (`.return`/`.escape`/arrows/single character) to
    /// an `NSMenuItem.keyEquivalent` string.
    private static func nsKeyEquivalent(_ token: String?) -> String? {
    ...
    private static func nsModifierFlags(_ source: String?) -> NSEvent.ModifierFlags {
    ...
    /// text-only fallbacks.
    private static func plainText(of nodes: [RenderNode]) -> String {
        nodes.map { plainText(of: $0) }.filter { !$0.isEmpty }.joined(separator: " ")
    }

    private static func plainText(of node: RenderNode) -> String {
        var parts: [String] = []
        if let text = node.text, !text.isEmpty { parts.append(text) }
        parts.append(contentsOf: node.children.map { plainText(of: $0) }.filter { !$0.isEmpty })
        return parts.joined(separator: " ")
    }

    /// First SF Symbol in the subtree (`Label`'s icon or a leading `Image`).
    private static func firstSystemImage(of node: RenderNode) -> String? {
        if let name = node.systemName, !name.isEmpty { return name }
        for child in node.children {
            if let name = firstSystemImage(of: child) { return name }
        }
        return nil
    }
}

/// Retained (via `representedObject`) target that forwards a menu item's
/// activation to the sidebar action dispatch.
@MainActor
private final class RenderMenuItemTarget: NSObject {
