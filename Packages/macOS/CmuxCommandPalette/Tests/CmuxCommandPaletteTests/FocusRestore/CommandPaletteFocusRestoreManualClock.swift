...
/// A manually-advanced virtual clock for the focus-restore timeout tests: time
/// only moves when the test calls ``advance(by:)``, so the bounded-timeout
/// behavior is exercised without real waiting. Sleeps are cancellation
/// responsive (resuming with `CancellationError`), matching the contract the
/// controller's re-arm/clear paths rely on.
final class CommandPaletteFocusRestoreManualClock: Clock, @unchecked Sendable {
    ...
    /// time only once the deadline under test is actually waiting.
