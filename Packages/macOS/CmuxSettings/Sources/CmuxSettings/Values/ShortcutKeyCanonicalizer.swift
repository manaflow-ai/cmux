...
/// Canonicalizes recorded shortcut keys at the settings boundary.
///
...
public enum ShortcutKeyCanonicalizer {
    /// Returns the canonical key for a newly recorded macOS key event.
    public static func recordedKey(
        keyCode: UInt16,
        charactersIgnoringModifiers: String?
    ) -> String? {
        if let physicalKey = physicalBaseKey(for: keyCode) {
            return physicalKey
        }
    ...
    public static func storedKey(_ key: String, keyCode: UInt16?) -> String {
        if key.lowercased().hasPrefix("media.") {
            return key
        }
        guard let keyCode,
              let physicalKey = physicalBaseKey(for: keyCode) else {
            return key.lowercased()
        }
        return physicalKey
    }

    private static func physicalBaseKey(for keyCode: UInt16) -> String? {
        switch keyCode {
        case 36, 76: return "\r"
        case 48: return "\t"
