...
public struct BetaFeaturesCatalogSection: SettingCatalogSection {
    ...
    /// remote tmux server running for resume. Defaults on so `cmux ssh-tmux`
    /// is discoverable from the CLI; while off, every remote-tmux entry point
    /// and socket command is gated out so the local terminal path is unaffected.
