...
public import CMUXProjectModel
...
public enum ProjectPanelTab: String, Sendable, Hashable, CaseIterable {
    ...
    /// Human-readable label shown in the panel's tab selector.
...
public enum ProjectPanelLoadState: Sendable, Equatable {
    ...
    /// The parsed model when a load has succeeded, otherwise `nil`.
...
///
/// Isolation: `@MainActor`. The panel mutates observable UI state and drives
/// AppKit focus, so it lives on the main actor like its view-layer callers; the
/// project parse runs off-main inside ``reload()`` via a detached task.
@MainActor
public final class ProjectPanel: NSObject, Panel, ObservableObject {
    ...
    private func applyLoadError(_ error: any Error, previousModel: ProjectModel?) {
