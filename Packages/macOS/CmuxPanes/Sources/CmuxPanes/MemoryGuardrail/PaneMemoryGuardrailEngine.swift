...
public struct PaneMemoryGuardrailEngine {
    ...
    public static let clearFraction = 0.8

    public private(set) var warnedPanes: Set<PaneMemoryPaneKey> = []
    public private(set) var dismissedPanes: Set<PaneMemoryPaneKey> = []

    public init() {}

    public var warnedWorkspaceIds: Set<UUID> { Set(warnedPanes.map(\.workspaceId)) }

    public mutating func ingest(samples: [PaneMemorySample], thresholdBytes: Int64) -> PaneMemoryGuardrailEngineOutput {
    ...
    public mutating func dismiss(_ key: PaneMemoryPaneKey) {
    ...
    public mutating func acknowledgeHandled(_ key: PaneMemoryPaneKey) {
        warnedPanes.remove(key)
        dismissedPanes.remove(key)
    }
