...
/// Resolves the ordered, deduplicated list of applications that can open a file,
/// with the system default first.
///
/// The four collaborators are constructor-injected `@Sendable` closures so the
/// resolver is testable without `NSWorkspace`: `defaultApplicationURL` and
/// `applicationURLs` query the candidate apps, `displayName` derives a label,
/// and `shouldIncludeApplication` filters out apps that must never appear (cmux
/// itself and its dev builds). `live` wires these to the real `NSWorkspace`.
public struct FileExternalOpenApplicationResolver: Sendable {
    /// Returns the system default handler URL for the given file, if any.
    public var defaultApplicationURL: @Sendable (URL) -> URL?
    /// Returns every application URL capable of opening the given file.
    public var applicationURLs: @Sendable (URL) -> [URL]
    /// Derives the menu display name for an application URL.
    public var displayName: @Sendable (URL) -> String
    /// Whether an application URL is eligible to be shown (filters out cmux).
    public var shouldIncludeApplication: @Sendable (URL) -> Bool

    /// Creates a resolver from its four injected collaborators.
    public init(
    ...
    /// The production resolver backed by `NSWorkspace` and `Bundle` metadata.
    ...
    /// The ordered, deduplicated applications for `fileURL`: the eligible system
    /// default first (flagged `isDefault`), then every other eligible
    /// application, with entries sharing an identity collapsed to the first.
    public func applications(for fileURL: URL) -> [FileExternalOpenApplication] {
        ...
        let defaultIdentity = defaultURL.map(\.fileExternalOpenApplicationIdentity)
        ...
        return orderedURLs.compactMap { applicationURL in
            let identity = applicationURL.fileExternalOpenApplicationIdentity
