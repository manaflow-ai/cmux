...
/// AppKit PDF-preview surface: an `NSSplitView` hosting a thumbnail/outline
/// sidebar and a magnifying `PDFView`, with floating sidebar and zoom chrome
/// overlays.
///
/// Owns the split/sidebar/content view stack, the PDF document loading (off the
/// main thread on a private queue, applied back on the main thread), display and
/// sidebar modes, sidebar-width persistence, viewport preservation across
/// zoom/fit/resize, page-control state, and the table-of-contents outline. Focus
/// ownership and app keyboard-focus reconciliation stay app-side: the view holds
/// an `any FilePreviewPDFFocusSeam` and registers its focus endpoints / announces
/// and queries focus intent / requests keyboard re-sync through it, rather than
/// referencing the concrete panel. The external-open chrome strings are injected
/// as a `FileExternalOpenStrings` value (resolved app-side so `String(localized:)`
/// binds to the app bundle).
public final class FilePreviewPDFContainerView: NSView, NSSplitViewDelegate, NSOutlineViewDataSource, NSOutlineViewDelegate {
    private enum Metrics {
        static let defaultSidebarWidth = FilePreviewPDFSizing.preview.defaultSidebarWidth
        static let minimumSidebarWidth = FilePreviewPDFSizing.preview.minimumSidebarWidth
        static let maximumSidebarWidth = FilePreviewPDFSizing.preview.maximumSidebarWidth
    ...
    private weak var focusSeam: (any FilePreviewPDFFocusSeam)?
    private var externalOpenStrings = FileExternalOpenStrings(
        openWithMenu: "",
        openExternally: "",
        revealInFinder: "",
        openInApplication: { _ in "" }
    )
    ...
    /// Injects the focus seam (and re-registers focus endpoints) so the view can
    /// route first-responder requests and intent announcements app-side.
    public func setFocusSeam(_ focusSeam: any FilePreviewPDFFocusSeam) {
        self.focusSeam = focusSeam
        registerFocusEndpoint()
    }

    /// Injects the app-resolved external-open chrome strings and refreshes the
    /// chrome views that render them.
    public func setExternalOpenStrings(_ strings: FileExternalOpenStrings) {
        externalOpenStrings = strings
    ...
    private func applyBackgroundAppearance() {
        ...
        contentHost.applyFilePreviewRootLayerBackground(
            backgroundColor: previewBackgroundColor,
            drawsBackground: drawsPreviewBackground
        )
        ...
        pdfView.applyFilePreviewScrollBackgrounds(
            backgroundColor: previewBackgroundColor,
            drawsBackground: drawsPreviewBackground
        )
    ...
    private func currentPDFScrollBackgroundAppearance(
        ...
        var hostIdentifiers = pdfView.filePreviewScrollBackgroundHostIdentifiers
    ...
    private func selectedVisiblePDFPage() -> PDFPage? {
        pdfView.filePreviewSelectedVisiblePage(scrollView: pdfScrollView())
    }

    private func topVisiblePDFPage() -> PDFPage? {
        pdfView.filePreviewTopVisiblePage(scrollView: pdfScrollView())
    }
    ...
    private func clampedSidebarWidth(_ proposedWidth: CGFloat) -> CGFloat {
        FilePreviewPDFSizing.preview.clampedSidebarWidth(
            proposedWidth,
    ...
    private func minimumSidebarWidthForCurrentMode() -> CGFloat {
        switch sidebarMode {
        case .thumbnails:
            FilePreviewPDFSizing.preview.minimumThumbnailSidebarWidth
    ...
    private func preferredSidebarWidthForCurrentMode() -> CGFloat {
        ...
        case .tableOfContents:
            FilePreviewPDFSizing.preview.preferredOutlineSidebarWidth(for: outlineRoot)
        }
    }
    ...
    private func logSidebarWidth(
        ...
        let tocWidth = FilePreviewPDFSizing.preview.preferredOutlineSidebarWidth(for: outlineRoot)
    ...
    private func registerFocusEndpoint() {
        focusSeam?.attachPreviewFocus(root: pdfView, primaryResponder: pdfView, intent: .pdfCanvas)
        focusSeam?.attachPreviewFocus(
            root: thumbnailView,
            primaryResponder: thumbnailView.focusResponder(),
            intent: .pdfThumbnails
        )
        focusSeam?.attachPreviewFocus(root: outlineView, primaryResponder: outlineView, intent: .pdfOutline)
    }
    ...
    private func setActivePDFRegion(_ region: FilePreviewPanelFocusIntent?) {
        ...
        focusSeam?.noteFilePreviewFocusIntent(region)
        focusSeam?.syncKeyboardFocus(in: window)
    }
    ...
    private func currentPDFFocusRegion() -> FilePreviewPanelFocusIntent? {
        guard window?.isKeyWindow == true,
              ...
              let intent = focusSeam?.currentFilePreviewFocusIntent(in: window) else { return nil }
        switch intent {
