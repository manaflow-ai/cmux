...
/// A captured PDF viewport position that can be restored after the content is
/// rescaled or the sidebar is resized.
///
/// Anchors the viewport to a page-relative point when one is available (so the
/// same content stays put across a zoom/fit) and falls back to a document-ratio
/// anchor otherwise. Captured against a `PDFView` + its enclosing scroll view at
/// a chosen vertical anchor, then restored against the same pair.
public struct FilePreviewPDFViewportSnapshot {
    ...
    /// Captures the current viewport of `pdfView` within `scrollView`, anchored
    /// at the given vertical anchor, or `nil` when there is no document/clip to
    /// anchor against.
    ...
    /// Restores this captured viewport into `pdfView` within `scrollView`.
    public func restore(in pdfView: PDFView, scrollView: NSScrollView?) {
        ...
        let nextOrigin = CGPoint.filePreviewClampedClipOrigin(
    ...
    #if DEBUG
    /// A compact debug description of the captured anchor, used by the PDF
    /// container's resize-probe logging.
