...
/// The browser omnibar's address-bar `NSTextField` subclass.
///
/// Owns the pointer and keyboard behavior the SwiftUI omnibar cannot express:
/// single-click caret placement and drag/Shift selection (without entering
/// `NSTextField`'s mouse-tracking loop), Chrome/Safari/Arc-style select-all on
/// the click that first focuses the field, multi-click word/line selection routed
/// to the field editor, and key routing into the owning chrome via closures.
///
/// All coupling to the owning view is expressed through the configured closures
/// (``onPointerDown`` and ``onHandleKeyEvent``) and ``panelId``; the field
/// reaches no app-side type. The select-all and end-editing focus decisions are
/// the pure ``BrowserOmnibarFocusGainingClick`` predicate from `CmuxBrowser`.
public final class OmnibarNativeTextField: NSTextField {
    /// Identifies the browser panel this field belongs to, for the owning chrome.
    public var panelId: UUID?

    /// Invoked at the start of `mouseDown`, before first responder changes, so the
    /// owning chrome can react to the pointer entering the field.
    public var onPointerDown: (() -> Void)?

    /// Invoked from `keyDown`/`performKeyEquivalent` to let the owning chrome
    /// handle a key event; returning `true` consumes the event.
    public var onHandleKeyEvent: ((NSEvent, NSTextView?) -> Bool)?

    /// When `true`, the next end-of-editing focus reacquisition is skipped once.
    public var suppressNextFocusReacquireOnEndEditing = false
    ...
    /// Creates an unbordered, single-line omnibar field with a transparent
    /// background and no focus ring, truncating its text at the tail.
