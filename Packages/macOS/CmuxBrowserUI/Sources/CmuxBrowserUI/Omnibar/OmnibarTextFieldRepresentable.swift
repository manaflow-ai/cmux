...
// CmuxBrowser is a public import: OmnibarInlineCompletion and
// OmnibarLiveFieldSnapshot appear in this representable's public API
// (the initializer parameters, the `inlineCompletion` property, and the
// `onSubmit` closure signature).
...
/// `NSViewRepresentable` wrapping the omnibar's native `OmnibarNativeTextField`,
/// bridging AppKit text-editing, focus, IME, and inline-completion behavior to
/// the SwiftUI browser address bar.
///
/// The browser panel view builds this, passing the text/focus bindings, the
/// fourteen action closures, and the per-panel-view-owned
/// ``BrowserOmnibarNativeFieldRegistry`` instance (de-singletonized from the
/// former process-wide registry).
public struct OmnibarTextFieldRepresentable: NSViewRepresentable {
    let panelId: UUID
    ...
    public init(
        ...
        placeholder: String,
        nativeFieldRegistry: BrowserOmnibarNativeFieldRegistry,
        onTap: @escaping () -> Void,
        onSubmit: @escaping (OmnibarLiveFieldSnapshot?) -> Void,
        onEscape: @escaping () -> Void,
        onFieldLostFocus: @escaping () -> Void,
        onMoveSelection: @escaping (Int) -> Void,
        onDeleteSelectedSuggestion: @escaping () -> Void,
        onAcceptInlineCompletion: @escaping () -> Void,
        onDeleteBackwardWithInlineSelection: @escaping () -> Void,
        onClearTypedPrefixWithInlineSelection: @escaping () -> Void,
        onDeleteWordBackwardWithInlineSelection: @escaping () -> Void,
        onSelectionChanged: @escaping (NSRange, Bool) -> Void,
        shouldSuppressWebViewFocus: @escaping () -> Bool
    ) {
        self.panelId = panelId
        self.fontSize = fontSize
        self._text = text
        self._isFocused = isFocused
        self.selectAllRequestId = selectAllRequestId
        ...
        self.onFieldLostFocus = onFieldLostFocus
        self.onMoveSelection = onMoveSelection
        self.onDeleteSelectedSuggestion = onDeleteSelectedSuggestion
        self.onAcceptInlineCompletion = onAcceptInlineCompletion
        self.onDeleteBackwardWithInlineSelection = onDeleteBackwardWithInlineSelection
        self.onClearTypedPrefixWithInlineSelection = onClearTypedPrefixWithInlineSelection
        self.onDeleteWordBackwardWithInlineSelection = onDeleteWordBackwardWithInlineSelection
        self.onSelectionChanged = onSelectionChanged
        self.shouldSuppressWebViewFocus = shouldSuppressWebViewFocus
    }
    ...
    public final class Coordinator: NSObject, NSTextFieldDelegate {
        ...
        func logFocusEvent(_ event: String, detail: String = "") {
            ...
            CMUXDebugLog.logDebugEvent(line)
        }
#endif
        ...
        public func control(_ control: NSControl, textView: NSTextView, doCommandBy commandSelector: Selector) -> Bool {
            ...
            case #selector(NSResponder.insertNewline(_:)):
                ...
                guard currentFlags.browserOmnibarShouldSubmitOnReturn else { return false }
        ...
        func handleKeyEvent(_ event: NSEvent, editor: NSTextView?) -> Bool {
            ...
            // TODO(refactor): KeyboardLayout still lives in the app target
            // (Sources/KeyboardLayout.swift); the orchestrator reconciles this
            // dangling reference after the keyboard-layout move lands.
            let lowered = KeyboardLayout.normalizedCharacters(for: event)

            // Ctrl+N and Ctrl+P should repeat while held.
            if let delta = event.modifierFlags.browserOmnibarSelectionDeltaForControlNavigation(
            ...
            case 36, 76: // Return / keypad Enter
                guard event.modifierFlags.browserOmnibarShouldSubmitOnReturn else { return false }
    ...
    public func makeNSView(context: Context) -> OmnibarNativeTextField {
        let field = OmnibarNativeTextField(frame: .zero)
        field.panelId = panelId
        nativeFieldRegistry.register(field, panelId: panelId)
        // TODO(refactor): browserOmnibarTextFieldIdentifier still lives in the
        // app target (Sources/Panels/BrowserPanelView.swift, shared with
        // AppDelegate); the orchestrator reconciles this dangling reference.
    ...
    public func updateNSView(_ nsView: OmnibarNativeTextField, context: Context) {
        context.coordinator.parent = self
        context.coordinator.parentField = nsView
        if let previousPanelId = nsView.panelId, previousPanelId != panelId {
            nativeFieldRegistry.unregister(nsView, panelId: previousPanelId)
        }
        nsView.panelId = panelId
        nativeFieldRegistry.register(nsView, panelId: panelId)
    ...
    public static func dismantleNSView(_ nsView: OmnibarNativeTextField, coordinator: Coordinator) {
        if let panelId = nsView.panelId {
            coordinator.parent.nativeFieldRegistry.unregister(nsView, panelId: panelId)
        }
