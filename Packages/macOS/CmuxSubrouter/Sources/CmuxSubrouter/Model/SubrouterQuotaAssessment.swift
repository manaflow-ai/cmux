...
public enum SubrouterQuotaAssessment: Sendable, Hashable {
    ...
    ///
    /// Mirrors `sr.go`: the first long window at 100% (clamped) wins as
    /// ``cooked(_:)``; otherwise the first short window at 100% wins as
    /// ``tempCooked(_:)``; otherwise ``ok``. Window order is preserved as
    ...
    public static func assess(_ windows: [SubrouterUsageWindow]) -> SubrouterQuotaAssessment {
        if let saturatedLong = windows.first(where: { $0.isLongQuotaWindow && $0.isFullyConsumed }) {
