...
extension UNUserNotificationCenter {
    ...
    /// Removes delivered notifications for `ids` off the main thread so a slow
    /// `usernoted` XPC round-trip never blocks the UI.
    public func removeDeliveredNotificationsOffMain(withIdentifiers ids: [String]) {
    ...
    /// Removes pending notification requests for `ids` off the main thread so a
    /// slow `usernoted` XPC round-trip never blocks the UI.
    public func removePendingNotificationRequestsOffMain(withIdentifiers ids: [String]) {
        guard !ids.isEmpty else { return }
        Self.removalQueue.async {
