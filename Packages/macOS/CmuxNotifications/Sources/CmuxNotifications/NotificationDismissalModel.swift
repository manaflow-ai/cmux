...
public final class NotificationDismissalModel: NotificationDismissing {
    ...
    public func dismissNotification(
        ...
        // focused surface. Without this, a banner delivered for a non-focused
        // surface in the now-visible workspace could be swept along; the
        // delivery gate (`shouldSuppressExternalDelivery`) already keys on the
        // focused surface, and this makes the withdraw side match it. Scoped to
        // the app-active contexts (`.activeFocus` / `.explicitWorkspaceResume`,
        // i.e. `requiresActiveApp`) so explicit per-surface interactions
        // (direct click, terminal typing) and workspace-level dismissals
        // (`surfaceId == nil`) stay broad. See issue #6601.
        if host.suppressOnlyFocusedSurface,
           context.requiresActiveApp,
           let surfaceId,
