...
struct AndroidEmulatorProcessIdentityTests {
    ...
    func matchesExactAVDArgumentForms() {
        ...
        let portsByProcessIdentifier: [Int32: Set<Int>] = [
            102: [5556],
            103: [5554],
        ]
        let identity = AndroidEmulatorProcessIdentity(
            processArguments: { processIdentifier in
                argumentsByProcessIdentifier[processIdentifier]
            },
            listeningPorts: { processIdentifier in
                portsByProcessIdentifier[processIdentifier]
            }
        )
