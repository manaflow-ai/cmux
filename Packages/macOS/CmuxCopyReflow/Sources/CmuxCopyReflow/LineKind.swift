...
public enum LineKind: Equatable, Sendable {
    ...
    init(_ rawLine: Substring, activeFence: FenceMarker?) {
        ...
        if let activeFence {
            ...
            self = .blank
        } else if lineKindIsURLLine(trimmed) {
            ...
            self = .urlLine
        } else if lineKindIsHeading(trimmed) {
            self = .heading
        } else if trimmed.first == ">" {
            self = .blockquote
        } else if lineKindIsListItem(trimmed) {
            self = .listItem
        } else if lineKindIsTableRow(trimmed) {
            self = .tableRow
        } else {
            self = .prose
        }
...
private let lineKindURLPrefixes = ["http://", "https://", "www."]

/// `#` .. `######` followed by a space (or end of line).
private func lineKindIsHeading(_ trimmed: Substring) -> Bool {
...
/// Markdown table rows with outer pipes or separator rows. Rows without outer
/// pipes are promoted by the reflow pass only when adjacent to a separator row,
/// so shell pipelines stay eligible for command reflow.
private func lineKindIsTableRow(_ trimmed: Substring) -> Bool {
    lineKindIsOuterPipeTableRow(trimmed) || lineKindIsMarkdownTableSeparator(trimmed)
}

func lineKindIsMarkdownTableCandidateRow(_ rawLine: Substring) -> Bool {
    let trimmed = rawLine.drop { $0 == " " || $0 == "\t" }
    return !trimmed.isEmpty && trimmed.contains("|") && !lineKindIsMarkdownTableSeparator(trimmed)
}

func lineKindIsMarkdownTableSeparatorRow(_ rawLine: Substring) -> Bool {
    let trimmed = rawLine.drop { $0 == " " || $0 == "\t" }
    return lineKindIsMarkdownTableSeparator(trimmed)
}

private func lineKindIsOuterPipeTableRow(_ trimmed: Substring) -> Bool {
    guard trimmed.first == "|", trimmed.last == "|" else { return false }
    return trimmed.filter { $0 == "|" }.count >= 2
}

private func lineKindIsMarkdownTableSeparator(_ trimmed: Substring) -> Bool {
...
private func lineKindIsListItem(_ trimmed: Substring) -> Bool {
...
private func lineKindIsURLLine(_ trimmed: Substring) -> Bool {
    let afterMarker = lineKindStripSingleLeadingMarker(trimmed)
    return !afterMarker.contains { $0 == " " || $0 == "\t" }
        && lineKindURLPrefixes.contains { afterMarker.lowercased().hasPrefix($0) }
}

/// Drop at most one leading list/quote marker (`- `, `1. `, `> `, etc.) so a
/// URL behind such a marker is still recognised.
private func lineKindStripSingleLeadingMarker(_ trimmed: Substring) -> Substring {
