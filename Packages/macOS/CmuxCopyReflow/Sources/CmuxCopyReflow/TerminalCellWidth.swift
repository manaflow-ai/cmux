...
/// Locale-independent terminal column measurement for wrap detection.
///
/// Swift's `Character.count` measures grapheme clusters, not terminal cells.
...
enum TerminalCellWidth {
    static func columns(in text: Substring) -> Int {
        text.reduce(into: 0) { width, character in
            width += columns(in: character)
        }
    }

    private static func columns(in character: Character) -> Int {
