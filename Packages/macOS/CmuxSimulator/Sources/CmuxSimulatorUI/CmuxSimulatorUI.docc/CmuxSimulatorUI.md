...
Private Simulator framework calls and framebuffer rendering remain isolated in `CmuxSimulatorWorker`; UI code communicates with them only through `CmuxSimulator` messages. The host uses a signature-checked, exception-contained `CALayerHost` setter solely to display the worker's remote Core Animation context.
