...
enum SimulatorDeviceChromeMeasurement {
    ...
    static func screenOpening(at url: URL) -> Opening? {
        guard let raster = rasterized(url), raster.width > 0, raster.height > 0 else { return nil }
    ...
    private static func rasterized(_ url: URL) -> Raster? {
        if let document = CGPDFDocument(url as CFURL), let page = document.page(at: 1) {
            ...
            return makeRaster(pointSize: bounds.size, pixelScale: 1) { context in
        ...
        if let raster = raster(image: image, pointSize: size) { return raster }
        return makeRaster(pointSize: size, pixelScale: 1) { context in
    ...
    private static func makeRaster(
        ...
        return rasterThroughPNG(image: image, pointSize: pointSize)
            ?? raster(image: image, pointSize: pointSize)
    }
    ...
    private static func rasterThroughPNG(image: CGImage, pointSize: CGSize) -> Raster? {
        ...
        return raster(image: decoded, pointSize: pointSize)
    }

    private static func raster(image: CGImage, pointSize: CGSize) -> Raster? {
