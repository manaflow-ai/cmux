...
struct SimulatorWebInspectorSocketDiscovery: Sendable {
    ...
    func socketPath(deviceIdentifier: String) async throws -> String {
        let service = "user/\(getuid())/com.apple.webinspectord"
        ...
        guard result.status == 0,
              let path = Self.parseSocketPath(result.standardOutput) else {
    ...
    static func parseSocketPath(_ launchctlOutput: String) -> String? {
        for line in launchctlOutput.split(whereSeparator: \.isNewline) {
            ...
            guard Self.isAllowedSocketPath(path) else { return nil }
            return path
        }
        return nil
    }

    private static func isAllowedSocketPath(_ path: String) -> Bool {
