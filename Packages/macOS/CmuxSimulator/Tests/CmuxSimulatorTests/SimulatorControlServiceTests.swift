...
private actor RecordingCommandRunner: CommandRunning, SimulatorBoundedCommandRunning {
    ...
    private static func capture(_ value: String?, limit: Int) -> (data: Data, truncated: Bool) {
...
private actor ClipboardInspectingCommandRunner: CommandRunning {
    ...
    func run(
        ...
        guard executable == "/bin/sh", let path = arguments.last else { return .success("") }
        let attributes = try? FileManager.default.attributesOfItem(atPath: path)
        let permissions = (attributes?[.posixPermissions] as? NSNumber)?.intValue ?? 0
        let text = (try? String(contentsOfFile: path, encoding: .utf8)) ?? ""
        observation = Observation(path: path, text: text, permissions: permissions)
        return .success("")
    }
}
