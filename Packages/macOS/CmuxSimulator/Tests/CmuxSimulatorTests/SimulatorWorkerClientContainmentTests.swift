...
extension SimulatorWorkerClientTests {
    ...
    func interactiveTouchCancellationRecovery() async throws {
        ...
        let cancellation = SimulatorWorkerInbound.pointer(.init(
            phase: .cancelled,
            primary: point
        ))
        for _ in 0..<1_000 {
            if worker.inboundMessages().contains(cancellation) { break }
            await Task.yield()
        }
        #expect(worker.inboundMessages().contains(cancellation))
        await client.stop()
    }
