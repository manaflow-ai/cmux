...
/// AppKit container that lays out the inline Feed reply/answer editor: a
/// `FeedInlineNativeTextView` plus an overlaid placeholder label. It owns the
/// font/enabled styling, computes its own fitting height for the SwiftUI
/// representable, and exposes `minimumHeight(for:)` so callers can pin a
/// `minHeight` frame before any text is laid out.
