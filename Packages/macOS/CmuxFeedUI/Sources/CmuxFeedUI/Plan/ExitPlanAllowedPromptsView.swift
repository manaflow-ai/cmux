...
/// Lists the tool/prompt pairs an ExitPlanMode plan would allow, each tagged
/// with its tool name when present.
public struct ExitPlanAllowedPromptsView: View {
    let prompts: [WorkstreamAllowedPrompt]

    /// Creates the allowed-prompts list view.
    /// - Parameter prompts: The tool/prompt pairs the plan would allow.
