...
///
/// In production every button renders the opaque `solid` treatment. The
/// `#if DEBUG` style-exploration path is driven by an injected
/// ``FeedButtonDebugStyle`` provider so that all debug-settings *reads* stay
/// app-side (the app's `FeedButtonDebugSettings` repository), while the package
/// owns only the rendering. The package keeps a tiny `@AppStorage` on the debug
/// generation counter so a settings change in the debug window re-renders every
/// live button exactly as before.
public struct FeedButton: View {
    /// The color family a ``FeedButton`` renders in.
    ...
    /// The size class a ``FeedButton`` renders at.
    public enum Size: Sendable {
        /// Filter bar / option pills.
        case compact
        /// Full-width CTAs.
        case medium
    }
...
#if DEBUG
    /// App-injected resolver for the Feed Button Style debug window. Absent (the
    /// default, and always in release builds) renders the byte-faithful
    /// production treatment.
    @Environment(\.feedButtonDebugStyleProvider) private var debugStyleProvider
#endif
    @State private var isHovered: Bool = false
#if DEBUG
    @AppStorage("feed.button.debug.generation") private var debugStyleGeneration = 0
#endif

    /// Creates a feed button.
    /// - Parameters:
    ///   - label: The visible title and accessibility help text.
    ///   - leadingIcon: Optional SF Symbol name shown before the label.
    ///   - trailingIcon: Optional SF Symbol name shown after the label.
    ///   - kind: The color family.
    ///   - size: The size class.
    ///   - fullWidth: When `true`, the button expands to its container width.
    ///   - isSelected: Selection state, affecting `ghost` styling.
    ///   - dimmed: When `true`, the button reads as disabled and swallows clicks.
    ///   - action: Invoked on click unless `dimmed`.
    public init(
        label: String,
...
#if DEBUG
    /// The app-side debug style for this button, resolved fresh on each access
    /// so a generation bump re-resolves it (matching the legacy static reads).
    private var resolvedDebugStyle: FeedButtonDebugStyle? {
        debugStyleProvider?.resolve(kind, colorScheme)
    }
#endif
    ...
    private var usesSystemGlassButtonStyle: Bool {
        ...
        guard let debugStyle = resolvedDebugStyle else { return false }
        switch debugStyle.visualStyle {
        ...
        private var systemGlassButton: some View {
            if resolvedDebugStyle?.visualStyle == .standardGlass {
                standardSystemGlassButtonBase
                    .buttonStyle(.glass)
            } else if resolvedDebugStyle?.visualStyle == .standardTintedGlass {
                standardSystemGlassButtonBase
                    .buttonStyle(.glass)
                    .tint(systemGlassTint)
            } else if resolvedDebugStyle?.visualStyle == .nativeProminentGlass {
    ...
    private var systemGlassTint: Color {
        glassEffectTint.opacity(resolvedDebugStyle?.glassTintOpacity ?? 0)
    }

    private var systemGlassForeground: Color {
        if let color = resolvedDebugStyle?.color(.foreground) {
            return color
        }

        switch resolvedDebugStyle?.visualStyle {
        case .nativeProminentGlass:
            return kind == .light ? .black : .white
        case .nativeGlass:
            return .primary
        case .standardGlass, .standardTintedGlass, .solid, .glass, .liquid, .halo, .command, .commandLight, .outline, .flat, nil:
            return foreground
        }
    }
#endif
...
#if DEBUG
        _ = debugStyleGeneration
        if let debugStyle = resolvedDebugStyle {
            return size == .compact
                ? debugStyle.compactCornerRadius
                : debugStyle.mediumCornerRadius
        }
    ...
    private var horizontalPadding: CGFloat {
        ...
        if let debugStyle = resolvedDebugStyle {
            return size == .compact
                ? debugStyle.compactHorizontalPadding
                : debugStyle.mediumHorizontalPadding
        }
    ...
    private var verticalPadding: CGFloat {
        ...
        if let debugStyle = resolvedDebugStyle {
            return size == .compact
                ? debugStyle.compactVerticalPadding
                : debugStyle.mediumVerticalPadding
        }
...
#if DEBUG
        _ = debugStyleGeneration
        if let color = resolvedDebugStyle?.color(isHovered ? .hoverBackground : .background) {
            return color
        }
#endif
        switch kind {
    ...
    private var buttonBackground: some View {
        ...
        let glassTintOpacity = resolvedDebugStyle?.glassTintOpacity ?? 0
        switch generation >= 0 ? (resolvedDebugStyle?.visualStyle ?? .solid) : .solid {
        case .solid:
        ...
        case .standardTintedGlass:
            shape
                ...
                .overlay(
                    shape.fill(
                        backgroundFill.opacity(glassTintOpacity)
                    )
                )
        case .glass:
            shape
        ...
        case .nativeGlass:
            ...
            if #available(macOS 26.0, *) {
                shape
                    ...
                    .glassEffect(
                        .regular
                            .tint(glassEffectTint.opacity(glassTintOpacity))
                            .interactive(!dimmed),
                        in: shape
                    )
            } else {
    ...
    private var buttonBorder: some View {
        ...
        let generation = debugStyleGeneration
        let shape = RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
        let borderWidth = resolvedDebugStyle?.borderWidth ?? 0
        switch generation >= 0 ? (resolvedDebugStyle?.visualStyle ?? .solid) : .solid {
        case .solid:
            EmptyView()
        case .standardGlass:
            shape.stroke(Color.white.opacity(0.12), lineWidth: borderWidth)
        case .standardTintedGlass:
            shape.stroke(backgroundFill.opacity(0.22), lineWidth: borderWidth)
        case .glass:
            shape.stroke(Color.white.opacity(0.16), lineWidth: 0.75)
        case .nativeGlass:
            shape.stroke(Color.white.opacity(0.14), lineWidth: borderWidth)
        case .nativeProminentGlass:
            shape.stroke(Color.white.opacity(0.18), lineWidth: borderWidth)
        case .liquid:
            shape.stroke(
                LinearGradient(
                    colors: [
        ...
        case .command:
            shape.stroke(Color.white.opacity(0.12), lineWidth: borderWidth)
        case .commandLight:
            shape.stroke(Color.black.opacity(0.12), lineWidth: borderWidth)
        case .outline:
            shape.stroke(backgroundFill.opacity(0.75), lineWidth: borderWidth)
        case .flat:
            EmptyView()
        }
#else
    ...
    private var buttonShadowColor: Color {
        ...
        case .nativeProminentGlass:
            return backgroundFill.opacity(isHovered || isSelected ? 0.18 : 0.10)
        case .standardGlass, .standardTintedGlass, .solid, .glass, .nativeGlass, .outline, .flat, nil:
            return Color.clear
        }
#else
        return Color.clear
    ...
    private var buttonShadowRadius: CGFloat {
        ...
        case .standardGlass, .standardTintedGlass, .solid, .glass, .nativeGlass, .outline, .flat, nil: return 0
        }
#else
        return 0
#endif
