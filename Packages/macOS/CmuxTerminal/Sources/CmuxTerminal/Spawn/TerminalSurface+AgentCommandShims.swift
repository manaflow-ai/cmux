...
extension TerminalSurface {
    ...
    public static func installAgentCommandShimsIfPossible(
        ...
        let availableDefinitions = TerminalSurfaceAgentCommandShimDefinition.bundled.compactMap { definition in
            let wrapperURL = wrapperDirectoryURL
                .appendingPathComponent(definition.wrapperName, isDirectory: false)
                .standardizedFileURL
            guard fileManager.isExecutableFile(atPath: wrapperURL.path) else { return nil }
            return (definition, wrapperURL)
        }
        ...
        let shims = availableDefinitions.compactMap { definition, wrapperURL in
            installAgentCommandShim(
