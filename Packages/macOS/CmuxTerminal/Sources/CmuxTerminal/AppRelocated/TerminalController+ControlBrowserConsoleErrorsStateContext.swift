...
extension TerminalController {
    ...
    func controlBrowserStateSave(
        ...
        case .success(let resolved):
            ...
            let cookies = (browserAutomation.cookies.allCookies(in: store) ?? []).map(browserAutomation.cookies.cookieDictionary(from:))
        ...
        case .success(let resolved):
            ...
            if let cookieRows = raw["cookies"] as? [[String: Any]] {
                ...
                for row in cookieRows {
                    if let cookie = browserAutomation.cookies.cookie(from: row, fallbackURL: browserPanel.currentURL) {
                        _ = browserAutomation.cookies.setCookie(cookie, in: store)
                    }
                }
            }
