...
extension TerminalSurface {
    ...
    // Named keys have layout-independent meanings. Supply the same logical
    // text and unshifted codepoint as physical input; the keycode alone is not
    // enough for Kitty or modifyOtherKeys to encode printable keys.
    private static func canonicalKeyText(keycode: UInt32) -> String? {
        switch keycode {
        case UInt32(kVK_Delete): return "\u{7F}"
        ...
        default: return letterKeyCodes.first { $0.value == keycode }?.key
        }
    }
    ...
    func sendKeyEvent(
        ...
        if mods.rawValue & GHOSTTY_MODS_SHIFT.rawValue != 0 {
            text = canonicalText == "\\" ? "|" : canonicalText?.uppercased()
        } else {
            text = canonicalText
        }
        ...
        // that tears down/replaces the runtime, so never release into that one.
    ...
    private func keycodeForLetter(_ letter: Character) -> UInt32? {
        Self.letterKeyCodes[String(letter).lowercased()]
    }
