...
struct CodexLegacyRestoreCommandTests {
    ...
    @Test(arguments: [
        "codex resume other-session", "codex resume $(cat session)", "launcher codex resume session",
        "codex resume session; codex resume other", "codex resume session | cat", "codex --last",
        "env CODEX_HOME=$(pwd) codex resume session"
    ])
