...
struct AgentResumeShellScriptBuilderTests {
    ...
    func codexRetryPolicyUsesBoundedStartupFailureLoop() {
        ...
        #expect(script.contains(#"/bin/dd bs=1 count=4096 of="$1" 2>/dev/null"#))
        #expect(script.contains(#"_cmux_resume_script_output="""#))
        #expect(script.contains(#"/usr/bin/mkfifo "$_cmux_resume_pipe""#))
        #expect(script.contains(#"if [ -n "$_cmux_resume_capture_pid" ] && [ -x /usr/bin/script ]; then"#))
