...
/// absolute inline CODEX_HOME is required for local resumes because login-shell
/// startup can override inherited environment. Complex commands fail closed.
public struct CodexLegacyRestoreCommand: Sendable {
    ...
    public init?(command: String, sessionID: String) {
        ...
        guard CodexWriterRestorePreflight().usesRemoteProvider(arguments: arguments)
            || environment["CODEX_HOME"]?.hasPrefix("/") == true else { return nil }
