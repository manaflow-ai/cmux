...
public struct AgentResumeShellScriptBuilder: Sendable, Equatable {
    ...
    private func retryingCommandLines(command: String, retryPolicy: AgentResumeRetryPolicy) -> [String] {
        ...
        return [
            ...
            #"  fi"#,
            #"  if [ -n "$_cmux_resume_pipe" ]; then"#,
            #"    rm -f -- "$_cmux_resume_pipe" 2>/dev/null || true"#,
            #"  fi"#,
            #"}"#,
            #"_cmux_resume_capture_log() {"#,
            #"  /bin/dd bs=1 count=4096 of="$1" 2>/dev/null"#,
            #"  /bin/cat >/dev/null"#,
            #"}"#,
            #"trap _cmux_resume_cleanup_log EXIT INT TERM"#,
            #"while true; do"#,
            #"  _cmux_resume_log="${TMPDIR:-/tmp}/cmux-agent-resume-${$}-${_cmux_resume_retry}.log""#,
            #"  _cmux_resume_pipe="${TMPDIR:-/tmp}/cmux-agent-resume-${$}-${_cmux_resume_retry}.fifo""#,
            #"  rm -f -- "$_cmux_resume_log" "$_cmux_resume_pipe" 2>/dev/null || true"#,
            #"  : > "$_cmux_resume_log" 2>/dev/null && chmod 600 "$_cmux_resume_log" 2>/dev/null || true"#,
            #"  _cmux_resume_script_output="""#,
            #"  _cmux_resume_capture_pid="""#,
            #"  if [ -x /usr/bin/script ] && [ -x /usr/bin/mkfifo ] && /usr/bin/mkfifo "$_cmux_resume_pipe" 2>/dev/null; then"#,
            #"    _cmux_resume_capture_log "$_cmux_resume_log" < "$_cmux_resume_pipe" &"#,
            #"    _cmux_resume_capture_pid=$!"#,
            #"    _cmux_resume_script_output="$_cmux_resume_pipe""#,
            #"  fi"#,
            #"  _cmux_resume_started_at=$(/bin/date +%s 2>/dev/null || echo 0)"#,
            #"  case "${_cmux_resume_shell:t}" in"#,
            #"    zsh|bash)"#,
            #"      if [ -n "$_cmux_resume_capture_pid" ] && [ -x /usr/bin/script ]; then"#,
            #"        /usr/bin/script -q -F "$_cmux_resume_script_output" "$_cmux_resume_shell" -lic "$_cmux_resume_command""#,
            #"      else"#,
            ...
            #"    csh|tcsh)"#,
            #"      if [ -n "$_cmux_resume_capture_pid" ] && [ -x /usr/bin/script ]; then"#,
            #"        /usr/bin/script -q -F "$_cmux_resume_script_output" "$_cmux_resume_shell" -c "$_cmux_resume_command""#,
            #"      else"#,
            ...
            #"  esac"#,
            #"  _cmux_resume_status=$?"#,
            #"  if [ -n "$_cmux_resume_capture_pid" ]; then"#,
            #"    wait "$_cmux_resume_capture_pid" 2>/dev/null || true"#,
            #"  fi"#,
            #"  rm -f -- "$_cmux_resume_pipe" 2>/dev/null || true"#,
