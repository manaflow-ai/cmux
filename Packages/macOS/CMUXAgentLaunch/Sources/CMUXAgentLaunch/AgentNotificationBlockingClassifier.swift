...
/// Decides whether an agent notification describes a genuinely blocking prompt
/// (the user must act now) rather than a routine waiting/idle reminder or a
/// completion.
///
...
/// prose) and only when `isBlockingPrompt` returns true. Everything else leaves the
/// lifecycle untouched so a finished agent stays hibernation-eligible across every
/// agent kind.
///
...
public enum AgentNotificationClassifier {
    /// `signal` is the structured event/reason field; `message` is the free text.
    public static func isBlockingPrompt(signal: String, message: String) -> Bool {
