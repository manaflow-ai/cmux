...
/// Detects which coding agent a terminal is running by inspecting workspace metadata
/// lines (restored agent identity, agent PID key, initial/tmux start commands) and
/// resolving the leading command through ``CmuxTaskManagerCodingAgentDefinition``.
public enum TextBoxAgentDetection: CaseIterable {
    case claudeCode
    case codex
    case opencode
    case pi
    ...
    /// Whether any metadata line in `context` identifies this agent.
    ...
    /// Whether the terminal `context` identifies any agent that supports prompt prefixes.
    public static func supportsAgentPrefixes(context: String) -> Bool {
        allCases.contains { $0.matches(context: context) }
    }

    /// Whether the terminal `context` identifies Claude Code specifically.
    public static func isClaudeCode(context: String) -> Bool {
