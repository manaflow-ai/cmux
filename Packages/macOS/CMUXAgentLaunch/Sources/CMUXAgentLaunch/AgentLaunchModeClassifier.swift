...
public enum AgentLaunchModeClassifier {
    ...
    static func mode(kind: String, arguments: [String]) -> AgentProcessLaunchMode {
        ...
        if containsOption(oneShotOptions(for: kind), in: arguments, policy: policy) {
            return .oneShot
        }
        if containsOption(interactiveOptions(for: kind), in: arguments, policy: policy) {
            return .interactive
        }
        if containsOption(unknownOptions(for: kind), in: arguments, policy: policy) {
            return .unknown
        }
