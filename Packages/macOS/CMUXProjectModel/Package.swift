...
let package = Package(
    ...
    // the tools-version declaration. The build and test entry points set
    // SWIFTPM_MIRROR_CONFIG to the tracked config/swiftpm/mirrors.json. Remove the
    // mirror and lockfile pin when
    // upstream PathKit publishes a modern manifest.
    dependencies: [
        .package(
