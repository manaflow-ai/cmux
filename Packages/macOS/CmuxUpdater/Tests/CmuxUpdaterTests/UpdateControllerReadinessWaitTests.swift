...
/// Tests for the bounded readiness-wait continuation predicate,
/// ``UpdateController/readinessWaitShouldContinue(whileModelState:)``.
///
...
@Suite struct UpdateControllerReadinessWaitTests {
    ...
    @Test func keepsWaitingWhileInstallPromptReResolves() {
        #expect(UpdateController.readinessWaitShouldContinue(whileModelState: updateAvailable))
    }
    ...
    @Test func keepsWaitingWhileCheckingPillAwaitsReadiness() {
        #expect(UpdateController.readinessWaitShouldContinue(whileModelState: .checking(.init(cancel: {}))))
    }
    ...
    @Test func stopsWaitingWhenPendingCheckIsCancelledToIdle() {
        #expect(!UpdateController.readinessWaitShouldContinue(whileModelState: .idle))
    }
}
