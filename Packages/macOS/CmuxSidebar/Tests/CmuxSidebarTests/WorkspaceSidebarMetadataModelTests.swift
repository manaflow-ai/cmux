...
@Suite struct WorkspaceSidebarMetadataModelTests {
    ...
    @Test func metadataBlocksStayBoundedUnderUnboundedDistinctKeys() {
        let model = makeModel()
        let cap = WorkspaceSidebarMetadataModel.maxMetadataBlocks

        for index in 0..<(cap * 3) {
            model.metadataBlocks["key_\(index)"] = SidebarMetadataBlock(
                key: "key_\(index)",
        ...
        #expect(model.metadataBlocks.count <= cap)
        #expect(model.metadataBlocks["key_\(cap * 3 - 1)"] != nil)
        #expect(model.metadataBlocks["key_0"] == nil)
    }

    @Test func metadataCapRetainsHighPriorityOverNewerLowPriorityFlood() {
        let model = makeModel()
        let cap = WorkspaceSidebarMetadataModel.maxMetadataBlocks

        model.metadataBlocks["important"] = SidebarMetadataBlock(
            key: "important",
            markdown: "m",
            priority: 100,
        ...
        for index in 0..<(cap * 2) {
            model.metadataBlocks["low_\(index)"] = SidebarMetadataBlock(
                key: "low_\(index)",
                markdown: "m",
                priority: 0,
        ...
        #expect(model.metadataBlocks.count <= cap)
        #expect(model.metadataBlocks["important"] != nil)
    }
