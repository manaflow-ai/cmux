...
///
/// Observer parity: the legacy properties were `@Published`, and the sidebar
/// observation publishers (`Workspace.sidebarObservationPublisher`) fused their
/// `$projection`s through `CombineLatest` + `removeDuplicates()`. Each property
/// here mirrors its retained value into a `CurrentValueSubject` in `didSet`; the
/// matching `…Publisher` accessor replaces the former `$property`.
@MainActor
@Observable
public final class WorkspaceSidebarMetadataModel {
    /// Upper bound on retained sidebar markdown metadata blocks per workspace.
    public static let maxMetadataBlocks = 200
    ...
    private static func cappedMetadataBlocksForDisplay(
        ...
        let keptKeys = Set(
            blocks
                ...
                .prefix(Self.maxMetadataBlocks)
                .map(\.key)
        )
        return blocks.filter { keptKeys.contains($0.key) }
    }
}
