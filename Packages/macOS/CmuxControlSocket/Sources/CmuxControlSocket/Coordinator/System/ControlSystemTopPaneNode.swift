...
/// One pane row of a `system.top` / `system.memory` workspace (the legacy
/// per-pane dictionary of `v2TopWorkspaceNode`, minus the coordinator-minted
/// refs and the process-annotation fields the nonisolated `system.top` pipeline
/// adds afterward).
public struct ControlSystemTopPaneNode: Sendable, Equatable {
    ...
    public let surfaces: [ControlSystemTopSurfaceNode]
    ...
    public init(
        ...
        surfaces: [ControlSystemTopSurfaceNode]
    ) {
        self.paneID = paneID
        self.index = index
