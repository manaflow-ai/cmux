...
///
/// The coordinator parses `name` / legacy `cwd` / `child_workspace_ids`
/// (resolving each child through the handle registry) and surfaces the
/// param-shape failures (`invalid_params` for a malformed
/// `child_workspace_ids` or unresolved handles) itself. The remaining
/// resolution depends on live app state — the
...
public enum ControlWorkspaceGroupCreateResolution: Sendable, Equatable {
    ...
    /// Every explicitly-listed child is already a group anchor, so there is
    /// no eligible child to use as the new group's ordering anchor (legacy
    /// `invalid_state` / `workspaceGroup.error.allChildrenAreAnchors`, `data:
