...
extension AgentLaunchSanitizer {
    ...
    static let campfirePolicy: Policy = {
        var policy = piPolicy
        policy.valueOptions.formUnion(["--relay", "--join-as", "--name"])
        policy.nonRestorableCommands.insert("init")
        policy.droppedOptions.formUnion(["--join-as", "--name", "--auto-exit"])
        policy.droppedOptionPrefixes.append(contentsOf: ["--join-as=", "--name="])
        return policy
    }()
