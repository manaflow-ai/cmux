...
final class TerminalInputTextView: UIView, UIKeyInput, UITextInput {
    ...
    /// one paste instead of executing line-by-line (falling back to ``onText`` on
    /// a host without the `terminal.paste.v1` capability). Images win when both
    /// are present. Accessing the pasteboard contents here is what shows iOS's
