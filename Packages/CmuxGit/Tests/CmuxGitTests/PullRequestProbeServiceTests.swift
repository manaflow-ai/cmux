...
@Suite struct PullRequestProbeServiceTests {
    ...
    @Test func refreshAllowsRepoCacheForTimerAndPeriodicReasons() {
        for reason in ["periodicPoll", "periodicPoll.followUp", "selectedPeriodicPoll", "selectedPeriodicPoll.followUp", "timer", "timer.followUp", "shellPrompt", "shellPrompt.followUp"] {
            #expect(PullRequestProbeService.refreshAllowsRepoCache(reason: reason), "\(reason) should allow cache")
        }
        for reason in ["branchChange", "branchChange.followUp", "commandHint:merge"] {
            #expect(!PullRequestProbeService.refreshAllowsRepoCache(reason: reason), "\(reason) should bypass cache")
        }
    }

    @Test func refreshOnlyForcesImmediateForStructuralReasons() {
        for reason in ["shellPrompt", "shellPrompt.followUp"] {
            #expect(!PullRequestProbeService.refreshForcesImmediate(reason: reason), "\(reason) should keep the scheduled poll")
        }
        for reason in ["branchChange", "directoryChange", "localGitProbe", "commandHint:merge"] {
            #expect(PullRequestProbeService.refreshForcesImmediate(reason: reason), "\(reason) should force an immediate refresh")
        }
    }
