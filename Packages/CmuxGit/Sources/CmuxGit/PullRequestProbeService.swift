...
public struct PullRequestProbeService: Sendable {
    ...
    /// Whether a refresh triggered by `reason` should bypass the normal due-time gate.
    ///
    /// Shell prompts are high-frequency and may arrive many times while an agent is active,
    /// so they keep the next scheduled poll instead of forcing an immediate request every time.
    public static func refreshForcesImmediate(reason: String) -> Bool {
