...
struct AgentModelCatalogTests {
    ...
    @Test func familyPrefixesDoNotBorrowAnotherModelsPrice() throws {
        ...
        #expect(catalog.info(forModelID: "gpt-5.2")?.pricing == nil)
        #expect(catalog.info(forModelID: "gpt-5")?.pricing?.inputPerMTok == 1.25)
    }

    @Test func legacyAndUnknownIDsFallBackToRawName() throws {
        ...
        let codex = try #require(catalog.info(forModelID: "gpt-6-astra"))
        #expect(codex.contextWindow == nil)
    ...
    @Test func snapshotFractionClampsAndHandlesUnknownWindow() {
        let over = AgentUsageSnapshot(modelID: "m", modelDisplayName: "m", contextTokens: 300, contextWindow: 200, estimatedCostUSD: nil)
        #expect(over.contextFraction == 1)
        let unknown = AgentUsageSnapshot(modelID: "m", modelDisplayName: "m", contextTokens: 300, contextWindow: nil, estimatedCostUSD: nil)
