...
extension V2ControlService {
    func requestDirectoryRefresh(run: UUID) {
        guard directorySyncTask == nil, socket != nil else { return }
        directorySyncTask = Task { [weak self] in
            ...
            await self.directorySyncFinished(run: run)
        }
    }

    private func directorySyncFinished(run: UUID) {
    ...
    private func maintain(run: UUID) async {
        while runID == run, status == .ready, !Task.isCancelled {
            ...
            guard runID == run, !Task.isCancelled, socket != nil else { return }
