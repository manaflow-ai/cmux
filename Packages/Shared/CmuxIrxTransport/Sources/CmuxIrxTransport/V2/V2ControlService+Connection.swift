...
extension V2ControlService {
    ...
    private func open(run: UUID) async throws {
        ...
        var proof: V2DeviceProof?
        let authorization: String
        if let ticket, !forceStackOnNextSetup, !cache.authorityRevoked {
            let issuedAt = Int(dependencies.now().timeIntervalSince1970)
            let bytes = try codec.request(device: descriptor, requestID: requestID, issuedAt: issuedAt, body: unsigned)
            let signature = try await dependencies.sign(bytes)
            try assertCurrent(run)
            proof = V2DeviceProof(issuedAt: issuedAt, requestID: requestID, signature: codec.base64URL(signature))
        ...
        let opened = try await dependencies.connect(request)
        ...
        failure = nil
        publish()
        // These requests cannot gate a cached peer connection. Each has one owner.
    ...
    func terminal(_ error: V2ControlFailure) -> Bool {
        switch error {
        case .scopeMismatch, .persistenceFailed: return true
    ...
    private func retryDelay(_ error: V2ControlFailure, attempt: Int) -> TimeInterval {
