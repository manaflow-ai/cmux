...
///
/// The wire format is unchanged from the legacy transport (the signing server
/// does not change in v3): an EdDSA compact JWS whose signing input is the
/// ASCII bytes of `base64url(header) + "." + base64url(payload)`.
public struct PeerRelayPolicyVerifier: Sendable {
    ...
    /// - Throws: ``PeerRelayPolicyError`` for signature, shape, or time failures.
    public func verify(
        _ token: String,
        trustRoot: PeerRelayPolicyTrustRoot,
        now: Date
    ) throws -> PeerRelayPolicy {
        ...
        do {
            ...
            throw PeerRelayPolicyError.invalidHeader
        }
        guard header.alg == "EdDSA",
              header.typ == Self.tokenType,
              PeerRelayPolicyVerificationKey.isSafeKeyID(header.kid) else {
            throw PeerRelayPolicyError.invalidHeader
        }
        guard let verificationKey = trustRoot.key(id: header.kid) else {
            throw PeerRelayPolicyError.unknownKeyID
        }
        ...
        guard publicKey.isValidSignature(signature, for: signingInput) else {
            throw PeerRelayPolicyError.invalidSignature
        }
    ...
    private static func validate(
        ...
        guard policy.relayProtocol == relayProtocol else {
            throw PeerRelayPolicyError.unsupportedRelayProtocol
        }
        ...
        guard policy.notBefore <= futureTolerance.partialValue else {
            throw PeerRelayPolicyError.notYetValid
        }
        guard policy.expiresAt > nowSeconds else {
            throw PeerRelayPolicyError.expired
        }
    }

    private static func validRelay(_ relay: PeerRelayDescriptor) -> Bool {
