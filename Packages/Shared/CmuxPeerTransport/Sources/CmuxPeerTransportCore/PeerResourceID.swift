/// A bounded opaque identifier for a terminal or artifact resource.
public struct PeerResourceID: Equatable, Hashable, Sendable {
    ...
    /// - Throws: ``PeerResourceIDError/invalidValue`` for unsafe values.
    public init(_ value: String) throws {
        let bytes = Array(value.utf8)
        guard (1 ... 128).contains(bytes.count), bytes.allSatisfy(Self.isAllowed) else {
            throw PeerResourceIDError.invalidValue
        }
        self.value = value
    }
...
/// Validation failures for identifiers carried in peer lane headers.
public enum PeerResourceIDError: Error, Equatable, Sendable {
