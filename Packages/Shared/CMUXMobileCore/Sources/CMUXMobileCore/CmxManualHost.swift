...
public struct CmxManualHost: Equatable, Sendable {
    ...
    public init?(_ rawHost: String) {
        guard let normalized = Self.normalize(rawHost) else {
            return nil
        }
        rawValue = normalized
    }
    ...
    private static func normalize(_ rawHost: String) -> String? {
        ...
        if host.contains(":"), let canonicalIPv6 = canonicalIPv6Literal(host) {
        ...
        if numericHost.contains("."), numericHost.utf8.allSatisfy({
            ...
            guard let canonicalIPv4 = canonicalIPv4Literal(numericHost),
    ...
    private static func canonicalIPv4Literal(_ host: String) -> String? {
        var address = in_addr()
    ...
    private static func canonicalIPv6Literal(_ host: String) -> String? {
