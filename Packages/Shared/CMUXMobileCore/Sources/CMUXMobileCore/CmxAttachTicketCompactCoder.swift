...
/// compatibility skew. A pairing QR never expires. Token-bearing tickets are
/// short-lived, Mac-minted credentials; the compact QR grammar drops the token
/// so the scannable code stays non-secret.
///
/// Compatibility:
...
public struct CmxAttachTicketCompactCoder: Sendable {
    ...
    /// intentionally not encoded: the compact QR grammar is non-secret, the
    /// name is read post-handshake from `mobile.host.status`, and a pairing QR
    /// never expires.
