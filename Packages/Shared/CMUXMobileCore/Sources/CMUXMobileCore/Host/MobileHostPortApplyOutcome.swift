...
/// Outcome of an explicit "Apply port" request from settings.
///
/// A pure value so the pre-bind classification is unit-testable without binding
/// a real `NWListener`. The `@MainActor` apply code lives app-side and forwards
/// to ``preBind(enabled:currentBoundPort:requestedPort:)`` for the cases that
/// need no bind attempt.
