...
public struct CmxAttachTicket: Codable, Equatable, Sendable {
    ...
    /// that never expire (the minimal pairing QR carries no token and no
    /// expiry). Expiry is data for the token consumers (`MobileCoreRPCClient`,
    /// the host's ticket store), not a structural validity condition; see
    /// ``isExpired(at:)``.
    ...
    /// (the minimal QR is not a token-bearing credential). Token-based
