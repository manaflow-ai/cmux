...
public struct CmxReconnectBackoffConfiguration: Equatable, Sendable {
    ...
    /// The largest server Retry-After directive honored as a delay floor.
    /// Anything above one day is treated as one day.
    public static let maximumServerRetryAfterSeconds = 24 * 60 * 60
...
public final class CmxReconnectBackoff: Sendable {
    private struct State {
        var previousDelay: TimeInterval?
        var rngState: UInt64
    }

    private let configuration: CmxReconnectBackoffConfiguration
    ...
    public init(
        configuration: CmxReconnectBackoffConfiguration = .foreground,
    ...
    ///   ``CmxReconnectBackoffConfiguration/maximumServerRetryAfterSeconds``.
