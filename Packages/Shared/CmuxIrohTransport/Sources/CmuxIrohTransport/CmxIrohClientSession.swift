...
public actor CmxIrohClientSession {
    ...
    /// firewall path can hold the whole reconnect owner until its much larger
    /// outer deadline. Cancellation crosses the Iroh FFI boundary, so the
    /// timed-out attempt is released before the next phase begins.
