...
public actor CmxIrohRelayPolicyCache {
    ...
    public func install(
        ...
        if let existing {
            guard policy.sequence > existing.highestSequence
                    || (policy.sequence == existing.highestSequence
                        && signedPolicy == existing.signedPolicy) else {
    ...
    public func load(
        ...
        guard policy.sequence == record.highestSequence else {
            throw CmxIrohRelayPolicyError.rollback
        }
        return policy
    }
