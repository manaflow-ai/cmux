...
public actor CmxConnectivityEngine {
    ...
    /// Returns the live path in the shared mobile transport vocabulary.  The
    /// diagnostic classifier above remains redacted; this boundary is used only
    /// for the in-process status surface and therefore may retain a concrete
    /// private address for the lifetime of the app.
    public func currentTransportPath(
        relayPolicy: CmxIrohEffectiveRelayPolicy?
    ) async -> CmxTransportPath {
        var selected: (
            peerID: CmxConnectivityPeerID,
            path: CmxIrohObservedConnectionPath
        )?
        for (peerID, peer) in peers {
            ...
            if let current = selected,
               !Self.pathSelectionPrecedes(
                ...
                // Keep the same deterministic peer ordering used by the
                // redacted API; a warm control peer must not replace focus.
                continue
            }
            selected = (peerID, candidate)
        }
        return Self.mobileTransportPath(
            from: selected?.path ?? .unavailable,
            relayPolicy: relayPolicy
        )
    }

    /// Emits the initial selected path and every subsequent path migration.
    public func currentTransportPathChanges(
        relayPolicy: CmxIrohEffectiveRelayPolicy?
    ) -> AsyncStream<CmxTransportPath> {
        let changes = selectedTransportPathChanges()
        return AsyncStream(bufferingPolicy: .bufferingNewest(1)) { continuation in
            let task = Task {
                continuation.yield(await self.currentTransportPath(relayPolicy: relayPolicy))
    ...
    private static func mobileTransportPath(
        ...
        case let .relay(url):
            let classified = CmxIrohSelectedTransportPathClassifier(policy: relayPolicy)
                .classify(.relay(url: url))
            switch classified {
            case let .managedRelay(_, region), let .customRelay(_, _, region):
                return .irohRelay(region: region)
            case .unavailable, .direct, .privateNetwork:
