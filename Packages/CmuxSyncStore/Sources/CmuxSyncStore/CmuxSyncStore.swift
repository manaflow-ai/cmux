...
public actor CmuxSyncStore: CmuxSyncStoring {
    ...
    // FULLMUTEX, matching MobilePairedMacStore. Package-internal (not `private`)
    // so the SQLite statement-helper extension in CmuxSyncStore+SQLite.swift can
    // reach it; still not part of the package's public API.
