...
/// Hand-curated settings-search table.
///
/// Ported from the legacy `SettingsSearchIndex.settingEntries` +
/// `SettingsSearchAliasIndex.settingAliases` tables in
/// `Sources/SettingsNavigation.swift` and `Sources/SettingsSearchAliases.swift`.
/// Each row is a (section, id, title, synonyms) tuple that surfaces a
/// specific setting in the search index with its natural name + the
/// search synonyms users actually type.
///
/// Today these strings are English-only; localization will come once
/// the package ships an `xcstrings` catalog. Until then, search still
/// works for English queries and the search index also still indexes
/// the raw catalog dotted-ids as a fallback.
enum CuratedSettingEntries {
    struct Entry {
    ...
    static let entries: [Entry] = [
        ...
        .init(section: .terminal, id: "textbox-max-lines", title: "Text Box Max Lines", synonyms: "textbox text box rich input prompt max height lines grow scroll"),
        ...
        .init(section: .sidebarAppearance, id: "sidebar-branch-layout", title: "Sidebar Branch Layout", synonyms: "sidebar.branchLayout git branch layout vertical inline cwd directory"),
