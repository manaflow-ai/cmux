...
public final class MobileShellComposite: MobileTerminalOutputSinking {
    ...
    public func pairAdditionalMac(_ rawValue: String) async -> Bool {
        ...
        case .needsUserApproval:
            // The scan triggered a version-skew warning; the connect has not
            // happened yet and the previous session is untouched. Leave the
            // pairing pending so the host picker's compatibility alert can drive
            // accept/cancel, and do not dismiss the sheet.
            return false
        case .superseded:
    ...
    private func resetTerminalOutputTracking() {
        ...
        // Clearing the capability set also clears paste support, so a reconnect
        // to an older host cannot send `terminal.paste` on a stale value before
        // the next status probe lands.
