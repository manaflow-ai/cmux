...
/// Removes terminal query REQUESTS from historical output before a local
/// emulator replays it.
///
/// Replaying retained output (a remounted surface, a reattach snapshot)
/// feeds the local emulator the queries the program sent long ago: DA1/DA2,
/// DSR/CPR, DECRQM, XTVERSION, XTWINOPS size reports, OSC 4/10/11/12 color
/// queries, DECRQSS/XTGETTCAP, and Kitty queries. An emulator that forwards
/// its replies to the PTY (SSH plain/tmux) would answer them again, and the
/// running program would read the answers as typed input. Only live output
/// may produce replies, so history is filtered before it is replayed.
///
/// Ported from the macOS persistent-SSH replay filter
/// (`CmuxFoundation.SSHPTYReplayOutputFilter`, manaflow-ai/cmux#10332). Only
/// the declared replay prefix is inspected; bytes after it pass through, and
/// malformed or oversized candidates fail open so visible output is not lost.
public struct TerminalReplayQueryFilter: Sendable {
    /// Filters one complete historical buffer (every byte is replay).
    public static func removingQueryRequests(_ replay: Data) -> Data {
    ...
    private static func oscQuery(in bytes: [UInt8], at start: Int) -> SequenceMatch {
        ...
        while cursor < bytes.count {
            ...
            if bytes[cursor] == Self.bell {
                return isColorQuery(command: command, commandDigitCount: commandDigitCount, payload: bytes[payloadStart..<cursor])
                    ? .strip(length: cursor - start + 1)
                    : .passThrough
            }
    ...
    /// OSC 10/11/12 query with `?` as the (first) color spec; OSC 4 is
    /// `index;spec` pairs and is a query when any spec is `?`.
    private static func isColorQuery(
        command: Int,
        commandDigitCount: Int,
        payload: ArraySlice<UInt8>
    ) -> Bool {
        guard commandDigitCount > 0 else { return false }
        switch command {
        case 10, 11, 12:
            return payload.first == 0x3F
        case 4:
            let fields = payload.split(separator: Self.semicolon, omittingEmptySubsequences: false)
            return stride(from: 1, to: fields.count, by: 2).contains { fields[$0].elementsEqual([0x3F]) }
        default:
            return false
        }
    }
