...
extension MobileShellComposite {
    ...
    func applyWorkspaceFocusEvent(_ event: MobileWorkspaceFocusEvent, macID: String?) {
        if let macID {
            ...
            Self.applyWorkspaceFocusEvent(event, to: &state.workspaces[index])
        ...
        mutateForegroundWorkspaces { workspaces in
            ...
            Self.applyWorkspaceFocusEvent(event, to: &workspaces[index])
        }
    }

    private static func applyWorkspaceFocusEvent(
