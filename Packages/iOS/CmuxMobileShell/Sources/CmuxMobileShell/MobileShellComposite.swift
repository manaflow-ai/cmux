...
public final class MobileShellComposite: MobileTerminalOutputSinking {
    ...
    private func performReconnectActiveMacAttempt(
        ...
        for (candidateIndex, mac) in candidates.enumerated() {
            ...
            // A relay-method pairing needs no locally persisted route: the
            // WebSocket route is synthesized at connect time, so the dial is
            // always possible.
            let localCanConnectSecurely = connectionMethod(for: mac) == .relay
                || localHasIroh
    ...
    private func performMacSwitch(
        ...
        // Same relay rule as the stored-Mac loop: the relay route is
        // synthesized at connect time, so a relay-method pairing always dials.
        let localCanConnectSecurely = connectionMethod(for: refreshedTarget) == .relay
            || localHasIroh
    ...
    func connect(
        ...
        routeLoop: for route in supportedRoutes {
            ...
            let client = MobileCoreRPCClient(
                ...
                // The pairing's Mac build tag pins a relay dial to that
                // build's own relay object; .adopt (no expectation) dials
                // the untagged object.
                expectedPeerInstanceTag: instanceTagExpectation.expectedTag,
                connectAttemptRegistry: connectAttemptRegistry,
                stackTokenGate: stackTokenGate,
                stackTokenForceRefreshGate: stackTokenForceRefreshGate,
                transportConnectObserver: transportConnectDiagnosticObserver(
    ...
    /// Whether ordered terminal input may skip the awaited round trip and
    /// pipeline through ``terminalInputRPCPipeline`` (up to four unsettled
    /// requests instead of one batch per RTT).
    ///
    /// Requires the host's ordered-input capability plus two route properties:
    ///
    /// - **Arrival order is preserved end to end.** Iroh provides one ordered
    ///   control stream. The relay provides one WebSocket per leg: the
    ///   Durable Object forwards data frames in arrival order and the Mac
    ///   decodes the RPC stream sequentially into its per-surface FIFO.
    ...
    private var pipelinedOrderedTerminalInputIsAvailable: Bool {
        ...
        case .iroh, .websocket:
            return true
        case .tailscale, .debugLoopback, nil:
            return false
        }
    }
