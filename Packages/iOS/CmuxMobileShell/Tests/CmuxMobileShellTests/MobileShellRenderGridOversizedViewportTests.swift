...
@MainActor
@Test func oversizedViewportEchoFromUnsupportedHostDoesNotEnableGuard() async throws {
    let clock = TestClock()
    let router = LivenessHostRouter()
    await router.setCapabilities([
        "events.v1", "terminal.bytes.v1", "terminal.render_grid.v1", "terminal.replay.v1",
    ...
    #expect(echoed?.columns == 90 && echoed?.rows == 40)
    ...
    try await waitForReplayResponsesServed(
        viewportReplayCount,
        router: router,
        "the unsupported-host setup replay count must settle before checking recovery requests"
    )
    ...
    await transport.deliver(try terminalBytesEventFrame(
        surfaceID: "live-terminal",
        seq: 9,
        text: "unsupported-host-bytes"
    ))
    let delivered = try await pollUntil { collector.lines.contains { $0.contains("unsupported-host-bytes") } }
    #expect(
        delivered,
