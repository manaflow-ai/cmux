...
@Suite struct DeviceRegistryRouteSelectionTests {
    ...
    private func peerRoute(
        routeID: String = "iroh",
        peerID: String = String(repeating: "a", count: 64),
        priority: Int = 0,
    ...
    @Test func registryPeerEndpointMetadataChangeIsWritten() throws {
        ...
        let cached = try peerRoute(pathHints: [CmxIrohPathHint(
            kind: .relayURL, value: "https://relay-old.example",
            source: .native, privacyScope: .publicInternet
        )])
        let updated = try peerRoute(pathHints: [CmxIrohPathHint(
            kind: .relayURL, value: "https://relay-new.example",
            source: .native, privacyScope: .publicInternet
        )])
        let selected = DeviceRegistryService.selectReconnectRoutes(local: [cached], registry: [updated])
