...
@Suite struct MobileConnectionLifecycleCanonicalRegressionTests {
    ...
    @Test func deferredCachedPersistenceCannotOverwriteLaterManualPairing() async throws {
        ...
        let writesAfterManualPairing = await pairedMacStore.currentUpsertCount()

        await pairedMacStore.release(teamID: nil)
        let staleWriteRan = try await pollUntil(attempts: 50) {
            await pairedMacStore.currentUpsertCount() > writesAfterManualPairing
        }
        #expect(!staleWriteRan)
        #expect(
            try await pairedMacStore.activeMac(
                stackUserID: "user-1",
                teamID: nil
    ...
    @Test func joinedNetworkChangeRefreshesStoredSecondaryMacs() async throws {
        let clock = TestClock()
        let router = LivenessHostRouter()
        let secondaryRoute = try loopbackRoute(id: "secondary", port: 51_009)
        let factory = RouteRecordingTransportFactory(
            router: router,
            box: TransportBox(),
            failingPorts: [51_009]
        ...
        let defaults = UserDefaults(suiteName: "joined-network-scope-\(UUID().uuidString)")!
        defaults.set(true, forKey: "multiMacAggregation")
        let store = MobileShellComposite(
            runtime: runtime,
            isSignedIn: true,
        ...
        #expect(try await pollUntil {
            factory.attemptedPorts().contains(51_009)
        })
        store.signOut()
    }
