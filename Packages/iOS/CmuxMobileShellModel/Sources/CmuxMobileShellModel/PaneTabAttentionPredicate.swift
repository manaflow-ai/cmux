...
public struct PaneTabAttentionPredicate: Sendable {
    ...
    ///
    /// The exact predicate is: the mirrored agent is waiting for user input
    /// (`agentStatus == .needsInput`) OR the layout DTO reports unread/bell
    /// activity (`hasUnread == true`). Running, idle, and unknown agents alone
