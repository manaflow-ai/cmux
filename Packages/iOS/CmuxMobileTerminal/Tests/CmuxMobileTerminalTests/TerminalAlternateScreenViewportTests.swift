...
struct TerminalAlternateScreenViewportTests {
    ...
    func transitionHoldsCapacity() {
        ...
        #expect(fence.snapshotForApply(portrait) == portrait)
        for _ in 0..<20 {
            #expect(!fence.sample(keyboard, transitionActive: true))
        }
        #expect(fence.snapshotForApply(keyboard) == portrait)
        // Rotation overlaps the keyboard leg; no earlier candidate can commit.
        #expect(!fence.sample(landscape, transitionActive: true))
        #expect(!fence.sample(landscape, transitionActive: false))
        #expect(!fence.sample(landscape, transitionActive: false))
        #expect(fence.sample(landscape, transitionActive: false))
        #expect(fence.snapshotForApply(landscape) == landscape)
    }
    ...
    func changingCandidateRestartsFence() {
        ...
        #expect(!fence.sample(intermediate, transitionActive: false))
        #expect(!fence.sample(intermediate, transitionActive: false))
        #expect(!fence.sample(final, transitionActive: false))
        #expect(fence.snapshotForApply(final) == initial)
        #expect(!fence.sample(final, transitionActive: false))
        #expect(fence.sample(final, transitionActive: false))
    ...
    func settledKeyboardMovesGridInsideSurface() {
        ...
        #expect(snapshot.layoutViewportRect.maxY == 874 - 34 - 120 - 44 - 8)
    }
