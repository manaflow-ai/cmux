...
public protocol GhosttySurfaceViewDelegate: AnyObject {
    ...
    /// Bytes generated by libghostty for the terminal's PTY (for example,
    /// device-attribute, cursor-position, and focus-report responses).
    ///
    /// Paired-Mac mirrors must not forward these bytes because the Mac owns the
    /// real PTY and already generates its own responses. A local terminal host
    /// can implement this callback to write the bytes back to its embedded PTY.
    /// Optional.
...
public extension GhosttySurfaceViewDelegate {
    ...
    func ghosttySurfaceView(_ surfaceView: GhosttySurfaceView, didChangeWindowAttachment isAttached: Bool) {}
    /// Default no-op so paired-Mac mirrors continue to discard local renderer
    /// responses unless a local PTY host explicitly opts in.
    func ghosttySurfaceView(
        _ surfaceView: GhosttySurfaceView,
        didProduceTerminalOutput data: Data
    ) {}
