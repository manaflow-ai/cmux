...
func handleBottomScrollStressDebugAction(_ action: ghostty_action_s, target: ghostty_target_s) -> Bool {
    ...
    if target.tag == GHOSTTY_TARGET_SURFACE, let surface = target.target.surface {
        Task { @MainActor in
            GhosttySurfaceView.view(for: surface)?.recordBottomScrollStressScrollbar(
                total: Int(sb.total),
