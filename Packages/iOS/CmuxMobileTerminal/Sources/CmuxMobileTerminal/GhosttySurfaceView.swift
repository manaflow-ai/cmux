...
public final class GhosttySurfaceView: UIView, TerminalSurfaceHosting {
    ...
    private func applyPendingFontSizeIfNeeded() -> Bool {
        ...
        let operation = SurfaceQueueOperation(
            surface: surface,
    ...
    func processOutput(
        ...
        let operation = SurfaceQueueOperation(surface: surface, generation: generation)
        workQueue.async { [weak self] in
            if let preparedConfigBits,
               let preparedConfig = ghostty_config_t(bitPattern: preparedConfigBits) {
                ghostty_surface_update_theme_config(operation.surface, preparedConfig)
            ...
            if a11yNow - workQueue.lastAccessibilityTextTime > 0.5 {
                ...
                accessibilityText = Self.accessibilitySurfaceText(operation.surface)
            }
            #endif
    ...
    func enqueueSurfaceFree(
        ...
        queue.async { [weak self] in
            let userdata = ghostty_surface_userdata(operation.surface)
            ghostty_surface_free(operation.surface)
            GhosttySurfaceBridge.releaseRetainedOpaque(userdata)
            Task { @MainActor in
                self?.surfaceFreeDrainWatchdog.cancel(generation: operation.generation)
                completion?()
            }
        }
    }
    ...
    private func requestRender() {
        ...
        outputQueue.async { [weak self] in
            ...
            ghostty_surface_render_now(operation.surface)
            DispatchQueue.main.async {
                guard let self else { return }
                guard self.surfaceGeneration == operation.generation else { return }
                #if DEBUG
    ...
    private func syncSurfaceGeometry(
        ...
        outputQueue.async { [weak self] in
            if pushContentScale {
                ghostty_surface_set_content_scale(operation.surface, scale, scale)
            }
            if !deferShrinkResize {
                ghostty_surface_set_size(operation.surface, containerPxW, containerPxH)
            }
            let measured = ghostty_surface_size(operation.surface)
            ...
            if let eff, !deferShrinkResize, eff.cols > 0, eff.rows > 0, cell.width > 0, cell.height > 0 {
                ...
                if shouldFitEffectiveGrid,
                   pinnedW + 0.5 < containerW || pinnedH + 0.5 < containerH {
                    let fitted = Self.fitSurfaceToGrid(
                        operation.surface,
                        cols: eff.cols,
                        rows: eff.rows,
                        cellPixelSize: cell
            ...
            Task { @MainActor in
                ...
                guard self.surfaceGeneration == operation.generation else {
                    completion?(false)
                    return
                }
...
/// One generation-bound raw pointer captured by a serial surface-queue operation.
///
/// Safety: the view owns the pointer until its generation's FIFO queue reaches
/// the queued free operation. Every dereference of this value occurs on that
/// same `GhosttySurfaceWorkQueue`; the wrapper conveys that ownership boundary
/// to Swift concurrency without making arbitrary raw pointers Sendable.
nonisolated private struct SurfaceQueueOperation: @unchecked Sendable {
    let surface: ghostty_surface_t
    let generation: UInt64
}
