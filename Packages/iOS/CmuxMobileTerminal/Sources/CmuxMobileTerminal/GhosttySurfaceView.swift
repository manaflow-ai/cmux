...
public final class GhosttySurfaceView: UIView, TerminalSurfaceHosting {
    ...
    public func applyConfirmedViewSize(cols: Int, rows: Int) {
        applyConfirmedViewSize(cols: cols, rows: rows, matchingReportedSize: nil)
    }
    ...
    public func applyConfirmedViewSize(
        cols: Int,
        rows: Int,
        ...
        applyViewSize(
            cols: cols,
            rows: rows,
            confirmedViewportEcho: true,
            matchingReportedSize: reportedSize
        )
    }

    public func markViewportReportConfirmed() {
        markViewportReportConfirmed(matchingReportedSize: nil)
    }

    public func markViewportReportConfirmed(matchingReportedSize reportedSize: TerminalGridSize?) {
        let confirmation = confirmViewportEcho(matchingReportedSize: reportedSize, effectiveGrid: nil)
        guard confirmation.isCurrent else { return }
        if confirmation.clearedPendingEcho {
    ...
    private func applyViewSize(
        ...
        let confirmation: (isCurrent: Bool, clearedPendingEcho: Bool)
        if confirmedViewportEcho {
            confirmation = confirmViewportEcho(
                matchingReportedSize: reportedSize,
                effectiveGrid: (cols: cols, rows: rows)
            )
        } else {
            confirmation = (isCurrent: true, clearedPendingEcho: false)
        }
        guard confirmation.isCurrent else { return }
        let changed = updateEffectiveGrid(cols: cols, rows: rows)
        guard changed || confirmation.clearedPendingEcho else { return }
    ...
    private func confirmViewportEcho(
        matchingReportedSize reportedSize: TerminalGridSize?,
        effectiveGrid responseGrid: (cols: Int, rows: Int)?
    ) -> (isCurrent: Bool, clearedPendingEcho: Bool) {
        ...
        guard viewportEchoPolicy.responseMatchesLatestReport(
            ...
            MobileDebugLog.anchormux(
                "zoom.viewport.stale reported=\(reportedSize.map { "\($0.columns)x\($0.rows)" } ?? "nil") "
                + "latest=\(lastReportedSize.map { "\($0.columns)x\($0.rows)" } ?? "nil")"
                + (responseGrid.map { " eff=\($0.cols)x\($0.rows)" } ?? "")
            )
            return (isCurrent: false, clearedPendingEcho: false)
        }
        ...
        return (isCurrent: true, clearedPendingEcho: clearsPendingEcho)
    }
