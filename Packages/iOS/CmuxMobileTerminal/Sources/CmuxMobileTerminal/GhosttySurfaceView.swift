...
public final class GhosttySurfaceView: UIView, TerminalSurfaceHosting {
    ...
    private func applyPendingFontSizeIfNeeded() -> Bool {
        ...
        Self.outputQueue.async { [handle = OutputQueueSurfaceHandle(surface: surface)] in
    ...
    deinit { MainActor.assumeIsolated { stopKeyboardHeightAnimation(); disposeSurface() } }
    ...
    private func processOutput(
        ...
        Self.outputQueue.async { [weak self, handle = OutputQueueSurfaceHandle(surface: surface)] in
        ...
        Self.outputQueue.async { [weak self, handle = OutputQueueSurfaceHandle(surface: surface)] in
            ...
            if let eff, eff.cols > 0, eff.rows > 0, cell.width > 0, cell.height > 0 {
                ...
                if !fillsNaturalGrid, !withinOneCell, pinnedW + 0.5 < containerW || pinnedH + 0.5 < containerH {
                    let fitted = Self.fitSurfaceToGrid(handle.surface, cols: eff.cols, rows: eff.rows, cellPixelSize: cell)
