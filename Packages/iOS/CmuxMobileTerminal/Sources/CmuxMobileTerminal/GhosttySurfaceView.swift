...
public final class GhosttySurfaceView: UIView, TerminalSurfaceHosting {
    ...
    private func applyPendingFontSizeIfNeeded() -> Bool {
        ...
        Self.outputQueue.async { [surfaceHandle = GhosttySurfaceHandle(surface: surface)] in
    ...
    private func processOutput(
        ...
        Self.outputQueue.async { [weak self, surfaceHandle = GhosttySurfaceHandle(surface: surface)] in
...
// Safe because this pointer is only dereferenced on `outputQueue`; disposal is
// enqueued on the same FIFO queue after the main-actor `surface` slot is nilled.
private struct GhosttySurfaceHandle: @unchecked Sendable { let surface: ghostty_surface_t }
