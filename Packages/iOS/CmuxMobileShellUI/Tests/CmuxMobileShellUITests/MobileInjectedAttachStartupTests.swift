...
struct MobileInjectedAttachStartupTests {
    ...
    func appLifetimeOwnerKeepsOneAttachAliveAcrossRootReconstruction() async throws {
        ...
        let initialStartAccepted = coordinator.startInjectedAttach(
            attachURL: attachURL,
            prepare: {},
            connect: { rawURL in
                await recorder.record(rawURL)
        ...
        let duplicateStartAccepted = coordinator.startInjectedAttach(
            attachURL: attachURL,
            prepare: {},
            connect: { rawURL in
                await recorder.record(rawURL)
