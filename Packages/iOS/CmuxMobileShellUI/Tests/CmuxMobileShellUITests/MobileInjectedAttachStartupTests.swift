...
struct MobileInjectedAttachStartupTests {
    ...
    func appLifetimeOwnerKeepsOneAttachAliveAcrossRootReconstruction() async throws {
        ...
        let initialAttachStarted = coordinator.startInjectedAttach(
            attachURL: attachURL,
            prepare: {},
            connect: { rawURL in
                await recorder.record(rawURL)
        ...
        let duplicateAttachAccepted = coordinator.startInjectedAttach(
            attachURL: attachURL,
            prepare: {},
            connect: { rawURL in
                await recorder.record(rawURL)
        ...
        #expect(duplicateAttachAccepted)
