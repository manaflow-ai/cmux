...
struct MacComputerDetailView: View {
    ...
    /// Drives the Forget confirmation.
    ...
    /// This Computer's Tailscale connection readiness. Tailscale is the only
    /// connection method; the section warns when this Computer has no
    /// authorized route yet and offers the add-connection sheet.
    @ViewBuilder
    private var connectionMethodSection: some View {
        Section {
            if !computerHasUsableTailscaleAuthorization {
                Label {
                    Text(L10n.string(
    ...
    /// Drops the local row for this pairing on every scope it owns (the same
    /// pipeline the hidden-computer Forget uses). The entry is built from the
    /// pairing's OWN stored scope so the delete targets the owning account
    /// even if the display scope changed.
    ...
    private var actionsSection: some View {
        Section {
            ...
            // Forget is the record's one whole-Computer deletion path: it
            // drops every stored row for this pairing.
