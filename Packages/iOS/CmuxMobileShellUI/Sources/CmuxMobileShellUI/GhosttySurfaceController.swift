...
/// Owns a `GhosttySurfaceView`, routes terminal output, and installs the native
...
@MainActor
final class GhosttySurfaceController: UIViewController {
    ...
    var onArtifactFilesRequested: @MainActor (_ anchor: CGPoint) -> Void
    ...
    private let terminalCoordinator: Coordinator
    private var mountedSurfaceView: GhosttySurfaceView?
    ...
    init(
        ...
        fontSize: Float32,
        autoFocusOnWindowAttach: Bool = true,
        isComposerActive: Bool = false,
        terminalTheme: TerminalTheme,
        terminalConfigTheme: TerminalTheme,
        configThemeGeneration: UInt64 = 0,
        artifactFilesEnabled: Bool = false,
        terminalFolderTapEnabled: Bool = true,
        terminalFilesChipEnabled: Bool = false,
        showMissingFiles: Bool = false,
        sessionArtifactCountEnabled: Bool = false,
        visibleArtifactCount: Int = 0,
        onArtifactFilesRequested: @escaping @MainActor (_ anchor: CGPoint) -> Void = { _ in },
        onArtifactPathTapped: @escaping @MainActor (_ path: String) -> Void = { _ in },
        onVisibleArtifactCountChanged: @escaping @MainActor (_ count: Int) -> Void = { _ in },
        onArtifactGalleryRefreshSignal: @escaping @MainActor (TerminalArtifactGalleryRefreshSignal) -> Void = { _ in }
    ) {
        self.workspaceID = workspaceID
        self.surfaceID = surfaceID
        self.store = store
        self.fontSize = fontSize
        self.autoFocusOnWindowAttach = autoFocusOnWindowAttach
        self.isComposerActive = isComposerActive
        self.terminalTheme = terminalTheme
        self.terminalConfigTheme = terminalConfigTheme
        self.configThemeGeneration = configThemeGeneration
        self.artifactFilesEnabled = artifactFilesEnabled
        self.terminalFolderTapEnabled = terminalFolderTapEnabled
        self.terminalFilesChipEnabled = terminalFilesChipEnabled
    ...
    override func loadView() {
        ...
        surfaceView.diagnosticLog = store.diagnosticLog
        #endif
        // Stamp the shell-level id so id-scoped registry lookups (the
        // "View as Text" capture) resolve this exact terminal.
        surfaceView.hostSurfaceID = surfaceID
        terminalCoordinator.attach(surfaceView: surfaceView)
        surfaceView.seedThemeParityPreviewIfRequested()
        ...
        // seed the surface's composer-active flag before the view enters a window.
        surfaceView.setComposerActive(isComposerActive)
        terminalCoordinator.setComposerMounted(isComposerActive)
        terminalCoordinator.themeApplicationScheduler.seed(generation: configThemeGeneration)
        mountedSurfaceView = surfaceView
        view = surfaceView
    }

    func update(
        autoFocusOnWindowAttach: Bool,
        isComposerActive: Bool,
        terminalTheme: TerminalTheme,
        terminalConfigTheme: TerminalTheme,
        configThemeGeneration: UInt64,
        ...
        // state write, so the update remains inside the controller's actor.
        guard let surfaceView = mountedSurfaceView else { return }
        surfaceView.autoFocusOnWindowAttach = autoFocusOnWindowAttach
        surfaceView.terminalTheme = terminalTheme
        surfaceView.terminalConfigTheme = terminalConfigTheme
        terminalCoordinator.onArtifactFilesRequested = onArtifactFilesRequested
        terminalCoordinator.onArtifactPathTapped = onArtifactPathTapped
        terminalCoordinator.onVisibleArtifactCountChanged = onVisibleArtifactCountChanged
        terminalCoordinator.onArtifactGalleryRefreshSignal = onArtifactGalleryRefreshSignal
        terminalCoordinator.terminalFolderTapEnabled = terminalFolderTapEnabled
        let artifactCountModeChanged = terminalCoordinator.updateArtifactCountMode(
            artifactFilesEnabled: artifactFilesEnabled,
            terminalFilesChipEnabled: terminalFilesChipEnabled,
            showMissingFiles: showMissingFiles,
            sessionArtifactCountEnabled: sessionArtifactCountEnabled
        ...
        let projectedArtifactCount = terminalCoordinator.artifactCountNeedsRefresh
            ? 0
            : visibleArtifactCount
        terminalCoordinator.updateArtifactChip(count: projectedArtifactCount)
        surfaceView.setComposerActive(isComposerActive)
        terminalCoordinator.setComposerMounted(isComposerActive)
        terminalCoordinator.scheduleTheme(terminalConfigTheme, generation: configThemeGeneration)
        ...
        terminalCoordinator.remeasureComposerForLayoutChange()
    }
    ...
    private func dismantle() {
        ...
        isDismantled = true
        mountedSurfaceView?.prepareForDismantle()
        terminalCoordinator.tearDownArtifactChip()
        terminalCoordinator.tearDownComposer()
        terminalCoordinator.detach()
    ...
    final class Coordinator: NSObject, GhosttySurfaceViewDelegate {
        ...
        init(
            ...
            sessionArtifactCountEnabled: Bool,
            visibleArtifactCount: Int,
            onArtifactFilesRequested: @escaping @MainActor (_ anchor: CGPoint) -> Void,
