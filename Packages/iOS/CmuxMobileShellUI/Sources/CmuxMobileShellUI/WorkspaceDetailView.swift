...
struct WorkspaceDetailView: View {
    ...
    private func selectBrowserStreamFromToolbar(_ panelID: String) {
        dismissTerminalKeyboardForChrome()
        browserStore.closeBrowser(for: workspace.id.rawValue)
        // A stream pick is an explicit navigation; it supersedes (not just
        // occludes) any selected Mac surface, matching `selectTerminal`.
    ...
    private func selectMacSurfaceFromPicker(_ surfaceID: MobileSurfacePreview.ID) {
        ...
        // an active stream must stop or this pick would change nothing.
        stopActiveBrowserStream()
        isChatMode = false
