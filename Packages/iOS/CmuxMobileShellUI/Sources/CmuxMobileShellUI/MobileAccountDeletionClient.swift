...
public struct MobileAccountDeletionClient: Sendable {
    ...
    private let responseParser = MobileAccountDeletionResponseParser()
    ...
    /// Sends the authenticated account deletion request and returns the server deletion state.
    public func deleteAccount() async throws -> MobileAccountDeletionResult {
        ...
        return try responseParser.deletionResult(statusCode: http.statusCode, data: data)
    }
}
