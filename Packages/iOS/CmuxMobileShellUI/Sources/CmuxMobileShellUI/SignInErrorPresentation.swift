...
struct SignInErrorPresentation {
    ...
    func failureReason(for error: Error) -> String {
        ...
        if let stackError = error as? StackAuthErrorProtocol {
            ...
            case "USER_EMAIL_ALREADY_EXISTS":
                return emailCodeFailurePolicy.action(for: error) == .showOriginalMethodSignIn
                    ? "email_unverified"
                    : "email_exists"
            default:
                return "oauth_error"
    ...
    func message(for error: Error) -> String {
        ...
        if let stackError = displayError as? StackAuthErrorProtocol {
            ...
            case "USER_EMAIL_ALREADY_EXISTS":
                if emailCodeFailurePolicy.action(for: displayError) == .showOriginalMethodSignIn {
                    return L10n.string(
                        "auth.error.email_requires_original_method",
                        defaultValue: "This account can’t use email codes yet. Enter your password, or use your original sign-in method."
                    )
                }
