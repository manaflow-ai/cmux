...
What that reduces the trust problem to: distributing the authentic Mac EndpointId. Two channels, both already trusted today for route distribution: the device registry (server-authenticated HTTPS, account-scoped) and the QR code (physical proximity). A compromised registry could substitute an attacker EndpointId, but that adversary class already owns route distribution today (it could substitute attacker host:port), and the Stack same-account check on every RPC means a swapped endpoint without the user's Stack identity still cannot do anything. Defense in depth later: pin the Mac's EndpointId in the phone's `MobilePairedMacStore` on first pair and surface changes.
...
3. **PR 3, phone dial lane**: `CmxIrohByteTransport` in `Packages/CmuxMobileTransport` plus registration, behind a feature flag (DEBUG default ON, release default OFF). Unit tests with the existing transport test doubles; one loopback-style integration test dialing a local iroh listener.
