...
| R-07 | T-CONNECT, M-INDEX | iOS/iPadOS | Arbitrary Linux/macOS/VPS/WSL/homelab OpenSSH host works without cmux helper installation; optional host helper discovery is explicit and authenticated. | Missing cmux-tui/moshi-hook/tmux still gives a usable shell; no connection side effect installs a binary or grants host access. | Bare-host fixture and install refusal audit. | U |
