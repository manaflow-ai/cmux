...
- Settings panel with category navigation, one-click setup presets, expanded themes/accent palettes, workspace names/colors, built-in/custom backgrounds, browser home page, import/export/reset, density, sidebar width, tabs/status bar visibility, toolbar shortcut visibility, performance mode, terminal font family, text size, line height, padding, cursor style/blink, default shell profile, and terminal scrollback.
...
- Pane drag/drop docking hints for left, right, top, and bottom terminal placement, with optimistic workspace/tab updates so moves, closes, and focus changes feel immediate.
