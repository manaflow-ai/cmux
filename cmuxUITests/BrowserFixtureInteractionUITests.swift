...
class BrowserFixtureSocketTestCase: XCTestCase {
    ...
    func socketEnvelope(
        ...
        return ControlSocketUITestClient(path: socketPath, responseTimeout: responseTimeout).sendJSON(request)
    }
        ...
        let ready = waitForControlSocketReady(
            ...
            pingReturnsPong: {
                for candidate in self.socketCandidates() {
                    ...
                    if ControlSocketUITestClient(path: candidate, responseTimeout: 1.0).sendLine("ping") == "PONG" {
