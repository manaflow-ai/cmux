...
final class BonsplitTabDragUITests: XCTestCase {
    ...
    private func launchConfiguredApp(
        ...
        if backendMode == .desktopCmx {
            ...
            let repoRoot = resolvedUITestRepoRoot()
            if let repoRoot {
                app.launchEnvironment["CMUXTERM_REPO_ROOT"] = repoRoot
            }
            if let cmxExecutable = resolvedUITestCmxExecutable(repoRoot: repoRoot) {
    ...
    private func resolvedUITestRepoRoot() -> String? {
    ...
    private func resolvedUITestCmxExecutable(repoRoot: String?) -> String? {
    ...
    private func firstAncestor(containing relativePath: String, from startURL: URL) -> URL? {
