...
///
/// The worker waits on the dispatch group while an async task owns Feed state.
/// `DispatchGroup.leave()` publishes the result before the waiting worker reads
/// it, so no separate mutable-state lock is needed.
