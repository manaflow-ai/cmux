...
private struct QuestionActionArea: View {
    ...
    private func customAnswerField(
        ...
        FeedInlineTextField(
            ...
            onBlur: onBlur,
            onSubmit: nil,
            moveFocusToFeedHost: Self.moveFeedKeyboardFocusToHost(in:),
            responderRetainsFeedFocus: Self.responderRetainsFeedFocus(_:)
        )
        .frame(
            maxWidth: .infinity,
    ...
    /// Focus seam for `FeedInlineTextField`: ask the app to move keyboard focus
    /// to the Feed right-sidebar host for `window`, returning whether it did.
    /// The app stays the only caller of `focusRightSidebarInActiveMainWindow`.
    @MainActor
    ...
    /// Focus seam for `FeedInlineTextField`: report whether a window's new first
    /// responder still belongs to the Feed focus domain (the Feed keyboard host
    /// or another inline editor), so end-of-editing does not fire `onBlur` while
    /// focus merely hops within the Feed sidebar. Inline editors are matched via
    /// the `FeedKeyboardFocusResponder` marker, which `FeedInlineNativeTextView`
