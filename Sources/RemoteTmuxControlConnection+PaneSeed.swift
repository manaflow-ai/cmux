...
extension RemoteTmuxControlConnection {
    ...
    func discardPendingPaneSeeds() {
        let failedPaneIDs = Array(pendingPaneSeeds.keys)
        ...
        for paneID in failedPaneIDs { observers.emitPaneSeedFailure(paneID) }
    }

    func discardPendingPaneSeeds(keeping livePanes: Set<Int>) {
        let removedPaneIDs = Set(pendingPaneSeeds.keys).subtracting(livePanes)
        ...
        for paneID in removedPaneIDs { observers.emitPaneSeedFailure(paneID) }
    }
    ...
    func discardPendingPaneSeeds(paneId: Int) {
        ...
        for seedID in removedSeedIDs { resolveReconnectSeed(seedID) }
        if !removedSeeds.isEmpty { observers.emitPaneSeedFailure(paneId) }
    }
