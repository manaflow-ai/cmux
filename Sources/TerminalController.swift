...
class TerminalController {
    ...
    private nonisolated func v2RunStrictCSPBrowserExpression(
        ...
        {
          \(framePrelude)
          \(v2BrowserControl.evaluationResultPrelude())
          ...
          void (async () => {
            ...
            return {
              ...
              __cmux_v: (typeof __value === 'undefined') ? null : __cmuxBridgeSafeValue(__value)
            };
          })().then(
            (__result) => {
              try {
    ...
    private nonisolated func v2RunBrowserJavaScript(
        ...
        if case .failure(let pageMessage) = rawResult,
           v2BrowserFailureLooksLikeCSPEvalBlock(pageMessage) {
            if useEval {
                let framePrelude = v2BrowserControl.evaluationFramePrelude(
                    frameSelector: v2BrowserCurrentFrameSelector(surfaceId: surfaceId)
                )
