...
class TerminalController {
    ...
    private func v2NoteCreate(params: [String: Any]) -> V2CallResult {
    ...
    private func v2NoteOpen(params: [String: Any]) -> V2CallResult {
    ...
    private func v2NoteOpenSplit(
        ...
        v2MainSync {
            ...
            if !createIfMissing {
                ...
                guard (try? NoteSupport.ensureNoteFile(slug: slug, projectRoot: projectRoot)) != nil else {
                    result = .err(
                        code: "io_error",
                        message: "Failed to create note file: \(notePath)",
                        data: ["slug": slug, "path": notePath]
                    )
                    return
                }
            }
            ...
            let direction = parseSplitDirection(v2String(params, "direction") ?? "right") ?? .right
            let orientation: SplitOrientation = direction.isHorizontal ? .horizontal : .vertical
            let insertFirst = (direction == .left || direction == .up)
    ...
    private func v2NoteList(params: [String: Any]) -> V2CallResult {
        guard let tabManager = v2ResolveTabManager(params: params) else {
            return .err(code: "unavailable", message: "TabManager not available", data: nil)
        }
    ...
    private func v2NotePath(params: [String: Any]) -> V2CallResult {
        guard let tabManager = v2ResolveTabManager(params: params) else {
            return .err(code: "unavailable", message: "TabManager not available", data: nil)
        }
        guard let rawSlug = v2String(params, "slug"), !rawSlug.isEmpty else {
    ...
    private func v2NoteDelete(params: [String: Any]) -> V2CallResult {
        guard let tabManager = v2ResolveTabManager(params: params) else {
            return .err(code: "unavailable", message: "TabManager not available", data: nil)
        }
        guard let rawSlug = v2String(params, "slug"), !rawSlug.isEmpty else {
