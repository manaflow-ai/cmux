...
final class WindowDecorationsController {
    ...
    private func applyDefaultsDrivenDecorationChangeIfNeeded() {
        let currentTitlebarHidden = WorkspaceTitlebarSettings.isHidden()
        let currentTitlebarSnapshot = MinimalModeTitlebarDebugSettings.snapshot()
        guard currentTitlebarHidden != lastKnownTitlebarHidden || currentTitlebarSnapshot != lastKnownTitlebarDebugSnapshot else { return }
        lastKnownTitlebarHidden = currentTitlebarHidden
