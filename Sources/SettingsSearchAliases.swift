enum SettingsSearchAliasIndex {
    ...
    private static let settingAliases: [String: String] = [
        ...
        "terminal:status-bar": localized("settings.search.alias.setting.terminal.status-bar", defaultValue: "terminal.statusBar persistent pinned bottom status bar statusline hardstatus command refresh interval rows claude codex tokens cost branch"),
