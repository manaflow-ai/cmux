...
extension TabManager {
    func sessionAutosaveFingerprint(
        ...
        for workspace in tabs.prefix(SessionPersistencePolicy.maxWorkspacesPerWindow) {
            ...
            hasher.combine(workspace.workspaceDirectory)
            hasher.combine(workspace.workspacePullRequest?.number)
            hasher.combine(workspace.workspacePullRequest?.url)
            hasher.combine(workspace.workspacePullRequest?.label)
            hasher.combine(workspace.workspacePullRequest?.status.rawValue)
            hasher.combine(workspace.workspacePullRequest?.branch)
