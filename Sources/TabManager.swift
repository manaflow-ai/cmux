...
extension TabManager {
    func sessionAutosaveFingerprint(
        ...
        for workspace in tabs.prefix(SessionPersistencePolicy.maxWorkspacesPerWindow) {
            ...
            Self.hashExternalTree(workspace.bonsplitController.treeSnapshot(), into: &hasher)
            Self.hashStatusEntries(workspace.statusEntries, into: &hasher)
            Self.hashMetadataBlocks(workspace.metadataBlocks, into: &hasher)
            Self.hashLogEntries(workspace.logEntries, into: &hasher)
            Self.hashUUIDStringDictionary(workspace.panelDirectories, into: &hasher)
            Self.hashUUIDStringDictionary(workspace.panelTitles, into: &hasher)
            Self.hashUUIDStringDictionary(workspace.panelCustomTitles, into: &hasher)
            Self.hashUUIDBranchDictionary(workspace.panelGitBranches, into: &hasher)
            Self.hashUUIDPullRequestDictionary(workspace.panelPullRequests, into: &hasher)
            Self.hashUUIDIntArrayDictionary(workspace.surfaceListeningPorts, into: &hasher)
            Self.hashUUIDStringDictionary(workspace.surfaceTTYNames, into: &hasher)
            Self.hashUUIDSet(workspace.pinnedPanelIds, into: &hasher)
            Self.hashUUIDSet(workspace.manualUnreadPanelIds, into: &hasher)
            ...
            for panelId in panelIds {
                ...
                if let panel = workspace.panels[panelId] {
                    Self.hashPanelSessionAutosaveState(panel, into: &hasher)
                } else {
            ...
            if let remote = workspace.remoteConfiguration?.sessionSnapshot() {
                hasher.combine(true)
                hasher.combine(remote.transport.rawValue)
                hasher.combine(remote.destination)
                hasher.combine(remote.port)
                Self.hashOptionalString(remote.identityFile, into: &hasher)
                hasher.combine(remote.sshOptions)
                hasher.combine(remote.skipDaemonBootstrap)
            } else {
                hasher.combine(false)
            }
        }
    ...
    nonisolated private static func hashExternalTree(_ node: ExternalTreeNode, into hasher: inout Hasher) {
        switch node {
        case .pane(let pane):
            hasher.combine("pane")
        ...
        case .split(let split):
            ...
            hashExternalTree(split.first, into: &hasher)
            hashExternalTree(split.second, into: &hasher)
        }
    }

    private static func hashPanelSessionAutosaveState(_ panel: any Panel, into hasher: inout Hasher) {
        hasher.combine(true)
        hasher.combine(panel.id)
        hasher.combine(panel.panelType.rawValue)
        hasher.combine(panel.displayTitle)
        hashOptionalString(panel.displayIcon, into: &hasher)
        hasher.combine(panel.isDirty)
        ...
        case .terminal:
            ...
            hasher.combine(true)
            hashOptionalString(terminalPanel.requestedWorkingDirectory, into: &hasher)
            hashOptionalString(Workspace.restorableTmuxStartCommand(terminalPanel.surface.debugTmuxStartCommand()), into: &hasher)
        case .browser:
            ...
            hasher.combine(true)
            hashOptionalString(browserPanel.preferredURLStringForOmnibar(), into: &hasher)
            hasher.combine(browserPanel.profileID)
            hasher.combine(browserPanel.shouldRenderWebViewForSessionSnapshot())
            hasher.combine(Int((Double(browserPanel.currentPageZoomFactor()) * 1000).rounded()))
            hasher.combine(browserPanel.isDeveloperToolsVisible())
            let history = browserPanel.sessionNavigationHistorySnapshot()
            hasher.combine(history.backHistoryURLStrings)
            hasher.combine(history.forwardHistoryURLStrings)
        case .markdown:
            ...
            hasher.combine(true)
            hasher.combine(markdownPanel.filePath)
        case .filePreview:
            guard let filePreviewPanel = panel as? FilePreviewPanel else {
                hasher.combine(false)
                return
            }
            hasher.combine(true)
            hasher.combine(filePreviewPanel.filePath)
        }
    }

    nonisolated private static func hashUUIDSet(_ values: Set<UUID>, into hasher: inout Hasher) {
        let ordered = values.sorted { $0.uuidString < $1.uuidString }
    ...
    nonisolated private static func hashUUIDStringDictionary(
        _ values: [UUID: String],
        into hasher: inout Hasher
    ) {
        hasher.combine(values.count)
        for key in values.keys.sorted(by: { $0.uuidString < $1.uuidString }) {
            hasher.combine(key)
            hasher.combine(values[key] ?? "")
        }
    }

    nonisolated private static func hashUUIDIntArrayDictionary(
        _ values: [UUID: [Int]],
        into hasher: inout Hasher
    ) {
        hasher.combine(values.count)
        for key in values.keys.sorted(by: { $0.uuidString < $1.uuidString }) {
            hasher.combine(key)
            hasher.combine((values[key] ?? []).sorted())
        }
    }

    nonisolated private static func hashUUIDBranchDictionary(
        _ values: [UUID: SidebarGitBranchState],
        into hasher: inout Hasher
    ) {
        hasher.combine(values.count)
        for key in values.keys.sorted(by: { $0.uuidString < $1.uuidString }) {
            ...
            hasher.combine(key)
            hasher.combine(value.branch)
            hasher.combine(value.isDirty)
        }
    }

    nonisolated private static func hashUUIDPullRequestDictionary(
        _ values: [UUID: SidebarPullRequestState],
        into hasher: inout Hasher
    ) {
        hasher.combine(values.count)
        for key in values.keys.sorted(by: { $0.uuidString < $1.uuidString }) {
            guard let value = values[key] else { continue }
            hasher.combine(key)
            hasher.combine(value.number)
            hasher.combine(value.label)
            hasher.combine(value.url.absoluteString)
            hasher.combine(value.status.rawValue)
            hashOptionalString(value.branch, into: &hasher)
            hasher.combine(value.isStale)
        }
    }

    nonisolated private static func hashStatusEntries(
        _ values: [String: SidebarStatusEntry],
        into hasher: inout Hasher
    ) {
        hasher.combine(values.count)
        for key in values.keys.sorted() {
            ...
            hashOptionalString(value.icon, into: &hasher)
            hashOptionalString(value.color, into: &hasher)
            hashOptionalString(value.url?.absoluteString, into: &hasher)
            hasher.combine(value.priority)
            hasher.combine(value.format.rawValue)
            hashOptionalDouble(value.timestamp.timeIntervalSince1970, into: &hasher)
        }
    }

    nonisolated private static func hashMetadataBlocks(
        _ values: [String: SidebarMetadataBlock],
        into hasher: inout Hasher
    ) {
        hasher.combine(values.count)
        for key in values.keys.sorted() {
            guard let value = values[key] else { continue }
            hasher.combine(key)
            hasher.combine(value.markdown)
            hasher.combine(value.priority)
            hashOptionalDouble(value.timestamp.timeIntervalSince1970, into: &hasher)
        }
    }

    nonisolated private static func hashLogEntries(
        _ values: [SidebarLogEntry],
        into hasher: inout Hasher
    ) {
        hasher.combine(values.count)
        for value in values {
            hasher.combine(value.message)
            hasher.combine(value.level.rawValue)
            hashOptionalString(value.source, into: &hasher)
            hashOptionalDouble(value.timestamp.timeIntervalSince1970, into: &hasher)
        }
    }
