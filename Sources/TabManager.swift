...
class TabManager: ObservableObject {
    ...
    /// Shared terminal construction seam for every workspace and Dock owned by
    /// this process. The app composition root injects one instance into every
    /// window's manager.
        ...
        let projectionHandler: TerminalBackendTopologyMutationCoordinator.ProjectionHandler = {
            [weak self] _ in
                ...
                if initialSurface == .terminal,
                   ...
                   let terminalPanel = workspace.panels[surfaceID] as? TerminalPanel {
                ...
                if applyCreationTitleAsCustomTitle {
                    self.applyCreationWorkspaceCustomization(
                        to: workspace,
                        explicitTitle: title,
    ...
    /// Synchronous compatibility entrypoint for embedded topology and
    /// presentation-only placeholders. Canonical terminal and browser
    /// creation must use `requestAddWorkspace` when cmuxd owns topology.
    @discardableResult
    func addWorkspace(
        id: UUID? = nil,
        title: String? = nil,
        ...
        precondition(
            ...
            "persistent terminal topology mutations must use requestAddWorkspace"
        )
        return addLocalWorkspace(
            id: id,
            title: title,
            titleSource: titleSource,
            workingDirectory: overrideWorkingDirectory,
...
extension TabManager {
    ...
    func restoreSessionSnapshot(
        ...
        let customizationSnapshots = workspaceSnapshots.map(\.snapshot)
        prepareLegacyWorkspaceCustomizationMigration(
            afterRestoring: customizationSnapshots
        )
