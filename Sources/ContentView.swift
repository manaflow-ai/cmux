...
struct ContentView: View {
    ...
    private func registerCommandPaletteHandlersImpl(
        ...
        registry.register(commandId: "palette.newSimulatorPane") { invocation in
        ...
        registry.register(commandId: "palette.reopenClosedWorkspace") { _ in
            let didReopen = if let appDelegate = AppDelegate.shared {
                appDelegate.reopenMostRecentlyClosedWorkspace(preferredTabManager: tabManager)
            } else {
                tabManager.reopenMostRecentlyClosedWorkspace()
            }
            return didReopen
                ? .completed
                : commandPaletteActionCouldNotStartResult(code: "history_unavailable")
        ...
        registerSurfaceNavigationCommandHandlers(&registry, context: context)
