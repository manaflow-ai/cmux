...
// installFileDropOverlay(on:tabManager:) and installFileDropOverlayWhenReady(...)
// were top-level free functions here; folded onto AppFileDropTarget as static
// methods (no-free-functions convention, CONVENTIONS s3). Call them as
// AppFileDropTarget.installFileDropOverlay(...) /
// AppFileDropTarget.installFileDropOverlayWhenReady(...). ContentView's call site
// uses AppFileDropTarget.installFileDropOverlayWhenReady(...). AppDelegate is a
// forbidden god for this slice and still calls the bare free function, so a thin
// shim forwarding to the static method lives next to the witness in
// AppFileDropTarget.swift (delete it when AppDelegate is next edited).
...
struct ContentView: View, CommandPaletteWorkspaceSnapshotProviding, CommandPaletteForkableAgentProbeHost {
    ...
    func commandPaletteCurrentFallbackSnapshotFingerprint(
        ...
        return currentFallbackSnapshot.commandPaletteForkFingerprint
    }
...
struct VerticalTabsSidebar: View {
    ...
    private func emptyAreaTabDropDelegate(renderContext: WorkspaceListRenderContext) -> SidebarTabDropDelegate {
        SidebarTabDropDelegate(
            targetTabId: nil,
            routing: SidebarWorkspaceTabRouter(tabManager: tabManager),
    ...
    private func workspaceRow(
        ...
        // App-side host carrying every `tabManager`/`notificationStore`/`tab`
        // computation. The lifted `CmuxSidebarUI.TabItemView` below reads only
        // the value snapshots and closures this model produces, keeping it on
        // the package side of the LazyVStack snapshot boundary.
        let rowModel = WorkspaceSidebarRowModel(
            tabManager: tabManager,
            notificationStore: notificationStore,
            tab: tab,
            index: index,
        ...
        let row = TabItemView(
            ...
            descriptionDebugLog: WorkspaceSidebarRowModel.sidebarDescriptionDebugLog,
            makeStyle: { colorScheme in rowModel.withColorScheme(colorScheme).makeStyle() },
            makeWorkspaceSnapshot: { rowModel.makeWorkspaceSnapshot() },
            makeContextMenuData: { snapshot in rowModel.contextMenuData(snapshot: snapshot) },
            makeContextMenuActions: { snapshot, requestShowInFinder in
                rowModel.contextMenuActions(
                    snapshot: snapshot,
                    requestShowInFinder: requestShowInFinder,
                    refreshSnapshot: {}
                )
            },
            onSelect: { rowModel.updateSelection() },
            onCloseWorkspace: { method in rowModel.closeWorkspace(method: method) },
            onReconnect: { rowModel.reconnect() },
            onMoveUp: { rowModel.moveBy(-1) },
            onMoveDown: { rowModel.moveBy(1) },
            onContextMenuAppear: onContextMenuAppear,
            onContextMenuDisappear: onContextMenuDisappear,
            onOpenPullRequest: { rowModel.openPullRequest($0) },
            onOpenPort: { rowModel.openPort($0) },
            pullRequestStatusLabel: { rowModel.pullRequestStatusLabel($0) },
            portLabel: { rowModel.portLabel($0) },
            portTooltip: { rowModel.portTooltip($0) },
            onDragStart: onDragStart,
            tabDropDelegateFactory: tabDropDelegateFactory,
            bonsplitDropDelegate: bonsplitDropDelegate,
            bonsplitDropContentTypes: BonsplitTabDragPayload.dropContentTypes
        )
        .equatable()
        .id(tab.id)
...
// The pure `com.splittabbar.tabtransfer` wire contract (the UTType identity, the
// `Transfer` decode, the current-process identity check, and
// `transfer(from:excludeFilePreview:)`) moved to
// `CmuxWorkspaces.BonsplitTabDragPayload`. The app-coupled members below stay
// app-side because they call the app target's `DragOverlayRoutingPolicy`
// pasteboard predicates, and forward the routing decision into the moved core.
// `currentTransfer()` stays here too: it reads the live `.drag` pasteboard and,
// exactly like the original, runtime-evaluates the file-preview routing guard on
// that pasteboard's types before decoding.
extension BonsplitTabDragPayload {
    /// The shared decoder bound to the live process id, used by all app-side
    /// drop wiring.
    static let appShared = BonsplitTabDragPayload()
    ...
    static func transfer(from pasteboard: NSPasteboard) -> Transfer? {
        appShared.transfer(
            from: pasteboard,
            excludeFilePreview: DragOverlayRoutingPolicy.hasFilePreviewTransfer(pasteboard.types)
        )
    }
}
