...
struct VerticalTabsSidebar: View, Equatable {
    ...
    var body: some View {
        ...
        let workspaceGroups = isPresented
            ? tabManager.workspaceGroups.filter {
                visibleGroupIDs.contains($0.id)
                    || ($0.isEmpty && creationContextID == SidebarCreationContextSelection.localID)
            }
            : []
    ...
    private func workspaceReorderPlan(
        ...
        let draggedBlockIds = SidebarWorkspaceDragBlockResolver().movingWorkspaceIds(
            ...
            anchorIds: Set(renderContext.workspaceGroups.compactMap(\.liveAnchorWorkspaceId))
        )
    ...
    private func selectWorkspaceRow(
        ...
        let anchorIds = Set(tabManager.workspaceGroups.compactMap { group in
            group.liveAnchorWorkspaceId.flatMap { visibleWorkspaceIDSet.contains($0) ? $0 : nil }
        })
