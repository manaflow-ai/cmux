...
struct ContentView: View {
    ...
    private func commandPaletteCommandsContext(
        ...
        snapshot.setBool(
            CommandPaletteContextKeys.extensionsExperimentalEnabled,
            extensionsExperimentalEnabled
        )
        snapshot.setBool(
            CommandPaletteContextKeys.customSidebarsExperimentalEnabled,
            customSidebarsExperimentalEnabled
        )
    ...
    private func commandPaletteCommandContributions() -> [CommandPaletteCommandContribution] {
        ...
        // extension sidebar and custom sidebars only while their beta flags
        // are enabled.
    ...
    private func registerCommandPaletteHandlers(_ registry: inout CommandPaletteHandlerRegistry) {
        ...
        for descriptor in CmuxExtensionSidebarSelection.allDescriptors {
            registry.register(commandId: commandPaletteExtensionSidebarCommandID(descriptor.id)) {
                guard CmuxExtensionSidebarSelection.canSelectProvider(descriptor.id) else { return }
...
enum CmuxExtensionSidebarSelection {
    ...
    /// Returns whether the provider can currently be selected through a stale
    /// command-palette handler or AppKit menu callback.
    static func canSelectProvider(_ providerId: String) -> Bool {
        if providerId == hostedExtensionsProviderId {
            return isEnabled
        }
        if providerId.hasPrefix(customSidebarProviderPrefix) {
            return customSidebarsEnabled
        }
        return true
    }
...
private final class CmuxExtensionSidebarMenuTarget: NSObject {
    ...
    @objc func selectProvider(_ sender: NSMenuItem) {
        ...
        guard CmuxExtensionSidebarSelection.canSelectProvider(providerId) else { return }
