...
struct VerticalTabsSidebar: View {
    ...
    /// has no per-row publisher subscription that would otherwise fire on
    /// `cd` while it's not selected.
    ...
    private func workspaceScrollArea(renderContext: WorkspaceListRenderContext) -> some View {
        // The workspace list is AppKit end to end: one NSViewRepresentable
        // mounts an NSTableView whose cells, menus, rename field, and drag
        // sources are plain AppKit views driven by immutable row values.
        // SwiftUI's lazy-layout feedback loops (#2586 / #5764 / #5845 / #6707)
        // are structurally unrepresentable in this path: no view below this
        // boundary can observe a model or write layout-relevant state.
        let listSnapshot = workspaceListSnapshot(renderContext: renderContext)
        let rows: [SidebarWorkspaceListRow] = renderContext.workspaceRenderItems.compactMap { item in
            switch item {
            case .groupHeader(let groupId, _):
                return listSnapshot.groupRowsById[groupId]
                    .map(SidebarWorkspaceListRow.init(groupHeader:))
            case .workspace(let workspaceId):
                guard let input = listSnapshot.workspaceRowsById[workspaceId] else { return nil }
                return SidebarWorkspaceListRow(workspace: input.rowSnapshot(list: listSnapshot))
            }
        }
        ...
        return SidebarWorkspaceTableView(
            rows: rows,
            listActions: workspaceTableActions(renderContext: renderContext),
            actionResolver: workspaceListActionResolver(listSnapshot: listSnapshot),
    ...
    /// Reduces live models to the immutable projections the AppKit table
    /// consumes. Built once per render pass; the table diffs by value and
    /// touches only rows whose projection changed.
    private func workspaceListSnapshot(
        renderContext: WorkspaceListRenderContext
    ) -> SidebarWorkspaceRowsSnapshot {
        ...
        return SidebarWorkspaceRowsSnapshot(
    ...
    /// Binds parent-owned capabilities to one row on demand (cell wiring or
    /// menu open), reusing the exact action factory the SwiftUI rows used.
    private func workspaceListActionResolver(
        ...
        return { row in
            ...
            case .workspace(let snapshot):
                guard let input = listSnapshot.workspaceRowsById[snapshot.workspaceId] else {
                    return nil
                }
                return .workspace(actionFactory(input))
            case .groupHeader(let snapshot):
                return .groupHeader(sidebarWorkspaceGroupHeaderActions(snapshot: snapshot))
            }
        }
    }
