...
/// Pure helpers used by `ContentView.syncFileExplorerDirectory` to compute
/// the remote home directory and effective root path for the file explorer
/// when a workspace is bound to an SSH remote.
///
/// Lives at top level (rather than nested in ContentView) so the cmuxTests
/// target can reach it via `@testable import cmux`.
enum SSHFileExplorerPathResolver {
    ...
    /// only). Whitespace around the destination and around the user token is
    /// trimmed; ports/IPv6 brackets after the host are ignored.
    ///
    /// Examples:
    ...
    static func remoteHomePath(from destination: String?) -> String {
    ...
    /// Returns true if `path` is on a macOS-only mount that cannot exist on a
    /// Linux remote: `/Users/...` (home) or `/Volumes/...` (mounted disks).
    /// We deliberately do NOT match `/private/...`, `/tmp/...`, `/etc/...` and
    /// other roots that exist on both platforms — those may legitimately be
    /// the user's intended remote target.
    static func isMacLocalPath(_ path: String) -> Bool {
...
struct ContentView: View {
    ...
    private func syncFileExplorerDirectory() {
        ...
        if tab.isRemoteWorkspace {
            ...
            // See SSHFileExplorerPathResolver for the rationale + test cases.
            let homePath = SSHFileExplorerPathResolver.remoteHomePath(from: config?.destination)
            let effectiveDir = SSHFileExplorerPathResolver.effectiveRootPath(
            ...
            if let existingProvider = fileExplorerStore.provider as? SSHFileExplorerProvider,
               existingProvider.destination == config?.destination {
                ...
                if isReady {
                    ...
                    let pathChanged = fileExplorerStore.rootPath != effectiveDir
                    fileExplorerStore.setRootPath(effectiveDir)
                    if pathChanged {
