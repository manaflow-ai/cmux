...
final class PostHogAnalytics {
    ...
    private nonisolated static func versionProperties(infoDictionary: [String: Any]) -> [String: Any] {
        var properties: [String: Any] = [:]
    ...
    func trackActive(reason: String) {
        dispatchAsyncOnWorkQueue { [weak self] in
            ...
            let didCaptureDaily = trackDailyActiveOnWorkQueue(reason: reason, flush: false)
            let didCaptureHourly = trackHourlyActiveOnWorkQueue(reason: reason, flush: false)
    ...
    private func scheduleActiveCheckTimer() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            activeCheckTimer = Timer.scheduledTimer(withTimeInterval: 30 * 60, repeats: true) { [weak self] _ in
                ...
                trackActive(reason: "activeTimer")
            }
        }
    }
