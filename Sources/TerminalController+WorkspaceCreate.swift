...
extension TerminalController {
    ...
    private func v2PerformWorkspaceCreate(
        ...
        if let operationID, !operationAlreadyAccepted {
            // Acceptance must be durable before workspace construction can run
            // a task command. A crash favors at-most-once task startup.
            do {
        ...
        v2MainSync {
            let configureWorkspace: @MainActor (Workspace) -> Void = { workspace in
                ...
                workspace.setCustomDescription(execution.description)
                if let layoutNode = execution.layoutNode {
                    workspace.applyCustomLayout(
                        layoutNode,
                        baseCwd: execution.workingDirectory ?? workspace.currentDirectory
                    )
                }
            ...
            case .submittedToBackend(let submission):
                newWorkspaceID = submission.workspaceID
        ...
        return workspaceCreateResult(
            workspaceID: newWorkspaceID,
            groupID: newWorkspace?.groupId ?? execution.groupID,
            surfaceID: newWorkspace?.focusedPanelId ?? initialSurfaceID,
    ...
    func v2MobileWorkspaceCreate(
        ...
        switch createResult {
        case let .ok(payload):
            guard var createPayload = payload as? [String: Any] else {
                return .err(code: "internal_error", message: "Invalid workspace create result", data: nil)
            }
            let createdWorkspaceID = createPayload["workspace_id"] as? String
            if createPayload["pending"] as? Bool == true {
                createPayload["created_workspace_id"] = createdWorkspaceID
