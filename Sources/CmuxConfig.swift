...
// The top-level `cmux.json` wire struct `CmuxConfigFile` (its fields,
// CodingKeys, memberwise init, validating `init(from:)`, and the
// `normalizedActions`/`validatedSurfaceTabBarButtons` static validators) now
// lives in CmuxWorkspaces/CustomLayout/CmuxConfigFile.swift, co-located with the
// action/command/ui/workspaceGroups wire schema it composes. The store's
// `parseConfig`/`loadAll` read it byte-identically through this typealias.
typealias CmuxConfigFile = CmuxWorkspaces.CmuxConfigFile

// The `cmux.json` `workspaceGroups` wire schema
// (CmuxConfigWorkspaceGroupsDefinition / CmuxConfigWorkspaceGroupEntry) now
// lives in CmuxWorkspaces/CustomLayout/, co-located with the
// CmuxConfigContextMenuItem value type its `contextMenu` field builds on. They
// are reached through `import CmuxWorkspaces`.
