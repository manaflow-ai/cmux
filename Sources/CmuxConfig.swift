...
/// Helpers for the `note` surface type. Notes are plain markdown files
/// stored at `.cmux/notes/<slug>.md` relative to the project root (the
/// nearest ancestor of the workspace cwd that already contains a `.cmux/`
/// directory; falling back to the cwd itself).
///
/// The renderer is the existing `MarkdownPanel` — notes do NOT introduce a
/// distinct in-app panel type. The "note" primitive lives at the public
/// `CmuxSurfaceType.note`, in the CLI subcommand `cmux note`, in the socket
/// methods `note.*`, and in the storage convention. Distinct in-app UI can
/// layer on later as a separate refactor.
enum NoteSupport {
    ...
    /// the slug. Otherwise nil. Used at snapshot-build time to detect that a
    /// markdown panel originated as a note so session restore can re-resolve
    /// it against the project root if it moves.
    static func slug(forNotePath path: String) -> String? {
        ...
        return (try? validateSlug(slug)) ?? slug
    }
    ...
    /// Returns false if the file was already absent (idempotent). Throws on
    /// FS errors other than "not found".
    @discardableResult
    static func deleteNote(slug: String, projectRoot: String) throws -> Bool {
        let path = notePath(forSlug: slug, projectRoot: projectRoot)
        let fs = FileManager.default
    ...
    enum NoteError: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case .fileMissing(let path): return "Note file not found: \(path)"
            }
        }
    }
}
