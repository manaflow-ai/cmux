...
/// Owns the mobile workspace-list RPC dispatch: the iOS-facing `workspace.list`
/// surface and its group collapse / workspace-close companions.
///
/// Serializes workspaces across every open Mac window into the mobile list
/// payload (workspace rows, terminal rows, collapsible group sections, and the
/// iMessage-style activity preview), and mutates group-collapse / workspace-close
/// state through the same `TabManager` mutators the CLI and sidebar use. It
/// reaches the terminal data plane (param/UUID coercion, tab-manager and
/// window-id resolution, notification store, terminal panels, and the localized
/// close-blocked message) only through the ``MobileWorkspaceListRPCHost`` seam.
/// This type replaces the former `TerminalController+MobileWorkspaceList`
/// extension: the same logic, relocated off the data-plane god object into a
/// constructor-injected owner.
///
/// `@MainActor` because every body it owns reads or mutates main-actor workspace
/// state and the seam it drives is `@MainActor`.
@MainActor
final class MobileWorkspaceListRPCHandler {
    private let host: any MobileWorkspaceListRPCHost

    /// - Parameter host: the terminal data-plane seam the list handlers drive
    ///   (param coercion, tab-manager / window resolution, notification store,
    ///   terminal panels, and the localized close-blocked message).
    init(host: any MobileWorkspaceListRPCHost) {
        self.host = host
    }
    ...
    /// mutation path stays shared. The host's tab-manager resolution routes by
    /// `group_id` to the owning window even in the multi-window case.
    func setGroupCollapsed(params: [String: Any], isCollapsed: Bool) -> TerminalController.V2CallResult {
        guard host.mobileWorkspaceListHasNonNullParam(params, "group_id"),
              let gid = host.mobileWorkspaceListUUID(params, "group_id") else {
            return .err(code: "invalid_params", message: "Missing or invalid group_id", data: nil)
        }
        guard let tabManager = host.mobileWorkspaceListResolveTabManager(params: params) else {
            return .err(code: "unavailable", message: "TabManager not available", data: nil)
        }
        var ok = false
        host.mobileWorkspaceListMainSync {
    ...
    func list(
        ...
        let requestedWorkspaceID = host.mobileWorkspaceListUUID(params, "workspace_id")
        if host.mobileWorkspaceListHasNonNullParam(params, "workspace_id"), requestedWorkspaceID == nil {
            return .err(code: "invalid_params", message: "Missing or invalid workspace_id", data: nil)
        }
        let requestedTerminalID: UUID?
        switch host.mobileWorkspaceListTerminalAliasUUID(params: params) {
        case .missing:
        ...
        let scopeToSingleWindow = resolvedTabManager != nil
            ...
            || host.mobileWorkspaceListHasNonNullParam(params, "window_id")
            || requestedTerminalID != nil
        ...
        let selectedWorkspaceID = scopeToSingleWindow
            ? nil
            : host.mobileWorkspaceListKeyWindowSelectedWorkspaceID
        ...
        if scopeToSingleWindow {
            guard let tabManager = resolvedTabManager ?? host.mobileWorkspaceListResolveTabManager(params: params) else {
            ...
            for workspace in visibleWorkspaces {
                host.mobileWorkspaceListAdoptDetectedAgentSessions(workspace: workspace)
            }
            let scopedWorkspaces = visibleWorkspaces.map { workspace in
                mobileWorkspacePayload(
                    workspace: workspace,
                    windowID: host.mobileWorkspaceListResolveWindowId(tabManager: tabManager),
            ...
            guard host.mobileWorkspaceListAppAvailable else {
                return .err(code: "unavailable", message: "Workspace context is unavailable", data: nil)
            }
            ...
            for summary in host.mobileWorkspaceListMainWindowSummaries() {
                guard seenWindowIDs.insert(summary.windowId).inserted else { continue }
                guard let windowTabManager = host.mobileWorkspaceListTabManager(windowId: summary.windowId) else { continue }
    ...
    /// of ``list(params:tabManager:createdWorkspaceID:createdTerminalID:)`` so the
    /// two never diverge. When `requestedTerminalID` is non-nil the terminals
    /// array is filtered to that one terminal (only the scoped branch passes it;
    /// the all-windows branch always passes nil, so it lists every terminal). The
    /// scoped terminal-not-found check is enforced by the caller after the list is
    /// built. `notificationStore` defaults to the app-global store; tests inject
    /// one so the unread/activity fields are deterministic.
    func mobileWorkspacePayload(
        ...
        let terminals = host.mobileWorkspaceListTerminalPanels(in: workspace).compactMap { terminal -> [String: Any]? in
        ...
        let store = notificationStore ?? host.mobileWorkspaceListNotificationStore
        ...
        return [
            ...
            "group_id": host.mobileWorkspaceListOrNull(workspace.groupId?.uuidString),
            ...
            "preview": host.mobileWorkspaceListOrNull(preview?.text),
            "preview_at": host.mobileWorkspaceListOrNull(preview?.epochSeconds),
    ...
    func close(params: [String: Any]) -> TerminalController.V2CallResult {
        if host.mobileWorkspaceListHasNonNullParam(params, "workspace_id"),
           host.mobileWorkspaceListUUID(params, "workspace_id") == nil {
            return .err(code: "invalid_params", message: "Missing or invalid workspace_id", data: nil)
        }
        guard let workspaceID = host.mobileWorkspaceListUUID(params, "workspace_id") else {
            return .err(code: "invalid_params", message: "Missing or invalid workspace_id", data: nil)
        }
        guard let tabManager = host.mobileWorkspaceListResolveTabManager(params: params) else {
            return .err(code: "unavailable", message: "Workspace context is unavailable", data: nil)
        ...
        var result: TerminalController.V2CallResult = .err(
            code: "internal_error",
            message: "Failed to close workspace",
            data: nil
        )
        host.mobileWorkspaceListMainSync {
            let windowID = host.mobileWorkspaceListResolveWindowId(tabManager: tabManager)
            ...
            guard tabManager.tabs.count > 1, tabManager.canCloseWorkspace(workspace) else {
                result = .err(
                    code: "protected",
                    message: host.mobileWorkspaceListCloseBlockedMessage,
                    data: [
                        "workspace_id": workspaceID.uuidString,
                        "workspace_ref": host.mobileWorkspaceListRef(kind: .workspace, uuid: workspaceID),
                        "window_id": host.mobileWorkspaceListOrNull(windowID?.uuidString),
                        "window_ref": host.mobileWorkspaceListRef(kind: .window, uuid: windowID),
                    ]
                )
                return
            }
