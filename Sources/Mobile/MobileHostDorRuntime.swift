...
/// macOS composition root for the dor transport: the account-scoped Durable
/// Object relay in `CmuxDorTransport`. Default-on: when the gate is on this
/// runtime owns the app's mobile transport slot — the Mac parks one standing
/// leg on the ACCOUNT's relay object and serves the full mobile dialect to
/// admitted phone sessions — and neither the irx runtime nor the legacy iroh
/// endpoint is started.
@MainActor
final class MobileHostDorRuntime {
    static let shared = MobileHostDorRuntime()

    nonisolated static let enabledDefaultsKey = "cmux.dor.enabled"
    ...
    /// One journal for every dor component on the Mac. The soak analyzer
    /// tails the JSONL file; every line mirrors to os_log at notice level.
    nonisolated static let journal: DorJournal = {
        let tag = MobileHostIdentity.instanceTag()
        let logger = Logger(subsystem: "dev.cmux", category: "dor-host")
        return DorJournal.file(
            url: URL(fileURLWithPath: "/tmp/cmux-dor-journal-mac-\(tag).jsonl"),
            mirror: { line in logger.notice("\(line, privacy: .public)") }
        )
    }()

    /// The proven terminal-lane server takes the irx journal type. Give it an
    /// os_log-only instance in the dor category: the dor JSONL file keeps
    /// exactly one writer (``journal``'s serial queue).
    ...
    private var acceptor: DorSessionAcceptor?
    ...
    /// the persisted trust snapshot admission reads.
    ...
    private func activate(accountID: String) async {
        ...
        do {
            ...
            // dor handshake authenticates equals the EndpointID every
            // existing pair grant pins (zero re-pairing). This deviceID also
            // names the Mac inside the account relay object and is what the
            // broker binding carries as `acceptor.deviceID` in grants —
            // exactly the value phones dial with.
            ...
            let signer = try MobileHostDorIdentitySigner(
                    ...
                    bundleIdentifier: Bundle.main.bundleIdentifier)
            {
                ...
                // Per-bundle, per-backend state (shared with the irx stack on
                // purpose: same identity, same binding slot, same trust).
            ...
            // never awaits the broker (steady-state independence). The
            // registration above just refreshed it when reachable.
            ...
            let admission = DorAdmissionMaterial(
                grantJWS: nil,
                grantVerificationKeys: verificationKeys,
                expectedPeerPublicKey: nil
            )
            let legConfiguration = DorLegConfiguration(
                ...
                tokenProvider: { [weak auth] in
                    guard let auth else {
                        throw MobileHostDorRuntimeError.signedOut
                    }
                    return try await auth.currentTokens().accessToken
                },
                journal: Self.journal
            )
            let acceptor = DorSessionAcceptor(
                configuration: DorSessionAcceptorConfiguration(
                    ...
                    judge: { _ in
                        ...
                        // pinned keys, and the relay admits only same-account
                        // Stack tokens. No additional host-side policy yet.
                    }
                )
            )
    ...
    /// hints): dor is always relay-carried — the phone resolves the Mac's
    /// deviceID from its cached grant and dials the account object with it.
    ...
    private func startAcceptLoop(acceptor: DorSessionAcceptor, token: UUID) {
    ...
    private func superviseSession(
        _ session: any DorSecureSessionProtocol,
        token: UUID
    ) async {
        let journal = Self.journal
        ...
        let eventWriter = MobileHostDorEventWriter(session: session, journal: journal)
        let rendezvous = MobileHostDorControlRendezvous()
        ...
        let controlTransport = MobileHostDorControlByteTransport(stream: control)
    ...
    private nonisolated static func runLanePump(
        session: any DorSecureSessionProtocol,
        rendezvous: MobileHostDorControlRendezvous,
    ...
    /// Maps the verified dor peer onto the admitted-peer value the
    /// `acceptTransport` authorization context expects.
    private nonisolated static func admittedPeer(
        for peer: DorAdmittedPeer
        ...
        CmxIrohAdmittedPeer(
            peer: CmxIrohGrantPeer(
                ...
                bindingID: peer.bindingID ?? "dor-device:\(peer.deviceID)",
    ...
    private nonisolated static func bidirectionalStream(
        ...
        CmxIrohBidirectionalStream(
            receiveStream: MobileHostDorReceiveStreamAdapter(stream: stream),
            sendStream: MobileHostDorSendStreamAdapter(stream: stream)
        )
    }
}

enum MobileHostDorRuntimeError: Error {
    case signedOut
}

/// `DorIdentitySigning` over the adopted keychain identity's Ed25519 seed:
/// the same key that IS the iroh EndpointID, so every existing pair grant
/// (pinned to its lowercase hex) keeps working over dor.
struct MobileHostDorIdentitySigner: DorIdentitySigning {
    let publicKey: Data
...
/// cancels the wait.
actor MobileHostDorControlRendezvous {
    private enum State {
        case waiting
        case delivered
        case cancelled
    ...
    private var pending: (any DorStream)?
    private var waiter: CheckedContinuation<(any DorStream)?, Never>?
    private var state: State = .waiting

    /// Returns true when the stream was accepted as THE control stream.
    func offer(_ stream: any DorStream) -> Bool {
...
/// opened the stream.
struct MobileHostDorControlByteTransport: CmxByteTransport {
...
/// Adapts one dor lane stream to the readable iroh stream half the proven
/// lane servers consume. Reads re-chunk to honor the `maximumByteCount`
/// contract; `stop` maps to closing the dor stream.
actor MobileHostDorReceiveStreamAdapter: CmxIrohReceiveStream {
...
/// `setPriority` is a no-op — scheduling fairness is the session mux's
/// concern (per-stream credit windows).
struct MobileHostDorSendStreamAdapter: CmxIrohSendStream {
    let stream: any DorStream
...
/// Server-events lane writer over dor: the host opens the `events` stream
/// lazily on first send and resets it on stall so the host service can
/// renegotiate.
actor MobileHostDorEventWriter: MobileHostIndependentEventWriting {
    private let session: any DorSecureSessionProtocol
    private let journal: DorJournal
    private var stream: (any DorStream)?

    init(session: any DorSecureSessionProtocol, journal: DorJournal) {
    ...
    private func openedStream() async throws -> any DorStream {
