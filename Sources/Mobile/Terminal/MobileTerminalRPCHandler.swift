...
/// Owns the `mobile.terminal.*` data-plane RPC dispatch: the iOS-facing terminal
/// create / replay / viewport / scroll / mouse / input / paste_image / paste
/// surface, plus the mobile workspace-create echo that seeds a workspace then
/// re-lists it for the phone.
///
/// These handlers serialize and mutate live ``Workspace`` / ``TerminalPanel`` and
/// ghostty-surface state: they create terminals, drive scroll/click/input/paste
/// into the real surface (reusing the same injection machinery local typing
/// uses), record paired-device viewport reports, and project render-grid /
/// VT-export snapshots back to the phone. They reach the terminal data plane
/// (v2 param coercion, tab-manager / workspace / surface resolution, the shared
...
/// reads) only through the ``MobileTerminalRPCHost`` seam. This type replaces the
/// former inline `v2Mobile*` methods on the data-plane god object: the same
/// logic, relocated off the god object into a constructor-injected owner.
///
/// `@MainActor` because every body it owns reads or mutates main-actor workspace
/// / surface state and the seam it drives is `@MainActor`.
@MainActor
final class MobileTerminalRPCHandler {
    private let host: any MobileTerminalRPCHost

    /// - Parameter host: the terminal data-plane seam the handlers drive (param
    ///   coercion, workspace / surface resolution, validation, viewport reports,
    ///   render-grid / scroll projection, ghostty / pasteboard / agent reads, and
    ///   the localized terminal error messages).
    ...
    func terminalCreate(params: [String: Any]) -> TerminalController.V2CallResult {
        ...
        guard let paneId = host.mobileTerminalDefaultPaneId(in: workspace) else {
    ...
    func terminalReplay(params: [String: Any]) -> TerminalController.V2CallResult {
        ...
        if let error = host.mobileTerminalAliasValidationError(params: params) {
            return error
        }
        guard let resolved = host.mobileTerminalResolveWorkspaceAndSurface(params: params, requireTerminal: true),
              let surfaceId = resolved.surfaceId,
              let terminalPanel = resolved.workspace.terminalPanel(for: surfaceId) else {
            #if DEBUG
            cmuxDebugLog("mobile.terminal.replay NOT_FOUND surface=\(host.mobileTerminalRawString(params, "surface_id") ?? "nil")")
            #endif
            return .err(code: "not_found", message: "Terminal surface not found", data: nil)
        }
        let state = MobileTerminalByteTee.shared.replayState(surfaceID: surfaceId)
        let seq = state?.seq ?? 0
        let renderGrid = host.mobileTerminalReplayRenderGridFrame(
        ...
        if let renderGrid,
           let renderGridObject = try? renderGrid.jsonObject() {
            ...
            let snapshotData = host.mobileTerminalActiveVTExportSnapshot(
                terminalPanel: terminalPanel
            )?.data(using: .utf8) ?? Data()
            let data = state?.data ?? Data()
            if let grid = host.mobileTerminalLiveGridSize(terminalPanel: terminalPanel, reason: "mobileTerminalReplay") {
    ...
    func terminalViewport(params: [String: Any]) -> TerminalController.V2CallResult {
        ...
        if host.mobileTerminalBool(params, "clear") == true {
            if let clientID = host.mobileTerminalString(params, "client_id") {
        ...
        if let grid = host.mobileTerminalLiveGridSize(terminalPanel: terminalPanel, reason: "mobileTerminalViewport") {
            payload["columns"] = grid.columns
            payload["rows"] = grid.rows
        }
        return .ok(payload)
    ...
    func terminalInput(params: [String: Any]) -> TerminalController.V2CallResult {
        guard let text = host.mobileTerminalRawString(params, "text"), !text.isEmpty else {
    ...
    func terminalPasteImage(params: [String: Any]) -> TerminalController.V2CallResult {
        guard let base64 = host.mobileTerminalRawString(params, "image_base64"),
        ...
        let format = host.mobileTerminalRawString(params, "image_format") ?? "png"
        if let error = host.mobileTerminalWorkspaceIDValidationError(params: params) {
            return error
        }
        if let error = host.mobileTerminalAliasValidationError(params: params) {
        ...
        guard let escapedPath = host.mobileTerminalSaveImageData(imageData, format: format) else {
    ...
    func terminalPaste(params: [String: Any]) -> TerminalController.V2CallResult {
        ...
        let submitKeyRaw = (host.mobileTerminalString(params, "submit_key") ?? "return").lowercased()
        ...
        if submitKeyWasReturnIntent,
           text.contains("\n") || text.contains("\r"),
           host.mobileTerminalIsClaudeCode(panel: terminalPanel, workspace: resolved.workspace) {
            submitKeyName = "ctrl+enter"
        }
