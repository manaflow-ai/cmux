...
final class MobileHostIrxRuntime {
    ...
    /// The latest safe native endpoint failure for Settings presentation.
    /// Only ``IrxEndpointError.errorDescription`` is accepted; broker and
    /// transport errors never cross this boundary as raw strings.
    ...
    func setSettingsPhase(_ phase: SettingsPhase, error: (any Error)? = nil) {
        ...
        guard phase != settingsPhase || nextFailure != relayFailureDescription else { return }
        settingsPhase = phase
    ...
    private func activate(accountID: String) async {
        ...
        do {
            ...
            // A successful native readiness check clears any prior relay
            // diagnosis while the remaining activation metadata is published.
            recordEndpointReadiness(token: token, phase: .activating)
            try Task.checkCancellation()
            ...
            if generationToken == token {
                ...
                setSettingsPhase(.failed, error: error)
            }
            ...
            while !Task.isCancelled {
                ...
                guard let inbound = await endpointSupervisor.acceptNextInbound() else {
                    ...
                    do {
                        ...
                        await self?.recordEndpointReadiness(token: token, phase: .active)
                        guard await self?.generationToken == token,
                              await self?.canStartNetworking == true else {
                            await endpointSupervisor.close()
                            return
                        ...
                        await self?.recordEndpointFailure(error, token: token)
    ...
    /// Records only the native endpoint's safe presentation error. This keeps
    /// Settings diagnostics independent from broker/network error payloads.
    private func recordEndpointFailure(_ error: any Error, token: UUID) {
        guard generationToken == token, canStartNetworking,
              error is IrxEndpointError else { return }
        setSettingsPhase(.failed, error: error)
    }

    private func recordEndpointReadiness(token: UUID, phase: SettingsPhase) {
        // The caller has completed native readiness. Check the generation
        // without another suspension before clearing this generation's error.
        guard generationToken == token, canStartNetworking else { return }
