...
final class AgentChatSessionRegistry {
    ...
    private var recordRemovalObservers: [(AgentChatSessionRecord) -> Void] = []
    func addRecordChangeObserver(_ observer: @escaping (AgentChatSessionRecord, AgentChatSessionRecord?) -> Void) { recordChangeObservers.append(observer) }
    func addRecordRemovalObserver(_ observer: @escaping (AgentChatSessionRecord) -> Void) { recordRemovalObservers.append(observer) }
    func notifyRecordChange(_ record: AgentChatSessionRecord, _ previous: AgentChatSessionRecord?) { recordChangeObservers.forEach { $0(record, previous) } }
    private func notifyRecordRemoval(_ record: AgentChatSessionRecord) { recordRemovalObservers.forEach { $0(record) } }
