...
/// Callback bundle handed to a native section row in place of a store reference.
/// Every capability the row needs is expressed as a closure so no child row
/// below the snapshot boundary can subscribe to broad store updates. Keeping
/// the store out of row types makes accidental broad subscriptions a type
/// error rather than a silent 100% CPU regression.
...
/// Callback bundle for native section reorder gaps.
