...
struct CloudPortScanResult: Equatable, Sendable {
    let ports: [Int]
    let loopbackOnlyPorts: [Int]
    let otherBindingPorts: [Int]

    init(ports: [Int], loopbackOnlyPorts: [Int] = [], otherBindingPorts: [Int] = []) {
        self.ports = ports
        self.loopbackOnlyPorts = loopbackOnlyPorts
        self.otherBindingPorts = otherBindingPorts
    }

    init?(socketListing: String) {
    ...
    var state: CloudPortDiscoveryState {
