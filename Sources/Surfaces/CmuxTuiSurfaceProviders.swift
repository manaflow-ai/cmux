...
final class CmuxTuiSurfaceProvider: SurfaceProvider {
    ...
    /// Close compatibility reads provider-owned receipts without exposing the maps.
    func fallbackTabIDForClose(_ resourceID: SurfaceResourceID) -> String? {
        pendingRemoteCreations[resourceID]?.tabID ?? tabByTerminal[resourceID.key]
    }

    func retirePendingCreation(_ resourceID: SurfaceResourceID) {
        pendingRemoteCreations.removeValue(forKey: resourceID)
    }
