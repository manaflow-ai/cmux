...
extension Workspace {
    ...
    func retryCloudTerminalMaterialization(surfaceID: UUID) -> Bool {
        ...
        let retryID = UUID()
        cloudMaterializationRetryID = retryID
        cloudMaterializationRetryTask = Task { @MainActor [weak self] in
            guard !Task.isCancelled else { return }
            if let provider = await CmuxTuiSurfaceProviderRegistry.shared.providerRefreshingIfMissing(machineID: machineID) {
                await provider.refresh(force: true)
            }
            guard let self, !Task.isCancelled, self.cloudMaterializationRetryID == retryID else { return }
            self.cloudMaterializationRetryTask = nil
            self.cloudMaterializationRetryID = nil
        }
        return true
    }
