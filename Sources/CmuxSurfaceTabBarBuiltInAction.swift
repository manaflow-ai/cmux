...
enum CmuxSurfaceTabBarBuiltInAction: String, Codable, Sendable, CaseIterable, Hashable {
    ...
    init?(configID: String) {
        ...
        case "cmux.newBrowser", "newBrowser":
            self = .newBrowser
        case "cmux.openFile", "openFile", "open-file":
            self = .openFile
    ...
    var resolvedConfigMetadata: (title: String, keywords: [String]) {
        ...
        case .openFile:
            return (String(localized: "command.openFile.title", defaultValue: "Open File…"), ["open", "file", "document", "choose", "import", "surface"])
        case .newSimulator:
    ...
    var bonsplitAction: BonsplitConfiguration.SplitActionButton.Action? {
        switch self {
        case .newWorkspace, .newAgentChat, .cloudVM, .mobileConnect, .openFile, .newSimulator:
            return nil
        case .newTerminal:
            return .newTerminal
        case .newBrowser:
