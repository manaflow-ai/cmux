...
extension Workspace {
  nonisolated static let remoteDaemonManifestInfoKey = WorkspaceRemoteSessionController
    .remoteDaemonManifestInfoKey

  nonisolated static func remoteDaemonManifest(from infoDictionary: [String: Any]?)
  ...
  func restoreSessionSnapshot(_ snapshot: SessionWorkspaceSnapshot) {
    ...
    let normalizedCurrentDirectory = snapshot.currentDirectory.trimmingCharacters(
    ...
    applySessionDividerPositions(
      snapshotNode: snapshot.layout, liveNode: bonsplitController.treeSnapshot())
    ...
    gitBranch = snapshot.gitBranch.map {
  ...
  private func sessionSurfaceUUID(for surfaceId: TabID) -> UUID? {
    ...
    guard let data = try? JSONEncoder().encode(surfaceId),
      let decoded = try? JSONDecoder().decode(EncodedSurfaceID.self, from: data)
    else {
      return nil
    }
    return decoded.id
  ...
  private func sessionPanelSnapshot(
    ...
    case .terminal:
      ...
      let capturedScrollback =
        includeScrollback && shouldPersistScrollback
  ...
  private func restoreSessionLayout(_ layout: SessionWorkspaceLayoutSnapshot)
  ...
  private func restoreSessionLayoutNode(
    ...
    case .split(let split):
      ...
      guard let anchorPanelId,
        ...
        let secondPaneId = self.paneId(forPanelId: newSplitPanel.id)
      else {
        leaves.append(
          SessionPaneRestoreEntry(
            paneId: paneId,
  ...
  private func createPanel(from snapshot: SessionPanelSnapshot, inPane paneId: PaneID) -> UUID? {
    ...
    case .terminal:
      ...
      let fallbackScrollback =
        shouldReplayScrollback
  ...
  private func applySessionPanelMetadata(_ snapshot: SessionPanelSnapshot, toPanelId panelId: UUID)
  {
    ...
    if let directory = snapshot.directory?.trimmingCharacters(in: .whitespacesAndNewlines),
      !directory.isEmpty
    {
    ...
    if let branch = snapshot.gitBranch {
      panelGitBranches[panelId] = SidebarGitBranchState(
        branch: branch.branch, isDirty: branch.isDirty)
    } else {
    ...
    if let ttyName = snapshot.ttyName?.trimmingCharacters(in: .whitespacesAndNewlines),
      !ttyName.isEmpty
    {
...
extension Workspace {
  ...
  func applyCustomLayout(_ layout: CmuxLayoutNode, baseCwd: String) {
    ...
    for leaf in leaves {
      populateCustomPane(
        leaf.paneId, surfaces: leaf.surfaces, baseCwd: baseCwd, focusPanelId: &focusPanelId)
    }
...
final class WorkspaceRemoteDaemonPendingCallRegistry {
  ...
  private let queue = DispatchQueue(
    label: "com.cmux.remote-ssh.daemon-rpc.pending.\(UUID().uuidString)")
...
enum WorkspaceRemoteSSHBatchCommandBuilder {
  ...
  static func reverseRelayControlMasterArguments(
    ...
    guard
      let controlPath = sshOptionValue(named: "ControlPath", in: configuration.sshOptions)?
...
private final class WorkspaceRemoteDaemonRPCClient {
  ...
  private let writeQueue = DispatchQueue(
    label: "com.cmux.remote-ssh.daemon-rpc.write.\(UUID().uuidString)")
  private let stateQueue = DispatchQueue(
    label: "com.cmux.remote-ssh.daemon-rpc.state.\(UUID().uuidString)")
  ...
  func start() throws {
    ...
    do {
      ...
      guard capabilities.contains(Self.requiredProxyStreamCapability) else {
        throw NSError(
          domain: "cmux.remote.daemon.rpc", code: 2,
          userInfo: [
            NSLocalizedDescriptionKey:
              "remote daemon missing required capability \(Self.requiredProxyStreamCapability)"
          ])
      }
    } catch {
  ...
  private func startViaSSHExec() throws {
    ...
    do {
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 1,
        userInfo: [
          NSLocalizedDescriptionKey:
            "Failed to launch SSH daemon transport: \(error.localizedDescription)"
        ])
    }
  ...
  private func startViaBakedVMSocketForward() throws {
    ...
    do {
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 18,
        userInfo: [
          NSLocalizedDescriptionKey:
            "Failed to launch SSH daemon socket forward: \(error.localizedDescription)"
        ])
    }
    ...
    if let startupFailure = Self.startupFailureDetail(
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 19,
        userInfo: [
    ...
    do {
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 20,
        userInfo: [
          NSLocalizedDescriptionKey:
            "Failed to connect VM daemon socket forward: \(error.localizedDescription)"
        ])
    }
  ...
  private func startViaWebSocket() throws {
    guard let endpoint = configuration.daemonWebSocketEndpoint else {
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 23,
        userInfo: [
    ...
    guard let url = URL(string: endpoint.url),
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 24,
        userInfo: [
    ...
    guard delegate.waitForOpen(timeout: 15.0) else {
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 25,
        userInfo: [
    ...
    do {
      ...
      stop(suppressTerminationCallback: true)
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 26,
        userInfo: [
          NSLocalizedDescriptionKey:
            "failed authenticating daemon websocket: \(error.localizedDescription)"
        ])
    }
  }
  ...
  func openStream(host: String, port: Int, timeoutMs: Int = 10000) throws -> String {
    ...
    let streamID =
      (result["stream_id"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    guard !streamID.isEmpty else {
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 3,
        userInfo: [
  ...
  func attachStream(
    ...
    guard !trimmedStreamID.isEmpty else {
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 17,
        userInfo: [
  ...
  private func call(method: String, params: [String: Any], timeout: TimeInterval) throws -> [String:
    Any]
  {
    ...
    do {
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 10,
        userInfo: [
          NSLocalizedDescriptionKey:
            "failed to encode daemon RPC request \(method): \(error.localizedDescription)"
        ])
    }
    ...
    case .timedOut:
      ...
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 11,
        userInfo: [
          NSLocalizedDescriptionKey: "daemon RPC timeout waiting for \(method) response"
        ])
    case .failure(let failure):
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 12,
        userInfo: [
          NSLocalizedDescriptionKey: failure
        ])
    case .missing:
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 13,
        userInfo: [
    ...
    let code =
      (errorObject["code"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines)
      ?? "rpc_error"
    let message =
      (errorObject["message"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines)
      ?? "daemon RPC call failed"
    throw NSError(
      domain: "cmux.remote.daemon.rpc", code: 14,
      userInfo: [
  ...
  private func writePayload(_ payload: Data) throws {
    ...
    if let webSocketTask {
      guard let text = String(data: payload, encoding: .utf8) else {
        throw NSError(
          domain: "cmux.remote.daemon.rpc", code: 27,
          userInfo: [
      ...
      if let sendError {
        stop(suppressTerminationCallback: false)
        throw NSError(
          domain: "cmux.remote.daemon.rpc", code: 16,
          userInfo: [
            NSLocalizedDescriptionKey:
              "failed writing daemon RPC request: \(sendError.localizedDescription)"
          ])
      }
      return
    }
    ...
    if stdinHandle === FileHandle.nullDevice {
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 15,
        userInfo: [
    ...
    do {
      ...
      throw NSError(
        ...
        userInfo: [
          NSLocalizedDescriptionKey:
            "failed writing daemon RPC request: \(error.localizedDescription)"
        ])
    }
  }
  ...
  private func consumeStdoutData(_ data: Data) {
    ...
    if stdoutBuffer.count > Self.maxStdoutBufferBytes {
      ...
      signalPendingFailureLocked(
        "daemon transport stdout exceeded \(Self.maxStdoutBufferBytes) bytes without message framing"
      )
      process?.terminate()
      return
    }
      ...
      self.stateQueue.async {
        ...
        case .failure(let error):
          if delegate.isClosed || self.isClosed {
            ...
            self.handleWebSocketTermination(
              "daemon websocket failed: \(error.localizedDescription)")
          }
        }
      }
    }
  ...
  private func handleProcessTermination(_ process: Process) {
    ...
    let detail =
      Self.bestErrorLine(stderr: stderrBuffer)
      ?? "daemon transport exited with status \(process.terminationStatus)"
  ...
  private func stop(suppressTerminationCallback: Bool) {
    let captured:
      (
        Process?, FileHandle?, FileHandle?, FileHandle?, URLSessionWebSocketTask?, URLSession?,
        Bool, String
      ) = stateQueue.sync {
  ...
  private static func usesSocketForwardTransport(configuration: WorkspaceRemoteConfiguration)
    -> Bool
  {
  ...
  private static func daemonArguments(
    configuration: WorkspaceRemoteConfiguration, remotePath: String
  ) -> [String] {
  ...
  private static func allocateLoopbackPort() throws -> Int {
    ...
    throw NSError(
      domain: "cmux.remote.daemon.rpc", code: 21,
      userInfo: [
  ...
  private static func connectLoopbackSocket(port: Int) throws -> FileHandle {
    guard port > 0 && port <= 65535 else {
      throw NSError(
        domain: "cmux.remote.daemon.rpc", code: 22,
        userInfo: [
    ...
    guard connectResult == 0 else {
      ...
      close(fd)
      throw NSError(
        domain: NSPOSIXErrorDomain, code: Int(errorCode),
        userInfo: [
...
enum RemoteLoopbackHTTPRequestRewriter {
  ...
  private static let requestLineMethods = [
    "GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS", "TRACE", "PRI",
  ]
  ...
  static func rewriteIfNeeded(data: Data, aliasHost: String, allowIncompleteHeadersAtEOF: Bool)
    -> Data
  {
    let headerData: Data
    let remainder: Data
  ...
  private static func requestLineLooksHTTP(_ requestLine: String) -> Bool {
    ...
    let method =
      trimmed.split(separator: " ", maxSplits: 1).first.map(String.init)?.uppercased() ?? ""
  ...
  private static func rewriteRequestLine(_ requestLine: String, aliasHost: String) -> String {
    ...
    guard let host = components?.host,
      BrowserInsecureHTTPSettings.normalizeHost(host)
        == BrowserInsecureHTTPSettings.normalizeHost(aliasHost)
    else {
      return requestLine
    }
    ...
    let trailingTrivia = String(
      requestLine.reversed().prefix { $0.isWhitespace || $0.isNewline }.reversed())
  ...
  private static func rewriteHeaderLine(_ line: String, aliasHost: String) -> String {
    ...
    switch name {
    case "host":
      guard let rewrittenHost = rewriteHostValue(rawValue, aliasHost: aliasHost) else {
        return line
      }
  ...
  private static func rewriteHostValue(_ value: String, aliasHost: String) -> String? {
    ...
    guard
      BrowserInsecureHTTPSettings.normalizeHost(trimmed)
        == BrowserInsecureHTTPSettings.normalizeHost(aliasHost)
    else {
      return nil
    }
...
enum RemoteLoopbackHTTPResponseRewriter {
  ...
  private static func rewriteHeaderLine(_ line: String, aliasHost: String) -> String {
    ...
    case "location", "content-location", "origin", "referer", "access-control-allow-origin":
      ...
      return "\(line[..<valueStart]) \(rewrittenURL)"
    case "set-cookie":
      guard let rewrittenCookie = rewriteCookieValue(rawValue, aliasHost: aliasHost) else {
        return line
      }
  ...
  private static func rewriteURLValue(_ value: String, aliasHost: String) -> String? {
    ...
    guard let host = components?.host,
      BrowserInsecureHTTPSettings.normalizeHost(host)
        == BrowserInsecureHTTPSettings.normalizeHost(canonicalLoopbackHost)
    else {
      return nil
    }
  ...
  private static func rewriteCookieValue(_ value: String, aliasHost: String) -> String? {
    ...
    let rewrittenParts = parts.map { part -> String in
      ...
      guard
        BrowserInsecureHTTPSettings.normalizeHost(domainValue)
          == BrowserInsecureHTTPSettings.normalizeHost(canonicalLoopbackHost)
      else {
        return part
      }
...
private final class WorkspaceRemoteDaemonProxyTunnel {
  private final class ProxySession {
    ...
    private func processSocksHandshakeStep() -> Bool {
      ...
      case .request:
        ...
        let pending =
          handshakeBuffer.count > request.consumedBytes
    ...
    private func parseSocksRequest(from data: Data) throws -> SocksRequest? {
      ...
      guard bytes[0] == 0x05 else {
        throw NSError(
          domain: "cmux.remote.proxy", code: 1,
          userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS version"])
      }
      ...
      default:
        throw NSError(
          domain: "cmux.remote.proxy", code: 2,
          userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS address type"])
      }

      guard !host.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
        throw NSError(
          domain: "cmux.remote.proxy", code: 3,
          userInfo: [NSLocalizedDescriptionKey: "empty SOCKS host"])
      }
      ...
      guard port > 0 && port <= 65535 else {
        throw NSError(
          domain: "cmux.remote.proxy", code: 4,
          userInfo: [NSLocalizedDescriptionKey: "invalid SOCKS port"])
      }
    ...
    private func processConnectHandshakeStep() -> Bool {
      ...
      let pending =
        headerRange.upperBound < handshakeBuffer.count
      ...
      openRemoteStream(
        host: host,
        port: port,
        successResponse: Self.httpResponse(
          status: "200 Connection Established", closeAfterResponse: false),
  ...
  private let queue = DispatchQueue(
    label: "com.cmux.remote-ssh.daemon-tunnel.\(UUID().uuidString)", qos: .utility)
    ...
    queue.sync {
      guard !isStopped else {
        capturedError = NSError(
          domain: "cmux.remote.proxy", code: 20,
          userInfo: [
  ...
  private static func makeLoopbackListener(port: Int) throws -> NWListener {
    guard let localPort = NWEndpoint.Port(rawValue: UInt16(port)) else {
      throw NSError(
        domain: "cmux.remote.proxy", code: 21,
        userInfo: [
    ...
    parameters.requiredLocalEndpoint = .hostPort(
      host: NWEndpoint.Host("127.0.0.1"), port: localPort)
    ...
    if let forcedLocalPort = entry.configuration.localProxyPort {
      ...
      guard let allocatedPort = Self.allocateLoopbackPort() else {
        notifyLocked(
          entry,
          update: .error(
            "Failed to allocate local proxy port\(Self.retrySuffix(delay: retryDelay))")
        )
...
private final class WorkspaceRemoteCLIRelayServer {
  private final class Session {
    ...
    private func receive() {
      guard !isClosed else { return }
      connection.receive(minimumIncompleteLength: 1, maximumLength: maximumFrameBytes) {
        [weak self] data, _, isComplete, error in
        guard let self else { return }
        self.queue.async {
          if error != nil {
    ...
    private func handleAuthLine(_ line: String) {
      ...
      let message = Self.authMessage(
        relayID: relayID, nonce: challengeNonce, version: challengeVersion)
    ...
    private func handleCommandLine(_ commandLine: Data) {
      ...
      DispatchQueue.global(qos: .utility).async { [localSocketPath, commandLine, queue] in
        let result = Result {
          try Self.roundTripUnixSocket(socketPath: localSocketPath, request: commandLine)
        }
        queue.async { [weak self] in
          guard let self else { return }
          switch result {
    ...
    private static func roundTripUnixSocket(socketPath: String, request: Data) throws -> Data {
      ...
      guard fd >= 0 else {
        throw NSError(
          domain: "cmux.remote.relay", code: 1,
          userInfo: [
      ...
      guard pathBytes.count <= MemoryLayout.size(ofValue: address.sun_path) else {
        throw NSError(
          domain: "cmux.remote.relay", code: 2,
          userInfo: [
      ...
      guard connectResult == 0 else {
        throw NSError(
          domain: "cmux.remote.relay", code: 3,
          userInfo: [
      ...
      try request.withUnsafeBytes { rawBuffer in
        ...
        while bytesRemaining > 0 {
          ...
          if written <= 0 {
            throw NSError(
              domain: "cmux.remote.relay", code: 4,
              userInfo: [
  ...
  private let relayToken: Data
  private let queue = DispatchQueue(
    label: "com.cmux.remote-ssh.cli-relay.\(UUID().uuidString)", qos: .utility)
  ...
  init(localSocketPath: String, relayID: String, relayTokenHex: String) throws {
    guard let relayToken = Session.hexData(from: relayTokenHex), !relayToken.isEmpty else {
      throw NSError(
        domain: "cmux.remote.relay", code: 7,
        userInfo: [
  ...
  func start() throws -> Int {
    ...
    if waitResult != .success {
      ...
      listener.cancel()
      throw NSError(
        domain: "cmux.remote.relay", code: 8,
        userInfo: [
...
final class WorkspaceRemoteSessionController {
  ...
  private let queue = DispatchQueue(
    label: "com.cmux.remote-ssh.\(UUID().uuidString)", qos: .utility)
  ...
  private var didConsumePrebootstrappedDaemon = false
  ...
  func uploadDroppedFiles(
    ...
    queue.async { [weak self] in
      guard let self else {
        Self.completeDroppedFileUploadOnMain(
          .failure(RemoteDropUploadError.unavailable),
      ...
      do {
        ...
        Self.completeDroppedFileUploadOnMain(.failure(error), completion: completion)
      }
    }
  }

  private static func completeDroppedFileUploadOnMain(
  ...
  private func completeDroppedFileUploadOnMain(
    ...
    Task { @MainActor [weak self] in
      if operation.isCancelled {
        if case .success(let remotePaths) = result, !remotePaths.isEmpty {
  ...
  private func beginConnectionAttemptLocked() {
    ...
    if reconnectRetryCount > 0 {
      connectDetail =
        "Reconnecting to \(configuration.displayTarget) (retry \(reconnectRetryCount))"
      bootstrapDetail =
        "Bootstrapping remote daemon on \(configuration.displayTarget) (retry \(reconnectRetryCount))"
    } else {
    ...
    do {
      ...
      } else if let prebootstrappedDaemon = configuration.prebootstrappedDaemon,
        !didConsumePrebootstrappedDaemon
      {
        didConsumePrebootstrappedDaemon = true
        hello = DaemonHello(
          name: prebootstrappedDaemon.name,
          version: prebootstrappedDaemon.version,
          capabilities: prebootstrappedDaemon.capabilities,
          remotePath: prebootstrappedDaemon.remotePath
        )
        debugLog(
          "remote.bootstrap.skipped reason=cmx-rust-prebootstrapped remotePath=\(hello.remotePath)"
        )
      } else {
        hello = try bootstrapDaemonLocked()
      }
      guard
        hello.capabilities.contains(WorkspaceRemoteDaemonRPCClient.requiredProxyStreamCapability)
      else {
        throw NSError(
          domain: "cmux.remote.daemon", code: 43,
          userInfo: [
            NSLocalizedDescriptionKey:
              "remote daemon missing required capability \(WorkspaceRemoteDaemonRPCClient.requiredProxyStreamCapability)"
          ])
      }
      daemonReady = true
      ...
      if configuration.skipDaemonBootstrap {
        debugLog(
          "remote.relay.skipped reason=vm-baked transport=\(configuration.transport.rawValue)")
  ...
  private func startReverseRelayLocked(remotePath: String) {
    ...
    do {
      ...
      let stderrPipe = Pipe()
      process.executableURL = URL(fileURLWithPath: "/usr/bin/ssh")
      process.arguments = reverseRelayArguments(
        relayPort: relayPort, localRelayPort: localRelayPort)
      process.standardInput = FileHandle.nullDevice
      process.standardOutput = FileHandle.nullDevice
      process.standardError = stderrPipe
      ...
      if let startupFailure = Self.reverseRelayStartupFailureDetail(
        ...
        debugLog(
          "remote.relay.startFailed relayPort=\(relayPort) " + "error=\(startupFailure)"
        )
        relayServer?.stop()
        publishDaemonStatus(
          .error,
      ...
      debugLog(
        "remote.relay.startFailed relayPort=\(relayPort) " + "error=\(error.localizedDescription)"
      )
      relayServer?.stop()
      cliRelayServer = nil
      scheduleReverseRelayRestartLocked(remotePath: remotePath, delay: 2.0)
  ...
  private func handleProxyBrokerUpdateLocked(_ update: WorkspaceRemoteProxyBroker.Update) {
    ...
    case .ready(let endpoint):
      debugLog(
        "remote.proxy.ready host=\(endpoint.host) port=\(endpoint.port) \(debugConfigSummary())")
      reconnectWorkItem?.cancel()
      reconnectWorkItem = nil
      reconnectRetryCount = 0
      ...
      publishState(
        .connected,
        detail:
          "Connected to \(configuration.displayTarget) via shared local proxy \(endpoint.host):\(endpoint.port)"
      )
      requestBootstrapRemoteTTYIfNeededLocked()
      recordHeartbeatActivityLocked()
    case .error(let detail):
      ...
      publishPortsSnapshotLocked()
      publishState(
        .error, detail: "Remote proxy to \(configuration.displayTarget) unavailable: \(detail)")
  ...
  private func publishPortsSnapshotLocked() {
    let controllerID = self.controllerID
    let detectedByPanel = remotePortScanTTYNames.keys.reduce(into: [UUID: [Int]]()) {
      result, panelId in
  ...
  private func requestBootstrapRemoteTTYIfNeededLocked() {
    ...
    let command =
      "sh -c \(Self.shellSingleQuoted("tty_path=\"$HOME/.cmux/relay/\(relayPort).tty\"; if [ -r \"$tty_path\" ]; then cat \"$tty_path\"; fi"))"
    do {
      ...
      debugLog(
        "remote.tty.bootstrap.failed error=\(error.localizedDescription) \(debugConfigSummary())")
      scheduleBootstrapRemoteTTYRetryLocked()
    }
  }
  ...
  private func startReverseRelayViaControlMasterLocked(forwardSpec: String) -> Bool {
    guard
      let arguments = WorkspaceRemoteSSHBatchCommandBuilder.reverseRelayControlMasterArguments(
        configuration: configuration,
    ...
    do {
      ...
      guard result.status == 0 else {
        let detail =
          Self.bestErrorLine(stderr: result.stderr, stdout: result.stdout)
      ...
      debugLog(
        "remote.relay.controlmaster.forwardFailed \(error.localizedDescription) \(debugConfigSummary())"
      )
      return false
    }
  }
  ...
  private func sshExec(arguments: [String], stdin: Data? = nil, timeout: TimeInterval = 15) throws
    -> CommandResult
  {
    try runProcess(
  ...
  private func runProcess(
    ...
    do {
      ...
      throw NSError(
        domain: "cmux.remote.process", code: 1,
        userInfo: [
          NSLocalizedDescriptionKey:
            "Failed to launch \(URL(fileURLWithPath: executable).lastPathComponent): \(error.localizedDescription)"
        ])
    }
    try? stdoutPipe.fileHandleForWriting.close()
    try? stderrPipe.fileHandleForWriting.close()
    ...
    if !didExitBeforeTimeout, process.isRunning {
      ...
      throw NSError(
        domain: "cmux.remote.process", code: 2,
        userInfo: [
          NSLocalizedDescriptionKey:
  ...
  private func bootstrapDaemonLocked() throws -> DaemonHello {
    ...
    let remotePath = Self.remoteDaemonPath(
      version: version, goOS: platform.goOS, goArch: platform.goArch)
    ...
    {
      debugLog(
        "remote.bootstrap.capabilityMissing remotePath=\(remotePath) capabilities=\(hello.capabilities.joined(separator: ","))"
      )
      let localBinary = try buildLocalDaemonBinary(
        goOS: platform.goOS, goArch: platform.goArch, version: version)
      try uploadRemoteDaemonBinaryLocked(localBinary: localBinary, remotePath: remotePath)
  ...
  private func ensureCLIRelayServerLocked(
    localSocketPath: String, relayID: String, relayToken: String
  ) throws -> WorkspaceRemoteCLIRelayServer {
  ...
  private func installRemoteRelayMetadataLocked(
    ...
    guard result.status == 0 else {
      let detail =
        ...
        ?? "ssh exited \(result.status)"
      throw NSError(
        domain: "cmux.remote.relay", code: 70,
        userInfo: [
  ...
  private func probeRemoteBootstrapStateLocked(version: String) throws -> RemoteBootstrapState {
    ...
    guard let unameOS, let unameArch else {
      let detail =
        ...
        ?? "ssh exited \(result.status)"
      throw NSError(
        domain: "cmux.remote.daemon", code: 11,
        userInfo: [
    ...
    guard let goOS = Self.mapUnameOS(unameOS),
      let goArch = Self.mapUnameArch(unameArch)
    else {
      throw NSError(
        domain: "cmux.remote.daemon", code: 12,
        userInfo: [
    ...
    if result.status != 0, binaryExists == nil {
      let detail =
        ...
        ?? "ssh exited \(result.status)"
      throw NSError(
        domain: "cmux.remote.daemon", code: 13,
        userInfo: [
  ...
  static func remoteDaemonManifest(from infoDictionary: [String: Any]?)
    -> WorkspaceRemoteDaemonManifest?
  {
    guard let rawManifest = infoDictionary?[remoteDaemonManifestInfoKey] as? String else {
      return nil
    }
  ...
  private static func explicitRemoteDaemonBinaryURL(
    ...
    guard
      let path = environment["CMUX_REMOTE_DAEMON_BINARY"]?.trimmingCharacters(
        in: .whitespacesAndNewlines),
      !path.isEmpty
    else {
      return nil
  ...
  private static func versionedRemoteDaemonBuildURL(goOS: String, goArch: String, version: String)
    -> URL
  {
  ...
  private static func fetchRemoteManifestLocked(releaseURL: String, version: String)
    -> WorkspaceRemoteDaemonManifest?
  {
    guard let manifestURL = URL(string: "\(releaseURL)/cmuxd-remote-manifest.json") else {
      return nil
    }
  ...
  private func downloadRemoteDaemonBinaryLocked(
    entry: WorkspaceRemoteDaemonManifest.Entry, version: String, releaseURL: String? = nil
  ) throws -> URL {
    guard let url = URL(string: entry.downloadURL) else {
      throw NSError(
        domain: "cmux.remote.daemon", code: 25,
        userInfo: [
          NSLocalizedDescriptionKey: "remote daemon manifest has an invalid download URL"
        ])
    }

    let cacheURL = try Self.remoteDaemonCachedBinaryURL(
      version: version, goOS: entry.goOS, goArch: entry.goArch)
    ...
    session.downloadTask(with: request as URLRequest) { localURL, response, error in
        ...
        !(200...299).contains(httpResponse.statusCode)
      {
        downloadError = NSError(
          domain: "cmux.remote.daemon", code: 26,
          userInfo: [
            NSLocalizedDescriptionKey:
              "remote daemon download failed with HTTP \(httpResponse.statusCode)"
          ])
        return
      }
    ...
    guard let downloadedURL else {
      throw NSError(
        domain: "cmux.remote.daemon", code: 27,
        userInfo: [
    ...
    if downloadedSHA != entry.sha256.lowercased() {
      ...
      {
        debugLog(
          "remote.download.checksum-fallback: embedded manifest checksum stale, live manifest matched for \(entry.assetName)"
        )
      } else {
        throw NSError(
          domain: "cmux.remote.daemon", code: 28,
          userInfo: [
  ...
  private func buildLocalDaemonBinary(goOS: String, goArch: String, version: String) throws -> URL {
    ...
    {
      let cacheURL = try Self.remoteDaemonCachedBinaryURL(
        version: manifest.appVersion, goOS: goOS, goArch: goArch)
      ...
      let downloadedURL = try downloadRemoteDaemonBinaryLocked(
        entry: entry, version: manifest.appVersion, releaseURL: manifest.releaseURL)
    ...
    guard Self.allowLocalDaemonBuildFallback() else {
      throw NSError(
        domain: "cmux.remote.daemon", code: 20,
        userInfo: [
          NSLocalizedDescriptionKey:
            "this build does not include a verified cmuxd-remote manifest for \(goOS)-\(goArch). Use a release/nightly build, or set CMUX_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 for a dev-only fallback."
        ])
    }
    ...
    guard let repoRoot = Self.findRepoRoot() else {
      throw NSError(
        ...
        userInfo: [
          NSLocalizedDescriptionKey:
            "cannot locate cmux repo root for dev-only cmuxd-remote build fallback"
        ])
    }
    ...
    guard FileManager.default.fileExists(atPath: goModPath) else {
      throw NSError(
        domain: "cmux.remote.daemon", code: 21,
        userInfo: [
    ...
    guard let goBinary = Self.which("go") else {
      throw NSError(
        domain: "cmux.remote.daemon", code: 22,
        userInfo: [
    ...
    let ldflags =
      goOS == "darwin"
      ? "-s -w -linkmode=external -X main.version=\(version)" : "-s -w -X main.version=\(version)"
    let result = try runProcess(
      executable: goBinary,
      arguments: [
        "build", "-trimpath", "-buildvcs=false", "-ldflags", ldflags, "-o", output.path,
        "./cmd/cmuxd-remote",
      ],
      environment: env,
    ...
    guard result.status == 0 else {
      ...
      throw NSError(
        domain: "cmux.remote.daemon", code: 23,
        userInfo: [
    ...
    guard FileManager.default.isExecutableFile(atPath: output.path) else {
      throw NSError(
        domain: "cmux.remote.daemon", code: 24,
        userInfo: [
  ...
  private func uploadRemoteDaemonBinaryLocked(localBinary: URL, remotePath: String) throws {
    ...
    guard mkdirResult.status == 0 else {
      let detail =
        Self.bestErrorLine(stderr: mkdirResult.stderr, stdout: mkdirResult.stdout)
        ?? "ssh exited \(mkdirResult.status)"
      throw NSError(
        domain: "cmux.remote.daemon", code: 30,
        userInfo: [
    ...
    guard scpResult.status == 0 else {
      let detail =
        Self.bestErrorLine(stderr: scpResult.stderr, stdout: scpResult.stdout)
        ?? "scp exited \(scpResult.status)"
      throw NSError(
        domain: "cmux.remote.daemon", code: 31,
        userInfo: [
    ...
    let finalizeResult = try sshExec(
      arguments: sshCommonArguments(batchMode: true) + [
        configuration.destination, finalizeCommand,
      ], timeout: 12)
    guard finalizeResult.status == 0 else {
      let detail =
        Self.bestErrorLine(stderr: finalizeResult.stderr, stdout: finalizeResult.stdout)
        ?? "ssh exited \(finalizeResult.status)"
      throw NSError(
        domain: "cmux.remote.daemon", code: 32,
        userInfo: [
  ...
  private func uploadDroppedFilesLocked(
    ...
    if configuration.transport == .ssh, Self.desktopCmxRemoteDropUploadEnabled() {
      return try uploadDroppedFilesWithDesktopCmxBackendLocked(fileURLs, operation: operation)
    }
  ...
  private func uploadDroppedFilesWithDesktopCmxBackendLocked(
    _ fileURLs: [URL],
    operation: TerminalImageTransferOperation
  ) throws -> [String] {
    guard let workspace else {
      throw RemoteDropUploadError.unavailable
    }
    ...
    operation.installCancellationHandler {
      Self.cancelDesktopCmxRemoteDropUpload(workspaceID: workspaceID, operationID: operationID)
    }
    defer { operation.clearCancellationHandler() }
    try operation.throwIfCancelled()

    do {
      let result = try Self.desktopCmxSocketV2Request(
        method: "workspace.remote.upload_dropped_files",
        params: [
      ...
      guard let remotePaths = result["remote_paths"] as? [String] else {
        throw RemoteDropUploadError.uploadFailed("cmx response did not include remote_paths")
      }
      return remotePaths
    } catch {
      Self.cancelDesktopCmxRemoteDropUpload(workspaceID: workspaceID, operationID: operationID)
  ...
  private func cleanupUploadedRemotePaths(_ remotePaths: [String]) {
    ...
    if configuration.transport == .ssh, Self.desktopCmxRemoteDropUploadEnabled(), let workspace {
      do {
        _ = try Self.desktopCmxSocketV2Request(
          method: "workspace.remote.cleanup_dropped_files",
          params: [
        ...
        debugLog("remote.drop.cleanup.cmx.failed error=\(error.localizedDescription)")
      }
      return
    }
  ...
  static func desktopCmxRemoteDropUploadEnabled(
    environment: [String: String] = ProcessInfo.processInfo.environment
  ) -> Bool {
    guard DesktopCmxBackendSettings.isEnabled(environment: environment) else {
      return false
    }
    if environmentFlag("CMUX_REMOTE_SSH_DROP_UPLOAD_IN_RUST_DISABLED", environment: environment) {
      return false
    }
    if environmentFlag("CMUX_REMOTE_SSH_DROP_UPLOAD_IN_RUST", environment: environment) {
      return true
    }
    if environmentFlag("CMUX_REMOTE_SSH_STACK_IN_RUST_DISABLED", environment: environment) {
      return false
    }
    return true
  }
  ...
  private static func cancelDesktopCmxRemoteDropUpload(workspaceID: String, operationID: String) {
    cancelDesktopCmxDropUpload(
      method: "workspace.remote.cancel_drop_upload",
      params: [
        "workspace_id": workspaceID,
        "operation_id": operationID,
      ]
  ...
  static func cancelDesktopCmxDropUpload(method: String, params: [String: Any]) {
    DispatchQueue.global(qos: .utility).async {
      _ = try? desktopCmxSocketV2Request(
        method: method,
        params: params,
        timeout: 5
      )
  ...
  static func desktopCmxSocketV2Request(
    method: String,
    params: [String: Any],
    timeout: TimeInterval
  ) throws -> [String: Any] {
    let socketPath = DesktopCmxBackendSettings.runtimePaths().compatibilitySocketPath
    var requestObjects: [[String: Any]] = []
    if let password = SocketControlPasswordStore.configuredPassword(allowLazyKeychainFallback: true) {
    ...
    let requestLines = try requestObjects.map { object in
      let data = try JSONSerialization.data(withJSONObject: object, options: [])
      guard let line = String(data: data, encoding: .utf8) else {
        throw RemoteDropUploadError.uploadFailed("failed to encode cmx request")
      }
      return line
    }
    let responseLines = try desktopCmxSocketRoundTrip(
      socketPath: socketPath,
      requestLines: requestLines,
      timeout: timeout
    )
    guard let responseLine = responseLines.last else {
      throw RemoteDropUploadError.uploadFailed("cmx did not return a response")
    }
    return try parseDesktopCmxV2Result(responseLine)
  }

  private static func parseDesktopCmxV2Result(_ responseLine: String) throws -> [String: Any] {
    guard let data = responseLine.data(using: .utf8),
      let object = try JSONSerialization.jsonObject(with: data) as? [String: Any]
    else {
      throw RemoteDropUploadError.uploadFailed("cmx returned an invalid response")
    }
    if let error = object["error"] as? [String: Any] {
      let message = (error["message"] as? String)
        ?? (error["code"] as? String)
        ?? "cmx request failed"
      throw RemoteDropUploadError.uploadFailed(message)
    }
    guard let result = object["result"] as? [String: Any] else {
      throw RemoteDropUploadError.uploadFailed("cmx response did not include a result")
    }
    return result
  }

  private static func desktopCmxSocketRoundTrip(
    socketPath: String,
    requestLines: [String],
    timeout: TimeInterval
  ) throws -> [String] {
    let fd = socket(AF_UNIX, SOCK_STREAM, 0)
    guard fd >= 0 else {
      throw RemoteDropUploadError.uploadFailed("failed to create cmx socket")
    }
    defer { Darwin.close(fd) }
    ...
    let pathBytes = Array(socketPath.utf8CString)
    guard pathBytes.count <= MemoryLayout.size(ofValue: address.sun_path) else {
      throw RemoteDropUploadError.uploadFailed("cmx socket path is too long")
    }
    let sunPathOffset = MemoryLayout<sockaddr_un>.offset(of: \.sun_path) ?? 0
    withUnsafeMutableBytes(of: &address) { rawBuffer in
      let destination = rawBuffer.baseAddress!.advanced(by: sunPathOffset)
    ...
    guard connectResult == 0 else {
      throw RemoteDropUploadError.uploadFailed("failed to connect to cmx socket")
    }
    ...
    for line in requestLines {
      ...
      try payload.withUnsafeBytes { rawBuffer in
        ...
        while bytesRemaining > 0 {
          let written = Darwin.write(fd, pointer, bytesRemaining)
          if written <= 0 {
            throw RemoteDropUploadError.uploadFailed("failed to write cmx request")
          }
          bytesRemaining -= written
          pointer = pointer.advanced(by: written)
        }
    ...
    var buffered = Data()
    var scratch = [UInt8](repeating: 0, count: 4096)
    while responses.count < requestLines.count {
      let count = Darwin.read(fd, &scratch, scratch.count)
      if count > 0 {
        buffered.append(scratch, count: count)
      ...
      throw RemoteDropUploadError.uploadFailed("failed to read cmx response")
    }
    guard responses.count == requestLines.count else {
      throw RemoteDropUploadError.uploadFailed("timed out waiting for cmx response")
    }
    return responses
  }
  ...
  private func helloRemoteDaemonLocked(remotePath: String) throws -> DaemonHello {
    ...
    guard result.status == 0 else {
      let detail =
        ...
        ?? "ssh exited \(result.status)"
      throw NSError(
        domain: "cmux.remote.daemon", code: 40,
        userInfo: [
    ...
    guard !responseLine.isEmpty,
      ...
      throw NSError(
        domain: "cmux.remote.daemon", code: 41,
        userInfo: [
    ...
    if let ok = payload["ok"] as? Bool, !ok {
      ...
      throw NSError(
        domain: "cmux.remote.daemon", code: 42,
        userInfo: [
    ...
    let version = (resultObject["version"] as? String)?.trimmingCharacters(
      in: .whitespacesAndNewlines)
  ...
  private func debugConfigSummary() -> String {
    let controlPath =
      Self.debugSSHOptionValue(named: "ControlPath", in: configuration.sshOptions) ?? "nil"
    return
      ...
      + "localSocket=\(configuration.localSocketPath ?? "nil") " + "controlPath=\(controlPath)"
  }
  ...
  private static let cachedRemoteDaemonSourceFingerprint: String? =
    computeRemoteDaemonSourceFingerprint()

  private static func remoteDaemonSourceFingerprint() -> String? {
    cachedRemoteDaemonSourceFingerprint
  }

  private static func computeRemoteDaemonSourceFingerprint(fileManager: FileManager = .default)
    -> String?
  {
    ...
    let digest = SHA256.hash(
      data: relativePaths.sorted().reduce(into: Data()) { partialResult, relativePath in
  ...
  private static func killOrphanedRemoteSSHProcesses(destination: String, relayPort: Int? = nil) {
    guard
      let output = captureCommandStandardOutput(
  ...
  private func updateRemotePortScanTTYsLocked(_ ttyNames: [UUID: String]) {
    ...
    keepPolledRemotePortsUntilTTYScan =
      ...
      ? keepPolledRemotePortsUntilTTYScan
      : shouldUseFallbackRemotePortPollingLocked() && !polledRemotePorts.isEmpty
    ...
    remoteScannedPortsByPanel = remoteScannedPortsByPanel.filter { panelId, _ in
      guard let oldTTY = previousTTYNames[panelId],
        let newTTY = nextTTYNames[panelId]
      else {
        return false
      }
  ...
  private func performRemotePortScanLocked() {
    ...
    do {
      ...
      debugLog(
        "remote.ports.scan.failed error=\(error.localizedDescription) \(debugConfigSummary())")
    }
  }
  ...
  private func scanRemotePortsByPanelLocked(ttyNamesByPanel: [UUID: String]) throws -> [UUID: [Int]]
  {
    ...
    let command =
      "sh -c \(Self.shellSingleQuoted(Self.remotePortScanScript(ttyNames: ttyNames, excluding: excludedRemoteScanPorts())))"
    let result = try sshExec(
      arguments: sshCommonArguments(batchMode: true) + [configuration.destination, command],
      timeout: 8
    )
    guard result.status == 0 else {
      let detail =
        ...
        ?? "ssh exited \(result.status)"
      throw NSError(
        domain: "cmux.remote.ports", code: 90,
        userInfo: [
  ...
  private func pollRemotePortsLocked() {
    ...
    if !remotePortScanTTYNames.isEmpty {
      ...
      if remotePortScanBurstActive || remotePortScanCoalesceWorkItem != nil
        || remotePortScanPendingReason != nil
      {
        return
      }
    ...
    let command =
      "sh -c \(Self.shellSingleQuoted(Self.remoteAllPortsScanScript(excluding: excludedRemoteScanPorts())))"
    do {
      let result = try sshExec(
        arguments: sshCommonArguments(batchMode: true) + [configuration.destination, command],
        timeout: 8
      ...
      debugLog(
        "remote.ports.poll.failed error=\(error.localizedDescription) \(debugConfigSummary())")
    }
  }
  ...
  private static func parseRemoteTTYPortPairs(output: String, trackedTTYNames: Set<String>)
    -> [String: [Int]]
  {
...
struct WorkspacePrebootstrappedDaemon: Equatable {
  let name: String
  let version: String
  let capabilities: [String]
  let remotePath: String
...
struct WorkspaceRemoteConfiguration: Equatable {
  ...
  init(
    ...
    prebootstrappedDaemon: WorkspacePrebootstrappedDaemon? = nil,
    ...
    self.prebootstrappedDaemon = prebootstrappedDaemon
...
enum SidebarBranchOrdering {
  ...
  private static func inferredHomeDirectory(
    ...
    guard let relativePath = relativePathFromTilde(tildeDirectory),
      let normalizedAbsolute = normalizedDirectory(absoluteDirectory)
    else { return nil }
  ...
  fileprivate static func inferredRemoteHomeDirectory(
    ...
    let tildeDirectories = candidates.compactMap { directory -> String? in
      guard let normalized = normalizedDirectory(directory),
        relativePathFromTilde(normalized) != nil
      else { return nil }
      return normalized
    }
    let absoluteDirectories = candidates.compactMap { directory -> String? in
      guard let normalized = normalizedDirectory(directory), normalized.hasPrefix("/") else {
        return nil
      }
  ...
  private static func expandedTildePath(
    ...
    guard let relativePath = relativePathFromTilde(directory),
      let homeDirectory = normalizedDirectory(homeDirectoryForTildeExpansion)
    else {
      return directory
    }
    if relativePath.isEmpty {
  ...
  private static func preferredDisplayedDirectory(
    ...
    if canonicalDirectoryKey(
      existing, homeDirectoryForTildeExpansion: homeDirectoryForTildeExpansion)
  ...
  static func orderedUniqueBranchDirectoryEntries(
    ...
    let defaultBranchDirty =
      shouldUseFallbackBranchPerPanel ? (fallbackBranch?.isDirty ?? false) : false
...
final class Workspace: Identifiable, ObservableObject {
  ...
  private var pendingTerminalInputObserversByPanelId:
  ...
  private var preservesSSHTerminalConnection: Bool {
    activeRemoteTerminalSessionCount > 0
      && remoteConfiguration?.terminalStartupCommand?.trimmingCharacters(
        in: .whitespacesAndNewlines
      ).isEmpty == false
  }
  ...
  private static func bonsplitAppearance(from config: GhosttyConfig)
  ...
  nonisolated static func bonsplitChromeColors(
    ...
    let paneBackgroundHex =
      usesBonsplitPaneTerminalBackdrop(
  ...
  private static func bonsplitChromeColorsEqual(
    ...
    lhs.backgroundHex == rhs.backgroundHex && lhs.tabBarBackgroundHex == rhs.tabBarBackgroundHex
      && lhs.splitButtonBackdropHex == rhs.splitButtonBackdropHex
      && lhs.paneBackgroundHex == rhs.paneBackgroundHex && lhs.borderHex == rhs.borderHex
  }

  private static func bonsplitChromeColorsLogDescription(
    _ colors: BonsplitConfiguration.Appearance.ChromeColors
  ) -> String {
    "bg=\(colors.backgroundHex ?? "nil") " + "tabBarBg=\(colors.tabBarBackgroundHex ?? "nil") "
      + "splitBackdrop=\(colors.splitButtonBackdropHex ?? "nil") "
      + "paneBg=\(colors.paneBackgroundHex ?? "nil") " + "border=\(colors.borderHex ?? "nil")"
  }
  ...
  func applyGhosttyChrome(
    backgroundColor: NSColor, backgroundOpacity: Double, reason: String = "unspecified"
  ) {
    let sharesWindowBackdrop = Self.usesWindowRootTerminalBackdrop()
    let renderingMode = WindowAppearanceSnapshot.terminalRenderingMode(
      usesHostLayerBackground: GhosttyApp.shared.usesHostLayerBackground
  ...
  init(
    ...
    desktopCmxBootstrapTerminalID: UInt64? = nil,
    desktopCmxCreateBootstrapTerminal: Bool = true
  ) {
    self.id = id
    ...
    if let initialDetachedSurface {
    ...
    } else if desktopCmxCreateBootstrapTerminal {
      let cmxTerminalID: UInt64? =
        if DesktopCmxBackendSettings.isEnabled() {
          desktopCmxBootstrapTerminalID ?? 0
        } else {
          nil
        }
	      let manualIOHandler: (@Sendable (Data) -> Void)? = cmxTerminalID.map { terminalID in
	        { data in
	          Task { @MainActor in
	            AppDelegate.shared?.sendDesktopCmxTerminalInput(
	              data,
	              terminalID: terminalID,
	              windowID: self.desktopCmxWindowID
	            )
	          }
	        }
	      }
      ...
      let terminalPanel = TerminalPanel(
        ...
        cmxTerminalID: cmxTerminalID,
        manualIOHandler: manualIOHandler
      )
      configureTerminalPanel(terminalPanel)
	      if let cmxTerminalID {
	        AppDelegate.shared?.registerDesktopCmxTerminalSurface(
      ...
      if let tabId = bonsplitController.createTab(
        ...
        if let cmxTerminalID {
          desktopCmxRecordSurface(
            terminalID: cmxTerminalID, surfaceID: tabId, panelID: terminalPanel.id)
        }
        initialTabId = tabId
      }
    }
        ...
        if button.terminalCommand != nil {
          return (
            ...
            SurfaceTabBarExecutableButton(
              ...
              terminalCommandSourcePath: button.actionSourcePath
                ?? terminalCommandSourcePaths[button.id]
            )
          )
        }
  ...
  private var desktopCmxSurfaceIDsByTerminalID: [UInt64: TabID] = [:]
  private var desktopCmxTerminalIDsBySurfaceID: [TabID: UInt64] = [:]
  private var desktopCmxSurfaceIDsByBrowserID: [UInt64: TabID] = [:]
  private var desktopCmxBrowserIDsBySurfaceID: [TabID: UInt64] = [:]
  private var desktopCmxLastBrowserUpdatesByBrowserID: [UInt64: CmxNativeBrowserInfo] = [:]
  private var desktopCmxLastBrowserFocusStatesByBrowserID: [UInt64: Bool] = [:]
  private var desktopCmxLastBrowserFocusRequestGeneration: UInt64?
  private var desktopCmxLastPortsKickGenerationsByTerminalID: [UInt64: UInt64] = [:]
  private var desktopCmxPaneIDsByPanelID: [UInt64: PaneID] = [:]
  private var desktopCmxLastAppliedRevision: UInt64?
  private var desktopCmxLastPublishedRemoteStatusJSON: String?
  private var isApplyingDesktopCmxSnapshot = false
  private var desktopCmxSelectionCoveredByMoveTabID: TabID?
  private var desktopCmxCurrentPanelTree: CmxNativePanelNode?
  ...
  private static func desktopCmxPanelUUID(for tabInfo: CmxNativeTabInfo) -> UUID {
    tabInfo.externalID ?? desktopCmxDerivedUUID(namespace: 2, id: tabInfo.id)
  }
  ...
  private var pendingRemoteSurfacePortKickReason:
  ...
  private var transferredRemoteCleanupConfigurationsByPanelId:
    [UUID: WorkspaceRemoteConfiguration] = [:]
  ...
  private func desktopCmxRecordSurface(terminalID: UInt64, surfaceID: TabID, panelID: UUID) {
    desktopCmxSurfaceIDsByTerminalID[terminalID] = surfaceID
    desktopCmxTerminalIDsBySurfaceID[surfaceID] = terminalID
    surfaceIdToPanelId[surfaceID] = panelID
  }

  private func desktopCmxRecordBrowserSurface(browserID: UInt64, surfaceID: TabID, panelID: UUID) {
    desktopCmxSurfaceIDsByBrowserID[browserID] = surfaceID
    desktopCmxBrowserIDsBySurfaceID[surfaceID] = browserID
    surfaceIdToPanelId[surfaceID] = panelID
  }

  private func desktopCmxBrowserProxyContext() -> CmxNativeBrowserProxyContext? {
    guard let remoteProxyEndpoint else { return nil }
    return CmxNativeBrowserProxyContext(
      host: remoteProxyEndpoint.host,
      port: UInt16(clamping: remoteProxyEndpoint.port),
      target: remoteConfiguration?.destination
    )
  }

  private func desktopCmxBrowserInfo(from browserPanel: BrowserPanel, browserID: UInt64)
    -> CmxNativeBrowserInfo
  {
    let historySnapshot = browserPanel.sessionNavigationHistorySnapshot()
    let rawTitle = browserPanel.pageTitle.trimmingCharacters(in: .whitespacesAndNewlines)
    return CmxNativeBrowserInfo(
      urlString: browserPanel.preferredURLStringForOmnibar(),
      title: rawTitle.isEmpty ? browserPanel.displayTitle : rawTitle,
      profileID: browserPanel.profileID.uuidString,
      shouldRenderWebView: browserPanel.shouldRenderWebViewForSessionSnapshot(),
      pageZoom: Double(browserPanel.currentPageZoomFactor()),
      developerToolsVisible: browserPanel.isDeveloperToolsVisible(),
      backHistoryURLStrings: historySnapshot.backHistoryURLStrings,
      forwardHistoryURLStrings: historySnapshot.forwardHistoryURLStrings,
      proxy: desktopCmxBrowserProxyContext(),
      reloadGeneration: desktopCmxLastBrowserUpdatesByBrowserID[browserID]?.reloadGeneration ?? 0
    )
  }

  private func desktopCmxSendBrowserUpdateIfNeeded(
    _ browserPanel: BrowserPanel, surfaceID: TabID, source: String
  ) {
    guard DesktopCmxBackendSettings.isEnabled(),
      let browserID = desktopCmxBrowserIDsBySurfaceID[surfaceID]
    else {
      return
    }
    let browserInfo = desktopCmxBrowserInfo(from: browserPanel, browserID: browserID)
    guard desktopCmxLastBrowserUpdatesByBrowserID[browserID] != browserInfo else {
      return
    }
	    desktopCmxLastBrowserUpdatesByBrowserID[browserID] = browserInfo
	    AppDelegate.shared?.sendDesktopCmxBrowserUpdate(
	      browserInfo,
	      browserID: browserID,
	      windowID: desktopCmxWindowID,
	      source: source)
  ...
  private func desktopCmxSendBrowserFocusUpdateIfNeeded(
    browserID: UInt64, webViewFocused: Bool, source: String
  ) {
    guard DesktopCmxBackendSettings.isEnabled() else {
      return
    }
    guard desktopCmxLastBrowserFocusStatesByBrowserID[browserID] != webViewFocused else {
      return
    }
    desktopCmxLastBrowserFocusStatesByBrowserID[browserID] = webViewFocused
	    AppDelegate.shared?.sendDesktopCmxBrowserFocusUpdate(
  ...
  private func desktopCmxRebuildSurfaceMappings() {
    let previousBrowserIDsBySurfaceID = desktopCmxBrowserIDsBySurfaceID
    desktopCmxSurfaceIDsByTerminalID.removeAll(keepingCapacity: true)
    desktopCmxTerminalIDsBySurfaceID.removeAll(keepingCapacity: true)
    desktopCmxSurfaceIDsByBrowserID.removeAll(keepingCapacity: true)
    desktopCmxBrowserIDsBySurfaceID.removeAll(keepingCapacity: true)

    for (surfaceID, panelID) in surfaceIdToPanelId {
      if let terminalPanel = panels[panelID] as? TerminalPanel,
        let terminalID = terminalPanel.surface.cmxTerminalID
      {
        desktopCmxSurfaceIDsByTerminalID[terminalID] = surfaceID
        desktopCmxTerminalIDsBySurfaceID[surfaceID] = terminalID
      } else if let browserID = previousBrowserIDsBySurfaceID[surfaceID],
        panels[panelID] is BrowserPanel
      {
        desktopCmxSurfaceIDsByBrowserID[browserID] = surfaceID
        desktopCmxBrowserIDsBySurfaceID[surfaceID] = browserID
      }
  ...
  private func desktopCmxSurfaceID(for tabInfo: CmxNativeTabInfo) -> TabID? {
    switch tabInfo.kind {
    case .terminal:
      return desktopCmxSurfaceIDsByTerminalID[tabInfo.id]
    case .browser:
      return desktopCmxSurfaceIDsByBrowserID[tabInfo.id]
    }
  }

  private func desktopCmxCmxTabID(forSurfaceID surfaceID: TabID) -> UInt64? {
    desktopCmxTerminalIDsBySurfaceID[surfaceID] ?? desktopCmxBrowserIDsBySurfaceID[surfaceID]
  }

  private func desktopCmxApplyTabTitleState(
    _ tabInfo: CmxNativeTabInfo,
    panelID: UUID,
    surfaceID: TabID? = nil
  ) {
    panelTitles[panelID] = tabInfo.title
    if tabInfo.explicitTitle {
      panelCustomTitles[panelID] = tabInfo.title
    } else {
    ...
    desktopCmxApplyTabMetadataState(tabInfo, panelID: panelID)

    guard let surfaceID, let existing = bonsplitController.tab(surfaceID) else {
      return
    }
    let titleUpdate: String? = existing.title == tabInfo.title ? nil : tabInfo.title
    let customTitleUpdate: Bool? =
      existing.hasCustomTitle == tabInfo.explicitTitle
      ? nil
      : tabInfo.explicitTitle
    let pinnedUpdate: Bool? = existing.isPinned == tabInfo.pinned ? nil : tabInfo.pinned
    guard titleUpdate != nil || customTitleUpdate != nil || pinnedUpdate != nil else {
      return
    }
  ...
  private func desktopCmxApplyTabMetadataState(_ tabInfo: CmxNativeTabInfo, panelID: UUID) {
    guard tabInfo.kind == .terminal else {
      return
    }

    if let cwd = tabInfo.cwd?.trimmingCharacters(in: .whitespacesAndNewlines), !cwd.isEmpty {
      updatePanelDirectory(panelId: panelID, directory: cwd)
    } else if panelDirectories.removeValue(forKey: panelID) != nil, panelID == focusedPanelId {
      surfaceTabBarDirectory = nil
    }

    if let gitBranch = tabInfo.gitBranch,
      !gitBranch.branch.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
      updatePanelGitBranch(
        panelId: panelID,
    ...
    if let pullRequest = tabInfo.pullRequest,
      let status = SidebarPullRequestStatus(rawValue: pullRequest.status),
      let url = URL(string: pullRequest.urlString)
    {
    ...
    if let ttyName = tabInfo.ttyName?.trimmingCharacters(in: .whitespacesAndNewlines),
      !ttyName.isEmpty
    {
      if surfaceTTYNames[panelID] != ttyName {
        surfaceTTYNames[panelID] = ttyName
    ...
    } else if surfaceTTYNames.removeValue(forKey: panelID) != nil {
      syncRemotePortScanTTYs()
    }

    if let shellState = Self.desktopCmxShellActivityState(from: tabInfo.shellState) {
      updatePanelShellActivityState(panelId: panelID, state: shellState)
    }

    if tabInfo.portsKickGeneration > 0,
      desktopCmxLastPortsKickGenerationsByTerminalID[tabInfo.id] != tabInfo.portsKickGeneration
    {
      desktopCmxLastPortsKickGenerationsByTerminalID[tabInfo.id] = tabInfo.portsKickGeneration
      if isRemoteWorkspace {
    ...
    let nextPorts = Array(Set(tabInfo.listeningPorts.filter { $0 > 0 && $0 <= 65_535 })).sorted()
    if nextPorts.isEmpty {
      if surfaceListeningPorts.removeValue(forKey: panelID) != nil {
        recomputeListeningPorts()
      }
    } else if surfaceListeningPorts[panelID] != nextPorts {
      surfaceListeningPorts[panelID] = nextPorts
      recomputeListeningPorts()
    }
  }

  private static func desktopCmxShellActivityState(
    from rawValue: String?
  ) -> PanelShellActivityState? {
    guard let rawValue else { return nil }
    switch rawValue.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() {
    case "prompt", "idle", "promptidle", "prompt_idle":
      return .promptIdle
    case "running", "busy", "command", "commandrunning", "command_running":
  ...
  private func desktopCmxPanelID(forPanelID panelID: UUID) -> UInt64? {
    guard let surfaceID = surfaceIdFromPanelId(panelID),
      let cmxTabID = desktopCmxCmxTabID(forSurfaceID: surfaceID)
    else {
      return nil
    }
    return desktopCmxCurrentPanelTree.flatMap {
      desktopCmxLeafPanelID(containing: cmxTabID, in: $0)
    }
  }

  private func desktopCmxPanelID(inPane paneID: PaneID) -> UInt64? {
    let selectedSurfaceID = bonsplitController.selectedTab(inPane: paneID)?.id
    let fallbackSurfaceID = bonsplitController.tabs(inPane: paneID).first?.id
    guard let surfaceID = selectedSurfaceID ?? fallbackSurfaceID,
      let cmxTabID = desktopCmxCmxTabID(forSurfaceID: surfaceID)
    else {
      return nil
    }
  ...
  private func desktopCmxPanelID(mappedToPane paneID: PaneID) -> UInt64? {
    desktopCmxPaneIDsByPanelID.first { $0.value == paneID }?.key
  }

  private var desktopCmxWindowID: UUID? {
    guard let app = AppDelegate.shared else { return nil }
    let manager = owningTabManager ?? app.tabManagerFor(tabId: id)
    return manager.flatMap { app.windowId(for: $0) }
  }

  private func sendDesktopCmxSelectTabInPanelCommand(
    tabId: TabID,
    inPane paneID: PaneID,
    source: String
  ) -> Bool {
    guard DesktopCmxBackendSettings.isEnabled(),
      ...
      let panelID = desktopCmxPanelID(inPane: paneID),
      let index = bonsplitController.tabs(inPane: paneID).firstIndex(where: { $0.id == tabId })
    else {
      return false
    }
    AppDelegate.shared?.sendDesktopCmxSelectTabInPanelCommand(
      panelID: panelID,
      index: index,
      windowID: desktopCmxWindowID,
      source: source
  ...
  private func sendDesktopCmxMoveTabToPanelCommand(
    tabId: TabID,
    ...
    guard DesktopCmxBackendSettings.isEnabled(),
      !isApplyingDesktopCmxSnapshot,
      let cmxTabID = desktopCmxCmxTabID(forSurfaceID: tabId),
      let sourceSelection = desktopCmxCurrentPanelTree?.selection(for: cmxTabID),
      let destinationPanelID = desktopCmxPanelID(mappedToPane: paneID) ?? desktopCmxPanelID(inPane: paneID)
    else {
      return false
    }
    AppDelegate.shared?.sendDesktopCmxMoveTabToPanelCommand(
      fromPanelID: sourceSelection.panelID,
      from: sourceSelection.index,
      toPanelID: destinationPanelID,
      to: max(0, index ?? bonsplitController.tabs(inPane: paneID).count),
      focus: focus,
      windowID: desktopCmxWindowID,
      source: source
    )
  ...
  private func sendDesktopCmxMoveTabToSplitCommand(
    tabId: TabID,
    originalPane: PaneID,
    newPane: PaneID,
    ...
    guard DesktopCmxBackendSettings.isEnabled(),
      ...
      let sourceSelection = desktopCmxCurrentPanelTree?.selection(for: cmxTabID),
      let targetPanelID =
        desktopCmxPanelID(mappedToPane: originalPane) ?? desktopCmxPanelID(inPane: originalPane)
    else {
      return false
    }
    AppDelegate.shared?.sendDesktopCmxMoveTabToSplitCommand(
      fromPanelID: sourceSelection.panelID,
      from: sourceSelection.index,
      targetPanelID: targetPanelID,
      edge: desktopCmxSplitDropEdge(originalPane: originalPane, newPane: newPane, orientation: orientation),
      focus: true,
      windowID: desktopCmxWindowID,
      source: source
    )
  ...
  private func desktopCmxSplitDropEdge(
    originalPane: PaneID,
    newPane: PaneID,
    orientation: SplitOrientation
  ) -> CmxSplitDropEdge {
    let snapshot = bonsplitController.layoutSnapshot()
    let originalFrame = snapshot.panes.first { $0.paneId == originalPane.id.uuidString }?.frame
    let newFrame = snapshot.panes.first { $0.paneId == newPane.id.uuidString }?.frame
    switch orientation {
    case .horizontal:
      guard let originalFrame, let newFrame else { return .right }
      return newFrame.x < originalFrame.x ? .left : .right
    case .vertical:
      guard let originalFrame, let newFrame else { return .bottom }
      return newFrame.y < originalFrame.y ? .top : .bottom
    }
  }

  private func sendDesktopCmxFocusPanelCommand(inPane paneID: PaneID, source: String) -> Bool {
    guard DesktopCmxBackendSettings.isEnabled(),
      !isApplyingDesktopCmxSnapshot,
      let panelID = desktopCmxPanelID(inPane: paneID)
    else {
    ...
    AppDelegate.shared?.sendDesktopCmxFocusPanelCommand(
      panelID: panelID,
      windowID: desktopCmxWindowID,
      source: source
    )
  ...
  private func desktopCmxManualIOHandler(for terminalID: UInt64) -> (@Sendable (Data) -> Void) {
    { data in
      Task { @MainActor in
        AppDelegate.shared?.sendDesktopCmxTerminalInput(
	          data,
  ...
  private func desktopCmxMakeTerminalPanel(
    terminalID: UInt64,
    panelID: UUID,
    title: String,
    ...
    let terminalPanel = TerminalPanel(
      ...
      portOrdinal: portOrdinal,
      cmxTerminalID: terminalID,
      manualIOHandler: desktopCmxManualIOHandler(for: terminalID)
    )
    configureTerminalPanel(terminalPanel)
    panels[terminalPanel.id] = terminalPanel
  ...
  private func desktopCmxBrowserProfileID(from rawValue: String?) -> UUID? {
    guard let rawValue,
  ...
  private func desktopCmxBrowserProxyEndpoint(from browserInfo: CmxNativeBrowserInfo?)
    -> BrowserProxyEndpoint?
  {
    guard let proxy = browserInfo?.proxy else {
      return remoteProxyEndpoint
    }
    return BrowserProxyEndpoint(host: proxy.host, port: Int(proxy.port))
  }

  private func desktopCmxBrowserSessionSnapshot(from browserInfo: CmxNativeBrowserInfo?)
    -> SessionBrowserPanelSnapshot
  {
    SessionBrowserPanelSnapshot(
      urlString: browserInfo?.urlString,
      profileID: desktopCmxBrowserProfileID(from: browserInfo?.profileID),
      shouldRenderWebView: browserInfo?.shouldRenderWebView ?? false,
      pageZoom: browserInfo?.pageZoom ?? 1.0,
      developerToolsVisible: browserInfo?.developerToolsVisible ?? false,
      backHistoryURLStrings: browserInfo?.backHistoryURLStrings ?? [],
      forwardHistoryURLStrings: browserInfo?.forwardHistoryURLStrings ?? []
    )
  }

  private func desktopCmxApplyBrowserInfoIfNeeded(
    _ tabInfo: CmxNativeTabInfo,
    panelID: UUID,
    surfaceID: TabID? = nil
  ) {
    guard tabInfo.kind == .browser,
      let browserInfo = tabInfo.browser,
      let browserPanel = panels[panelID] as? BrowserPanel
    else {
      return
    }
    let previous = desktopCmxLastBrowserUpdatesByBrowserID[tabInfo.id]
    guard previous != browserInfo else {
      return
    }

    desktopCmxLastBrowserUpdatesByBrowserID[tabInfo.id] = browserInfo
    let snapshot = desktopCmxBrowserSessionSnapshot(from: browserInfo)
    browserPanel.setRemoteProxyEndpoint(desktopCmxBrowserProxyEndpoint(from: browserInfo))
    _ = browserPanel.setPageZoomFactor(CGFloat(snapshot.pageZoom))
    browserPanel.restoreSessionSnapshot(snapshot)

    if snapshot.developerToolsVisible && BrowserAvailabilitySettings.isEnabled() {
      _ = browserPanel.showDeveloperTools()
      browserPanel.requestDeveloperToolsRefreshAfterNextAttach(reason: "desktop_cmx_update")
    } else if previous?.developerToolsVisible == true || browserPanel.isDeveloperToolsVisible() {
      _ = browserPanel.hideDeveloperTools()
    }

    if previous?.reloadGeneration != nil,
      previous?.reloadGeneration != browserInfo.reloadGeneration
    {
      browserPanel.reload()
    }
    ...
    #if DEBUG
      cmuxDebugLog(
        "desktopCmxBackend.workspace.browser.applied workspace=\(id.uuidString.prefix(5)) "
          + "browserID=\(tabInfo.id) surface=\(surfaceID.map { String($0.uuid.uuidString.prefix(5)) } ?? "-") "
          + "reloadGeneration=\(browserInfo.reloadGeneration)"
      )
    #endif
  }

  private func desktopCmxApplyBrowserFocusRequestIfNeeded(_ request: CmxNativeBrowserFocusRequest?)
  {
    guard let request,
      desktopCmxLastBrowserFocusRequestGeneration != request.generation,
      let surfaceID = desktopCmxSurfaceIDsByBrowserID[request.tabID],
    ...
    desktopCmxLastBrowserFocusRequestGeneration = request.generation
    desktopCmxSelectSurfaceIfNeeded(surfaceID, inPane: paneID)
    let focused = browserPanel.requestExplicitWebViewFocus()

    #if DEBUG
      cmuxDebugLog(
        "desktopCmxBackend.workspace.browser.focusRequest workspace=\(id.uuidString.prefix(5)) "
          + "browserID=\(request.tabID) generation=\(request.generation) focused=\(focused ? 1 : 0)"
      )
    #endif
  }

  private func desktopCmxMakeBrowserPanel(_ tabInfo: CmxNativeTabInfo) -> BrowserPanel {
    let browserInfo = tabInfo.browser
    let proxyEndpoint = desktopCmxBrowserProxyEndpoint(from: browserInfo)
    let usesRemoteProxy = proxyEndpoint != nil || isRemoteWorkspace
    let browserPanel = BrowserPanel(
      workspaceId: id,
      id: Self.desktopCmxPanelUUID(for: tabInfo),
      profileID: desktopCmxBrowserProfileID(from: browserInfo?.profileID),
      initialURL: nil,
      renderInitialNavigation: false,
      proxyEndpoint: proxyEndpoint,
      isRemoteWorkspace: usesRemoteProxy,
      remoteWebsiteDataStoreIdentifier: usesRemoteProxy ? id : nil
    )
    panels[browserPanel.id] = browserPanel
    browserPanel.onWebViewFocusStateChanged = { [weak self] webViewFocused in
      self?.desktopCmxSendBrowserFocusUpdateIfNeeded(
        browserID: tabInfo.id,
        webViewFocused: webViewFocused,
        source: "browser.focus"
      )
    }
    desktopCmxApplyTabTitleState(tabInfo, panelID: browserPanel.id)
    if let browserInfo {
      desktopCmxLastBrowserUpdatesByBrowserID[tabInfo.id] = browserInfo
    }
    let snapshot = desktopCmxBrowserSessionSnapshot(from: browserInfo)
    ...
    if snapshot.developerToolsVisible && BrowserAvailabilitySettings.isEnabled() {
      _ = browserPanel.showDeveloperTools()
      browserPanel.requestDeveloperToolsRefreshAfterNextAttach(reason: "desktop_cmx_restore")
    }
  ...
  @discardableResult
  private func desktopCmxCreateTab(
    _ tabInfo: CmxNativeTabInfo,
    inPane paneID: PaneID,
    cmxPanelID: UInt64
  ) -> TabID? {
    if let existingSurfaceID = desktopCmxSurfaceID(for: tabInfo) {
      desktopCmxPaneIDsByPanelID[cmxPanelID] = paneID
      if let currentPanelID = panelIdFromSurfaceId(existingSurfaceID),
        let currentPaneID = paneId(forPanelId: currentPanelID),
        currentPaneID != paneID
      {
        _ = bonsplitController.moveTab(existingSurfaceID, toPane: paneID)
      }
      if let currentPanelID = panelIdFromSurfaceId(existingSurfaceID) {
        desktopCmxApplyTabTitleState(tabInfo, panelID: currentPanelID, surfaceID: existingSurfaceID)
        desktopCmxApplyBrowserInfoIfNeeded(
          tabInfo, panelID: currentPanelID, surfaceID: existingSurfaceID)
      }
      return existingSurfaceID
    }

    if tabInfo.kind == .browser {
      let browserPanel = desktopCmxMakeBrowserPanel(tabInfo)
      guard
        let surfaceID = bonsplitController.createTab(
          title: tabInfo.title,
          hasCustomTitle: tabInfo.explicitTitle,
        ...
        panelSubscriptions.removeValue(forKey: browserPanel.id)
        return nil
      }

      desktopCmxRecordBrowserSurface(
        browserID: tabInfo.id, surfaceID: surfaceID, panelID: browserPanel.id)
      desktopCmxApplyTabTitleState(tabInfo, panelID: browserPanel.id, surfaceID: surfaceID)
      desktopCmxSendBrowserUpdateIfNeeded(
        browserPanel, surfaceID: surfaceID, source: "browser.restore")
      desktopCmxPaneIDsByPanelID[cmxPanelID] = paneID
      desktopCmxClosePlaceholderTabs(inPane: paneID, preserving: surfaceID)
      #if DEBUG
        cmuxDebugLog(
          "desktopCmxBackend.workspace.browser.created workspace=\(id.uuidString.prefix(5)) "
            + "browserID=\(tabInfo.id) cmxPanelID=\(cmxPanelID) pane=\(paneID.id.uuidString.prefix(5))"
        )
      #endif
      return surfaceID
    }

    let terminalPanel = desktopCmxMakeTerminalPanel(
      terminalID: tabInfo.id,
      panelID: Self.desktopCmxPanelUUID(for: tabInfo),
      title: tabInfo.title,
    ...
    guard
      let surfaceID = bonsplitController.createTab(
        ...
        kind: SurfaceKind.terminal,
        isDirty: false,
        showsNotificationBadge: tabInfo.hasActivity || tabInfo.bellCount > 0,
        isPinned: tabInfo.pinned,
        inPane: paneID
      )
    else {
    ...
    desktopCmxRecordSurface(terminalID: tabInfo.id, surfaceID: surfaceID, panelID: terminalPanel.id)
    desktopCmxApplyTabTitleState(tabInfo, panelID: terminalPanel.id, surfaceID: surfaceID)
    desktopCmxPaneIDsByPanelID[cmxPanelID] = paneID
    desktopCmxClosePlaceholderTabs(inPane: paneID, preserving: surfaceID)
    #if DEBUG
      cmuxDebugLog(
        "desktopCmxBackend.workspace.tab.created workspace=\(id.uuidString.prefix(5)) "
          + "terminalID=\(tabInfo.id) cmxPanelID=\(cmxPanelID) pane=\(paneID.id.uuidString.prefix(5))"
      )
    #endif
    return surfaceID
  }

  private func desktopCmxClosePlaceholderTabs(
    inPane paneID: PaneID, preserving preservedSurfaceID: TabID
  ) {
    for tab in bonsplitController.tabs(inPane: paneID) where tab.id != preservedSurfaceID {
      if panelIdFromSurfaceId(tab.id) == nil {
        _ = bonsplitController.closeTab(tab.id, inPane: paneID)
      }
    }
  }

  private func desktopCmxSelectSurfaceIfNeeded(_ surfaceID: TabID, inPane paneID: PaneID) {
    let selectedSurfaceID = bonsplitController.selectedTab(inPane: paneID)?.id
    let paneIsFocused = bonsplitController.focusedPaneId == paneID
    if !paneIsFocused {
      bonsplitController.focusPane(paneID)
    }
    if selectedSurfaceID != surfaceID {
      bonsplitController.selectTab(surfaceID)
    } else if !paneIsFocused {
      applyTabSelection(tabId: surfaceID, inPane: paneID)
    }
  }

  private func desktopCmxApplyLeaf(
    cmxPanelID: UInt64,
    tabs: [CmxNativeTabInfo],
    active: Int,
    activeTabID: UInt64,
    inPane paneID: PaneID
  ) {
    ...
    desktopCmxPaneIDsByPanelID[cmxPanelID] = paneID
    for tabInfo in tabs {
      _ = desktopCmxCreateTab(tabInfo, inPane: paneID, cmxPanelID: cmxPanelID)
    }
  }

  private func desktopCmxFirstMappedPane(in node: CmxNativePanelNode) -> PaneID? {
    switch node {
    case .leaf(let panelID, let tabs, _, _):
      if let paneID = desktopCmxPaneIDsByPanelID[panelID] {
        return paneID
      }
      for tab in tabs {
        if let surfaceID = desktopCmxSurfaceID(for: tab),
          let panelID = panelIdFromSurfaceId(surfaceID),
          let paneID = paneId(forPanelId: panelID)
        {
          return paneID
    ...
    case .split(_, _, let first, let second):
      return desktopCmxFirstMappedPane(in: first) ?? desktopCmxFirstMappedPane(in: second)
    }
  }

  private func desktopCmxFirstTab(in node: CmxNativePanelNode) -> CmxNativeTabInfo? {
    switch node {
    case .leaf(_, let tabs, _, _):
      return tabs.first
    case .split(_, _, let first, let second):
      return desktopCmxFirstTab(in: first) ?? desktopCmxFirstTab(in: second)
    }
  }

  private func desktopCmxReusableUnmappedPane(excluding excludedPaneIDs: Set<UUID>) -> PaneID? {
    for paneID in bonsplitController.allPaneIds where !excludedPaneIDs.contains(paneID.id) {
      let hasMappedCmxTerminal = bonsplitController.tabs(inPane: paneID).contains { tab in
        desktopCmxCmxTabID(forSurfaceID: tab.id) != nil
      }
      if !hasMappedCmxTerminal {
        return paneID
      }
    }
    return nil
  ...
  private func desktopCmxLeafPanelID(containing terminalID: UInt64, in node: CmxNativePanelNode)
    -> UInt64?
  {
    switch node {
    case .leaf(let panelID, let tabs, _, _):
      return tabs.contains(where: { $0.id == terminalID }) ? panelID : nil
    case .split(_, _, let first, let second):
      return desktopCmxLeafPanelID(containing: terminalID, in: first)
        ?? desktopCmxLeafPanelID(containing: terminalID, in: second)
    }
  }

  private func desktopCmxApplyPanelNode(_ node: CmxNativePanelNode, fallbackPane: PaneID?) {
    switch node {
    case .leaf(let panelID, let tabs, let active, let activeTabID):
      guard
        let paneID = desktopCmxPaneIDsByPanelID[panelID] ?? fallbackPane
          ?? desktopCmxFirstMappedPane(in: node) ?? bonsplitController.focusedPaneId
    ...
    case .split(let direction, _, let first, let second):
      let orientation: SplitOrientation = direction == .vertical ? .vertical : .horizontal
      ...
      if firstPane == nil, secondPane == nil {
        ...
        if let firstPane,
          let secondTab = desktopCmxFirstTab(in: second)
        {
          let panelID = desktopCmxLeafPanelID(containing: secondTab.id, in: node) ?? secondTab.id
          desktopCmxEnsureSplitPane(
            secondTab,
            cmxPanelID: panelID,
          ...
          secondPane = desktopCmxFirstMappedPane(in: second)
        }
        desktopCmxApplyPanelNode(second, fallbackPane: secondPane)
        return
      }

      if firstPane == nil, let secondPane, let firstTab = desktopCmxFirstTab(in: first) {
        if let reusablePane = desktopCmxReusableUnmappedPane(excluding: [secondPane.id]) {
          let panelID = desktopCmxLeafPanelID(containing: firstTab.id, in: node) ?? firstTab.id
          desktopCmxPaneIDsByPanelID[panelID] = reusablePane
        } else {
          let panelID = desktopCmxLeafPanelID(containing: firstTab.id, in: node) ?? firstTab.id
          desktopCmxEnsureSplitPane(
            firstTab,
            cmxPanelID: panelID,
            sourcePane: secondPane,
            orientation: orientation,
            insertFirst: true
          )
        }
      } else if secondPane == nil, let firstPane, let secondTab = desktopCmxFirstTab(in: second) {
        if let reusablePane = desktopCmxReusableUnmappedPane(excluding: [firstPane.id]) {
          let panelID = desktopCmxLeafPanelID(containing: secondTab.id, in: node) ?? secondTab.id
          desktopCmxPaneIDsByPanelID[panelID] = reusablePane
        } else {
          let panelID = desktopCmxLeafPanelID(containing: secondTab.id, in: node) ?? secondTab.id
      ...
      desktopCmxApplyPanelNode(
        first, fallbackPane: desktopCmxFirstMappedPane(in: first) ?? fallbackPane)
      desktopCmxApplyPanelNode(
        second, fallbackPane: desktopCmxFirstMappedPane(in: second) ?? fallbackPane)
    }
  }

  private func desktopCmxEnsureSplitPane(
    _ tabInfo: CmxNativeTabInfo,
    cmxPanelID: UInt64,
    sourcePane: PaneID,
    orientation: SplitOrientation,
    ...
    if let existingSurfaceID = desktopCmxSurfaceID(for: tabInfo) {
      ...
      if let newPane = bonsplitController.splitPane(
        sourcePane,
        orientation: orientation,
        movingTab: existingSurfaceID,
        insertFirst: insertFirst
      ) {
        desktopCmxPaneIDsByPanelID[cmxPanelID] = newPane
      }
      return
    }

    desktopCmxCreateSplitTerminal(
      tabInfo,
  ...
  private func desktopCmxCreateSplitTerminal(
    _ tabInfo: CmxNativeTabInfo,
    sourcePane: PaneID,
    orientation: SplitOrientation,
    insertFirst: Bool
  ) {
    guard desktopCmxSurfaceID(for: tabInfo) == nil else {
      return
    }
    let cmxPanelID =
      desktopCmxCurrentPanelTree
      .flatMap { desktopCmxLeafPanelID(containing: tabInfo.id, in: $0) } ?? tabInfo.id
    if tabInfo.kind == .browser {
      let browserPanel = desktopCmxMakeBrowserPanel(tabInfo)
      ...
      surfaceIdToPanelId[tab.id] = browserPanel.id
      ...
      desktopCmxRecordBrowserSurface(
        browserID: tabInfo.id, surfaceID: tab.id, panelID: browserPanel.id)
      desktopCmxApplyTabTitleState(tabInfo, panelID: browserPanel.id, surfaceID: tab.id)
      desktopCmxSendBrowserUpdateIfNeeded(
        browserPanel, surfaceID: tab.id, source: "browser.restoreSplit")
      desktopCmxPaneIDsByPanelID[cmxPanelID] = newPane
      #if DEBUG
        cmuxDebugLog(
          "desktopCmxBackend.workspace.browserSplit.created workspace=\(id.uuidString.prefix(5)) "
            + "browserID=\(tabInfo.id) cmxPanelID=\(cmxPanelID) pane=\(newPane.id.uuidString.prefix(5))"
        )
      #endif
      return
    }
    ...
    surfaceIdToPanelId[tab.id] = terminalPanel.id
    ...
    desktopCmxRecordSurface(terminalID: tabInfo.id, surfaceID: tab.id, panelID: terminalPanel.id)
    desktopCmxApplyTabTitleState(tabInfo, panelID: terminalPanel.id, surfaceID: tab.id)
    desktopCmxPaneIDsByPanelID[cmxPanelID] = newPane
    #if DEBUG
      cmuxDebugLog(
        "desktopCmxBackend.workspace.split.created workspace=\(id.uuidString.prefix(5)) "
          + "terminalID=\(tabInfo.id) cmxPanelID=\(cmxPanelID) pane=\(newPane.id.uuidString.prefix(5))"
      )
    #endif
  }

  private func desktopCmxPruneStaleTerminalSurfaces(validTerminalIDs: Set<UInt64>) {
    let staleTerminalIDs = desktopCmxSurfaceIDsByTerminalID
      .keys
      .filter { !validTerminalIDs.contains($0) }
    let staleSurfaceIDs =
      desktopCmxSurfaceIDsByTerminalID
      .filter { terminalID, _ in !validTerminalIDs.contains(terminalID) }
      .map(\.value)

    guard !staleSurfaceIDs.isEmpty else {
      return
    }

    for terminalID in staleTerminalIDs {
      desktopCmxLastPortsKickGenerationsByTerminalID.removeValue(forKey: terminalID)
    }

    for surfaceID in staleSurfaceIDs {
      if let browserID = desktopCmxBrowserIDsBySurfaceID[surfaceID] {
        desktopCmxLastBrowserUpdatesByBrowserID.removeValue(forKey: browserID)
      }
      forceCloseTabIds.insert(surfaceID)
      _ = bonsplitController.closeTab(surfaceID)
    }
    desktopCmxRebuildSurfaceMappings()
    #if DEBUG
      cmuxDebugLog(
        "desktopCmxBackend.workspace.prunedStaleTerminals workspace=\(id.uuidString.prefix(5)) "
          + "count=\(staleSurfaceIDs.count)"
      )
    #endif
  }

  private func desktopCmxPruneStaleBrowserSurfaces(validBrowserIDs: Set<UInt64>) {
    let staleBrowserIDs = desktopCmxSurfaceIDsByBrowserID
      .keys
      .filter { !validBrowserIDs.contains($0) }
    let staleSurfaceIDs =
      desktopCmxSurfaceIDsByBrowserID
      .filter { browserID, _ in !validBrowserIDs.contains(browserID) }
      .map(\.value)
    ...
    for browserID in staleBrowserIDs {
      desktopCmxLastBrowserFocusStatesByBrowserID.removeValue(forKey: browserID)
    }
    desktopCmxRebuildSurfaceMappings()
    #if DEBUG
      cmuxDebugLog(
        "desktopCmxBackend.workspace.prunedStaleBrowsers workspace=\(id.uuidString.prefix(5)) "
          + "count=\(staleSurfaceIDs.count)"
      )
    #endif
  }
  ...
  func applyDesktopCmxNativeSnapshot(_ snapshot: CmxNativeSnapshot) {
    ...
    desktopCmxSelectionCoveredByMoveTabID = nil

    let previousRevision = desktopCmxLastAppliedRevision
    let validTerminalIDs = Set(
      snapshot.panels.flattenedTabs.filter { $0.kind == .terminal }.map(\.id))
    let validBrowserIDs = Set(
      snapshot.panels.flattenedTabs.filter { $0.kind == .browser }.map(\.id))
    desktopCmxRebuildSurfaceMappings()
    desktopCmxCurrentPanelTree = snapshot.panels
    desktopCmxApplyPanelNode(snapshot.panels, fallbackPane: bonsplitController.allPaneIds.first)
    desktopCmxPruneStaleTerminalSurfaces(validTerminalIDs: validTerminalIDs)
    desktopCmxPruneStaleBrowserSurfaces(validBrowserIDs: validBrowserIDs)
    desktopCmxLastAppliedRevision = snapshot.revision
    tmuxLayoutSnapshot = bonsplitController.layoutSnapshot()

    if let focusedSurfaceID = desktopCmxSurfaceIDsByTerminalID[snapshot.focusedTabID]
      ?? desktopCmxSurfaceIDsByBrowserID[snapshot.focusedTabID],
      let focusedPanelID = panelIdFromSurfaceId(focusedSurfaceID),
      let paneID = paneId(forPanelId: focusedPanelID)
    {
      desktopCmxSelectSurfaceIfNeeded(focusedSurfaceID, inPane: paneID)
    }
    desktopCmxApplyBrowserFocusRequestIfNeeded(snapshot.browserFocusRequest)

    scheduleTerminalGeometryReconcile()
    #if DEBUG
      if previousRevision != snapshot.revision {
        cmuxDebugLog(
          "desktopCmxBackend.workspace.snapshot.applied workspace=\(id.uuidString.prefix(5)) "
            + "revision=\(snapshot.revision) terminals=\(desktopCmxSurfaceIDsByTerminalID.count) "
            + "browsers=\(desktopCmxSurfaceIDsByBrowserID.count)"
        )
      }
    #endif
  }
  ...
  private func installBrowserPanelSubscription(_ browserPanel: BrowserPanel) {
    ...
    .sink { [weak self, weak browserPanel] _, _, isLoading, favicon in
      guard let self = self,
        let browserPanel = browserPanel,
        let tabId = self.surfaceIdFromPanelId(browserPanel.id)
      else { return }
      self.publishBrowserOpenTabSuggestion(for: browserPanel)
      self.desktopCmxSendBrowserUpdateIfNeeded(
        browserPanel, surfaceID: tabId, source: "browser.subscription")
  ...
  private func installFilePreviewPanelSubscription(_ filePreviewPanel: FilePreviewPanel) {
    ...
    .sink { [weak self, weak filePreviewPanel] titleAndDirty, displayIcon in
      guard let self,
        ...
        let tabId = self.surfaceIdFromPanelId(filePreviewPanel.id)
      else { return }
  ...
  func desktopCmxBrowserPanel(forCmxTabID tabID: UInt64) -> (panelID: UUID, panel: BrowserPanel)? {
    guard
      let surfaceID = desktopCmxSurfaceIDsByBrowserID[tabID],
      let panelID = panelIdFromSurfaceId(surfaceID),
      let browserPanel = panels[panelID] as? BrowserPanel
    else {
      return nil
    }
    return (panelID, browserPanel)
  }
  ...
  private func hasUnreadNotification(panelId: UUID) -> Bool {
    AppDelegate.shared?.notificationStore?.hasVisibleNotificationIndicator(
      forTabId: id, surfaceId: panelId) ?? false
  }
  ...
  private func syncUnreadBadgeStateForPanel(_ panelId: UUID) {
    ...
    if let existing = bonsplitController.tab(tabId),
      existing.showsNotificationBadge == shouldShowUnread
    {
      return
    }
  ...
  private func normalizePinnedTabs(in paneId: PaneID) {
    ...
    for (index, desiredTab) in desiredOrder.enumerated() {
      ...
      guard let currentIndex = currentTabs.firstIndex(where: { $0.id == desiredTab.id }) else {
        continue
      }
  ...
  private func insertionIndexToRight(of anchorTabId: TabID, inPane paneId: PaneID) -> Int {
    let tabs = bonsplitController.tabs(inPane: paneId)
    guard let anchorIndex = tabs.firstIndex(where: { $0.id == anchorTabId }) else {
      return tabs.count
    }
  ...
  func setPanelCustomTitle(panelId: UUID, title: String?) {
    ...
    if DesktopCmxBackendSettings.isEnabled(),
      ...
      let cmxTabID = desktopCmxCmxTabID(forSurfaceID: surfaceID)
    {
      let explicit = !trimmed.isEmpty
      let tabHasCustomTitle = bonsplitController.tab(surfaceID)?.hasCustomTitle == true
      if explicit {
        guard panelCustomTitles[panelId] != trimmed || !tabHasCustomTitle else { return }
	        AppDelegate.shared?.sendDesktopCmxSetTabTitleCommand(
	          tabID: cmxTabID,
	          title: trimmed,
	          explicit: true,
	          windowID: desktopCmxWindowID,
	          source: "workspace.setPanelCustomTitle"
	        )
      } else {
        guard panelCustomTitles[panelId] != nil || tabHasCustomTitle else { return }
	        AppDelegate.shared?.sendDesktopCmxSetTabTitleCommand(
	          tabID: cmxTabID,
	          title: nil,
	          explicit: false,
  ...
  func preloadTerminalPanelForDebugStress(
    ...
    let isVisibleSelection =
      ...
      && terminalPanel.hostedView.window != nil && terminalPanel.hostedView.superview != nil
  ...
  func setCustomColor(_ hex: String?) {
    let nextColor = hex.flatMap(WorkspaceTabColorSettings.normalizedHex)
    guard customColor != nextColor else {
      return
    }
  ...
  func setCustomTitle(_ title: String?) {
    ...
    if trimmed.isEmpty {
      ...
      if self.title != processTitle {
        self.title = processTitle
      }
    } else {
      if customTitle != trimmed {
        customTitle = trimmed
      }
  ...
  func setCustomDescription(_ description: String?) {
    ...
    guard customDescription != normalizedDescription else {
      return
    }
    #if DEBUG
      let inputNewlines =
      ...
      let normalizedNewlines =
        normalizedDescription?.reduce(into: 0) { count, character in
          if character == "\n" { count += 1 }
        } ?? 0
      cmuxDebugLog(
  ...
  func pruneSurfaceMetadata(validSurfaceIds: Set<UUID>) {
    for panelId in Array(pendingTerminalInputObserversByPanelId.keys)
    ...
    invalidatedRestoredAgentFingerprintsByPanelId =
      invalidatedRestoredAgentFingerprintsByPanelId.filter {
        validSurfaceIds.contains($0.key)
      }
    syncRemotePortScanTTYs()
    recomputeListeningPorts()
  ...
  func setLatestSubmittedMessagePreview(_ preview: String?) {
    guard latestSubmittedMessage != preview else { return }
    latestSubmittedMessage = preview
  }
  ...
  func kickRemotePortScan(
    panelId: UUID, reason: WorkspaceRemoteSessionController.PortScanKickReason = .command
  ) {
    guard isRemoteWorkspace else { return }
    syncRemotePortScanTTYs()
  ...
  private func desktopCmxRemoteStatusJSON() -> String? {
  ...
  private func publishDesktopCmxRemoteStatusIfNeeded(source: String) {
    guard DesktopCmxBackendSettings.isEnabled(),
      let cmxWorkspaceID = owningTabManager?.desktopCmxWorkspaceID(forWorkspaceID: id)
    else {
      return
    }
    let statusJSON = desktopCmxRemoteStatusJSON()
    guard desktopCmxLastPublishedRemoteStatusJSON != statusJSON else {
      return
    }
	    let didSend = AppDelegate.shared?.sendDesktopCmxSetWorkspaceRemoteStatusCommand(
    ...
    if didSend {
      desktopCmxLastPublishedRemoteStatusJSON = statusJSON
    }
  }

  func configureRemoteConnection(
    _ configuration: WorkspaceRemoteConfiguration, autoConnect: Bool = true
  ) {
    defer { publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.configure") }
  ...
  func notifyRemoteForegroundAuthenticationReady(token: String? = nil) {
    ...
    guard
      Self.normalizedForegroundAuthToken(remoteConfiguration.foregroundAuthToken)
        == foregroundAuthToken
    else {
      return
    }
  ...
  func disconnectRemoteConnection(clearConfiguration: Bool = false) {
    defer { publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.disconnect") }
  ...
  {
    guard
      configuration.terminalStartupCommand?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        == false
    else {
      return
    }
  ...
  private func trackRemoteTerminalSurface(_ panelId: UUID) {
    ...
    publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.trackSurface")
  ...
  private func untrackRemoteTerminalSurface(_ panelId: UUID) {
    ...
    activeRemoteTerminalSessionCount = activeRemoteTerminalSurfaceIds.count
    publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.untrackSurface")
  ...
  private func maybeDemoteRemoteWorkspaceAfterSSHSessionEnded() {
    ...
    if !hasBrowserPanels {
      if remoteConnectionState == .error || remoteDaemonStatus.state == .error
        || remoteConnectionState == .connecting
      {
        return
      }
  ...
  private func applyPendingRemoteSurfaceTTYIfNeeded(to panelId: UUID) {
    guard
      let ttyName = pendingRemoteSurfaceTTYName?.trimmingCharacters(in: .whitespacesAndNewlines),
      !ttyName.isEmpty
    else {
      return
    }
  ...
  private func cleanupTransferredRemoteConnectionIfNeeded(surfaceId: UUID, relayPort: Int?) -> Bool
  {
    guard let relayPort,
      ...
      cleanupConfiguration.relayPort == relayPort
    else {
      return false
    }
  ...
  func markRemoteTerminalSessionEnded(surfaceId: UUID, relayPort: Int?) {
    ...
    guard let relayPort,
      relayPort > 0,
      remoteConfiguration?.relayPort == relayPort
    else {
      return
    }
  ...
  private static func requestSSHControlMasterCleanupIfNeeded(
    configuration: WorkspaceRemoteConfiguration
  ) {
    guard let arguments = sshControlMasterCleanupArguments(configuration: configuration) else {
      return
    }
  ...
  private static func sshControlMasterCleanupArguments(configuration: WorkspaceRemoteConfiguration)
    -> [String]?
  {
    ...
    if let identityFile = configuration.identityFile?.trimmingCharacters(
      in: .whitespacesAndNewlines),
  ...
  func applyRemoteConnectionStateUpdate(
    ...
    defer { publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.connectionState") }
  ...
  fileprivate func applyRemoteDaemonStatusUpdate(
    _ status: WorkspaceRemoteDaemonStatus, target: String
  ) {
    defer { publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.daemonStatus") }
    ...
    let trimmedDetail =
      status.detail?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "remote daemon error"
  ...
  fileprivate func applyRemoteProxyEndpointUpdate(_ endpoint: BrowserProxyEndpoint?) {
    defer { publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.proxyEndpoint") }
    remoteProxyEndpoint = endpoint
    for panel in panels.values {
      guard let browserPanel = panel as? BrowserPanel else { continue }
      browserPanel.setRemoteProxyEndpoint(endpoint)
      if let surfaceID = surfaceIdFromPanelId(browserPanel.id) {
        desktopCmxSendBrowserUpdateIfNeeded(
          browserPanel, surfaceID: surfaceID, source: "workspace.remote.proxyEndpoint")
      }
    }
    applyBrowserRemoteWorkspaceStatusToPanels()
  }
  ...
  fileprivate func applyRemoteHeartbeatUpdate(count: Int, lastSeenAt: Date?) {
    ...
    applyBrowserRemoteWorkspaceStatusToPanels()
    publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.heartbeat")
  }
  ...
  fileprivate func applyRemoteDetectedSurfacePortsSnapshot(
    ...
    defer { publishDesktopCmxRemoteStatusIfNeeded(source: "workspace.remote.ports") }
  ...
  func newTerminalSplit(
    ...
    #endif
    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxSplitCommand(
	        orientation: orientation,
	        panelID: desktopCmxPanelID(forPanelID: panelId),
	        windowID: desktopCmxWindowID,
	        source: "workspace.newTerminalSplit"
	      )
      return nil
    }
    ...
    let splitWorkingDirectory: String? = {
      if let panelDirectory = panelDirectories[panelId]?.trimmingCharacters(
        in: .whitespacesAndNewlines),
    ...
    guard
      bonsplitController.splitPane(
        paneId, orientation: orientation, withTab: newTab, insertFirst: insertFirst) != nil
    else {
    ...
    #if DEBUG
      cmuxDebugLog(
        "split.created pane=\(paneId.id.uuidString.prefix(5)) orientation=\(orientation)")
      cmuxDebugLog(
        "split.timing workspace=\(id.uuidString.prefix(5)) panel=\(panelId.uuidString.prefix(5)) "
  ...
  func newTerminalSurface(
    ...
    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxNewTabCommand(
	        panelID: desktopCmxPanelID(inPane: paneId),
	        workingDirectory: workingDirectory,
	        initialInput: initialInput,
	        windowID: desktopCmxWindowID,
	        source: "workspace.newTerminalSurface"
	      )
      return nil
    }
  ...
  private func remoteTerminalStartupCommand() -> String? {
    guard
      let command = remoteConfiguration?.terminalStartupCommand?
        .trimmingCharacters(in: .whitespacesAndNewlines),
      !command.isEmpty
    else {
      return nil
  ...
  func newBrowserSplit(
    ...
    if DesktopCmxBackendSettings.isEnabled(), creationPolicy == .userInitiated {
	      AppDelegate.shared?.sendDesktopCmxNewBrowserSplitCommand(
	        url: url,
	        orientation: orientation,
	        panelID: desktopCmxPanelID(forPanelID: panelId),
	        windowID: desktopCmxWindowID,
	        source: "workspace.newBrowserSplit"
	      )
      return nil
    }
  ...
  func newBrowserSurface(
    ...
    if DesktopCmxBackendSettings.isEnabled(), creationPolicy == .userInitiated {
	      AppDelegate.shared?.sendDesktopCmxNewBrowserTabCommand(
	        url: url ?? initialRequest?.url,
	        panelID: desktopCmxPanelID(inPane: paneId),
	        windowID: desktopCmxWindowID,
	        source: "workspace.newBrowserSurface"
	      )
      return nil
    }
  ...
  func teardownAllPanels() {
    ...
    desktopCmxSurfaceIDsByTerminalID.removeAll(keepingCapacity: false)
    desktopCmxTerminalIDsBySurfaceID.removeAll(keepingCapacity: false)
    desktopCmxSurfaceIDsByBrowserID.removeAll(keepingCapacity: false)
    desktopCmxBrowserIDsBySurfaceID.removeAll(keepingCapacity: false)
    desktopCmxLastBrowserUpdatesByBrowserID.removeAll(keepingCapacity: false)
    desktopCmxLastPortsKickGenerationsByTerminalID.removeAll(keepingCapacity: false)
    desktopCmxLastPublishedRemoteStatusJSON = nil
  ...
  func closePanel(_ panelId: UUID, force: Bool = false) -> Bool {
    if let tabId = surfaceIdFromPanelId(panelId) {
      if !force,
        DesktopCmxBackendSettings.isEnabled(),
        let cmxTabID = desktopCmxCmxTabID(forSurfaceID: tabId)
      {
	        AppDelegate.shared?.sendDesktopCmxCloseTabCommand(
	          terminalID: cmxTabID,
	          windowID: desktopCmxWindowID,
	          source: "workspace.closePanel"
	        )
        return true
      }
      if force {
    ...
    guard targetIsActive,
      ...
      let selected = bonsplitController.selectedTab(inPane: focusedPane)
    else {
      #if DEBUG
        cmuxDebugLog(
    ...
    let closed = bonsplitController.closeTab(selected.id)
      ...
      cmuxDebugLog(
        "surface.close.fallback panel=\(panelId.uuidString.prefix(5)) "
          + "selectedTab=\(String(describing: selected.id).prefix(5)) " + "closed=\(closed ? 1 : 0)"
      )
    #endif
    return closed
  }
  ...
  private func browserPathToPane(targetPaneId: String, node: ExternalTreeNode)
    -> [BrowserPaneBreadcrumb]?
  {
    switch node {
  ...
  private func browserCollectPaneNodes(
    node: ExternalTreeNode, into output: inout [ExternalPaneNode]
  ) {
    switch node {
    case .pane(let paneNode):
  ...
  private func browserCollectNormalizedPaneBounds(
    ...
    case .split(let splitNode):
      ...
      browserCollectNormalizedPaneBounds(
        node: splitNode.first, availableRect: firstRect, into: &output)
      browserCollectNormalizedPaneBounds(
        node: splitNode.second, availableRect: secondRect, into: &output)
    }
  }
  ...
  private func browserNearestPaneId(
    ...
    if let targetCenter {
      bestPane = panes.min { lhs, rhs in
        ...
        let lhsDistance =
          pow(lhsCenter.x - targetCenter.x, 2) + pow(lhsCenter.y - targetCenter.y, 2)
        let rhsDistance =
          pow(rhsCenter.x - targetCenter.x, 2) + pow(rhsCenter.y - targetCenter.y, 2)
  ...
  @discardableResult
  func moveSurface(
    panelId: UUID, toPane paneId: PaneID, atIndex index: Int? = nil, focus: Bool = true
  ) -> Bool {
    if DesktopCmxBackendSettings.isEnabled() {
      guard let surfaceID = surfaceIdFromPanelId(panelId),
        let cmxTabID = desktopCmxCmxTabID(forSurfaceID: surfaceID),
        ...
        let destinationPanelID = desktopCmxPanelID(inPane: paneId)
      else {
        return false
      }
	      AppDelegate.shared?.sendDesktopCmxMoveTabToPanelCommand(
  ...
  private func moveSurfaceToAdjacentPane(panelId: UUID, direction: NavigationDirection) -> Bool {
    guard panels[panelId] != nil,
      ...
      let targetPaneId = bonsplitController.adjacentPane(to: sourcePaneId, direction: direction)
    else {
      return false
    }
  ...
  func moveSurfaceToSplit(
    ...
    if DesktopCmxBackendSettings.isEnabled() {
      guard let surfaceID = surfaceIdFromPanelId(panelId),
        ...
        let sourceSelection = desktopCmxCurrentPanelTree?.selection(for: cmxTabID),
        bonsplitController.allPaneIds.contains(paneID),
        let targetPanelID = desktopCmxPanelID(inPane: paneID)
      else {
        return false
      }
	      AppDelegate.shared?.sendDesktopCmxMoveTabToSplitCommand(
	        ...
	        from: sourceSelection.index,
	        targetPanelID: targetPanelID,
	        edge: Self.desktopCmxSplitDropEdge(orientation: orientation, insertFirst: insertFirst),
	        focus: focus,
	        windowID: desktopCmxWindowID,
	        source: "workspace.moveSurfaceToSplit"
	      )
      return true
    }
    ...
    if focus {
      owningTabManager?.focusTab(id, surfaceId: panelId, suppressFlash: true)
    } else {
      scheduleFocusReconcile()
    }
    scheduleTerminalGeometryReconcile()
  ...
  private static func desktopCmxSplitDropEdge(
    orientation: SplitOrientation,
    insertFirst: Bool
  ) -> CmxSplitDropEdge {
      ...
      guard let surfaceID = surfaceIdFromPanelId(panelId),
        ...
        return false
	      ...
	      AppDelegate.shared?.sendDesktopCmxMoveTabToPanelCommand(
	        fromPanelID: sourceSelection.panelID,
	        from: sourceSelection.index,
	        toPanelID: sourceSelection.panelID,
	        to: max(0, index),
	        focus: focus,
	        windowID: desktopCmxWindowID,
	        source: "workspace.reorderSurface"
	      )
      return true
    }
  ...
  func detachSurface(panelId: UUID) -> DetachedSurfaceTransfer? {
    ...
    if shouldSkipControlMasterCleanupAfterDetach, let detachedTransfer = detached,
  ...
  func attachDetachedSurface(
    ...
    if let ttyName = detached.ttyName?.trimmingCharacters(in: .whitespacesAndNewlines),
      !ttyName.isEmpty
    {
    ...
    guard
      let newTabId = bonsplitController.createTab(
        ...
        hasCustomTitle: detached.customTitle?.trimmingCharacters(in: .whitespacesAndNewlines)
          .isEmpty == false,
        icon: detached.icon,
  ...
  private func reassertFocusAfterNonFocusSplit(
    ...
    guard
      matchesPendingNonFocusSplitFocusReassert(
        generation: generation,
        preferredPanelId: preferredPanelId,
        splitPanelId: splitPanelId
      )
  ...
  func focusPanel(
    ...
    markExplicitFocusIntent(on: panelId)
      ...
      cmuxDebugLog(
        "focus.panel panel=\(panelId.uuidString.prefix(5)) pane=\(pane) trigger=\(triggerLabel)")
    ...
    let shouldSuppressReentrantRefocus =
      trigger == .terminalFirstResponder && selectionAlreadyConverged
    #if DEBUG
      let targetPaneShort = targetPaneId.map { String($0.id.uuidString.prefix(5)) } ?? "nil"
      let focusedPaneShort =
        bonsplitController.focusedPaneId.map { String($0.id.uuidString.prefix(5)) } ?? "nil"
      let selectedTabShort =
      ...
      let currentPanelShort =
        currentlyFocusedPanelId.map { String($0.uuidString.prefix(5)) } ?? "nil"
      cmuxDebugLog(
        "focus.panel.begin workspace=\(id.uuidString.prefix(5)) "
          ...
          + "converged=\(selectionAlreadyConverged ? 1 : 0) " + "currentPanel=\(currentPanelShort)"
      )
  ...
  private func maybeAutoFocusBrowserAddressBarOnPanelFocus(
    ...
    guard browserPanel.isShowingNewTabPage || browserPanel.preferredURLStringForOmnibar() == nil
    else { return }
  ...
  func moveFocus(direction: NavigationDirection) {
    if DesktopCmxBackendSettings.isEnabled(),
      ...
      let targetPaneId = bonsplitController.adjacentPane(to: sourcePaneId, direction: direction),
      sendDesktopCmxFocusPanelCommand(inPane: targetPaneId, source: "workspace.moveFocus")
    {
      return
    }
  ...
  /// Select the next surface in the currently focused pane
	  func selectNextSurface() {
	    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxNextTabCommand(
	        windowID: desktopCmxWindowID,
	        source: "workspace.selectNextSurface")
	      return
	    }
  ...
  func selectPreviousSurface() {
	    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxPreviousTabCommand(
	        windowID: desktopCmxWindowID,
	        source: "workspace.selectPreviousSurface")
	      return
	    }
  ...
  func selectSurface(at index: Int) {
	    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxSelectTabCommand(
	        index: index,
	        windowID: desktopCmxWindowID,
	        source: "workspace.selectSurface")
	      return
	    }
    ...
    guard !tabs.isEmpty else { return }
	    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxSelectTabCommand(
	        ...
	        windowID: desktopCmxWindowID,
	        source: "workspace.selectLastSurface"
	      )
      return
    }
    let last = tabs[tabs.count - 1]
  ...
  @discardableResult
  func newTerminalSurfaceInFocusedPane(focus: Bool? = nil, initialInput: String? = nil)
    -> TerminalPanel?
  {
  ...
  func toggleSplitZoom(panelId: UUID) -> Bool {
    ...
    beginEventDrivenLayoutFollowUp(
      ...
      browserExitFocusPanelId: (wasSplitZoomed && !bonsplitController.isSplitZoomed)
        ? panelId : nil,
  ...
  private static func replacementShellScriptWithBanner(target: String) -> String {
    ...
    let reconnectLine = String(
      ...
      defaultValue:
        "[cmux] falling back to a local shell. Reconnect with the original cmux ssh or cmux vm attach command."
    )
  ...
  func createReplacementTerminalPanel() -> TerminalPanel {
    ...
    let replacementInitialCommand: String? = bannerTarget.map {
      Self.replacementShellScriptWithBanner(target: $0)
    }
    let newPanel = TerminalPanel(
      workspaceId: id,
      context: GHOSTTY_SURFACE_CONTEXT_TAB,
  ...
  private func installLayoutFollowUpObservers() {
    ...
    layoutFollowUpObservers.append(
    ...
    layoutFollowUpPanelsCancellable =
      $panels
      .map { _ in () }
      .sink { _ in
        enqueueAttempt()
  ...
  private func browserPortalAnchorReady(for browserPanel: BrowserPanel) -> Bool {
    ...
    return
      anchorView.window != nil && anchorView.superview != nil && anchorView.bounds.width > 1
      && anchorView.bounds.height > 1
  }

  private func browserPortalReady(for browserPanel: BrowserPanel) -> Bool {
    browserPortalAnchorReady(for: browserPanel) && browserPanel.webView.window != nil
      && browserPanel.webView.superview != nil
      && BrowserWindowPortalRegistry.isWebView(
        browserPanel.webView, boundTo: browserPanel.portalAnchorView)
  }
  ...
  private func attemptEventDrivenLayoutFollowUp() {
    ...
    if let browserPanelId = layoutFollowUpBrowserPanelId {
      if let browserPanel = browserPanel(for: browserPanelId) {
        ...
        if isReady,
          !wasReady
            || BrowserWindowPortalRegistry.debugSnapshot(for: browserPanel.webView)?.containerHidden
              == true
        {
    ...
    let needsMoreWork =
      layoutFollowUpNeedsGeometryPass || terminalPortalPending || browserVisibilityPending
      || terminalFocusPending || browserPanelPending || browserExitPending
  ...
  private func renderedVisiblePanelIdsForCurrentLayout() -> Set<UUID> {
    guard portalRenderingEnabled else { return [] }
    let renderedPaneIds =
      bonsplitController.zoomedPaneId.map { [$0] } ?? bonsplitController.allPaneIds
  ...
  private func reconcileBrowserPortalVisibilityForCurrentRenderedLayout(reason: String) -> Bool {
    ...
    for panel in panels.values {
      ...
      if shouldBeVisible {
        ...
        let portalNeedsHide =
          snapshot?.visibleInUI == true || snapshot?.containerHidden == false
        if portalNeedsHide {
  ...
  private func browserPortalVisibilityNeedsFollowUp() -> Bool {
    ...
    for panel in panels.values {
      ...
      let anchorReady =
        anchorView.window != nil && anchorView.superview != nil && anchorView.bounds.width > 1
        && anchorView.bounds.height > 1
      if !anchorReady || browserPanel.webView.window == nil || browserPanel.webView.superview == nil
  ...
  private func createBrowserToRight(of anchorTabId: TabID, inPane paneId: PaneID, url: URL? = nil) {
    let targetIndex = insertionIndexToRight(of: anchorTabId, inPane: paneId)
    let preferredProfileID = panelIdFromSurfaceId(anchorTabId).flatMap {
      browserPanel(for: $0)?.profileID
    }
    guard
      let newPanel = newBrowserSurface(
        inPane: paneId,
        url: url,
        focus: true,
        preferredProfileID: preferredProfileID
  ...
  private func promptRenamePanel(tabId: TabID) {
    ...
    alert.informativeText = String(
      localized: "alert.renameTab.message", defaultValue: "Enter a custom name for this tab.")
    let currentTitle = panelCustomTitles[panelId] ?? panelTitles[panelId] ?? panel.displayTitle
    let input = NSTextField(string: currentTitle)
    input.placeholderString = String(
      localized: "alert.renameTab.placeholder", defaultValue: "Tab name")
  ...
  private func bonsplitTabMoveDestinations(for tabId: TabID) -> [TabContextMoveDestination] {
    ...
    destinations.append(
      contentsOf: workspaceTargets.map { target in
        TabContextMoveDestination(
  ...
  private func showMoveTabFailureAlert() {
    ...
    failure.messageText = String(
      localized: "alert.moveTab.failed.title", defaultValue: "Move Failed")
  ...
  func splitPaneWithNewTerminal(
    ...
    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxSplitCommand(
	        ...
	        initialInput: initialInput,
	        windowID: desktopCmxWindowID,
	        source: "workspace.splitPaneWithNewTerminal"
	      )
      return nil
    }
  ...
  func handleExternalTabDrop(_ request: BonsplitController.ExternalTabDropRequest) -> Bool {
    ...
    case .insert(let paneId, let index):
      ...
      #if DEBUG
        destinationLabel =
          "insert pane=\(paneId.id.uuidString.prefix(5)) index=\(index.map(String.init) ?? "nil")"
      #endif
    case .split(let paneId, let orientation, let insertFirst):
      targetPane = paneId
      targetIndex = nil
      ...
      #if DEBUG
        destinationLabel =
          "split pane=\(paneId.id.uuidString.prefix(5)) orientation=\(orientation.rawValue) insertFirst=\(insertFirst ? 1 : 0)"
      #endif
    }
...
extension Workspace: BonsplitDelegate {
  ...
  private func confirmClosePanel(for tabId: TabID) async -> Bool {
    ...
    let panelName: String? = {
      ...
      if let custom = panelCustomTitles[panelId],
        !custom.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      {
        return custom
      }
      if let title = panelTitles[panelId],
    ...
    if let panelName {
      message = String(
        localized: "dialog.closeTab.messageNamed", defaultValue: "This will close \"\(panelName)\"."
      )
    } else {
      message = String(
        localized: "dialog.closeTab.message", defaultValue: "This will close the current tab.")
    }

    if let confirmCloseHandler =
      (owningTabManager
      ?? AppDelegate.shared?.tabManagerFor(tabId: id)
      ?? AppDelegate.shared?.tabManager)?.confirmCloseHandler
    {
  ...
  private func applyTabSelectionNow(
    ...
    let previousFocusedPanelId = focusedPanelId
    #if DEBUG
      let focusedPaneBefore =
        bonsplitController.focusedPaneId.map { String($0.id.uuidString.prefix(5)) } ?? "nil"
      let selectedTabBefore =
        bonsplitController.focusedPaneId
        .flatMap { bonsplitController.selectedTab(inPane: $0)?.id }
        .map { String($0.uuid.uuidString.prefix(5)) } ?? "nil"
      cmuxDebugLog(
  ...
  private func activationWindow(for panel: any Panel) -> NSWindow? {
    ...
    if let browserPanel = panel as? BrowserPanel {
      return browserPanel.webView.window ?? browserPanel.portalAnchorView.window ?? NSApp.keyWindow
        ?? NSApp.mainWindow
    }
  ...
  private func yieldForeignOwnedFocusIfNeeded(
    ...
    for (panelId, panel) in panels where panelId != targetPanelId {
      guard let ownedIntent = panel.ownedFocusIntent(for: firstResponder, in: window) else {
        continue
      }
      #if DEBUG
        cmuxDebugLog(
  ...
  private func matchesPendingNonFocusSplitFocusReassert(
    ...
    return pending.generation == generation && pending.preferredPanelId == preferredPanelId
  ...
  func splitTabBar(
    _ controller: BonsplitController, shouldCloseTab tab: Bonsplit.Tab, inPane pane: PaneID
  ) -> Bool {
    ...
    if DesktopCmxBackendSettings.isEnabled(),
      let cmxTabID = desktopCmxCmxTabID(forSurfaceID: tab.id)
    {
	      AppDelegate.shared?.sendDesktopCmxCloseTabCommand(
	        terminalID: cmxTabID,
	        windowID: desktopCmxWindowID,
	        source: "workspace.splitTabBar.close"
	      )
      clearStagedClosedBrowserRestoreSnapshot(for: tab.id)
      return false
    }
  ...
  func splitTabBar(
    ...
    let closedBrowserRestoreSnapshot = pendingClosedBrowserRestoreSnapshots.removeValue(
      forKey: tabId)
    ...
    #if DEBUG
      NSLog(
        "[Workspace] didCloseTab panelId=\(panelId) remainingPanels=\(panels.count - 1) remainingPanes=\(controller.allPaneIds.count)"
      )
    #endif

    let panel = panels[panelId]
    removePendingTerminalInputObservers(forPanelId: panelId)
    let transferredRemoteCleanupConfiguration =
      transferredRemoteCleanupConfigurationsByPanelId.removeValue(forKey: panelId)
    ...
    if !isDetaching, let transferredRemoteCleanupConfiguration {
      Self.requestSSHControlMasterCleanupIfNeeded(
        configuration: transferredRemoteCleanupConfiguration)
    }
  ...
  func splitTabBar(
    _ controller: BonsplitController, didSelectTab tab: Bonsplit.Tab, inPane pane: PaneID
  ) {
    if desktopCmxSelectionCoveredByMoveTabID == tab.id {
      desktopCmxSelectionCoveredByMoveTabID = nil
    } else {
      _ = sendDesktopCmxSelectTabInPanelCommand(
        tabId: tab.id,
        inPane: pane,
        source: "workspace.splitTabBar.select"
      )
    }
  ...
  func splitTabBar(
    _ controller: BonsplitController, didMoveTab tab: Bonsplit.Tab, fromPane source: PaneID,
    toPane destination: PaneID
  ) {
    let movedIndex = controller.tabs(inPane: destination).firstIndex { $0.id == tab.id }
    let sentDesktopCmxMove = sendDesktopCmxMoveTabToPanelCommand(
      tabId: tab.id,
      toPane: destination,
      atIndex: movedIndex,
      focus: true,
      source: "workspace.splitTabBar.moveTab"
    )
    if sentDesktopCmxMove, source == destination {
      desktopCmxSelectionCoveredByMoveTabID = tab.id
    }
    #if DEBUG
      ...
      let selectedBefore =
        controller.selectedTab(inPane: destination)
    ...
    if let movedPanelId = panelIdFromSurfaceId(tab.id) {
      ...
      let movedPanelFocused =
        (movedPanelIdAfter != nil && movedPanelIdAfter == focusedPanelId) ? 1 : 0
      cmuxDebugLog(
  ...
  func splitTabBar(_ controller: BonsplitController, didFocusPane pane: PaneID) {
    ...
    _ = sendDesktopCmxFocusPanelCommand(inPane: pane, source: "workspace.splitTabBar.focusPane")
    #if DEBUG
      FocusLogStore.shared.append(
  ...
  func splitTabBar(
    ...
    orientation: SplitOrientation
      ...
      let originalSelectedKind =
        controller.selectedTab(inPane: originalPane).map { panelKindForTab($0.id) } ?? "none"
      let newSelectedKind =
        controller.selectedTab(inPane: newPane).map { panelKindForTab($0.id) } ?? "none"
      cmuxDebugLog(
    ...
    let rearmBrowserPortalHostReplacement: (PaneID, String) -> Void = { paneId, reason in
      for tab in controller.tabs(inPane: paneId) {
        guard let panelId = self.panelIdFromSurfaceId(tab.id),
          let browserPanel = self.browserPanel(for: panelId)
        else {
          continue
        }
        browserPanel.preparePortalHostReplacementForNextDistinctClaim(
    ...
    if !controller.tabs(inPane: newPane).isEmpty {
      if DesktopCmxBackendSettings.isEnabled(), !isApplyingDesktopCmxSnapshot,
        let movedTab = controller.tabs(inPane: newPane).first(where: {
          panelIdFromSurfaceId($0.id) != nil
        })
      {
        _ = sendDesktopCmxMoveTabToSplitCommand(
          tabId: movedTab.id,
          originalPane: originalPane,
          newPane: newPane,
          orientation: orientation,
          source: "workspace.didSplitPane.drag"
        )
        scheduleTerminalGeometryReconcile()
        return
      }
      ...
      if !hasRealSurface {
        ...
        if let replacementTab = placeholderTabs.first {
          ...
          seedTerminalInheritanceFontPoints(
            panelId: replacementPanel.id, configTemplate: inheritedConfig)
    ...
    if DesktopCmxBackendSettings.isEnabled() {
	      AppDelegate.shared?.sendDesktopCmxSplitCommand(
	        orientation: orientation,
	        panelID: desktopCmxPanelID(inPane: originalPane),
	        windowID: desktopCmxWindowID,
	        source: "workspace.didSplitPane"
	      )
      scheduleTerminalGeometryReconcile()
      return
    }
  ...
  private func executeSurfaceTabBarCommandButton(identifier: String, inPane pane: PaneID) {
    ...
    let presentingWindow =
      selectedTerminalPanel(inPane: pane)?.hostedView.window
      ?? NSApp.keyWindow
      ?? NSApp.mainWindow
    ...
    if let workspaceCommand = executable.workspaceCommand {
      ...
      let baseCwd =
        trimmedCwd.isEmpty ? FileManager.default.homeDirectoryForCurrentUser.path : trimmedCwd
  ...
  func splitTabBar(
    _ controller: BonsplitController, didRequestNewTab kind: String, inPane pane: PaneID
  ) {
    if DesktopCmxBackendSettings.isEnabled(), kind == "terminal" {
	      AppDelegate.shared?.sendDesktopCmxNewTabCommand(
	        panelID: desktopCmxPanelID(inPane: pane),
	        windowID: desktopCmxWindowID,
	        source: "workspace.splitTabBar.newTab"
	      )
      return
    }
  ...
  func splitTabBar(
    _ controller: BonsplitController, didRequestCustomAction identifier: String, inPane pane: PaneID
  ) {
    #if DEBUG
      cmuxDebugLog(
  ...
  func splitTabBar(
    _ controller: BonsplitController, didRequestTabContextAction action: TabContextAction,
    for tab: Bonsplit.Tab, inPane pane: PaneID
  ) {
    switch action {
    case .rename:
    ...
    case .moveToNewWorkspace:
      _ = AppDelegate.shared?.moveBonsplitTabToNewWorkspace(
        tabId: tab.id.uuid, focus: true, focusWindow: false)
    case .moveToLeftPane:
      guard let panelId = panelIdFromSurfaceId(tab.id) else { return }
  ...
  func splitTabBar(
    _ controller: BonsplitController, didRequestTabMoveToDestination destinationId: String,
    for tab: Bonsplit.Tab, inPane pane: PaneID
  ) {
