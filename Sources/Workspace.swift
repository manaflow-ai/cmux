...
final class Workspace: Identifiable, ObservableObject {
    ...
    /// forwarded dictionaries without bound, which both leaks memory and makes
    /// the sidebar observation `removeDuplicates` equality check and the
    ...
    static let maxSidebarMetadataBlocks = WorkspaceSidebarMetadataModel.maxMetadataBlocks
    ...
    private func trimSidebarStatusEntriesIfNeeded(
        ...
        // Assign the backing model directly so the computed setter does not
        // recalculate the triggering write's previous-key set during trim.
        sidebarMetadata.statusEntries = statusEntries.filter { kept.contains($0.key) }
    }
