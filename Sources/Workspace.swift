...
extension Workspace {
    ...
    // `internal` (not `private`) so the closed-panel-history host conformance in
    // `Workspace+ClosedPanelHistoryHosting.swift` can build the captured snapshot
    // behind the `WorkspaceClosedPanelHistoryHosting` seam.
    ...
    // The closed-panel-history cluster (capture eligibility, entry build, and
    // restore routing) lives in `ClosedPanelHistoryCoordinator` (CmuxWorkspaces).
    // `Workspace` is its `WorkspaceClosedPanelHistoryHosting` host
    // (`Workspace+ClosedPanelHistoryHosting.swift`); the live panel/snapshot
    // creation the legacy bodies performed inline stays app-side in that host
    // conformance. These forwarders keep the in-file and external call sites
    // (TabManager, AppDelegate history menu) unchanged.

    private func closedPanelHistoryEntry(panelId: UUID, tabId: TabID, pane: PaneID) -> ClosedPanelHistoryEntry? {
        closedPanelHistoryCoordinator.closedPanelHistoryEntry(panelId: panelId, tabId: tabId, pane: pane)
    }

    private func consumeCloseHistoryEligibility(tabId: TabID, panelId: UUID?) -> Bool {
        closedPanelHistoryCoordinator.consumeCloseHistoryEligibility(tabId: tabId, panelId: panelId)
    }

    private func clearCloseHistoryEligibility(tabId: TabID, panelId: UUID? = nil) {
        closedPanelHistoryCoordinator.clearCloseHistoryEligibility(tabId: tabId, panelId: panelId)
    }

    @discardableResult
    private func pushClosedPanelHistoryIfEligible(for tab: Bonsplit.Tab, inPane pane: PaneID) -> Bool {
        closedPanelHistoryCoordinator.pushClosedPanelHistoryIfEligible(for: tab, inPane: pane)
    }

    // `WorkspaceClosedPanelHistoryHosting` witnesses for the two eligibility-flag
    // ops, co-located here (not in `Workspace+ClosedPanelHistoryHosting.swift`)
    // because they read the `private surfaceRegistry`, which a separate-file
    // extension cannot see. They keep the surface→panel resolution at the call
    // site (the coordinator passes the resolved panel id), mirroring the legacy
    // `Workspace` bodies.
    ...
    func surfaceRegistryClearCloseHistoryEligibility(tabId: TabID, panelId: UUID?) {
        surfaceRegistry.clearCloseHistoryEligibility(tabId: tabId, panelId: panelId)
    }
    ...
    // `internal` (not `private`) so the closed-panel-history host conformance in
    // `Workspace+ClosedPanelHistoryHosting.swift` can recreate a live panel from a
    // captured snapshot behind the `WorkspaceClosedPanelHistoryHosting` seam.
    func createPanel(
...
// `SharedLiveAgentIndex` (the @MainActor @Observable hook-store + process-detection
// cache) moved verbatim to `Sources/SharedLiveAgentIndex.swift` (one-major-type-per-file).
// Referenced by type only from WorkspaceHostEnvironment.swift; no forwarding shim needed.
...
final class Workspace: Identifiable, WorkspaceUnreadHosting, SurfaceMetadataHosting, WorkspaceTitleHosting, WorkspaceAppearanceHosting, SurfaceRegistryHosting {
    ...
    private var surfaceTabBarCommandButtons: [String: WorkspaceSurfaceTabBarButtonResolution<CmuxSurfaceTabBarButton>] = [:]
    ...
    /// The closed-panel-history coordinator (CmuxWorkspaces): owns the
    /// recently-closed-panel decisions (eligibility consume/clear, capture into a
    /// `ClosedPanelHistoryEntry`, and restore routing across original/anchor/
    /// fallback/focused panes). `Workspace` is its
    /// `WorkspaceClosedPanelHistoryHosting` host (see
    /// `Workspace+ClosedPanelHistoryHosting.swift`); the live panel/snapshot
    /// creation those legacy bodies performed inline stays app-side behind the
    /// seam, and the few call sites forward here.
    let closedPanelHistoryCoordinator = ClosedPanelHistoryCoordinator<SessionPanelSnapshot>()
    ...
    private static func bonsplitAppearance(
        ...
        let chromeColors = BonsplitChromeColorResolver().chromeColors(
    ...
    func applyGhosttyChrome(from config: GhosttyConfig, reason: String = "unspecified") {
        ...
        if GhosttyApp.shared.backgroundLogEnabled {
            GhosttyApp.shared.logBackground(
                ...
                "resulting=[\(chromeColorResolver.chromeColorsLogDescription(bonsplitController.configuration.appearance.chromeColors))] " +
    ...
    init(
        ...
        closedPanelHistoryCoordinator.attach(host: self)
    ...
    func applySurfaceTabBarButtons(
        ...
        let executableButtons = WorkspaceSurfaceTabBarButtonResolution.resolutionMap(
            buttons: buttons,
    ...
    /// this map stays here because it holds `ClosedPanelHistoryEntry` values bound
    /// to the app-target `SessionPanelSnapshot` (the package owns the type, generic
    /// over that app-side snapshot), captured during the `BonsplitDelegate`
    /// pane-close flow.
