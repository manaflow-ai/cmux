...
final class WorkspaceRemoteSessionController: @unchecked Sendable {
    ...
    private func startPTYBridgeWhenReady(
        ...
        let lock = NSLock()
        var captured: Result<RemotePTYBridgeServer.Endpoint, Error>?
        let isCancelled: () -> Bool = {
            lock.lock()
        ...
        let complete: (Result<RemotePTYBridgeServer.Endpoint, Error>) -> Void = { result in
            lock.lock()
            if captured == nil {
                captured = result
                semaphore.signal()
        ...
        do {
            ...
            guard missingCapabilities.isEmpty else {
                throw NSError(domain: "cmux.remote.daemon", code: 43, userInfo: [
                    NSLocalizedDescriptionKey: RemoteDaemonStrings.appLocalized.missingRequiredCapabilitiesMessage(missingCapabilities),
    ...
    static func userFacingRemoteDaemonBootstrapErrorMessage(_ error: Error) -> String {
        ...
        if lowered.contains("missing required capability") ||
            ...
            return RemoteDaemonStrings.appLocalized.missingRequiredCapabilitiesMessage([
    ...
    private func ensureCLIRelayServerLocked(localSocketPath: String, relayID: String, relayToken: String) throws -> RemoteCLIRelayServer {
        ...
        let relayServer = try RemoteCLIRelayServer(
            ...
            commandRewriter: WorkspaceRemoteRelayCommandRewriter()
        )
