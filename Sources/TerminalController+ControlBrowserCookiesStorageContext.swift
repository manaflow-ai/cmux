...
extension TerminalController {
    ...
    func controlBrowserCookiesGet(
        ...
        case .success(let resolved):
            ...
            guard var cookies = browserAutomation.cookies.allCookies(in: store) else {
                return .timedOut
            }
            if let nameFilter {
        ...
        case .success(let resolved):
            ...
            for row in cookieRows {
                guard case .object = row,
                      ...
                      let cookie = browserAutomation.cookies.cookie(from: raw, fallbackURL: fallbackURL) else {
                    return .invalidCookie(row: row)
                }
                if browserAutomation.cookies.setCookie(cookie, in: store) {
                    setCount += 1
                } else {
    ...
    func controlBrowserCookiesClear(
        ...
        case .success(let resolved):
            let store = resolved.browserPanel.webView.configuration.websiteDataStore.httpCookieStore
            guard let cookies = browserAutomation.cookies.allCookies(in: store) else {
                return .timedOut
            }
            ...
            for cookie in targets {
                if browserAutomation.cookies.deleteCookie(cookie, in: store) {
                    removed += 1
                }
            }
            return .resolved(
    ...
    /// `browserAutomation.cookies.cookieDictionary(from:)`.
