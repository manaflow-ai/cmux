...
final class SurfacePipController {
    ...
    /// Returns the most recently focused PiP surface whose home workspace belongs
    /// to the routed tab manager. A routed return must never fall back to a
    /// floater owned by another main window.
    func mostRecentActivePanelId(for tabManager: TabManager?) -> UUID? {
        guard let tabManager else { return nil }
        return activePanelOrder.reversed().first { panelId in
            guard let record = recordsByPanelId[panelId],
                  let home = workspace(id: record.homeWorkspaceId) else {
                return false
            }
            return home.tabManager === tabManager
        }
    }
