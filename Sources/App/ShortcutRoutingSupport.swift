...
enum BrowserZoomShortcutAction: Equatable {
    ...
    /// The libghostty binding-action string that performs this font-size change
    /// on the focused terminal surface.
    ///
    ...
    /// a dedicated key (Spanish, German QWERTZ, …) the re-match for increase
    /// failed while decrease (`minus`) and reset (`zero`) still matched, so only
    /// increase appeared broken (manaflow-ai/cmux#5981).
