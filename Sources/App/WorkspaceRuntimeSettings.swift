...
enum AgentSessionTrustedSourcesSettings {
    static let trustedResumeSourcesKey = "terminal.trustedResumeSources"
    static let didChangeNotification = Notification.Name("cmux.agentSessionTrustedSourcesSettingsDidChange")

    /// Resume-command sources that are auto-trusted without a stored, signed
    /// approval record and without showing the "Allow Resume Command?" prompt.
    ///
    /// Matching is case-insensitive against a binding's `source` first, then its
    /// `name`, so entries like `rovo`, `claude`, or `codex` cover every future
    /// session from that agent even though each session carries a unique
    /// per-session resume command (e.g. `rovo run --restore <session-id>`).
    static func trustedSources(defaults: UserDefaults = .standard) -> [String] {
        guard let raw = defaults.string(forKey: trustedResumeSourcesKey) else {
            return []
        }
        return raw
    ...
    static func isTrusted(source: String?, name: String? = nil, defaults: UserDefaults = .standard) -> Bool {
        isTrusted(source: source, name: name, trustedSources: trustedSources(defaults: defaults))
    }

    static func isTrusted(source: String?, name: String?, trustedSources: [String]) -> Bool {
        guard !trustedSources.isEmpty else { return false }
        let candidates = [source, name]
            .compactMap { $0?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() }
            .filter { !$0.isEmpty }
        guard !candidates.isEmpty else { return false }
        let allowlist = Set(trustedSources)
        return candidates.contains { allowlist.contains($0) }
    }

    static func setTrustedSources(
        _ sources: [String],
        defaults: UserDefaults = .standard,
        notificationCenter: NotificationCenter = .default
    ) {
        let normalized = sources
            ...
            .filter { !$0.isEmpty }
        let previous = trustedSources(defaults: defaults)
        defaults.set(normalized.joined(separator: "\n"), forKey: trustedResumeSourcesKey)
        if previous != trustedSources(defaults: defaults) {
            notifyDidChange(notificationCenter: notificationCenter)
        }
    }
    ...
    static func reset(
        ...
        let previous = trustedSources(defaults: defaults)
        defaults.removeObject(forKey: trustedResumeSourcesKey)
        let didChange = previous != trustedSources(defaults: defaults)
        if didChange {
            notifyDidChange(notificationCenter: notificationCenter)
        }
        return didChange
