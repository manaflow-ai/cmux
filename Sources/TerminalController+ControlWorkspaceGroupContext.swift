...
extension TerminalController: ControlWorkspaceGroupContext {
    func controlWorkspaceGroupStrings() -> ControlWorkspaceGroupStrings {
        ControlWorkspaceGroupStrings(
            ...
            idempotencyKeyMustBeString: String(
                localized: "workspaceGroup.error.idempotencyKeyMustBeString",
                defaultValue: "external_id and idempotency_key must be strings"
            ),
            idempotencyKeysMustMatch: String(
                localized: "workspaceGroup.error.idempotencyKeysMustMatch",
                defaultValue: "external_id and idempotency_key must match"
            ),
            idempotencyKeyMustNotBeEmpty: String(
                localized: "workspaceGroup.error.idempotencyKeyMustNotBeEmpty",
                defaultValue: "The group identity must not be empty"
            ),
            removeGeneratedAnchorMustBeBoolean: String(
                localized: "workspaceGroup.error.removeGeneratedAnchorMustBeBoolean",
                defaultValue: "remove_generated_anchor must be a boolean"
            ),
            generatedAnchorRequiresAnchorOnly: String(
                localized: "workspaceGroup.error.generatedAnchorRequiresAnchorOnly",
                defaultValue: "Generated-anchor cleanup requires an anchor-only group"
            ),
            generatedAnchorNotOwned: String(
                localized: "workspaceGroup.error.generatedAnchorNotOwned",
                defaultValue: "The group anchor was not generated by cmux"
            ),
            generatedAnchorRemovalFailed: String(
    ...
    func controlCreateWorkspaceGroup(
        ...
        // A retry owns the identity it supplied. Resolve it before validating
        // child handles so a transiently stale child ref cannot turn a safe
        // retry into a second create (or an unrelated error).
        if let normalizedExternalID,
           let existingGroup = tabManager.workspaceGroups.first(where: { $0.externalID == normalizedExternalID }) {
            return .existing(controlWorkspaceGroupSnapshot(existingGroup, tabManager: tabManager))
        }
        ...
        // user's active workspace. The identity lookup and create call both run
        // on the main actor without suspension, so this remains atomic across
    ...
    func controlUngroupWorkspaceGroup(
        ...
        case .groupNotFound:
            return .notFound
        case .dissolved(let keptWorkspaceCount):
            return .dissolved(keptWorkspaceCount: keptWorkspaceCount)
        case .removedGeneratedAnchor(let workspaceID):
            return .removedGeneratedAnchor(workspaceID: workspaceID)
        case .generatedAnchorRequiresAnchorOnly(let memberWorkspaceCount):
            return .generatedAnchorRequiresAnchorOnly(memberWorkspaceCount: memberWorkspaceCount)
