...
enum TerminalSSHSessionDetector {
    ...
    private enum RemoteShellTransport {
        case ssh
        case eternalTerminal

        init?(executableName: String) {
            switch TerminalSSHSessionDetector.normalizedExecutableName(executableName) {
            case "ssh":
                self = .ssh
            case "et":
    ...
    private static func normalizedExecutableName(_ executableName: String) -> String {
    ...
    private static func parseCommandLine(
        ...
        case .eternalTerminal:
            return parseEternalTerminalCommandLine(arguments)
        }
    }
    ...
    private static func parseSSHCommandLine(_ arguments: [String]) -> DetectedSSHSession? {
        ...
        var index = 0
        if normalizedExecutableName(arguments[0]) == RemoteShellTransport.ssh.executableName {
            index = 1
        }
    ...
    private static func parseEternalTerminalCommandLine(_ arguments: [String]) -> DetectedSSHSession? {
        guard !arguments.isEmpty else { return nil }
