...
private enum TextBoxAgentDetection: CaseIterable {
    ...
    func matches(title: String) -> Bool {
        title.range(of: titlePattern, options: [.caseInsensitive, .regularExpression]) != nil
    }

    static func supportsAgentPrefixes(title: String) -> Bool {
        allCases.contains { $0.matches(title: title) }
    }

    static func isClaudeCode(title: String) -> Bool {
        claudeCode.matches(title: title)
    }
}
...
final class TextBoxInputTextView: NSTextView {
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if string.isEmpty,
           ...
           char.count == 1,
           TextBoxAgentDetection.supportsAgentPrefixes(title: terminalTitle) {
            switch char {
            case "?":
