...
struct TextBoxAttachment: Identifiable {
    ...
    static func shouldCleanupLocalURLWhenDisposed(_ fileURL: URL) -> Bool {
        GhosttyApp.terminalPasteboard.isOwnedTemporaryImageFile(fileURL)
            || GhosttyApp.textBoxDraftAttachmentCoordinator.isOwnedDraftCopy(fileURL)
    }
...
/// App-side glue that gates draft-attachment durable copies on the Ghostty
/// image-ownership predicate and bridges between the app-only `TextBoxAttachment`
/// / `SessionTextBoxInputAttachmentSnapshot` types and the package
/// `CmuxWorkspaces.TextBoxDraftAttachmentStore`.
///
/// The path/URL-based durable-copy state machine moved into the package; only the
/// app-coupled half remains here because it reaches `GhosttyApp.terminalPasteboard`
/// (the injected image-ownership seam) and the app's snapshot type. The single
/// process-wide instance is constructed at the composition root
/// (`GhosttyApp.textBoxDraftAttachmentCoordinator`) and referenced from there,
/// matching the original process-wide storage; tests construct their own instance
/// with an injected `store`.
final class TextBoxDraftAttachmentCoordinator {
    private let store: TextBoxDraftAttachmentStore

    init(store: TextBoxDraftAttachmentStore = TextBoxDraftAttachmentStore()) {
        self.store = store
    }

    func snapshot(for attachment: TextBoxAttachment) -> SessionTextBoxInputAttachmentSnapshot {
        ...
        store.prepareDurableCopy(forTemporaryFileAtPath: standardizedLocalURL.path)
        guard let durableURL = store.copiedDraftURL(forOriginalURL: standardizedLocalURL) else {
            return fallbackSnapshot(for: attachment)
        }
    ...
    private func fallbackSnapshot(for attachment: TextBoxAttachment) -> SessionTextBoxInputAttachmentSnapshot {
    ...
    func prepareDurableCopy(for attachment: TextBoxAttachment) {
        guard let localURL = attachment.localURL,
              GhosttyApp.terminalPasteboard.isOwnedTemporaryImageFile(localURL) else {
            return
        }
    ...
    func removeIfOwnedDraftCopy(_ fileURL: URL) -> Bool {
        store.removeIfOwnedDraftCopy(fileURL)
    }

    func isOwnedDraftCopy(_ fileURL: URL) -> Bool {
        store.isOwnedDraftCopy(fileURL)
    }

    func removeCopiedDraftForOriginalTemporaryFile(_ fileURL: URL) {
        store.removeCopiedDraftForOriginalTemporaryFile(fileURL)
    }

    func flushPendingCopiesSynchronously() {
        store.flushPendingCopiesSynchronously()
    }
...
#if DEBUG
    func debugPrepareDurableCopySynchronously(for attachment: TextBoxAttachment) -> URL? {
        guard let localURL = attachment.localURL,
              GhosttyApp.terminalPasteboard.isOwnedTemporaryImageFile(localURL) else {
            return nil
        }
        return store.debugPrepareDurableCopySynchronously(
            forTemporaryFileAtPath: localURL.standardizedFileURL.path
        )
    }
#endif
}
...
extension TextBoxAttachment {
    func debugPrepareSessionDraftCopySynchronouslyForTesting() -> URL? {
        GhosttyApp.textBoxDraftAttachmentCoordinator.debugPrepareDurableCopySynchronously(for: self)
    }

    func debugCancelSessionDraftCopyForTesting() {
        guard let localURL else { return }
        GhosttyApp.textBoxDraftAttachmentCoordinator.removeCopiedDraftForOriginalTemporaryFile(localURL)
    }
}
#endif
...
extension TextBoxInputTextView {
    static func flushPendingSessionDraftAttachmentCopies() {
        GhosttyApp.textBoxDraftAttachmentCoordinator.flushPendingCopiesSynchronously()
    }
}
...
extension SessionTextBoxInputAttachmentSnapshot {
    init(_ attachment: TextBoxAttachment) {
        self = GhosttyApp.textBoxDraftAttachmentCoordinator.snapshot(for: attachment)
    }
...
@MainActor
enum TextBoxSubmit {
    /// Held metadata detector over the built-in agent catalog. Constructed once and reused so the
    /// catalog is not rebuilt per submission; the agent-detection logic lives in `CMUXAgentLaunch`.
    private static let agentMetadataDetector = AgentMetadataDetector()
    ...
    static func dispatchEvents(
        ...
        let isClaude = agentMetadataDetector.isClaudeCode(context: terminalAgentContext)
...
private final class TextBoxSubmitEventRunner {
    ...
    private let pasteboardFileURLReader: any ServiceFileURLReading = PasteboardServiceFileURLReader()
    ...
    private func pasteFilePath(_ path: String) -> Bool {
        ...
        if originalPasteboardItems == nil {
            ...
            fileURLReader: pasteboardFileURLReader
        ) {
            originalPasteboardItems = Self.snapshotPasteboardItems(pasteboard)
...
struct TextBoxInputContainer: View {
    ...
    var body: some View {
        ...
        let canSend = TextBoxInputView.shouldEnableTextBoxSubmit(
            text: text,
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            ZStack(alignment: .leading) {
                ...
                if TextBoxInputView.shouldShowTextBoxPlaceholder(
                    text: text,
    ...
    private func submit() {
        ...
        guard TextBoxInputView.shouldSubmitTextBox(
...
struct TextBoxInputView: NSViewRepresentable {
    ...
    func updateNSView(_ scrollView: NSScrollView, context: Context) {
        ...
        if TextBoxInputView.shouldSynchronizeExternalTextToTextBox(
...
extension TextBoxInputView {
    static func shouldSynchronizeExternalTextToTextBox(
    ...
    static func shouldShowTextBoxPlaceholder(
        text: String,
        attachmentCount: Int,
        hasMarkedText: Bool
    ) -> Bool {
    ...
    static func shouldEnableTextBoxSubmit(
        text: String,
        attachmentCount: Int,
    ...
    static func shouldSubmitTextBox(
        hasPendingAttachmentUpload: Bool,
        hasMarkedText: Bool
    ) -> Bool {
...
final class TextBoxInputTextView: NSTextView {
    ...
    /// Held metadata detector over the built-in agent catalog; the agent-detection logic lives in
    /// `CMUXAgentLaunch`. Constructed once per text view so the catalog is not rebuilt per keystroke.
    ...
    func replacePendingAttachmentUploadPlaceholder(
        ...
        attachments.forEach(GhosttyApp.textBoxDraftAttachmentCoordinator.prepareDurableCopy)
    ...
    override func performKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        guard flags.contains(.command),
              ...
              Self.textBoxCommandShortcutKey(for: event) == "z" else {
            return super.performKeyEquivalent(with: event)
        }
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if Self.shouldHandleTextBoxPlainArrowLocally(
        ...
        if string.isEmpty,
           ...
           char.count == 1,
           agentMetadataDetector.supportsAgentPrefixes(context: terminalTitle) {
            switch char {
            case "?":
    ...
    private func handleStandardEditShortcut(_ event: NSEvent) -> Bool {
        ...
        switch Self.textBoxCommandShortcutKey(for: event) {
        case "c":
            copy(nil)
            return true
        case "x":
    ...
    func cleanupDisposableAttachmentFiles(
        ...
        let ghosttyTemporaryURLs = urlsToClean.filter { url in
            GhosttyApp.textBoxDraftAttachmentCoordinator.removeCopiedDraftForOriginalTemporaryFile(url)
            return !GhosttyApp.textBoxDraftAttachmentCoordinator.removeIfOwnedDraftCopy(url)
        }
...
extension TextBoxInputTextView {
    static func shouldHandleTextBoxPlainArrowLocally(
        keyCode: UInt16,
    ...
    static func textBoxCommandShortcutKey(
        for event: NSEvent,
