...
extension RemoteTmuxSessionMirror {
    func routeOutput(paneId: Int, data: Data) {
        ...
        if let workspace,
           ...
           registry.surfaceID(workspaceID: workspace.id, tmuxPaneID: paneId) != nil {
            registry.receiveOutput(cleaned, workspaceID: workspace.id, tmuxPaneID: paneId)
            return
        }
    ...
    func routeSeed(paneId: Int, seed: RemoteTmuxPaneSeed) {
        ...
        if let workspace,
           let registry = workspace.terminalClientComposition.remoteTmuxSurfaceRegistry,
           registry.surfaceID(workspaceID: workspace.id, tmuxPaneID: paneId) != nil {
            let leaf = authoritativeGrid(forPane: paneId)
            registry.receiveSeed(
                RemoteTmuxPaneSeed(bytes: renderedBytes),
                workspaceID: workspace.id,
                tmuxPaneID: paneId,
                columns: leaf?.columns ?? 80,
                rows: leaf?.rows ?? 24,
                noReflow: noReflowByPane[paneId] ?? true
            )
            return
        }
