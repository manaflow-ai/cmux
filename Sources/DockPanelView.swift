...
final class DockControlRuntime: ObservableObject, Identifiable {
    ...
    private static func shellStartupScript(command: String, workingDirectory: String) -> String {
        ...
        cmux_dock_child=""
        cmux_dock_interrupt() { if [ -n "$cmux_dock_child" ]; then kill -INT "$cmux_dock_child" 2>/dev/null || true; wait "$cmux_dock_child" 2>/dev/null || true; fi; }
        ...
        cmux_dock_child=$!
        wait "$cmux_dock_child" 2>/dev/null || true
        trap - INT
        printf '\\n'
