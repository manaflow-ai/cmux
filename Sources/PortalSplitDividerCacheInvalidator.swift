...
final class PortalSplitDividerCacheInvalidator {
    ...
    func observe(_ views: [NSView], onChange: @escaping () -> Void) {
        ...
        notificationObservers = views.flatMap { view in
            ...
            return [
                NotificationCenter.default.addObserver(forName: NSView.frameDidChangeNotification, object: view, queue: .main) { _ in onChange() },
                NotificationCenter.default.addObserver(forName: NSView.boundsDidChangeNotification, object: view, queue: .main) { _ in onChange() },
            ]
        }
