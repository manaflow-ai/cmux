...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    private var configuredMenuBarController: ConfiguredMenuBarController?
    ...
    func configuredMenuBarRuntimeContext(preferredWindow: NSWindow? = nil) -> ConfiguredMenuBarRuntimeContext {
        let context = preferredRegisteredMainWindowContext(preferredWindow: preferredWindow)
        ...
        return ConfiguredMenuBarRuntimeContext(
            menus: store?.menuBarMenus ?? [],
            extensions: store?.menuBarExtensions ?? [],
            configStore: store,
            workingDirectory: workingDirectory
        )
    }
    ...
    func notifyConfiguredMenuBarDynamicFailure(
    ...
    func configuredDynamicMenuTaskManagerPayload() -> [[String: Any]] {
        configuredMenuBarController?.taskManagerPayload() ?? []
    }
    ...
    private func configuredMenuBarControllerForApp() -> ConfiguredMenuBarController {
        if let configuredMenuBarController {
            return configuredMenuBarController
        }
        let controller = ConfiguredMenuBarController(owner: self)
        configuredMenuBarController = controller
        return controller
    }

    private func installConfiguredMenuBarObserversIfNeeded() {
        configuredMenuBarControllerForApp().installObserversIfNeeded()
    }
