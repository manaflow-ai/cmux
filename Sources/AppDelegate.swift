...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    func configure(tabManager: TabManager, notificationStore: TerminalNotificationStore, sidebarState: SidebarState) {
        ...
        setupMarkdownPaneDragUITestIfNeeded()
    ...
    private func setupMarkdownPaneDragUITestIfNeeded() {
        ...
        guard let fixtureDirectory = env["CMUX_UI_TEST_MARKDOWN_PANE_DRAG_FIXTURE_DIR"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !fixtureDirectory.isEmpty else {
            writeMarkdownPaneDragUITestData(["setupError": "Missing fixture directory"], at: path)
            return
        }
        ...
        func runSetupWhenWindowReady() {
            guard Date() < deadline else {
                self.writeMarkdownPaneDragUITestData(["setupError": "Timed out waiting for main window"], at: path)
                return
            }
    ...
    private func runMarkdownPaneDragUITestScenario(
        ...
        do {
            let fixtures = try createMarkdownPaneDragFixtures(
                fixtureDirectory: fixtureDirectory,
                scenario: scenario
            )
