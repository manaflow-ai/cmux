...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    private func persistWindowGeometry(from window: NSWindow?) {
        ...
        let display = displaySnapshot(for: window)
        persistWindowGeometry(
            frame: frame,
            display: display
        )
        SharedWindowGeometryHintStore.save(
            frame: frame,
            display: display,
    ...
    private func resolvedPersistedWindowGeometryFrame() -> NSRect? {
        ...
        return WindowGeometryResolver.resolvedWindowFrame(
    ...
    private func attemptStartupSessionRestoreIfNeeded(primaryWindow: NSWindow) {
        ...
        if let primaryWindowSnapshot {
            ...
            let restoredFrame = WindowGeometryResolver.resolvedStartupPrimaryWindowFrame(
                primarySnapshot: nil,
    ...
    private func positionNewMainWindow(_ window: NSWindow, relativeTo sourceWindow: NSWindow) {
        ...
        window.setFrame(
            WindowGeometryResolver.clampFrame(
                frame,
                within: visibleFrame,
    ...
    func createMainWindow(
        ...
        let persistedGeometryFrame = (restoredFrame == nil && sourceWindow == nil)
            ...
            : nil
        let freshFallbackFrame: NSRect? = {
            guard restoredFrame == nil, sourceWindow == nil, persistedGeometryFrame == nil else {
                return nil
            }

            let displays = currentDisplayGeometries()
            let sharedGeometryHint = SharedWindowGeometryHintStore.load()
            return WindowGeometryResolver.resolvedFreshMainWindowFrame(
                sharedGeometryHint: sharedGeometryHint,
                availableDisplays: displays.available,
                fallbackDisplay: displays.fallback
            )
        }()
        let explicitInitialFrame = restoredFrame ?? persistedGeometryFrame ?? freshFallbackFrame
        ...
        } else if let explicitInitialFrame {
...
#if DEBUG
            let explicitInitialFrameSource: String = {
                if restoredFrame != nil {
                    return "sessionSnapshot"
                }
                if persistedGeometryFrame != nil {
                    return "persistedGeometry"
                }
                return "freshFallback"
            }()
            cmuxDebugLog(
                "mainWindow.initialFrameApplied source=\(explicitInitialFrameSource) window=\(windowId.uuidString.prefix(8)) " +
                    "applied={\(debugNSRectDescription(window.frame))}"
            )
#endif
        }
