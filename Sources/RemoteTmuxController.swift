...
final class RemoteTmuxController {
    ...
    func handleNewWorkspaceRequested(in manager: TabManager) -> Bool {
        // Same lookup the New Local Workspace menu item reads through
        // ``wouldNewWorkspaceSpawnRemote(in:)``, so the item and the routing agree.
        ...
        newSessionRoutingTask = Task { @MainActor in
            do {
                ...
                // Revalidate across the ssh round trip: the manager must still be a
                // REGISTERED main-window context. `windowId(for:)` would also answer for a
                // closed window's recoverable route and resurrect a dead manager, so it is
                // deliberately not used here. On a closed window, skip — the detached
                // session is picked up on the next attach. This precedes the failure check
                // so a closed window is never told about a failure it can no longer act on.
                ...
                // The user may have moved on to another tab while the round trip ran
                // (mirror, but don't steal selection).
