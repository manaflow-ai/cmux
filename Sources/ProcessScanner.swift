...
enum ProcessScanner {
    /// Maps a comma-separated TTY name list, such as `"ttys001,ttys003"`,
    /// to `[pid: tty_name]` for every process whose controlling terminal
    ...
    /// Returns `[pid: ppid]` for every process in the system.
    ...
    /// Returns `[pid: set<listening_tcp_port>]`, filtered by caller-supplied PIDs.
