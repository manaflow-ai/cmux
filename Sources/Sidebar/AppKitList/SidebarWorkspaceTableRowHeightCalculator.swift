...
/// Supplies deterministic estimates while the exact AppKit row heights resolve.
struct SidebarWorkspaceTableRowHeightCalculator {
    ...
    func estimatedHeight(
        ...
        let magnification = CGFloat(globalFontMagnificationPercent) / 100
        switch row.content {
        case .groupHeader(let snapshot):
            return estimatedGroupHeaderHeight(fontScale: snapshot.fontScale * magnification)
        case .workspace(let snapshot):
            return estimatedWorkspaceHeight(
                fontScale: snapshot.settings.sidebarFontScale * magnification,
                titleLineCount: 1,
