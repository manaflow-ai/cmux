...
struct SidebarRowPalette {
    ...
    static func applyRowOwnedLinkStyling(to mutable: NSMutableAttributedString, linkColor: NSColor) {
        let fullRange = NSRange(location: 0, length: mutable.length)
        guard fullRange.length > 0 else { return }
        var runs: [(url: URL?, range: NSRange)] = []
        mutable.enumerateAttribute(.link, in: fullRange) { value, range, _ in
            guard value != nil else { return }
            runs.append((SidebarRowWebLink.url(from: value), range))
        }
        for run in runs {
...
/// Shared web-URL contract for sidebar row links (parity with
/// `upsertSidebarMetadata`): only HTTP(S) destinations are actionable.
enum SidebarRowWebLink {
