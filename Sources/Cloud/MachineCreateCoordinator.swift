...
final class MachineCreateCoordinator {
    ...
    private func completionHandler(for id: UUID, generation: UInt64) -> @MainActor (CloudVMActionLauncher.Completion) -> Void {
        { [weak self] completion in
            guard let self, (self.attemptGeneration[id] == generation || self.cancelledCreates[id]?.generation == generation) else { return }
    ...
    private func finish(id: UUID, completion: CloudVMActionLauncher.Completion) {
        ...
        // Only the strict stdout protocol can claim a machine. Retain the id
        // observed in progress when its line was evicted from the transcript.
