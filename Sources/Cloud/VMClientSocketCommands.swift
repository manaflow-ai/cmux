...
extension TerminalController {
    nonisolated func socketWorkerCloudVMResponse(
        ...
        switch method {
        case "vm.list":
            return v2VmCall(id: id, jsonRPC: jsonRPC) {
        ...
        case "vm.create":
            ...
            guard let idempotencyKey, !idempotencyKey.isEmpty else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.create requires `idempotency_key`")
            }
            return v2VmCall(id: id, jsonRPC: jsonRPC) {
        ...
        case "vm.destroy":
            guard let vmId = params["id"] as? String else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.destroy requires `id`")
            }
            return v2VmCall(id: id, jsonRPC: jsonRPC) {
        ...
        case "vm.exec":
            guard let vmId = params["id"] as? String else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.exec requires `id`")
            }
            guard let command = params["command"] as? String else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.exec requires `command`")
            }
        ...
        case "vm.ssh_info":
            guard let vmId = params["id"] as? String else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.ssh_info requires `id`")
            }
            return v2VmCall(id: id, jsonRPC: jsonRPC) {
        ...
        case "vm.attach_info":
            guard let vmId = params["id"] as? String else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.attach_info requires `id`")
            }
