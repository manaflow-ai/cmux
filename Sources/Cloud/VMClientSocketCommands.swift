    ...
    nonisolated func socketWorkerCloudVMResponse(
        ...
        case "vm.tunnel_config":
            ...
            return v2VmCall(id: id) {
                ...
                return [
                    ...
                    // Bind the acknowledgement from `cmux vpn up` to the
                    // exact config bytes returned by this enrollment.
        ...
        case "vm.tunnel_revoke":
            ...
            return v2VmCall(id: id) {
                ...
                try? FileManager.default.removeItem(at: manager.appliedDigestURL)
                return ["revoked": true]
            }
        case "vm.ssh_info":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
