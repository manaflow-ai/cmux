...
extension TerminalController {
    nonisolated func socketWorkerCloudVMResponse(
        ...
        case "vm.exec":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.exec requires `id`. Run `cmux vm ls` to find one.")
            }
        ...
        case "vm.attach_info":
            guard let vmId = Self.socketWorkerString(params["id"]), !vmId.isEmpty else {
                return v2Error(id: id, jsonRPC: jsonRPC, code: "invalid_params", message: "vm.attach_info requires `id`. Run `cmux vm ls` to find one, then `cmux vm ssh <id>`.")
            }
