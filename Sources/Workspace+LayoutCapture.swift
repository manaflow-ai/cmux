...
extension Workspace {
    ...
    private func captureSurfaceDefinition(
        ...
        case .markdown, .filePreview, .rightSidebarTool, .customSidebar, .simulator, .agentSession, .extensionBrowser, .workspaceTodo, .links, .notifications, .cloudVMLoading, .mobilePairing, .accountSignIn, .cloudVPNSetup:
            unsupportedSurfaceCount += 1
            definition = CmuxSurfaceDefinition(type: .terminal)
        }
