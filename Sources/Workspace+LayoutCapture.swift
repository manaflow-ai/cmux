...
extension Workspace {
    ...
    private func captureSurfaceDefinition(
        ...
        case .markdown, .filePreview, .rightSidebarTool, .customSidebar, .simulator, .agentSession, .extensionBrowser, .workspaceTodo, .notifications, .cloudVMLoading, .mobilePairing, .accountSignIn, .settings:
            unsupportedSurfaceCount += 1
            definition = CmuxSurfaceDefinition(type: .terminal)
        }
