...
/// so quitting drains CEF browsers on the live run loop first. Chromium's
/// crashing atexit handlers are skipped by the _exit handler CEFKit
/// registers at initialize (it runs inside AppKit's exit(), after every
/// willTerminate observer). Every gate is a no-op unless the CEF debug
/// browser was used this session.
