...
final class CmuxSettingsFileStore {
    ...
    private func boolValue(_ key: String, defaultValue: Bool, defaults: UserDefaults) -> Bool {
        defaults.object(forKey: key) as? Bool ?? defaultValue
    }

    private func intValue(_ key: String, defaultValue: Int, defaults: UserDefaults) -> Int {
        defaults.object(forKey: key) as? Int ?? defaultValue
    }

    private func doubleValue(_ key: String, defaultValue: Double, defaults: UserDefaults) -> Double {
        defaults.object(forKey: key) as? Double ?? defaultValue
    }

    private func stringValue(_ key: String, defaultValue: String, defaults: UserDefaults) -> String {
        defaults.string(forKey: key) ?? defaultValue
    }

    private func stringListValue(_ key: String, defaultValue: String, defaults: UserDefaults) -> [String] {
    ...
    private func persistSettingsJSONValues(_ values: [String: ManagedSettingsValue]) throws {
        bootstrapPrimaryTemplateIfNeeded()
        let fileURL = URL(fileURLWithPath: primaryPath, isDirectory: false)
        var root = try loadWritableSettingsRoot(from: fileURL)
        if root["$schema"] == nil {
    ...
    private func loadWritableSettingsRoot(from fileURL: URL) throws -> [String: Any] {
    ...
    private func setSettingsJSONValue(_ value: Any, at path: String, in root: inout [String: Any]) {
        let components = path.split(separator: ".").map(String.init)
        guard !components.isEmpty else { return }
        setSettingsJSONValue(value, path: components[...], in: &root)
    }

    private func setSettingsJSONValue(
        _ value: Any,
