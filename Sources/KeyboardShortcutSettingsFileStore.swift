...
final class CmuxSettingsFileStore {
    ...
    private func parseSidebarSection(
        ...
        if section.keys.contains(SidebarSettingsFileMapping.workspaceDescriptionColorJSONKey) {
            guard let value = parseNullableHex(
                section[SidebarSettingsFileMapping.workspaceDescriptionColorJSONKey],
                path: SidebarSettingsFileMapping.workspaceDescriptionColorPath,
                sourcePath: sourcePath
            ) else { return }
            snapshot.managedUserDefaults[
