...
/// bodies performed, with the per-item encoding (`FeedSocketEncoder.itemDict`)
...
extension TerminalController: ControlFeedContext {
    func controlFeedJump(workstreamID: String) -> Bool {
        FeedCoordinator.shared.requestFocusIfPossible(workstreamId: workstreamID)
    }
    ...
    func controlFeedSnapshotItems(pendingOnly: Bool) -> [JSONValue] {
        FeedCoordinator.shared.snapshot(pendingOnly: pendingOnly).map { item in
            // `FeedSocketEncoder.itemDict` only ever produces valid JSON
            // (strings, bools, arrays, nested dicts), so the bridge never fails;
            // the empty-object fallback exists solely to keep the map total.
            JSONValue(
                foundationObject: FeedCoordinator.shared.socketEncoder.itemDict(item)
            ) ?? .object([:])
        }
    }
}
