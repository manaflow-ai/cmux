...
final class UpdateController {
    ...
    init() {
        ...
        userDriver = UpdateDriver(viewModel: .init(), hostBundle: hostBundle)
        updater = SPUUpdater(
    ...
    func attemptUpdate() {
        ...
        attemptInstallCancellable = viewModel.$state
            ...
            .sink { [weak self] state in
                ...
                guard didObserveAttemptUpdateProgress, !state.isInstallable else {
                    return
                }
                stopAttemptUpdateMonitoring()
            }
    ...
    private func scheduleNoUpdateDismiss(for state: UpdateState, overrideState: UpdateState?) {
        ...
        guard case let .notFound(notFound) = state else { return }

        recordUITestTimestamp(key: "noUpdateShownAt")
        let workItem = DispatchWorkItem { [weak self] in
            guard let self else { return }
            guard viewModel.overrideState == nil,
                  case .notFound = viewModel.state else { return }
