...
final class CmuxWebView: WKWebView {
    ...
    var debugPointerFocusAllowanceDepth: Int {
    ...
    private static func parseDataURL(_ url: URL) -> ParsedDataURL? {
        ...
        guard absolute.hasPrefix("data:"),
              let commaIndex = absolute.firstIndex(of: ",")
        else {
            return nil
        }
    ...
    private func isDownloadSupportedScheme(_ url: URL) -> Bool {
        isDownloadableScheme(url) || isDataURLScheme(url)
    }
    ...
    private func isLikelyImageURL(_ url: URL) -> Bool {
        if isDataURLScheme(url) {
            guard let parsed = Self.parseDataURL(url),
                  let mime = parsed.mimeType?.lowercased()
            else {
                return false
            }
    ...
    @objc private func contextMenuOpenLinkInDefaultBrowser(_ sender: Any?) {
        ...
        resolveContextMenuLinkURL(at: point) { [weak self] url in
            guard let self, let url, canOpenInDefaultBrowser(url) else { return }
            openContextMenuLinkInDefaultBrowser(url)
        }
    }

    @objc private func contextMenuOpenLinkInNewTab(_: Any?) {
        let point = lastContextMenuPoint
        resolveContextMenuLinkURL(at: point) { [weak self] url in
            guard let self, let url else { return }
            onContextMenuOpenLinkInNewTab?(url)
        }
    }
        ...
        findImageURLAtPoint(point) { [weak self] url in
            ...
            if let url {
                ...
                if scheme == "data" {
                    ...
                    let normalized = normalizedLinkedDownloadURL(url)
                    debugContextDownload(
                    ...
                    if isLikelyImageURL(normalized) {
                        if !isLikelyFaviconURL(normalized) {
                    ...
                    } else if isDownloadableScheme(normalized), !isLikelyFaviconURL(normalized) {
            ...
            findLinkURLAtPoint(point) { linkURL in
                self.debugContextDownload(
    ...
    @objc private func contextMenuDownloadLinkedFile(_ sender: Any?) {
        ...
        findLinkURLAtPoint(point) { [weak self] url in
            ...
            if let url {
                ...
                if isDownloadSupportedScheme(normalized) {
                    startContextMenuDownload(
                        normalized,
                        sender: sender,
                        fallbackAction: fallback.action,
            ...
            findImageURLAtPoint(point) { imageURL in
                self.debugContextDownload(
