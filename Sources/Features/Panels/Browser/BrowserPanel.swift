...
enum GhosttyBackgroundTheme {
    ...
    static func color(
        ...
        let opacity: Double = if let value = userInfo?[GhosttyNotificationKey.backgroundOpacity] as? Double {
            value
...
enum BrowserSearchEngine: String, CaseIterable, Identifiable {
    ...
    func searchURL(query: String) -> URL? {
        ...
        var components: URLComponents? = switch self {
            case .google:
                URLComponents(string: "https://www.google.com/search")
            case .duckduckgo:
                URLComponents(string: "https://duckduckgo.com/")
            case .bing:
                URLComponents(string: "https://www.bing.com/search")
            case .kagi:
                URLComponents(string: "https://kagi.com/search")
            case .startpage:
                URLComponents(string: "https://www.startpage.com/do/dsearch")
        }
...
enum BrowserThemeMode: String, CaseIterable, Identifiable {
    ...
    var displayName: String {
        switch self {
            case .system:
                String(localized: "theme.system", defaultValue: "System")
            case .light:
                String(localized: "theme.light", defaultValue: "Light")
            case .dark:
                String(localized: "theme.dark", defaultValue: "Dark")
        }
    }
...
// MARK: - BrowserInsecureHTTPSettings
...
enum BrowserInsecureHTTPSettings {
    ...
    static func normalizedAllowlistPatterns(rawValue: String?) -> [String] {
        let source: String = if let rawValue, !rawValue.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            rawValue
        } else {
            defaultAllowlistText
        }
...
final class BrowserHistoryStore: ObservableObject {
    ...
    private nonisolated static func defaultHistoryFileURL() -> URL? {
    ...
    private nonisolated static func legacyTaggedHistoryFileURL() -> URL? {
    ...
    private nonisolated static func persistSnapshot(_ snapshot: [Entry], to fileURL: URL) throws {
    ...
    private func makeSuggestionCandidate(entry: Entry) -> SuggestionCandidate {
        ...
        let pathAndQueryLower: String = if query.isEmpty {
            path
        } else {
            "\(path)?\(query)"
        }
    ...
    private func normalizedHistoryKey(components: inout URLComponents) -> String? {
        ...
        let portPart = if let port = components.port {
            ":\(port)"
        } else {
            ""
        }
        ...
        let queryPart = if let query = components.percentEncodedQuery, !query.isEmpty {
            "?\(query.lowercased())"
        } else {
            ""
        }
...
// MARK: - BrowserSearchSuggestionService
...
// MARK: - BrowserInsecureHTTPNavigationIntent
...
final class BrowserPanel: Panel, ObservableObject {
    ...
    @Published private(set) var webViewInstanceID: UUID = .init()
            ...
            if let searchState {
                ...
                searchNeedleCancellable = searchState.$needle
                    ...
                    .sink { [weak self] needle in
                        ...
                        NSLog("Find: browser needle updated panel=%@ needle=%@", id.uuidString, needle)
    ...
    init(workspaceId: UUID, initialURL: URL? = nil, bypassInsecureHTTPHostOnce: String? = nil) {
        ...
        insecureHTTPBypassHostOnce = BrowserInsecureHTTPSettings.normalizeHost(bypassInsecureHTTPHostOnce ?? "")
        browserThemeMode = BrowserThemeSettings.mode()
        ...
        navDelegate.didFinish = { webView in
            ...
            Task { @MainActor [weak self] in
                guard let self, isCurrentWebView(webView) else { return }
    ...
    private func setupObservers(for webView: WKWebView) {
        ...
        NotificationCenter.default
            .publisher(for: .ghosttyDefaultBackgroundDidChange)
    ...
    private func refreshFavicon(from webView: WKWebView) {
        ...
        faviconTask = Task { @MainActor [weak self, weak webView] in
            ...
            guard isCurrentWebView(webView, instanceID: refreshWebViewInstanceID) else { return }
            guard isCurrentFaviconRefresh(generation: refreshGeneration) else { return }
    ...
    private func handleWebViewLoadingChanged(_ newValue: Bool) {
        ...
        let work = DispatchWorkItem { [weak self] in
            ...
            guard loadingGeneration == genAtEnd else { return }
...
extension BrowserPanel {
    ...
    private func installDetachedDeveloperToolsWindowCloseObserver() {
        ...
        detachedDeveloperToolsWindowCloseObserver = NotificationCenter.default.addObserver(
            ...
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                guard preferredDeveloperToolsPresentation == .detached else { return }
    ...
    func toggleDeveloperTools() -> Bool {
        ...
        let handled = enqueueDeveloperToolsVisibilityTransition(to: targetVisible, source: "toggle")
            ...
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                dlog(
                    "browser.devtools toggle.tick panel=\(id.uuidString.prefix(5)) " +
                        "\(debugDeveloperToolsStateSummary()) \(debugDeveloperToolsGeometrySummary())"
                )
            }
        #endif
    ...
    func showDeveloperTools() -> Bool {
        enqueueDeveloperToolsVisibilityTransition(to: true, source: "show")
    }
    ...
    func hideDeveloperTools() -> Bool {
        enqueueDeveloperToolsVisibilityTransition(to: false, source: "hide")
    }
    ...
    func prepareFocusIntentForActivation(_ intent: PanelFocusIntent) {
        guard case let .browser(target) = intent else { return }
    ...
    func restoreFocusIntent(_ intent: PanelFocusIntent) -> Bool {
        guard case let .browser(target) = intent else { return false }
    ...
    private func restoreAddressBarPageFocusAttemptIfNeeded(
        ...
        webView.evaluateJavaScript(Self.addressBarFocusRestoreScript) { [weak self] result, error in
            ...
            let hasNextAttempt = attempt + 1 < delays.count
                ...
                if let error {
                    dlog(
                        "browser.focus.addressBar.restore panel=\(id.uuidString.prefix(5)) " +
...
private extension BrowserPanel {
    ...
    func makeBrowserThemeModeScript(mode: BrowserThemeMode) -> String {
        let colorSchemeLiteral = switch mode {
            case .system:
                "null"
            case .light:
                "'light'"
...
extension WKWebView {
    ...
    func cmuxInspectorFrontendWebView() -> WKWebView? {
        ...
        guard inspector.responds(to: selector),
              let inspectorWebView = inspector.perform(selector)?.takeUnretainedValue() as? WKWebView
        else {
            return nil
        }
...
// MARK: - BrowserNavigationDelegate
...
private final class BrowserNavigationDelegate: NSObject, WKNavigationDelegate {
    ...
    func webView(
        ...
        {
            let intent: BrowserInsecureHTTPNavigationIntent = if shouldOpenInNewTab || navigationAction.targetFrame == nil {
                .newTab
            } else {
                .currentTab
            }
