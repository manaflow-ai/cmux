...
// MARK: - BrowserDevToolsIconOption
...
enum BrowserDevToolsIconOption: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
            case .wrenchAndScrewdriver: "Wrench + Screwdriver"
            case .wrenchAndScrewdriverFill: "Wrench + Screwdriver (Fill)"
            case .curlyBracesSquare: "Curly Braces"
            case .curlyBraces: "Curly Braces (Plain)"
            case .terminalFill: "Terminal (Fill)"
            case .terminal: "Terminal"
            case .hammer: "Hammer"
            case .hammerCircle: "Hammer Circle"
            case .ladybug: "Bug"
            case .ladybugFill: "Bug (Fill)"
            case .scope: "Scope"
            case .codeChevrons: "Code Chevrons"
            case .gearshape: "Gear"
            case .gearshapeFill: "Gear (Fill)"
            case .globe: "Globe"
            case .globeAmericas: "Globe Americas (Fill)"
        }
    }
...
// MARK: - BrowserDevToolsIconColorOption
...
enum BrowserDevToolsIconColorOption: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
            case .bonsplitInactive: "Bonsplit Inactive (Terminal/Globe)"
            case .bonsplitActive: "Bonsplit Active (Terminal/Globe)"
            case .accent: "Accent"
...
// MARK: - BrowserDevToolsButtonDebugSettings
...
enum BrowserDevToolsButtonDebugSettings {
    ...
    static func iconOption(defaults: UserDefaults = .standard) -> BrowserDevToolsIconOption {
        guard let raw = defaults.string(forKey: iconNameKey),
              let option = BrowserDevToolsIconOption(rawValue: raw)
        else {
            return defaultIcon
        }
        return option
    ...
    static func colorOption(defaults: UserDefaults = .standard) -> BrowserDevToolsIconColorOption {
        guard let raw = defaults.string(forKey: iconColorKey),
              let option = BrowserDevToolsIconColorOption(rawValue: raw)
        else {
            return defaultColor
        }
        return option
...
// MARK: - OmnibarAddressButtonStyle
...
// MARK: - OmnibarAddressButtonStyleBody
...
func resolvedBrowserOmnibarPillBackgroundColor(
    ...
    let darkenMix: CGFloat = switch colorScheme {
        case .light:
            0.04
        case .dark:
            0.05
...
struct BrowserPanelView: View {
    ...
    @State private var omnibarSelectionRange: NSRange = .init(location: NSNotFound, length: 0)
    ...
    private func commitSuggestion(_ suggestion: OmnibarSuggestion) {
        ...
        switch suggestion.kind {
            case let .switchToTab(tabId, panelId, _, _):
    ...
    private func deleteSelectedSuggestionIfPossible() {
        ...
        guard case let .history(url, _) = target.kind else { return }
    ...
    private func refreshSuggestions() {
        ...
        let staleRemote: [String] = if query.isEmpty || isSingleCharacterQuery {
            []
        } else {
            staleRemoteSuggestionsForDisplay(query: query)
        }
    ...
    private func matchingOpenTabSuggestions(for query: String, limit: Int) -> [OmnibarOpenTabMatch] {
        ...
        let currentPanelWorkspaceId = tabManager?.tabs
            .first(where: { tab in
...
func omnibarSuggestionCompletion(for suggestion: OmnibarSuggestion) -> String? {
    switch suggestion.kind {
        case let .navigate(url):
            url
        case let .history(url, _):
            url
        case let .switchToTab(_, _, url, _):
            url
        default:
            nil
    }
...
func omnibarSuggestionTitle(for suggestion: OmnibarSuggestion) -> String? {
    switch suggestion.kind {
        case let .history(_, title):
            title
        case let .switchToTab(_, _, _, title):
            title
        default:
            nil
    }
...
func omnibarStrippedURL(_ value: String) -> String {
    stripHTTPSchemeAndWWWPrefix(value)
}
...
func buildOmnibarSuggestions(
    ...
    func insert(_ suggestion: OmnibarSuggestion, score: Double) {
        ...
        if let existing = bestByCompletion[key] {
            let shouldReplaceExisting: Bool = // For identical completions, keep "go to URL" over "switch to tab" so
    ...
    if !(isSingleCharacterQuery && shouldSuppressSingleCharacterSearchResult) {
        let searchBaseScore: Double = switch intent {
            case .queryLike: 820
            case .ambiguous: 540
            case .urlLike: 140
        }
    ...
    if let resolvedURL {
        ...
        let navigateBaseScore: Double = switch intent {
            case .urlLike: 1020
            case .ambiguous: 760
            case .queryLike: 470
        }
    ...
    for (index, entry) in filteredHistoryEntries.prefix(max(limit * 2, limit)).enumerated() {
        let intentBaseScore: Double = switch intent {
            case .urlLike: 780
            case .ambiguous: 690
            case .queryLike: 600
        }
    ...
    for (index, match) in filteredOpenTabMatches.prefix(limit).enumerated() {
        ...
        let resolvedURLBonus: Double = if let resolvedURL,
    ...
    if shouldIncludeRemoteSuggestions {
        for (index, remoteQuery) in remoteQueries.prefix(limit).enumerated() {
            ...
            let remoteBaseScore: Double = switch intent {
                case .queryLike: 690
                case .ambiguous: 450
                case .urlLike: 110
            }
...
func omnibarInlineCompletionForDisplay(
    ...
    let displayText: String = if typedQueryHasExplicitPathOrQuery(query) {
        if typedIncludesScheme {
            acceptedText
        } else if typedIncludesWWWPrefix {
            stripHTTPSchemePrefix(acceptedText)
        } else {
            stripHTTPSchemeAndWWWPrefix(acceptedText)
        }
...
// MARK: - OmnibarPillFramePreferenceKey
...
// MARK: - BrowserAddressBarHeightPreferenceKey
...
func omnibarReduce(state: inout OmnibarState, event: OmnibarEvent) -> OmnibarEffects {
    ...
    switch event {
        ...
        case let .focusLostRevertBuffer(url):
        ...
        case let .focusLostPreserveBuffer(url):
            state.isFocused = false
            state.currentURLString = url
            state.isUserEditing = false
            state.suggestions = []
        ...
        case let .bufferChanged(newValue):
        ...
        case let .suggestionsUpdated(items):
...
struct OmnibarSuggestion: Identifiable, Hashable {
    ...
    var id: String {
        switch kind {
            case let .search(engineName, query):
                "search|\(engineName.lowercased())|\(query.lowercased())"
            case let .navigate(url):
                "navigate|\(url.lowercased())"
            case let .history(url, _):
                "history|\(url.lowercased())"
            case let .switchToTab(tabId, panelId, url, _):
                "switch-tab|\(tabId.uuidString.lowercased())|\(panelId.uuidString.lowercased())|\(url.lowercased())"
            case let .remote(query):
                "remote|\(query.lowercased())"
        }
    }

    var completion: String {
        switch kind {
            case let .search(_, q): q
            case let .navigate(url): url
            case let .history(url, _): url
            case let .switchToTab(_, _, url, _): url
            case let .remote(q): q
        }
    }
    ...
    var primaryText: String {
        switch kind {
            case let .search(engineName, q):
                "Search \(engineName) for \"\(q)\""

            case let .navigate(url):
                Self.displayURLText(for: url)

            case let .history(url, title):
                (title?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
                    ? Self.singleLineText(title) : Self.displayURLText(for: url)

            case let .switchToTab(_, _, url, title):
                (title?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false)
                    ? Self.singleLineText(title) : Self.displayURLText(for: url)
    ...
    var listText: String {
        switch kind {
            case let .history(url, title), let .switchToTab(_, _, url, title):
    ...
    var trailingBadgeText: String? {
        switch kind {
            case .switchToTab:
                String(localized: "browser.switchToTab", defaultValue: "Switch to tab")
            default:
                nil
        }
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        if currentEditor() == nil {
            ...
            if event.modifierFlags.contains(.shift) {
                ...
                let newRange = if safeIndex >= anchor {
                    NSRange(location: anchor, length: safeIndex - anchor)
                } else {
                    NSRange(location: safeIndex, length: anchor - safeIndex)
                }
...
// MARK: - OmnibarTextFieldRepresentable
...
private struct OmnibarTextFieldRepresentable: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTextFieldDelegate {
        ...
        var lastPublishedSelection: NSRange = .init(location: NSNotFound, length: 0)
            ...
            if parent.isFocused {
                if shouldReacquireFocusAfterEndEditing(window: parentField?.window) {
                    ...
                    DispatchQueue.main.async { [weak self] in
                        ...
                        #if DEBUG
                            logFocusEvent("controlTextDidEndEditing.reacquire.tick")
                        #endif
                        guard parent.isFocused else { return }
                        guard let field = parentField, let window = field.window else { return }
                        guard shouldReacquireFocusAfterEndEditing(window: window) else {
                            #if DEBUG
                                logFocusEvent("controlTextDidEndEditing.reacquire.cancel")
                            #endif
                        ...
                        if !isAlreadyFocused {
                            #if DEBUG
                                logFocusEvent("controlTextDidEndEditing.reacquire.apply")
                            #endif
                            window.makeFirstResponder(field)
                        } else {
                            #if DEBUG
                                logFocusEvent("controlTextDidEndEditing.reacquire.skip", detail: "reason=already_focused")
                            #endif
                        }
                    }
                    return
...
private struct OmnibarSuggestionsView: View {
    ...
    private func rowHeight(for _: OmnibarSuggestion) -> CGFloat {
        singleLineRowHeight
    }
    ...
    final class HostContainerView: NSView {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            guard let hostedInspectorHit = hostedInspectorDividerHit(at: point),
                  isHostedInspectorSideDockHit(hostedInspectorHit)
            else {
                super.mouseDown(with: event)
                return
            }
        ...
        func promoteHostedInspectorSideDockFromCurrentLayoutIfNeeded() -> Bool {
            guard !isHostedInspectorSideDockActive(),
                  ...
                  let hit = hostedInspectorDividerCandidateUsingKnownWebViews(in: slotView)
            else {
                return false
            }
        ...
        fileprivate func scheduleHostedInspectorDividerReapply(reason: String) {
            ...
            let workItem = DispatchWorkItem { [weak self] in
                guard let self else { return }
                hostedInspectorReapplyWorkItem = nil
                _ = promoteHostedInspectorSideDockFromCurrentLayoutIfNeeded()
                if isHostedInspectorSideDockActive() {
                    reapplyHostedInspectorDividerToStoredWidthIfNeeded(reason: reason)
                } else if !hasStoredHostedInspectorWidthPreference {
                    captureHostedInspectorPreferredWidthFromCurrentLayout(reason: reason)
                }
            }
        ...
        private func isHostedInspectorSideDockActive() -> Bool {
            guard let hostedInspectorSideDockContainerView,
                  let hostedInspectorSideDockPageView,
                  let hostedInspectorSideDockInspectorView
            else {
                return false
            }
        ...
        private func layoutHostedInspectorSideDockIfNeeded(reason: String) {
            guard let containerView = hostedInspectorSideDockContainerView,
                  let pageView = hostedInspectorSideDockPageView,
                  let inspectorView = hostedInspectorSideDockInspectorView,
                  let dockSide = hostedInspectorSideDockDockSide
            else {
                return
            }
        ...
        private func enforceAdaptiveBottomDockIfNeeded(reason: String) -> Bool {
            guard let hit = hostedInspectorDividerCandidate(),
                  shouldForceHostedInspectorBottomDock(using: hit)
            else {
                return false
            }
        ...
        private func hostedInspectorDividerHit(at point: NSPoint) -> HostedInspectorDividerHit? {
            guard let hit = hostedInspectorDividerCandidate(),
                  hostedInspectorDividerHitRect(for: hit).contains(point)
            else {
                return nil
            }
            return hit
        ...
        private func hostedInspectorDividerCandidateUsingKnownWebViews(in root: NSView) -> HostedInspectorDividerHit? {
            guard let pageLeaf = hostedWebView,
                  ...
                  Self.isVisibleHostedInspectorCandidate(inspectorLeaf)
            else {
                return nil
            }
        ...
        private func captureHostedInspectorPreferredWidthFromCurrentLayout(reason: String) {
            ...
            let fractionMatches: Bool = switch (preferredHostedInspectorWidthFraction, currentFraction) {
                case (nil, nil):
                    true
                case let (lhs?, rhs?):
                    abs(lhs - rhs) <= 0.001
