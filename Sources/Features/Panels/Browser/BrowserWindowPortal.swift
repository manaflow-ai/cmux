...
enum HostedInspectorDockSide {
    ...
    func dividerX(pageFrame _: NSRect, inspectorFrame: NSRect) -> CGFloat {
        switch self {
            case .leading:
                inspectorFrame.maxX
            case .trailing:
    ...
    func inspectorWidth(forDividerX dividerX: CGFloat, in containerBounds: NSRect) -> CGFloat {
        switch self {
            case .leading:
                max(0, dividerX - containerBounds.minX)
            case .trailing:
                max(0, containerBounds.maxX - dividerX)
        }
    }
...
final class WindowBrowserHostView: NSView {
    ...
    private func hostedInspectorDividerCandidateScore(_ hit: HostedInspectorDividerHit) -> CGFloat {
        ...
        return (overlap * 1000) + coverageWidth + pageFrame.width
    }
    ...
    private func hostedInspectorPageCandidateScore(_ pageView: NSView, inspectorView: NSView) -> CGFloat {
        ...
        return (overlap * 1000) + coverageWidth + pageView.frame.width
    }
...
// MARK: - BrowserDropZoneOverlayView
...
// MARK: - BrowserPortalSearchOverlayConfiguration
...
struct BrowserPaneDragTransfer: Equatable {
    ...
    static func decode(from data: Data) -> BrowserPaneDragTransfer? {
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              ...
              let sourcePaneId = UUID(uuidString: sourcePaneIdRaw)
        else {
            return nil
        }
...
enum BrowserPaneDropRouting {
    ...
    static func action(
        ...
        let splitTarget: BrowserPaneSplitTarget? = switch zone {
            case .center:
                nil
            case .left:
                BrowserPaneSplitTarget(orientation: .horizontal, insertFirst: true)
            case .right:
                BrowserPaneSplitTarget(orientation: .horizontal, insertFirst: false)
            case .top:
                BrowserPaneSplitTarget(orientation: .vertical, insertFirst: true)
            case .bottom:
                BrowserPaneSplitTarget(orientation: .vertical, insertFirst: false)
        }
...
// MARK: - BrowserPaneDropTargetView
...
final class BrowserPaneDropTargetView: NSView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        ...
        switch action {
            ...
            case let .move(tabId, workspaceId, targetPane, splitTarget):
...
final class WindowBrowserSlotView: NSView {
    ...
    func yieldSearchOverlayFocusIfOwned(by panelId: UUID, in window: NSWindow) -> Bool {
        guard let firstResponder = window.firstResponder,
              searchOverlayPanelId(for: firstResponder) == panelId
        else {
            return false
        }
    ...
    private func yieldOwnedFirstResponderIfNeeded(in window: NSWindow, reason: String) -> Bool {
        guard let firstResponder = window.firstResponder,
              ...
              owningView === self || owningView.isDescendant(of: self)
        else {
            return false
        }
        #if DEBUG
    ...
    private func applyResolvedDropZoneOverlay() {
        ...
        guard let zone = resolvedZone else {
            ...
            NSAnimationContext.runAnimationGroup { context in
                ...
                guard dropZoneOverlayAnimationGeneration == animationGeneration else { return }
                guard displayedDropZone == nil else { return }
                dropZoneOverlayView.isHidden = true
...
final class WindowBrowserPortal: NSObject {
    ...
    private func scheduleDeferredFullSynchronizeAll() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            #if DEBUG
                dlog("browser.portal.sync.defer.tick entries=\(entriesByWebViewId.count)")
            #endif
            synchronizeAllWebViews(excluding: nil, source: "deferredTick")
        }
    }
...
// MARK: - BrowserWindowPortalRegistry
