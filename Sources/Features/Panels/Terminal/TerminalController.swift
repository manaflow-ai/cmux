...
class TerminalController {
    ...
    private nonisolated static let acceptFailureMaxBackoffMs = 5000
    ...
    private nonisolated static let socketProbePollRetryBackoffUs: useconds_t = 50000
    ...
    func start(tabManager: TabManager, socketPath: String, accessMode: SocketControlMode) {
        ...
        if case let .failure(failedPath, failedStage, failedErrnoCode) = bindAttempt,
        ...
        switch bindAttempt {
            case let .success(boundPath):
                activeSocketPath = boundPath
                withListenerState {
                    self.socketPath = activeSocketPath
                }

            case let .pathTooLong(failedPath):
            ...
            case let .failure(failedPath, failedStage, failedErrnoCode):
    ...
    private func passwordLoginV2ResponseIfNeeded(for command: String, authenticated: inout Bool) -> String? {
        ...
        guard let params = dict["params"] as? [String: Any],
              let provided = params["password"] as? String
        else {
    ...
    private nonisolated func acceptLoop(listenerSocket: Int32, generation: UInt64) {
        ...
        while shouldContinueAcceptLoop(generation: generation) {
            ...
            guard clientSocket >= 0 else {
                ...
                if backoffMs > 0 {
                    usleep(useconds_t(backoffMs * 1000))
                }
                continue
            }
    ...
    private nonisolated func scheduleListenerRearm(
        ...
        DispatchQueue.main.asyncAfter(deadline: deadline) { [weak self] in
            ...
            guard let restartPath = withListenerState({ () -> String? in
            ...
            stop()
            start(tabManager: tabManager, socketPath: restartPath, accessMode: restartMode)
        }
    }
    ...
    private func v2Result(id: Any?, _ res: V2CallResult) -> String {
        switch res {
            case let .ok(payload):
                v2Ok(id: id, result: payload)
            case let .err(code, message, data):
                v2Error(id: id, code: code, message: message, data: data)
        }
    }
        ...
        v2MainSync {
            ...
            switch action {
                ...
                case "close_below":
                    ...
                    let candidates: [Workspace] = if index + 1 < tabManager.tabs.count {
                        Array(tabManager.tabs.suffix(from: index + 1)).filter { !$0.isPinned }
                    } else {
                        []
                    }
                    let closed = closeWorkspaces(candidates)
        ...
        v2MainSync {
            ...
            switch action {
                ...
                case "close_others", "close_other_tabs":
                    ...
                    let targetIds = workspace.bonsplitController
                        .tabs(inPane: paneId)
                        .map(\.id)
    ...
    private func v2SurfaceCreate(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            let newPanelId: UUID? = if panelType == .browser {
                ws.newBrowserSurface(inPane: paneId, url: url, focus: v2FocusAllowed())?.id
            } else {
                ws.newTerminalSurface(inPane: paneId, focus: v2FocusAllowed())?.id
            }
        ...
        v2MainSync {
            ...
            if let anchorSurfaceId = beforeSurfaceId ?? afterSurfaceId {
                guard let anchor = app.locateSurface(surfaceId: anchorSurfaceId),
                      ...
                      let anchorIndex = anchorWorkspace.indexInPane(forPanelId: anchorSurfaceId)
                else {
                ...
                guard let tm = app.tabManagerFor(tabId: workspaceUUID),
                      let ws = tm.tabs.first(where: { $0.id == workspaceUUID })
                else {
                ...
                guard let selectedWorkspaceId = tm.selectedTabId,
                      let ws = tm.tabs.first(where: { $0.id == selectedWorkspaceId })
                else {
    ...
    private func v2SurfaceReorder(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            guard let app = AppDelegate.shared,
                  ...
                  let sourcePane = ws.paneId(forPanelId: surfaceId)
            else {
                result = .err(code: "not_found", message: "Surface not found", data: ["surface_id": surfaceId.uuidString])
                return
            }
            ...
            if let index {
                ...
                guard let anchorPane = ws.paneId(forPanelId: beforeSurfaceId),
                      ...
                      let anchorIndex = ws.indexInPane(forPanelId: beforeSurfaceId)
                else {
                ...
                guard let anchorPane = ws.paneId(forPanelId: afterSurfaceId),
                      anchorPane == sourcePane,
                      let anchorIndex = ws.indexInPane(forPanelId: afterSurfaceId)
                else {
                    result = .err(code: "invalid_params", message: "Anchor surface must be in the same pane", data: nil)
                    return
                }
    ...
    private func readTerminalTextBase64(terminalPanel: TerminalPanel, includeScrollback: Bool = false, lineLimit: Int? = nil) -> String {
        ...
        let rawData = if let ptr = text.text, text.text_len > 0 {
            Data(bytes: ptr, count: Int(text.text_len))
        } else {
            Data()
        }
    ...
    private func v2PaneSwap(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            guard let selectedSourceTab = workspace.bonsplitController.selectedTab(inPane: sourcePane),
                  ...
                  let targetSurfaceId = workspace.panelIdFromSurfaceId(selectedTargetTab.id)
            else {
    ...
    private func v2PaneJoin(params: [String: Any]) -> V2CallResult {
        ...
        if surfaceId == nil, let sourcePaneUUID = v2UUID(params, "pane_id") {
            guard let sourceLocated = v2LocatePane(sourcePaneUUID),
                  ...
                  let selectedSurface = sourceLocated.workspace.panelIdFromSurfaceId(selected.id)
            else {
    ...
    private func v2FeedbackOpen(params: [String: Any]) -> V2CallResult {
        ...
        DispatchQueue.main.async {
            let targetWindow: NSWindow? = if let windowId, let app = AppDelegate.shared {
                app.mainWindow(for: windowId)
            } else if let workspaceId, let app = AppDelegate.shared {
                app.mainWindowContainingWorkspace(workspaceId)
            } else {
                nil
            }
    ...
    private func v2RunBrowserJavaScript(
        ...
        if !useEval, case let .failure(pageMessage) = rawResult, #available(macOS 11.0, *) {
            ...
            switch isolatedResult {
                ...
                case let .failure(isolatedMessage):
    ...
    private func v2BrowserBack(params: [String: Any]) -> V2CallResult {
        v2BrowserNavSimple(params: params, action: "back")
    }

    private func v2BrowserForward(params: [String: Any]) -> V2CallResult {
        v2BrowserNavSimple(params: params, action: "forward")
    }

    private func v2BrowserReload(params: [String: Any]) -> V2CallResult {
        v2BrowserNavSimple(params: params, action: "reload")
    }
    ...
    private func v2BrowserElementNotFoundResult(
        ...
        let message = if count > 0, visibleCount == 0 {
            "Element \"\(selector)\" is present but not visible."
        } else if count > 1 {
            "Selector \"\(selector)\" matched multiple elements."
        } else {
            "Element \"\(selector)\" not found or not visible. Run 'browser snapshot' to see current page elements."
        }
    ...
    private func v2BrowserAppendPostSnapshot(
        ...
        switch v2BrowserSnapshot(params: snapshotParams) {
            ...
            case let .err(code: code, message: message, data: data):
                var err: [String: Any] = [
                    "code": code,
                    "message": message,
                ]
    ...
    private func v2BrowserEval(params: [String: Any]) -> V2CallResult {
        ...
        return v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            switch v2RunBrowserJavaScript(browserPanel.webView, surfaceId: surfaceId, script: script, timeout: 10.0) {
                case let .failure(message):
                    .err(code: "js_error", message: message, data: nil)
                case let .success(value):
                    .ok([
                        "workspace_id": ws.id.uuidString,
                        "workspace_ref": v2Ref(kind: .workspace, uuid: ws.id),
                ...
                case let .success(value):
                    ...
                    for entry in entries {
                        guard let selector = entry["selector"] as? String,
                              !selector.isEmpty,
                              !seenSelectors.contains(selector)
                        else {
                            continue
                        }
    ...
    private func v2BrowserWait(params: [String: Any]) -> V2CallResult {
        let timeoutMs = max(1, v2Int(params, "timeout_ms") ?? 5000)
                ...
                case let .success(value):
                    if let b = value as? Bool, b {
                        return .ok([
                            ...
                            "surface_ref": v2Ref(kind: .surface, uuid: surfaceIdOut),
                            "waited": true,
                        ])
                    }
    ...
    private func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        v2BrowserWithPanel(params: params) { _, ws, surfaceId, browserPanel in
            ...
            let screenshotsDirectory = FileManager.default
                .temporaryDirectory
    ...
    private func v2BrowserDownloadWait(params: [String: Any]) -> V2CallResult {
        v2BrowserWithPanel(params: params) { _, ws, surfaceId, _ in
            let timeoutMs = max(1, v2Int(params, "timeout_ms") ?? v2Int(params, "timeout") ?? 10000)
            let timeout = Double(timeoutMs) / 1000.0
    ...
    private func v2BrowserTabNew(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            ...
            guard let paneUUID,
                  let pane = ws.bonsplitController.allPaneIds.first(where: { $0.id == paneUUID })
            else {
    ...
    private func v2BrowserHighlight(params: [String: Any]) -> V2CallResult {
        v2BrowserSelectorAction(params: params, actionName: "highlight") { selectorLiteral in
            """
            (() => {
              const el = document.querySelector(\(selectorLiteral));
              if (!el) return { ok: false, error: 'not_found' };
    ...
    private func v2BrowserInputTouch(params _: [String: Any]) -> V2CallResult {
        ...
        private func v2DebugShortcutSet(params: [String: Any]) -> V2CallResult {
            guard let name = v2String(params, "name"),
                  let combo = v2String(params, "combo")
            else {
    ...
    private func readTerminalTextBase64(surfaceArg: String, includeScrollback: Bool = false, lineLimit: Int? = nil) -> String {
        ...
        DispatchQueue.main.sync {
            ...
            let panelId: UUID? = if trimmedSurfaceArg.isEmpty {
                tab.focusedPanelId
            } else {
                resolveSurfaceId(from: trimmedSurfaceArg, tab: tab)
            }
    ...
    private func helpText() -> String {
        ...
        private func seedDragPasteboardFileURL() -> String {
            seedDragPasteboardTypes("fileurl")
        }

        private func seedDragPasteboardTabTransfer() -> String {
            seedDragPasteboardTypes("tabtransfer")
        }

        private func seedDragPasteboardSidebarReorder() -> String {
            seedDragPasteboardTypes("sidebarreorder")
        }
        ...
        private func parseOverlayEventType(_ token: String) -> (isKnown: Bool, eventType: NSEvent.EventType?) {
            switch token {
                case "leftmousedragged":
                    (true, .leftMouseDragged)
                case "rightmousedragged":
                    (true, .rightMouseDragged)
                case "othermousedragged":
                    (true, .otherMouseDragged)
        ...
        private func renderStats(_ args: String) -> String {
            ...
            DispatchQueue.main.sync {
                ...
                let panelId: UUID? = if panelArg.isEmpty {
                    tab.focusedPanelId
                } else {
                    resolveSurfaceId(from: panelArg, tab: tab)
                }
    ...
    private func moveWorkspaceToWindow(_ args: String) -> String {
        ...
        v2MainSync {
            guard let srcTM = AppDelegate.shared?.tabManagerFor(tabId: wsId),
                  ...
                  let ws = srcTM.detachWorkspace(tabId: wsId)
            else {
                ok = false
                return
            }
    ...
    private func notifyTarget(_ args: String) -> String {
        ...
        var result = "OK"
        DispatchQueue.main.sync {
            let tab: Tab? = if let tabId = UUID(uuidString: tabArg) {
                tabForSidebarMutation(id: tabId)
            } else {
                resolveTab(from: tabArg, tabManager: tabManager)
            }
            guard let tab else {
                result = "ERROR: Tab not found"
                return
    ...
    private func clearNotifications(_ args: String) -> String {
        ...
        guard let tabOption = parsed.options["tab"],
              !tabOption.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        else {
    ...
    private func simulateAppDidBecomeActive() -> String {
        ...
        #if DEBUG
            private struct PanelSnapshotState {
                let width: Int
                let height: Int
                let bytesPerRow: Int
                let rgba: Data
                ...
                previous.rgba.withUnsafeBytes { prevRaw in
                    current.rgba.withUnsafeBytes { curRaw in
                        guard let prev = prevRaw.bindMemory(to: UInt8.self).baseAddress,
                              let cur = curRaw.bindMemory(to: UInt8.self).baseAddress
                        else {
                            return
                        }
        ...
        private struct LayoutDebugSelectedPanel: Codable {
            let paneId: String
        ...
        private struct LayoutDebugSplitView: Codable {
            let isVertical: Bool
        ...
        private struct LayoutDebugResponse: Codable {
    ...
    private func resolveSurface(from arg: String, tabManager: TabManager) -> ghostty_surface_t? {
        ...
        resolveTerminalSurface(from: arg, tabManager: tabManager, waitUpTo: 2.0)
    }
        ...
        DispatchQueue.main
            .sync {
                guard let targetManager = AppDelegate.shared?.tabManagerFor(tabId: workspaceId)
                    ?? (tabManager.tabs.contains(where: { $0.id == workspaceId }) ? tabManager : nil)
                else {
    ...
    private func navigateBrowser(_ args: String) -> String {
        ...
        DispatchQueue.main.sync {
            guard let tabId = tabManager.selectedTabId,
                  ...
                  let browserPanel = tab.browserPanel(for: panelId)
            else {
                return
            }
    ...
    private func upsertSidebarMetadata(_ args: String, missingError: String) -> String {
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self, let tab = tabForSidebarMutation(id: targetTabId) else { return }
    ...
    private func setAgentPID(_ args: String) -> String {
        ...
        guard parsed.positional.count >= 2,
              let pid = Int32(parsed.positional[1]), pid > 0
        else {
    ...
    private func listLog(_ args: String) -> String {
        ...
        DispatchQueue.main.sync {
            ...
            let entries: [SidebarLogEntry] = if let limit {
                Array(tab.logEntries.suffix(limit))
            } else {
                tab.logEntries
            }
    ...
    private func closeSurface(_ args: String) -> String {
        ...
        DispatchQueue.main.sync {
            ...
            let surfaceId: UUID? = if trimmed.isEmpty {
                tab.focusedPanelId
            } else {
                resolveSurfaceId(from: trimmed, tab: tab)
            }
    ...
    private func newSurface(_ args: String) -> String {
        ...
        DispatchQueue.main.sync {
            ...
            let newPanelId: UUID? = if panelType == .browser {
                tab.newBrowserSurface(inPane: targetPaneId, url: url, focus: shouldFocus)?.id
            } else {
                tab.newTerminalSurface(inPane: targetPaneId, focus: shouldFocus)?.id
            }
