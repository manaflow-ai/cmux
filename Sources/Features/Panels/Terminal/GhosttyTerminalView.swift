...
// MARK: - GhosttyDefaultBackgroundUpdateScope
...
// MARK: - GhosttyDefaultBackgroundNotificationDispatcher
...
// MARK: - TerminalKeyboardCopyModeSelectionMove
...
// MARK: - TerminalKeyboardCopyModeAction
...
// MARK: - TerminalKeyboardCopyModeInputState
...
// MARK: - TerminalKeyboardCopyModeResolution
...
private let terminalKeyboardCopyModeMaxCount = 9999
...
// MARK: - GhosttySurfaceCallbackContext
...
final class GhosttyApp {
    ...
    private lazy var defaultBackgroundNotificationDispatcher: GhosttyDefaultBackgroundNotificationDispatcher =
        ...
        .init(delay: 0, logEvent: { [weak self] message in
            guard let self, backgroundLogEnabled else { return }
    ...
    func synchronizeThemeWithAppearance(_ appearance: NSAppearance?, source: String) {
        ...
        if backgroundLogEnabled {
            let previousLabel = switch lastAppearanceColorScheme {
                case .light:
                    "light"
                case .dark:
                    "dark"
    ...
    private func splitDirection(from direction: ghostty_action_split_direction_e) -> SplitDirection? {
        switch direction {
            case GHOSTTY_SPLIT_DIRECTION_RIGHT: .right
    ...
    private func focusDirection(from direction: ghostty_action_goto_split_e) -> NavigationDirection? {
        switch direction {
            ...
            case GHOSTTY_GOTO_SPLIT_PREVIOUS: .left
            case GHOSTTY_GOTO_SPLIT_NEXT: .right
            case GHOSTTY_GOTO_SPLIT_UP: .up
            case GHOSTTY_GOTO_SPLIT_DOWN: .down
            case GHOSTTY_GOTO_SPLIT_LEFT: .left
            case GHOSTTY_GOTO_SPLIT_RIGHT: .right
            default: nil
        }
    }
    ...
    private func resizeDirection(from direction: ghostty_action_resize_split_direction_e) -> ResizeDirection? {
        switch direction {
            ...
            case GHOSTTY_RESIZE_SPLIT_RIGHT: .right
            default: nil
        }
    }
    ...
    private func handleAction(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        if target.tag != GHOSTTY_TARGET_SURFACE {
            ...
            if action.tag == GHOSTTY_ACTION_DESKTOP_NOTIFICATION {
                let actionTitle = action.action
                    .desktop_notification
                    .title
                    .flatMap { String(cString: $0) } ?? ""
                let actionBody = action.action
                    .desktop_notification
                    .body
                    .flatMap { String(cString: $0) } ?? ""
...
final class TerminalSurface: Identifiable, ObservableObject {
    ...
    var currentKeyStateIndicatorText: String? {
        surfaceView.currentKeyStateIndicatorText
    }
    ...
    func forceRefresh(reason: String = "unspecified") {
        ...
        guard let currentSurface = surface else { return }
    ...
    func requestBackgroundSurfaceStartIfNeeded() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            createSurface(for: view)
                ...
                dlog(
                    "surface.background_start surface=\(id.uuidString.prefix(8)) inWindow=\(view.window != nil ? 1 : 0) ready=\(surface != nil ? 1 : 0) ms=\(String(format: "%.2f", elapsedMs))"
                )
            #endif
        }
    }
...
final class GhosttyNSView: NSView, NSUserInterfaceValidations {
    // MARK: Nested Types
    ...
    fileprivate var isKeyboardCopyModeActive: Bool {
    ...
    fileprivate var isVisibleInUI: Bool {
        visibleInUI
    }
    ...
    override func scrollWheel(with event: NSEvent) {
        ...
        let momentum = switch event.momentumPhase {
            case .began:
    ...
    @IBAction func pasteAsPlainText(_: Any?) {
        _ = performBindingAction("paste_from_clipboard")
    }
    ...
    private func activeSurfaceResizeDeferralReason() -> String? {
        Self.shouldDeferSurfaceResizeForActiveDrag() ? "tabDrag" : nil
    }
    ...
    private func sendGhosttyKey(_ surface: ghostty_surface_t, _ keyEvent: ghostty_input_key_s) -> Bool {
        ...
        private func sendTimedGhosttyKey(
            ...
            let mergedExtra: String = if let extra, !extra.isEmpty {
                "\(baseExtra) \(extra)"
            } else {
                baseExtra
            }
    ...
    @objc private func splitHorizontally(_: Any?) {
        _ = splitCurrentSurface(direction: .down)
    }

    @objc private func splitVertically(_: Any?) {
    ...
    @objc private func triggerFlash(_: Any?) {
        onTriggerFlash?()
    }
...
// MARK: - GhosttyPassthroughVisualEffectView
...
final class GhosttySurfaceScrollView: NSView {
    ...
    enum SearchFocusTarget {
        ...
        func debugSampleIOSurface(normalizedCrop: CGRect) -> DebugFrameSample? {
            ...
            guard CFGetTypeID(cf) == IOSurfaceGetTypeID() else {
                var fnv: UInt64 = 1_469_598_103_934_665_603
    ...
    init(surfaceView: GhosttyNSView) {
        ...
        let initialTerminalBackground = GhosttyApp.shared
    ...
    override func viewDidMoveToWindow() {
        ...
        windowObservers.append(NotificationCenter.default.addObserver(
            ...
            guard let self else { return }
            let searchActive = surfaceView.terminalSurface?.searchState != nil
            #if DEBUG
                dlog("find.window.didBecomeKey surface=\(surfaceView.terminalSurface?.id.uuidString.prefix(5) ?? "nil") searchActive=\(searchActive) focusTarget=\(searchFocusTarget) firstResponder=\(String(describing: self.window?.firstResponder))")
            #endif
        ...
        windowObservers.append(NotificationCenter.default.addObserver(
            ...
            {
                #if DEBUG
                    dlog("find.window.didResignKey surface=\(surfaceView.terminalSurface?.id.uuidString.prefix(5) ?? "nil") searchActive=\(searchActive) resigningFirstResponder")
                #endif
                window.makeFirstResponder(nil)
            } else {
                #if DEBUG
                    dlog("find.window.didResignKey surface=\(surfaceView.terminalSurface?.id.uuidString.prefix(5) ?? "nil") searchActive=\(searchActive) firstResponder=\(String(describing: window.firstResponder)) (not terminal, skipping)")
                #endif
            }
        })
    ...
    func setSearchOverlay(searchState: TerminalSurface.SearchState?) {
        ...
        if let overlay = searchOverlayHostingView {
            ...
            if overlay.superview !== self {
                scheduleDeferredSearchOverlayMutation(generation: mutationGeneration) { [weak self, weak overlay] in
                    ...
                    addSubview(overlay)
                    updateKeyboardCopyModeBadgeZOrder(relativeTo: overlay)
                    requestMountedSearchFieldFocus(
        ...
        scheduleDeferredSearchOverlayMutation(generation: mutationGeneration) { [weak self, weak overlay] in
            guard let self, let overlay else { return }
            guard searchOverlayHostingView === overlay else { return }
            overlay.removeFromSuperview()
            overlay.frame = bounds
            overlay.autoresizingMask = [.width, .height]
            addSubview(overlay)
    ...
    func setDropZoneOverlay(zone: DropZone?) {
        ...
        if let zone {
            ...
            if dropZoneOverlayView.isHidden {
                ...
                NSAnimationContext.runAnimationGroup { context in
                    ...
                    dropZoneOverlayView.animator().alphaValue = 1
                        ...
                        logDropZoneOverlay(event: "showComplete", zone: zone, frame: targetFrame)
                    #endif
                }
                return
            }
            ...
            NSAnimationContext.runAnimationGroup { context in
                ...
                guard activeDropZone == nil else { return }
                dropZoneOverlayView.isHidden = true
                dropZoneOverlayView.alphaValue = 1
                #if DEBUG
    ...
    func triggerFlash(style: FlashStyle = .standardFocus) {
        DispatchQueue.main.async { [weak self] in
            ...
            flashLayer.add(animation, forKey: "cmux.flash")
        }
    }
    ...
    func moveFocus(from previous: GhosttySurfaceScrollView? = nil, delay: TimeInterval? = nil) {
        ...
        let work = { [weak self] in
            ...
            #if DEBUG
                dlog(
                    "find.moveFocus.apply to=\(surfaceView.terminalSurface?.id.uuidString.prefix(5) ?? "nil") " +
    ...
    func ownedPanelFocusIntent(for responder: NSResponder) -> TerminalPanelFocusIntent? {
        ...
        let resolvedResponder: NSResponder = if let editor = responder as? NSTextView,
    ...
    func yieldPanelFocusIntent(_ intent: TerminalPanelFocusIntent, in window: NSWindow) -> Bool {
        guard let firstResponder = window.firstResponder,
              ownedPanelFocusIntent(for: firstResponder) == intent
        else {
            return false
        }
...
// MARK: - GhosttyNSView + NSTextInputClient
