...
// MARK: - NotificationSoundSettings
...
enum NotificationSoundSettings {
    ...
    enum CustomSoundPreparationIssue: Error {
        ...
        var logMessage: String {
            switch self {
                case .emptyPath:
                    "Notification custom sound path is empty"
                case let .missingFile(path):
                    "Notification custom sound file does not exist: \(path)"
                case let .missingFileExtension(path):
                    "Notification custom sound requires a file extension: \(path)"
                case let .stagingFailed(path, details):
                    "Failed to stage custom notification sound from \(path): \(details)"
            }
        }
    }
    ...
    private static func transcodeStagedSoundIfNeeded(
        ...
        guard process.terminationStatus == 0 else {
            ...
            let description: String = if let errorOutput, !errorOutput.isEmpty {
                errorOutput
            } else {
...
// MARK: - TerminalNotificationStore
