...
extension Workspace {
    ...
    private func applySessionDividerPositions(
        ...
        switch (snapshotNode, liveNode) {
            case let (.split(snapshotSplit), .split(liveSplit)):
...
enum SidebarBranchOrdering {
    ...
    static func orderedPaneIds(tree: ExternalTreeNode) -> [String] {
        switch tree {
            ...
            case let .split(split):
                ...
                orderedPaneIds(tree: split.first) + orderedPaneIds(tree: split.second)
        }
    }
    ...
    static func orderedUniqueBranchDirectoryEntries(
        ...
        for panelId in orderedPanelIds {
            ...
            let key = if let directoryKey = canonicalDirectoryKey(directory) {
                // Keep one line per directory and allow the latest branch state to overwrite.
                EntryKey(directory: directoryKey, branch: nil)
            } else {
                EntryKey(directory: nil, branch: branch)
            }
...
// MARK: - ClosedBrowserPanelRestoreSnapshot
...
final class Workspace: Identifiable, ObservableObject {
    ...
    private nonisolated static let manualUnreadFocusGraceInterval: TimeInterval = 0.2
    private nonisolated static let manualUnreadClearDelayAfterFocusFlash: TimeInterval = 0.2
    ...
    private var isDetachingCloseTransaction: Bool {
    ...
    init(
        ...
        currentDirectory = hasWorkingDirectory
    ...
    private static func bonsplitAppearance(
        ...
        BonsplitConfiguration.Appearance(
            splitButtonTooltips: currentSplitButtonTooltips(),
    ...
    private func installBrowserPanelSubscription(_ browserPanel: BrowserPanel) {
        ...
        .sink { [weak self, weak browserPanel] _, isLoading, favicon in
            guard let self,
                  let browserPanel,
                  let tabId = surfaceIdFromPanelId(browserPanel.id) else { return }
    ...
    private func installMarkdownPanelSubscription(_ markdownPanel: MarkdownPanel) {
        let subscription = markdownPanel.$displayTitle
            ...
            .sink { [weak self, weak markdownPanel] newTitle in
                guard let self,
                      let markdownPanel,
                      let tabId = surfaceIdFromPanelId(markdownPanel.id) else { return }
                guard let existing = bonsplitController.tab(tabId) else { return }

                if panelTitles[markdownPanel.id] != newTitle {
                    panelTitles[markdownPanel.id] = newTitle
                }
                let resolvedTitle = resolvedPanelTitle(panelId: markdownPanel.id, fallback: newTitle)
    ...
    private func browserCloseFallbackPlan(
        ...
        switch node {
            case .pane:
                return nil
            case let .split(splitNode):
                if case let .pane(firstPane) = splitNode.first, firstPane.id == targetPaneId {
                ...
                if case let .pane(secondPane) = splitNode.second, secondPane.id == targetPaneId {
                    return BrowserCloseFallbackPlan(
                        orientation: splitNode.orientation.lowercased() == "vertical" ? .vertical : .horizontal,
                        insertFirst: false,
                        anchorPaneId: browserNearestPaneId(
    ...
    private func browserNearestPaneId(
        ...
        let bestPane: ExternalPaneNode? = if let targetCenter {
    ...
    private func scheduleTerminalFirstResponderReassert(panelId: UUID, remainingPasses: Int = 2) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            scheduleTerminalFirstResponderReassert(
                panelId: panelId,
    ...
    private func runScheduledTerminalGeometryReconcile(remainingPasses: Int) {
        ...
        if shouldRunAgain, remainingPasses > 1 {
            ...
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                runScheduledTerminalGeometryReconcile(remainingPasses: remainingPasses - 1)
            }
            return
        }
    ...
    private func scheduleTerminalGeometryReconcile() {
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            runScheduledTerminalGeometryReconcile(remainingPasses: 4)
        }
    }
    ...
    private func scheduleTerminalPortalVisibilityReconcileAfterSplitZoom(remainingPasses: Int) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if terminalPortalVisibilityNeedsFollowUp(), remainingPasses > 1 {
                scheduleTerminalPortalVisibilityReconcileAfterSplitZoom(
                    remainingPasses: remainingPasses - 1
                )
            }
        }
    ...
    private func scheduleBrowserPortalVisibilityReconcileAfterSplitZoom(
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if browserPortalVisibilityNeedsFollowUp(), remainingPasses > 1 {
                scheduleBrowserPortalVisibilityReconcileAfterSplitZoom(
                    remainingPasses: remainingPasses - 1,
                    reason: reason
                )
    ...
    private func scheduleBrowserPortalReconcileAfterSplitZoom(panelId: UUID, remainingPasses: Int) {
        guard remainingPasses > 0 else { return }
        DispatchQueue.main.async { [weak self] in
            guard let self, let browserPanel = browserPanel(for: panelId) else { return }
            ...
            if portalNeedsFollowUpPass {
                scheduleBrowserPortalReconcileAfterSplitZoom(
                    panelId: panelId,
                    remainingPasses: remainingPasses - 1
                )
            }
    ...
    private func scheduleBrowserSplitZoomExitFocusReassert(panelId: UUID, remainingPasses: Int) {
        guard remainingPasses > 0 else { return }
        DispatchQueue.main.async { [weak self] in
            guard let self, browserPanel(for: panelId) != nil else { return }
            ...
            if !selectionConverged || !anchorReady {
                scheduleBrowserSplitZoomExitFocusReassert(
                    panelId: panelId,
                    remainingPasses: remainingPasses - 1
                )
            }
        ...
        switch destination {
            ...
            case .selectedWorkspaceInNewWindow:
                ...
                guard let destinationManager = app.tabManagerFor(windowId: newWindowId),
                      let destinationWorkspaceId = destinationManager.selectedTabId
                else {
                    return
                }
                moved = app.moveSurface(
            ...
            case let .existingWorkspace(workspaceId):
                moved = app.moveSurface(
                    panelId: panelId,
    ...
    private func handleExternalTabDrop(_ request: BonsplitController.ExternalTabDropRequest) -> Bool {
        ...
        switch request.destination {
            ...
            case let .split(paneId, orientation, insertFirst):
                targetPane = paneId
                targetIndex = nil
...
extension Workspace: BonsplitDelegate {
    ...
    @MainActor
    private func confirmClosePanel(for _: TabID) async -> Bool {
        let alert = NSAlert()
    ...
    func splitTabBar(_: BonsplitController, didSelectTab tab: Bonsplit.Tab, inPane pane: PaneID) {
    ...
    func splitTabBar(_ controller: BonsplitController, didMoveTab tab: Bonsplit.Tab, fromPane source: PaneID, toPane destination: PaneID) {
            ...
            let sincePrev = if debugLastDidMoveTabTimestamp > 0 {
                String(format: "%.2f", (now - debugLastDidMoveTabTimestamp) * 1000)
            } else {
                "first"
            }
    ...
    func splitTabBar(_: BonsplitController, didClosePane paneId: PaneID) {
    ...
    func splitTabBar(_ controller: BonsplitController, didSplitPane originalPane: PaneID, newPane: PaneID, orientation: SplitOrientation) {
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            if bonsplitController.focusedPaneId == newPane {
                bonsplitController.selectTab(newTabId)
            }
            scheduleTerminalGeometryReconcile()
            scheduleFocusReconcile()
    ...
    func splitTabBar(_: BonsplitController, didChangeGeometry snapshot: LayoutSnapshot) {
        _ = snapshot
        scheduleTerminalGeometryReconcile()
        if !isDetachingCloseTransaction {
            scheduleFocusReconcile()
