...
struct CmuxCLIPathInstaller {
    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
                case let .bundledCLIMissing(expectedPath):
                    "Bundled cmux CLI was not found at \(expectedPath)."
                case let .destinationParentNotDirectory(path):
                    "Expected \(path) to be a directory."
                case let .destinationIsDirectory(path):
                    "\(path) is a directory. Remove or rename it and try again."
                case let .installVerificationFailed(path):
                    "Installed symlink at \(path) did not point to the bundled cmux CLI."
                case let .uninstallVerificationFailed(path):
                    "Failed to remove \(path)."
                case let .privilegedCommandFailed(message):
                    "Administrator action failed: \(message)"
            }
        }
    }
    ...
    private func verifyInstalledSymlinkTarget(sourceURL: URL) throws {
        guard let installedTargetURL = symlinkDestinationURL(),
              installedTargetURL == sourceURL.standardizedFileURL
        else {
