...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    ...
    override func viewDidMoveToWindow() {
        ...
        windowOcclusionObservers.append(NotificationCenter.default.addObserver(
            ...
            MainActor.assumeIsolated {
                self?.syncRendererWindowVisibility(
                    for: occludedWindow,
                    assumeVisibleWhenUnknown: false
                )
            }
        })
        windowOcclusionObservers.append(NotificationCenter.default.addObserver(
    ...
    private func syncRendererWindowVisibility(
        ...
        if !window.isVisible || window.isMiniaturized {
            ...
            // The first occlusion sample can be produced while AppKit is still
            // ordering a transparent/headless window. Callers that are doing a
            // lifecycle convergence pass opt into the optimistic value; the
            // dedicated occlusion notification passes `false` and therefore
            // remains authoritative for genuinely covered windows.
