...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
        ...
        if mouseCaptured, !capturedRightClickMenuLatch {
            return nil
        }

        window?.makeFirstResponder(self)
        if !mouseCaptured {
            // Keep the synthetic right-press for the uncaptured path only: on
            // the latched bypass path nothing may reach the core, or the TUI
            // would see a position update / button report for a click cmux
            // decided to handle locally.
            let point = convert(event.locationInWindow, from: nil)
            ghostty_surface_mouse_pos(surface, point.x, bounds.height - point.y, mouseModsFromEvent(event))
            _ = ghostty_surface_mouse_button(surface, GHOSTTY_MOUSE_PRESS, GHOSTTY_MOUSE_RIGHT, mouseModsFromEvent(event))
        }
