...
class GhosttyApp {
    ...
    private static func openEmbeddedBrowserLink(
        ...
        guard let openResult = workspace.openTerminalLinkInBrowser(
            url: url,
            ...
            cmuxDebugLog(
                ...
                "host=\(host) placement=\(placement.rawValue) url=\(url)"
            )
            #endif
            return NSWorkspace.shared.open(url)
        }
        ...
        #if DEBUG
        cmuxDebugLog(
            "link.openURL opened in browser placement=\(openResult.placement.rawValue) " +
            "strategy=\(openResult.placementStrategy) createdSplit=\(openResult.createdSplit ? 1 : 0) " +
            "surfaceId=\(openResult.panel.id)"
        )
        #endif
        return true
    }
