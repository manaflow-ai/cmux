...
final class GhosttySurfaceScrollView: NSView {
    ...
    private var cloudReconnectDismissals = CloudBannerDismissalStore(defaults: .standard)
    ...
    private func synchronizeCloudTerminalReconnectOverlay() {
        ...
        guard let terminalSurface = surfaceView.terminalSurface,
              let presentation = currentCloudTerminalReconnectPresentation() else {
            cloudTerminalReconnectOverlayView?.removeFromSuperview()
            cloudTerminalReconnectOverlayView = nil
            return
        }
        let dismissalID = "cloud.remote-reconnect.\(terminalSurface.id.uuidString)"
        guard !cloudReconnectDismissals.isDismissed(id: dismissalID, signature: presentation.copyableError) else {
            cloudTerminalReconnectOverlayView?.removeFromSuperview()
            cloudTerminalReconnectOverlayView = nil
            return
        }
        ...
        overlay.onDismiss = { [weak self] in
            guard let self else { return }
            self.cloudReconnectDismissals.dismiss(id: dismissalID, signature: presentation.copyableError)
            self.synchronizeCloudTerminalReconnectOverlay()
        }
