...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    ...
    @IBAction func copy(_ sender: Any?) {
        ...
        guard let cleaned = cleanedSelectionTextForCopy(selection),
    ...
    @IBAction func copyCleaned(_ sender: Any?) {
        ...
        guard let cleaned = cleanedSelectionTextForCopy(selection) else {
            copyRaw(sender)
            return
        }
    ...
    private func selectionIsStructuredOrCode(_ text: String) -> Bool {
        let normalized = text
    ...
    private func cleanedSelectionTextForCopy(_ text: String) -> String? {
    ...
    private func shouldMergeSoftWrappedLine(previous: String, next: String) -> Bool {
    ...
    private func previousLineLooksTerminalWrapped(_ line: String) -> Bool {
    ...
    private func selectionLooksLikeSingleSoftWrappedPath(_ lines: [String]) -> Bool {
    ...
    private func shouldMergePathContinuation(previous: String, next: String) -> Bool {
        let previousTrimmed = previous.trimmingCharacters(in: .whitespaces)
        let nextTrimmed = next.trimmingCharacters(in: .whitespaces)
        guard !previousTrimmed.isEmpty, !nextTrimmed.isEmpty else { return false }
    ...
    private func joinSoftWrappedLines(previous: String, next: String) -> String {
        let previousTrimmed = previous.trimmingCharacters(in: .whitespaces)
        let nextTrimmed = next.trimmingCharacters(in: .whitespaces)
    ...
    private func joinSeparatorForCopy(previous: String, next: String) -> String {
    ...
    private func cleanPathWrapPadding(in text: String) -> String {
    ...
    private func needsPathWrapPaddingCleaning(_ text: String) -> Bool {
    ...
    private func isPathSegmentStartChar(_ char: Character) -> Bool {
    ...
    private func cleanProseWrapPadding(in text: String) -> String {
    ...
    private func shouldCollapseProseWrapPadding(previous: UnicodeScalar, next: UnicodeScalar) -> Bool {
        if isCJKScalar(previous) || isCJKScalar(next) { return true }
        return isAlphaNumericASCII(previous) && isAlphaNumericASCII(next)
    }

    private func shouldInsertSpaceWhenCollapsingWrap(previous: UnicodeScalar, next: UnicodeScalar) -> Bool {
        if isCJKScalar(previous) && isCJKScalar(next) { return false }
        return true
    }

    private func isAlphaNumericASCII(_ scalar: UnicodeScalar) -> Bool {
        (scalar.value >= 48 && scalar.value <= 57)
            || (scalar.value >= 65 && scalar.value <= 90)
            || (scalar.value >= 97 && scalar.value <= 122)
    }

    private func startsNewStructuredBlock(_ line: String) -> Bool {
        // Code fence (markdown)
    ...
    private func endsWithHardBoundary(_ line: String) -> Bool {
    ...
    private func matchesStackTracePattern(_ line: String) -> Bool {
    ...
    private func startsWithDiffMarker(_ line: String) -> Bool {
    ...
    private func matchesLogPattern(_ line: String) -> Bool {
    ...
    private func matchesKeyValuePattern(_ line: String) -> Bool {
        // JSON key: "key": value
    ...
    private func startsWithPromptMarker(_ line: String) -> Bool {
        let promptPrefixes = ["$ ", "# ", ">>> ", "... ", "% ", ">> "]
        return promptPrefixes.contains(where: { line.hasPrefix($0) })
    }

    private func isPathStart(_ line: String) -> Bool {
        if line == "/" { return true }
        if line.hasPrefix("/") {
    ...
    private func looksLikePathFragment(_ line: String) -> Bool {
    ...
    private func looksLikePathContinuation(_ line: String) -> Bool {
        guard !line.isEmpty, !line.contains(where: \.isWhitespace) else { return false }
        guard line.unicodeScalars.allSatisfy(isPathSafeScalar) else { return false }
        return line.contains("/") || line.count >= 2
    }

    private func previousPathLineLooksTerminalWrapped(_ line: String) -> Bool {
    ...
    private func isPathSafeScalar(_ scalar: UnicodeScalar) -> Bool {
    ...
    private func startsWithPathPattern(_ line: String) -> Bool {
        isPathStart(line)
    }

    /// Returns true if the line contains code-like symbols (parens, braces, semicolons, operators)
    private func containsCodeSymbols(_ line: String) -> Bool {
    ...
    private func hasHighSymbolDensity(_ line: String) -> Bool {
    ...
    private func isLowercaseASCII(_ scalar: UnicodeScalar) -> Bool {
        scalar.value >= 97 && scalar.value <= 122
    }
