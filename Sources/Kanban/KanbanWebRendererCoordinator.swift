...
final class KanbanWebRendererCoordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandlerWithReply {
    ...
    /// One board repository per coordinator, rooted at the shared cmux runtime
    /// state directory (`~/.local/state/cmux/kanban`) so the CLI can read/write
    /// the same files without a TCC prompt.
    private let repository: KanbanBoardRepository = {
        ...
        return KanbanBoardRepository(baseDirectory: base)
    }()

    /// Cached board for this workspace. The coordinator is `@MainActor`, so the
    /// cached copy is the in-memory source of truth between disk writes; it is
    /// loaded lazily on the first board request.
    ...
    private func handle(_ request: KanbanBridgeRequest) async throws -> Any {
        ...
        case "getBoard":
            let board = try await loadedBoard()
        ...
        case "createTask":
            ...
            let updated = try await loadedBoard().upserting(card, now: now)
            try await persist(updated)
            return Self.boardPayload(updated)
        case "moveCard":
            let cardId = try request.requiredUUID("cardId")
            let column = try request.requiredColumn()
            let updated = try await loadedBoard().movingCard(id: cardId, to: column, now: Date())
            try await persist(updated)
            return Self.boardPayload(updated)
        default:
    ...
    private func loadedBoard() async throws -> KanbanBoard {
        if let board {
            return board
        }
        let loaded = try await repository.load(workspaceId: workspaceId, now: Date())
        board = loaded
        return loaded
    }

    private func persist(_ newBoard: KanbanBoard) async throws {
        board = newBoard
        try await repository.save(newBoard)
        sendEvent([
            "type": "kanban.boardUpdated",
            "board": Self.boardPayload(newBoard)
        ])
    }
