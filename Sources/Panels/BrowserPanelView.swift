...
// WKWebView hide/show rendering-state reattach extension moved to
// CmuxBrowserUI (WKWebView+RenderingStateReattach.swift). The in-window
// rendering-state pokes (cmuxBrowserPanelNotifyHidden / ReattachRenderingState
// / ForceRenderingStateRefresh / RequiresRenderingStateReattach /
// IsInspectorFrontend) are now public WKWebView API on that package, which this
// file imports.

/// View for rendering a browser panel with address bar
struct BrowserPanelView: View {
    @ObservedObject var panel: BrowserPanel
    @State private var browserProfileStore = BrowserProfileStore.shared
    let paneId: PaneID
    let isFocused: Bool
    ...
    /// Per-panel-view live-field lookup cache for the omnibar native text field,
    /// injected into the omnibar text-field representable and the pointer-forwarding
    /// interaction representable. De-singletonized from the former process-wide
    /// `BrowserOmnibarNativeFieldRegistry.shared`.
    @State private var nativeFieldRegistry = BrowserOmnibarNativeFieldRegistry()
    ...
    private var browserThemeModeButton: some View {
        ...
        .popover(isPresented: $isBrowserThemeMenuPresented, arrowEdge: .bottom) {
            BrowserThemeModePopoverView(
                items: BrowserThemeMode.allCases.map {
                    BrowserThemeModeItem(displayName: $0.displayName, mode: $0)
                },
    ...
    private var browserProfilePopover: some View {
        BrowserProfilePopoverView(
            ...
            items: browserProfileStore.profiles.map { profile in
                BrowserProfileItem(
                    id: profile.id,
            ...
            newProfileTitle: String(localized: "browser.profile.new", defaultValue: "New Profile..."),
            importTitle: String(localized: "menu.view.importFromBrowser", defaultValue: "Import Browser Data…"),
            renameTitle: String(localized: "browser.profile.rename", defaultValue: "Rename Current Profile..."),
            canRename: browserProfileStore.canRenameProfile(id: panel.profileID),
            ...
            onSelect: { profileID in
                applyBrowserProfileSelection(profileID)
            },
            onCreate: {
    ...
    private var omnibarField: some View {
        ...
        return HStack(spacing: 4) {
            ...
            OmnibarTextFieldRepresentable(
                ...
                nativeFieldRegistry: nativeFieldRegistry,
                onTap: {
                    handleOmnibarTap()
                },
        ...
        .overlay {
            BrowserOmnibarInteractionRepresentable(panelId: panel.id, nativeFieldRegistry: nativeFieldRegistry)
        }
        .overlay(
            RoundedRectangle(cornerRadius: omnibarPillCornerRadius, style: .continuous)
    ...
    private func canHandleOmnibarSelectionNavigation() -> Bool {
        ...
        if let field = nativeFieldRegistry.field(forPanelId: panel.id, in: fieldWindow),
