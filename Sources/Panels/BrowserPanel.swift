...
final class BrowserHistoryStore: ObservableObject {
    ...
    // main thread once history grew to a few thousand rows. `nil` means "not
    // built / just invalidated"; it is rebuilt only when `entries` changes, so
    // steady-state typing reuses it and pays only cheap substring scoring.
