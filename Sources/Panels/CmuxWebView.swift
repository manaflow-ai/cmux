...
/// Observes WebKit's private, KVO-compliant `_isPlayingAudio` property on a
/// `WKWebView` — the Chrome/Safari-style "this tab is producing sound" signal.
///
/// `_isPlayingAudio` is not surfaced in the Swift `WKWebView` interface, so it
/// cannot be watched with a Swift key-path `observe(_:)`; classic string-based
/// KVO is required. cmux already ships notarized outside the App Store and
/// already uses the sibling private `_setPageMuted:` route (`cmuxSetPageAudioMuted`),
/// so this is consistent with existing practice.
///
/// This is the *audible* signal — muting a page drives it to `false` — which is
/// deliberately distinct from the DOM-level `BrowserPanel.isPlayingMedia`
/// keep-alive signal that intentionally counts muted video.
final class WebViewAudioPlaybackObserver: NSObject {
    private static let keyPath = "_isPlayingAudio"
    private weak var webView: WKWebView?
    private let onChange: (Bool) -> Void
    private var isObserving = false

    /// Returns `nil` when the running WebKit does not expose `_isPlayingAudio`,
    /// so callers degrade gracefully instead of trapping on an unknown key.
    init?(webView: WKWebView, onChange: @escaping (Bool) -> Void) {
        guard webView.responds(to: NSSelectorFromString(Self.keyPath)) else { return nil }
        self.webView = webView
        self.onChange = onChange
        super.init()
        webView.addObserver(self, forKeyPath: Self.keyPath, options: [.initial, .new], context: nil)
        isObserving = true
    }

    var isPlayingAudio: Bool {
        (webView?.value(forKey: Self.keyPath) as? NSNumber)?.boolValue ?? false
    }
    ...
    override func observeValue(
        ...
        let playing = (change?[.newKey] as? NSNumber)?.boolValue ?? isPlayingAudio
        onChange(playing)
    }

    /// Detaches the observer. Safe to call repeatedly. Must run while the
    /// observed web view is still alive (KVO removal on a deallocated object
    /// traps), so callers invalidate before releasing/replacing the web view.
    func invalidate() {
        guard isObserving, let webView else {
            isObserving = false
            return
        }
        webView.removeObserver(self, forKeyPath: Self.keyPath)
        isObserving = false
        self.webView = nil
    }
