...
final class MarkdownPanel: Panel, FilePreviewTextEditingPanel {
    ...
    @ObservationIgnored let panelType: PanelType = .markdown
    ...
    /// Legacy Combine bridge for the remaining content test subscriber. Emits
    /// the new value during willSet and replays the current value on subscribe.
    @ObservationIgnored let contentPublisher = CurrentValueSubject<String, Never>("")
    private(set) var content: String = "" {
        willSet { contentPublisher.send(newValue) }
    }
    ...
    /// Legacy Combine bridge for the remaining `markdownPanel.$isDirty`
    /// subscriber; preserves `Published.Publisher` timing and replay.
    ...
    /// Legacy Combine bridge for the remaining `markdownPanel.$displayTitle`
    /// subscriber; preserves `Published.Publisher` timing and replay.
    ...
    @ObservationIgnored let rendererSession = MarkdownRendererSession()
    ...
    @ObservationIgnored private var originalTextContent: String = ""
    @ObservationIgnored private var textEncoding: String.Encoding = .utf8
    @ObservationIgnored private var saveGeneration: Int = 0
    @ObservationIgnored private var activeSaveGeneration: Int?
    @ObservationIgnored private var pendingSearchNeedle: String?
    @ObservationIgnored private weak var textView: NSTextView?
    @ObservationIgnored private var isClosed: Bool = false
    ...
    @ObservationIgnored private var followedFontSize: Double
    @ObservationIgnored private var followedFontFamily: String
    @ObservationIgnored private var followedMaxContentWidth: Double
    ...
    init(workspaceId: UUID, filePath: String, fontSize: Double? = nil) {
        ...
        let initialDisplayTitle = (filePath as NSString).lastPathComponent
