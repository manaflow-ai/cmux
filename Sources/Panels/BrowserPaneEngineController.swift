...
final class BrowserPaneEngineController {
    ...
    private var chromiumInputFailureHandler: ((any Error) -> Void)?
    ...
    func setChromiumInputFailureHandler(_ handler: @escaping (any Error) -> Void) {
        chromiumInputFailureHandler = handler
        (adapter as? ChromiumBrowserPaneEngineAdapter)?.hostView.onInputFailure = handler
    }
    ...
    func replaceChromium(
        ...
        if let oldChromium = adapter as? ChromiumBrowserPaneEngineAdapter {
            ...
            replacement.hostView.onInputFailure = chromiumInputFailureHandler
            adapter = replacement
            return true
        }
        adapter.stop()
    ...
    func fallBackToWebKit(_ webView: WKWebView) {
        (adapter as? ChromiumBrowserPaneEngineAdapter)?.onSnapshot = nil
        adapter.stop()
        adapter = WebKitBrowserPaneEngineAdapter(webView: webView)
    }
}
