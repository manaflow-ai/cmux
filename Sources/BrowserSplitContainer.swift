...
enum BrowserSplitContainer {
    ...
    func openBrowserToRight(
        ...
        if let targetPane = preferredRightSidePane(from: sourcePane) {
            return BrowserSplitPlacement(
                outcome: createBrowserSurface(
                    in: targetPane,
                    request: request
                ),
                createdSplit: false
    ...
    private func createBrowserSurface(
        ...
        case .dock(let dock):
            ...
            guard let panel = dock.browserPanel(for: panelID) else {
                return .failed
            }
    ...
    private func createBrowserSplit(
        ...
        switch self {
        case .workspace(let workspace):
            return workspace.requestNewBrowserSplit(
                from: sourcePanelID,
