...
The workflow runs from the base branch and reads the PR as data. It never checks out or executes pull-request code. Bot review coverage can be enabled for a repository once its providers expose reliable current-head review records by setting `REQUIRE_BOT_REVIEW_COVERAGE=1` and configuring `REVIEW_BOTS`.
