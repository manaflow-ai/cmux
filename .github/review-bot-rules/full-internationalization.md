...
- Web UI text, API response copy, user-facing web data, metadata, route copy, rendered markdown, changelog copy, or message keys that are not represented across all locales in `web/i18n/routing.ts` and every matching file in `web/messages/`.
...
- Web message, API response, rendered markdown, changelog, and user-facing data changes update all locale entries consistently and keep the locale registry aligned with available messages.
...
When reporting, identify the new or changed user-facing text, the missing localization API or locale entries, and the exact catalog or message files that need updates.
