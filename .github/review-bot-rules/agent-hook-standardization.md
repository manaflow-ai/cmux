...
- A diff adds or changes Feed bridge events without preserving the longer Feed timeout and either source/event routing through `cmux hooks feed --source <agent> --event <event>` for CLI-backed hooks or the existing direct plugin bridge contract such as OpenCode `feed.push` events.
