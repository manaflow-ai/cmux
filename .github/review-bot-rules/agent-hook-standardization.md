...
Apply this rule to cmux-owned agent hook installation, command generation, hook config writers, hook docs, and hook tests. In mixed files, apply it only to hook and Feed bridge hunks.
...
- A diff adds or changes Feed bridge events or `feed.push` socket-handler hunks without preserving the longer Feed timeout and either source/event routing through `cmux hooks feed --source <agent> --event <event>` for CLI-backed hooks or the existing direct plugin bridge contract such as OpenCode `feed.push` events.
