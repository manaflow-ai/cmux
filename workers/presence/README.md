...
`scripts/gen-control-plane-types.sh`, drift-guarded in CI). The account control
plane has no Vercel origin and does not use `CMUX_WEB_BASE_URL`.
...
For operator traces, set the Axiom dataset and token on the Worker. Account
Durable Objects emit one bounded request event and one alarm event. Events have
the request trace id, operation, path, status, and duration. They never include
tokens, request bodies, endpoint ids, or exception text.
...
Keep `observability.enabled = true` in both Wrangler configs. Cloudflare logs
remain the fallback when Axiom is unavailable. Telemetry failures are swallowed
and cannot fail a request or an alarm.
