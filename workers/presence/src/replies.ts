...
export function parsePhoneReply(body: Record<string, unknown>): ParsePhoneReplyResult {
  ...
  const macInstanceTag = body.macInstanceTag == null ? "" : boundedId(body.macInstanceTag, MAX_PHONE_REPLY_TARGET_ID_CHARS);
  if (macInstanceTag === null) return { ok: false, error: "invalid_mac_instance_tag" };
  const encryptedPayload = body.encryptedPayload;
  if (encryptedPayload === null || typeof encryptedPayload !== "object" || Array.isArray(encryptedPayload)) {
    return { ok: false, error: "missing_encrypted_payload" };
  }
  return {
    ok: true,
    reply: {
      ...
      macDeviceId,
      macInstanceTag,
      encryptedPayload: encryptedPayload as Record<string, unknown>,
    },
  };
}
