...
async function seedScopedBackupFromUnscopedIfNeeded(
  ...
  for (const stored of ordered) {
    ...
    await upsertRecord<PairedMacBackupRecord>(
      ...
      scopedCollection,
      stored.id,
      sanitizePairedMacRecord(stored.payload),
      nowMs,
      pairedMacShapeEqual,
...
function recordWithFreshUnscopedRoutes(
  ...
  if ((unscoped.lastSeenAt ?? 0) <= (scoped.lastSeenAt ?? 0)) {
    return sanitizePairedMacRecord(scoped);
  }
  return {
    ...sanitizePairedMacRecord(scoped),
    displayName: unscoped.displayName,
    routes: sanitizePublishedRoutes(unscoped.routes) ?? [],
