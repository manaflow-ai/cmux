...
export function verifyPairGrant(
  ...
  const header = decodeJson(encodedHeader, "invalid_pair_grant");
  ...
  const valid = verify(
    ...
    publicKey,
    decodeCanonicalBase64url(encodedSignature, 64, "invalid_pair_grant"),
  );
  if (!valid) throw new IrohForbiddenError({ code: "invalid_pair_grant_signature" });
  const claims = decodeJson(encodedClaims, "invalid_pair_grant") as unknown as PairGrantClaims;
...
export function verifyEndpointAttestation(
  ...
  const header = decodeJson(encodedHeader, "invalid_endpoint_attestation");
  ...
  const valid = verify(
    ...
    Buffer.from(`${encodedHeader}.${encodedClaims}`, "ascii"),
    publicKey,
    decodeCanonicalBase64url(encodedSignature, 64, "invalid_endpoint_attestation"),
  );
  if (!valid) {
