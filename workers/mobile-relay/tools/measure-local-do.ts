 ...
 *   RELAY_TICKET_SECRET  The secret configured on the target Worker.
...
import { encodeDataFrame } from "../src/protocol";
import { mintTicket } from "../src/ticket";
...
if (!secret || secret.length < 32) {
  throw new Error("RELAY_TICKET_SECRET must be set and contain at least 32 characters");
}

const userId = `latency-${crypto.randomUUID()}`;
