...
final class cmuxUITests: XCTestCase {
    ...
    private func assertExpansionTogglePreservesTranscriptPosition(
        ...
        let expectation = XCTNSPredicateExpectation(predicate: expanded, object: button)
        ...
        XCTAssertLessThanOrEqual(
            abs(after.distanceFromBottom - before.distanceFromBottom),
            120,
            "Tapping \(buttonID) must preserve the transcript's distance from the live tail instead of snapping to bottom. before=\(before) after=\(after)",
            file: file,
            line: line
        )
        XCTAssertGreaterThan(
