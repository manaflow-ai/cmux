...
/// iOS composition root for the dor transport (the account-scoped Durable
/// Object relay in `CmuxDorTransport`). Default ON: when enabled, cmuxApp
/// routes ALL `.iroh` traffic here and neither the irx rebuild nor the legacy
/// iroh runtime is configured. The phone dials its paired Mac through the
/// ACCOUNT's relay object on the presence worker origin, reusing every
/// existing pairing artifact (identity, pair grants, stored Macs/routes) with
/// zero re-pairing.
public actor MobileDorRuntimeComposition {
    public static let enabledDefaultsKey = "cmux.dor.enabled"

    /// dor is the PRIMARY transport: on by default in every configuration.
    ...
    public nonisolated static var isEnabled: Bool {
        if ProcessInfo.processInfo.environment["CMUX_DOR_ENABLED"] == "1" {
    ...
    public static let forceOnlyDefaultsKey = "cmux.dor.force-only"
    ...
    public nonisolated static var forceTransportOnly: Bool {
        if ProcessInfo.processInfo.environment["CMUX_DOR_FORCE_ONLY"] == "1" {
            return true
        }
    ...
    /// One journal for every dor component on the phone; the JSONL file lives
    ...
    nonisolated static let journal: DorJournal = {
        let logger = Logger(subsystem: "dev.cmux.ios", category: "dor-client")
        ...
        guard
            ...
            return DorJournal(write: mirror)
        }
        return DorJournal.file(
            url: documents.appendingPathComponent("dor-journal.jsonl"),
            mirror: mirror
        )
    }()

    /// The broker-reuse plumbing journals through the irx JSONL schema; a
    /// sibling file keeps `dor-journal.jsonl` single-writer.
    nonisolated static let brokerJournal: IrxJournal = {
        ...
        return IrxJournal(
            ...
            category: "dor-broker",
            journalFileURL: documents?.appendingPathComponent("dor-broker-journal.jsonl")
        )
    }()

    /// The stable factory cmuxApp registers for `.iroh` routes in dor mode.
    /// The kind name is legacy wire compat: stored routes and attach tickets
    /// carry the Mac's identity under `.iroh`, and dor resolves the Mac's
    /// device id (the dial input) from the cached grant for that identity.
    ...
    /// The relay lives on the SAME origin as the presence worker (the
    /// account's `AccountRelay` DO is hosted alongside `TeamPresence`),
    /// including the `CMUXPresenceBaseURL` Info.plist bake for per-dev
    /// isolated workers.
    private let relayBaseURL: URL?
    private let clientNamespace: String
    private let tag: String
    /// irx's per-bundle/per-broker state directory ON PURPOSE: the broker
    /// reuse below shares the binding/trust/grant disk caches the iroh stacks
    /// populated, so every pair grant already minted keeps working over dor
    ...
    private var enginesByPeer: [String: DorPeerEngine] = [:]
    ...
    private var engineBuilds: [String: Task<DorPeerEngine, any Error>] = [:]
    ...
    private func provisionOnce() async throws -> IrxBrokerService {
        ...
        // plane is the account relay DO, so no relay credentials, endpoint
        ...
        if cachedBinding == nil || cachedTrust == nil {
            ...
            // trust/discovery snapshot in the background; nothing here sits
            // on the dial path.
            Task {
                _ = try? await broker.register(pairingEnabled: false, relayURLHint: nil)
                _ = try? await broker.discover()
    ...
    private func buildEngine(peerHex: String) async throws -> DorPeerEngine {
        ...
        Self.journal.record(
            ...
            attributes: [
                ...
                "mac_device": String(macDeviceID.prefix(12)),
                "relay": relayBaseURL.host() ?? "-",
            ]
        )
        let leg = DorLegConfiguration(
        ...
        return DorPeerEngine(
            configuration: .init(
                leg: leg,
                identity: DorAdoptedIdentitySigner(identity: identity),
                admission: admission
            )
        )
    }
    ...
    /// phone: `acceptor.deviceID` names the Mac inside the account relay
    /// object, `acceptor.endpointID` pins the E2E handshake peer, and the
    /// persisted trust snapshot supplies the grant-verification keys. Claims
    /// are decoded here for ROUTING only; cryptographic verification stays
    /// inside the dor handshake.
    private func admissionMaterial(
        peerHex: String,
        broker: IrxBrokerService
    ) async throws -> (macDeviceID: String, admission: DorAdmissionMaterial) {
            ...
            let pump = Task {
                for await event in events {
                    ...
                    case let .inboundStream(stream):
                        guard stream.descriptor.lane == DorLaneDescriptor.events.lane
                        else {
    ...
    public func openArtifactLane(
        ...
        return DorArtifactLane(reader: DorBoundedStreamReader(stream: stream))
    }

    public func simulatorStreamLaneUnavailable() throws -> Never {
        // Simulator streaming is not served by dor v1; the viewer surfaces
    ...
    public func transport(
        ...
        let peerHex = try peerTarget(for: request)
        return DorControlByteTransport { [weak self] in
            guard let self else {
                throw CompositionError.notSignedIn
            }
    ...
    private func claimControlStream(
        ...
        // The phone INITIATES the control stream on first connect.
        return try await session.openStream(.control)
    }
}

extension MobileDorRuntimeComposition {
    /// The legacy-seam view of a dor lane, so the proven terminal envelope
    /// handlers run over dor unchanged.
    fileprivate static func bidirectional(
        _ stream: any DorStream
    ) -> CmxIrohBidirectionalStream {
        CmxIrohBidirectionalStream(
            receiveStream: DorReceiveStreamAdapter(
                reader: DorBoundedStreamReader(stream: stream)
            ),
            sendStream: DorSendStreamAdapter(stream: stream)
        )
    }
}

extension MobileDorRuntimeComposition: CmxIrohDeferredTransportProviding {}

/// Adapts the adopted legacy identity (the same Ed25519 keypair the iroh
/// stacks authenticate with) to the dor handshake's signing seam, so the key
/// a grant pins as `initiator.endpointID` is the key the Mac verifies.
private struct DorAdoptedIdentitySigner: DorIdentitySigning {
    let identity: IrxIdentity
...
/// The control stream as a `CmxByteTransport`: raw passthrough of the app's
/// MobileSyncFrameCodec frames. Releasing this transport closes the control
/// STREAM only, never the session — terminal, artifact, and event lanes ride
/// the same session and the peer engine stays the single owner of session
/// lifetime.
private actor DorControlByteTransport: CmxByteTransport {
    typealias Establish = @Sendable () async throws -> any DorStream

    private let establish: Establish
    private var stream: (any DorStream)?
    private var connectInFlight: Task<any DorStream, any Error>?
    ...
    private func establishedStream() async throws -> any DorStream {
        if let stream { return stream }
        if let connectInFlight {
            return try await connectInFlight.value
        }
        let task = Task<any DorStream, any Error> {
...
/// Rebounds `DorStream.read()` chunks to callers that read with an explicit
/// byte bound, carrying leftovers across reads so no byte is dropped or
/// duplicated.
private actor DorBoundedStreamReader {
    private let stream: any DorStream
    private var pending = Data()

    init(stream: any DorStream) {
        self.stream = stream
    }
...
private struct DorReceiveStreamAdapter: CmxIrohReceiveStream {
    let reader: DorBoundedStreamReader
...
private struct DorSendStreamAdapter: CmxIrohSendStream {
    let stream: any DorStream
...
/// Artifact lane over dor: bounded reads down, no upstream bytes.
private struct DorArtifactLane: MobileArtifactLaneConnection {
    let reader: DorBoundedStreamReader
