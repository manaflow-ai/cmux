...
struct GhosttySurfaceRepresentable: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, GhosttySurfaceViewDelegate {
        ...
        func ghosttySurfaceView(_ surfaceView: GhosttySurfaceView, didResize size: TerminalGridSize) {
            // Future: forward to Mac so it resizes its surface too. The
            // Mac is currently authoritative; we'll wire the round trip
            // once `terminal.size` events flow in both directions.
        }
    }
}
#endif
