...
命令面板的导航快捷键（包括 ⌃ P）同样可以自定义，也可以清除，以便按键能传递到当前活动的终端。
...
请在 [GitHub Issues](https://github.com/manaflow-ai/cmux/issues) 或 [Discord 的 #nightly-bugs](https://discord.gg/xsgFEVrWCZ) 中报告每夜版的 Bug。
...
当 hooks 保存了原生会话 ID 时，受支持的 agent 会话可以恢复。请在安装好 agent 的 CLI 之后再安装 hooks，以确保其二进制文件已在 `PATH` 中：
...
`cmux hooks setup` 会安装它能找到的受支持 agent，并为被跳过的 agent 打印一份摘要。受支持的恢复集成包括 Claude Code、Codex、Grok、OpenCode、Pi、Amp、Cursor CLI、Gemini、Rovo Dev、Copilot、CodeBuddy、Factory 和 Qoder。当在设置中启用 Claude 集成时，Claude Code 由 cmux 的 Claude 包装器处理。
...
这个绑定会继续关联到 cmux surface。通过公开 CLI 或 socket 创建的绑定会被保存，用于检查和手动恢复，除非你为自动恢复批准了一个签名的命令前缀。已批准的前缀在存在相应信息时，还会绑定到工作目录和确切的环境变量值。可在 **设置 > 终端 > 恢复命令** 中查看或编辑这些批准。cmux 只会自动运行它标记为可信的恢复绑定，例如从运行中进程检测到的 tmux 绑定，或用户批准的前缀。令牌、密码、密钥和 API key 等敏感环境变量键会在保存恢复绑定前被丢弃。

若要让恢复的 agent 终端保持空闲、而不自动运行其恢复命令，请关闭 **设置 > 终端 > 重新打开时恢复 Agent 会话**，或在 `~/.config/cmux/cmux.json` 中设置：
...
这只会禁用 agent 的自动恢复命令。cmux 仍会恢复已保存的布局、工作目录、回滚缓冲区和浏览器历史记录。

如果你需要手动重新应用上一次保存的快照，可使用：
- `文件 > 重新打开上一个会话`
- `⌘ ⇧ O`
- `cmux restore-session`

在底层，cmux 会在 `~/Library/Application Support/cmux/` 下写入带版本的快照，agent hooks 则在 `~/.cmuxterm/` 下写入会话映射。恢复时，cmux 会先重建布局，然后在启用 agent 自动恢复的情况下运行受支持 agent 的原生恢复命令。

完整指南请阅读 <https://cmux.com/docs/session-restore>。
