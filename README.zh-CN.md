...
命令面板导航快捷键（包括 ⌃ P）同样支持自定义，可以清空，让按键透传到当前活动终端。
...
如遇 nightly 版本的问题，请在 [GitHub Issues](https://github.com/manaflow-ai/cmux/issues) 或 Discord 的 [#nightly-bugs 频道](https://discord.gg/xsgFEVrWCZ) 反馈。
...
当 hooks 保存了原生会话 ID 时，受支持的 agent 会话可以恢复。请在安装好 agent CLI 之后再安装 hooks，以保证对应可执行文件已经在 `PATH` 中：
...
`cmux hooks setup` 会安装所有能够识别到的受支持 agent，并对跳过的 agent 打印说明。当前支持的恢复集成包括 Claude Code、Codex、Grok、OpenCode、Pi、Amp、Cursor CLI、Gemini、Rovo Dev、Copilot、CodeBuddy、Factory 和 Qoder。当在设置中启用了 Claude 集成时，Claude Code 由 cmux 自带的 Claude 包装器接管。
...
绑定会持续关联到 cmux surface。通过公开 CLI 或 socket 创建的绑定默认只用于查看和手动恢复，除非你为命令前缀做了签名授权，才会自动执行。已批准的前缀同时会绑定到当时的工作目录和环境变量值（若存在）。可在**设置 > 终端 > 恢复命令**中查看或修改授权。cmux 只会自动执行被标记为可信的恢复绑定，例如从运行中进程检测到的 tmux 绑定，或用户手动批准的前缀。令牌、密码、密钥和 API key 等敏感环境变量键会在保存恢复绑定前被丢弃。

如果希望恢复后的 agent 终端保持空闲，而不是自动执行恢复命令，可以关闭**设置 > 终端 > 重新打开时恢复 Agent 会话**，或在 `~/.config/cmux/cmux.json` 中配置：
...
这只会禁用自动恢复 agent 命令的行为。cmux 仍然会恢复保存的布局、工作目录、回滚缓冲区和浏览器历史。

如果需要手动重新加载上一次保存的快照，可使用：
- `文件 > 重新打开上一次会话`
- `⌘ ⇧ O`
- `cmux restore-session`

实现层面，cmux 把版本化的快照写入 `~/Library/Application Support/cmux/`，agent hooks 把会话映射写入 `~/.cmuxterm/`。恢复时，cmux 会先重建布局，然后在启用自动 agent 恢复的情况下，运行受支持 agent 的原生恢复命令。
