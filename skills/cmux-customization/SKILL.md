...
| Custom actions, workspace layouts/commands, tab bar buttons, plus-button behavior, Command Palette entries, terminal right-click menu entries (`terminalContextMenu: true`; shift+right-click opens the menu while a TUI captures the mouse, gated by `terminal.shiftRightClickShowsMenu`), notification hooks | `~/.config/cmux/cmux.json` globally or `.cmux/cmux.json` in the project |
