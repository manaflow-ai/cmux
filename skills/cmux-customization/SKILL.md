...
- New workspace button: set `ui.newWorkspace.action` to replace the normal plus-button click, `ui.newWorkspace.contextMenu` to control right-click actions, and `ui.newWorkspace.showLayoutManagement` to show or hide the built-in layout controls. `ui.newWorkspace.rightClick` is accepted as an alias, but new examples should use `contextMenu`.
