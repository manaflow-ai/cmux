...
- Project Worktrees sidebar: set `ui.projectWorktrees.createAction` and/or `ui.projectWorktrees.openAction` to replace the bundled preset's creation or opening flow. Omit either field to keep its built-in behavior; project-local values override global values independently.
