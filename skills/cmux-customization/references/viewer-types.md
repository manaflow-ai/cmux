...
| `markdown` | Default prose size, prose font, and reading-column width | `markdown.fontSize`, `markdown.fontFamily`, `markdown.maxWidth` | `cmux-settings validate`; `cmux reload-config` |
...
| `filePreview` | Whether a file opens in cmux, where double-click routes, and text-editor rendering | `app.openSupportedFilesInCmux`, `app.preferredEditor`, `fileExplorer.doubleClickAction`, and `fileEditor.*`; generic Quick Look media has no additional cmux template knobs | `cmux-settings validate`; `cmux reload-config` |
