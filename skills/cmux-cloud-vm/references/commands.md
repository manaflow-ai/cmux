...
cmux vm tree <id> --refresh            # one machine (`local` for This Mac), re-synced first; omit the id to refresh the fleet too
...
cmux vpn down                          # take this build's tunnel down (sudo)
cmux vm tree                           # the surface catalog: This Mac (terminals by workspace, browsers), then every machine → Workspaces, Ports, VNC Displays, Terminals
cmux vm tree <id> --refresh            # one machine (`local` for This Mac), re-synced first; omit the id to refresh the fleet too (fleet + provider refresh)
