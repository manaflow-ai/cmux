...
Base is the one pinned persistent machine per user. `open` (`vm.base_open`) reuses the same VM every time, creating it on first use with the chosen kind (shell-only unless `--desktop`, like `vm new`); an existing Base keeps its image. `reset` (`vm.base_reset`) mints a new Base generation and retains the previous VM so an accidental reset is recoverable. Both open a plain terminal unless `--detach`; text `OK <id>`, `--json` the payload. Sidebar: Open Base / Set Up Base sheet.
...
- **#11609** (`freestyle-vm-primitives`) is the big one — everything below is on that branch and none of it is runnable here yet:
  ...
  - **Port previews go live**: the first `openPort` driver implementation — a TLS edge on tokened random `style.dev` subdomains, advertised as `capabilities.ports` in `vm ls --json`, revoked on sign-out. The dormant `vm open <id> <port>` / `:port/<n>` forms above start working when this lands.
  ...
  - A bare `vm new` (and `vm base open` first-create) flips to **desktop-by-default** — `--base`/`--no-desktop` become the way to ask for shell-only. docs/cli-contract.md already describes the flipped default; this branch's CLI still defaults to shell-only until that lands.
