...
- **#11609** (`freestyle-vm-primitives`) adds `cmux vm link <src> <dst>` — grant machine `<src>` a cmux-remote link to `<dst>` so the in-VM `cmux` on `<src>` can drive `<dst>` directly (exec/tree/terminals; brokered by the Mac with a single-use enrollment invitation, no control-plane credential in any VM) — plus an in-VM `cmux vm connect <dst>` counterpart, `capabilities.attach_transports` in `vm ls --json`, and a TLS edge for **port previews** that brings the dormant open-port path live (`ports: true` capability, tokened `style.dev` subdomains, revoked on sign-out).
