...
Workspace row › **Go to Workspace** (the open verb's label once the workspace is showing locally), click, Return | `cmux workspace select <local-id>` (the local workspace from `vm tree --json` projections) | `workspace.select` | ✅ live — one open verb; never opens a second copy
Workspace row › **Open Workspace** (not open yet), click, Return | `cmux vm workspace open <m> <ws>` (also `cmux vm open <m>/<ws>`) — opens as its own local workspace | `vm.workspace_open` | ✅ live — an empty workspace opens nothing: the row is inert, the CLI answers `opened=0` (D9)
...
Workspace row › **Close Workspace…**, hover × (confirms when it has terminals) | `cmux vm workspace rm <m> <ws>` | `vm.workspace_delete` | ✅ live — same `CloudTreeNodeActions.deleteWorkspaceAndTerminals`: kills every terminal viewed there (their local panes close), then closes it; the CLI prints how many
(no menu verb — CLI only) | `cmux vm workspace close <m> <ws>` | `vm.workspace_close` | ✅ live — the protocol's keep-terminals close: they keep running in the Terminals pool (only `terminal close` kills)
