...
--- | --- | --- | ---
**Machines panel ＋ / palette "New Cloud Machine…"** (name, Desktop/Base, size; Create closes the sheet and the create runs in the background) | `cmux vm new [--desktop\|--base] [--size 20g] [--name <label>] [--focus false] [--detach] [--json]` — the sheet passes `--focus false` | `vm.create` | ✅ the pending row in the tree ("Creating…", then failure with Retry / Show Error / Dismiss) is `MachineCreateCoordinator`; `--detach` is the agent's way to get the same non-blocking outcome without a workspace
