...
`viewport` width and height must be integer CSS pixels between 0 and 100000 and
within the current pane's emulation range. `0 0` clears emulation. If the
browser pane has not been laid out yet, retry after a `not_ready` response.
Successful calls return `handled: true`; `changed` reports whether the stored
viewport state or magnification changed.
