...
The paid Cloud VM allowance and shared resource pool (up to 50 machines per billing team, with 20 GB / 5 vCPU / 200 GB disk shared across those machines) live in `web/services/vms/machineSpec.ts`; `web/tests/pro-pricing.test.ts` pins the pricing copy to those constants.
