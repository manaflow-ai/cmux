...
- `/dashboard/admin` (nav group `admin`) and `/api/admin/users` let verified `cmux.com`, `manaflow.ai`, and `manaflow.com` accounts search users and grant, change, or remove the `cmuxVmPlan` override. `services/admin/access.ts` is the gate, `services/admin/proGrants.ts` writes under the account-mutation lease and records `serverMetadata.cmuxAdminPlanGrant` (who, when, which plan). Non-admins get 404 on the page and 403 on the API.
