...
| Key | Type | Default | Description |
|---|---|---|---|
| `workspaceColors.indicatorStyle` | `"solidFill"` or `"leftRail"` or `"leftRailAuto"` or `"rail"` or `"border"` or `"wash"` or `"lift"` or `"typography"` or `"washRail"` or `"blueWashColorRail"` | `"leftRail"` | Workspace color presentation: solidFill colors the whole row, leftRail shows only colors chosen manually, and leftRailAuto gives every uncolored workspace its own persisted palette color on the rail. Auto assignments stay distinct until the palette runs out, and a color chosen manually always wins. Automatic assignment needs at least one usable palette color, so an empty workspaceColors.colors leaves every automatic rail blank. Legacy aliases are accepted and normalized. |
