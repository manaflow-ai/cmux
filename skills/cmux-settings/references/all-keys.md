...
| `workspaceColors.autoAssignColors` | boolean | `false` | Give every workspace without an explicit color its own color from the palette, drawn on the left rail. No two workspaces share a color until the palette runs out, after which the least-used color is reused. Assignments are saved, so a workspace keeps its color across restarts, and a color you set by hand always wins. Ignored unless indicatorStyle is leftRail, because solidFill colors the whole row and would compete with the selected-workspace highlight. |
