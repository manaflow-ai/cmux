...
`shutdown_cleanup` reports unresolved shutdown work, whether a retry is active, and whether cleanup requires another explicit stop request. Its pending count includes terminal owners, unpublished child-reaper reservations, and ownerless post-fence work such as durable registry or browser-runtime cleanup. It is an additive protocol 11 field, so clients must accept its absence from older protocol 11 servers.
