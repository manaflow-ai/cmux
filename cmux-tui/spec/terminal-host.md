...
`FLAG_TERMINATE_ONLY` is bit 4 in protocol v4 and is valid only as the sole
flag in `ClientHello` and `HostHello`. The host grants only termination and
does not start a renderer stream. A version-2 record that advertises
`supports_terminate_only` uses legacy bit 2. A version-3 record that advertises
the capability uses legacy bit 3. Clients reject records that do not advertise
termination support. Version-4 hosts never accept legacy bit 2 or bit 3 as
termination authority, and old clients reject version-4 records before flag
interpretation.
