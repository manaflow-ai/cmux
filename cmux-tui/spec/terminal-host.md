...
`FLAG_TERMINATE_ONLY` is bit 3 and is valid only as the sole flag in
`ClientHello` and `HostHello`. The host grants only the termination request and
does not start a renderer stream. A version-2 host record that advertises
`supports_terminate_only` uses the legacy bit-2 termination flag. A version-3
record that advertises the capability uses bit 3. Clients reject records that
do not advertise termination support. A version-3 host always interprets bit 2
as `FLAG_SMART_RENDERER`, never as termination authority.
