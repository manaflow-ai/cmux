...
| `report-agent` | implemented; telemetry protocol 12 | `--surface <id> --state <state> --source socket\|hook` | `--root-session`, `--session <id>`, `--label <label>`, `--detail <detail>`, `--started-at-ms <n>`, `--tasks-completed <n>`, `--tasks-total <n>`, `--jobs-running <n>`, `--agents-active <n>` | none |
