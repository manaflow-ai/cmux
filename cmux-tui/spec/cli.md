...
| `report-agent` | implemented; telemetry protocol 12 | `--surface <id> --state <state> --source socket\|hook` | `--session <id>`, `--label`, `--detail`, `--started-at-ms`, `--tasks-completed`, `--tasks-total`, `--jobs-running`, `--agents-active` | none |
