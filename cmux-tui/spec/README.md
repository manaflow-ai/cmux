...
Protocol v7 is the implemented baseline. It adds stable ids to canonical split nodes and exact split-ratio mutation while preserving the protocol-v5 `set-ratio` command. Proposed additions in this directory target the next minor protocol unless a later spec says otherwise.
