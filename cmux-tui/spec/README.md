...
Protocol v9 adds stack layout nodes and `new-pane`. Clients must negotiate v9 before decoding a stack node or sending `new-pane`. Protocol-v9 clients may use `clear-history` only when `clear-history-v1` is advertised; its structured fallback additionally requires `clear-history-key-v1`.

Protocol v10 scopes client-sizing participation to one terminal surface. Protocol v11 preserves the additive viewport/layout capabilities. Protocol v12 is the implemented baseline and additively introduces optional agent telemetry fields, the `error` agent state, `agent-state-changed`, and optional notification subtitles. Missing v12 fields decode as null/absent, preserving older producers.
