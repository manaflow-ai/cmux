...
All bindings must expose the implemented protocol v7 commands, events, transports, stable split ids, and `set-split-ratio` API. APIs newer than the connected server must be guarded by explicit version checks or feature gates.
