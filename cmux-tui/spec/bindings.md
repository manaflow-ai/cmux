...
The protected branch used a second private terminal API. Current resource
operations own those semantics. The old commands and generated SDK layouts are
not restored.

| Protected semantic group | Canonical owner |
| --- | --- |
| Topology snapshot, ordered replay, and explicit gap recovery | `session.snapshot` and `session.events` |
| Stable frontend window projection and optimistic replacement | `frontend_projection.get` and `frontend_projection.put` |
| Idempotent terminal creation and recovery correlation | `tab.create_terminal` with its request idempotency key and `correlation_key` |
| Terminal placement and reparenting | `terminal.move` and `terminal.project` |
| Text, byte, key, mouse, and focus input | `terminal.input.write`, `terminal.input.keys`, `terminal.input.mouse`, and `terminal.input.focus` |
| Connection-owned geometry and release | `terminal.viewer.resize` and `terminal.viewer.release` |
| Byte or render attachment and initial state | `terminal.attach` |
| Renderer authorization and expiry | `terminal.renderer_grant.create` |
| Screen, history, process, selection, copy, and viewport reads | `terminal.screen.read`, `terminal.history.read`, `terminal.process.get`, `terminal.state.read`, and `terminal.copy` |
| Compatibility viewport mutation | `terminal.viewport.scroll` |
| Terminal activity and reconnect delivery | `notification.list` and `session.events` |
| Configuration reload and presentation defaults | `session.reload_config` and `session.terminal_defaults.update` |
| Durable session identity and recovery | `session.creation.resolve`, `session.open`, `session.snapshot`, and `session.events` |
