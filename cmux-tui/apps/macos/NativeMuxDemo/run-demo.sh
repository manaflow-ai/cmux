...
if [[ "$REUSE_BUILD" != "1" ]]; then
  ...
  echo "Preparing the pinned GhosttyKit.xcframework..."
  PATH="$(dirname "$ZIG"):$PATH" "$REPO_ROOT/scripts/ensure-ghosttykit.sh"
fi
...
if [[ "$LAUNCH_GHOSTTY" == "1" ]]; then
  ...
  "$SCRIPT_DIR/launch-ghostty-client.sh" "$GHOSTTY_APP" "$CMUX_TUI" \
