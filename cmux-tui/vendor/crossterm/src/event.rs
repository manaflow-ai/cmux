...
bitflags! {
    ...
    pub struct KeyboardEnhancementFlags: u8 {
        ...
        /// in addition to the active-layout keycode. Reported values are preserved in
        /// [`EnhancedKeyEvent`] without replacing the active-layout key.
        const REPORT_ALTERNATE_KEYS = 0b0000_0100;
        /// Represent all keyboard events as CSI-u sequences. This is required to get repeat/release
        /// events for plain-text keys.
        const REPORT_ALL_KEYS_AS_ESCAPE_CODES = 0b0000_1000;
        /// Send the text generated by the key event as Unicode codepoints.
...
pub enum Event {
    ...
    Key(KeyEvent),
    /// A Kitty keyboard event whose layout and generated-text fields were reported separately.
    EnhancedKey(EnhancedKeyEvent),
...
impl Event {
    ...
    pub fn is_key_press(&self) -> bool {
        matches!(self.as_key_event(), Some(KeyEvent { kind: KeyEventKind::Press, .. }))
    }

    /// Returns `true` if the event is a key release event.
    #[inline]
    pub fn is_key_release(&self) -> bool {
        matches!(self.as_key_event(), Some(KeyEvent { kind: KeyEventKind::Release, .. }))
    }

    /// Returns `true` if the event is a key repeat event.
    #[inline]
    pub fn is_key_repeat(&self) -> bool {
        matches!(self.as_key_event(), Some(KeyEvent { kind: KeyEventKind::Repeat, .. }))
    }
    ...
    pub fn as_key_event(&self) -> Option<KeyEvent> {
        match self {
            ...
            Event::EnhancedKey(event) => Some(event.key_event),
            _ => None,
        }
    }

    /// Returns the lossless Kitty keyboard event when alternate key or text data was reported.
    #[inline]
    pub fn as_enhanced_key_event(&self) -> Option<&EnhancedKeyEvent> {
        match self {
            Event::EnhancedKey(event) => Some(event),
            _ => None,
        }
    }
    ...
    pub fn as_key_press_event(&self) -> Option<KeyEvent> {
        match self {
            Event::Key(event) if event.kind == KeyEventKind::Press => Some(*event),
            Event::EnhancedKey(event) if event.key_event.kind == KeyEventKind::Press => {
                Some(event.key_event)
            }
            _ => None,
        }
    }
    ...
    pub fn as_key_release_event(&self) -> Option<KeyEvent> {
        match self {
            ...
            Event::EnhancedKey(event) if event.key_event.kind == KeyEventKind::Release => {
                Some(event.key_event)
            }
            _ => None,
        }
    ...
    pub fn as_key_repeat_event(&self) -> Option<KeyEvent> {
        match self {
            ...
            Event::EnhancedKey(event) if event.key_event.kind == KeyEventKind::Repeat => {
                Some(event.key_event)
            }
            _ => None,
        }
...
/// Lossless fields from a Kitty CSI-u key event.
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, PartialOrd, PartialEq, Eq, Clone, Hash)]
pub struct EnhancedKeyEvent {
    /// The unshifted key and modifier state from the active keyboard layout.
    pub key_event: KeyEvent,
    /// The shifted character from the active keyboard layout, when reported.
    pub shifted_key: Option<char>,
    /// The key at the same physical position in the standard PC-101 layout, when reported.
    pub base_layout_key: Option<char>,
    /// Text generated by the key event, including composed multi-codepoint text.
    pub text: String,
}
...
impl KeyEvent {
    ...
    pub const fn new_with_kind(
        ...
        KeyEvent { code, modifiers, kind, state: KeyEventState::empty() }
    }
...
impl PartialEq for KeyEvent {
    fn eq(&self, other: &KeyEvent) -> bool {
        let KeyEvent { code: lhs_code, modifiers: lhs_modifiers, kind: lhs_kind, state: lhs_state } =
            self.normalize_case();
        let KeyEvent { code: rhs_code, modifiers: rhs_modifiers, kind: rhs_kind, state: rhs_state } =
