...
use crate::event::{
    EnhancedKeyEvent, Event, KeyCode, KeyEvent, KeyEventKind, KeyEventState, KeyModifiers,
...
pub(crate) fn parse_csi_u_encoded_key_code(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let codepoint = codepoints
        ...
        .parse::<u32>()
        .map_err(|_| could_not_parse_event_error())?;
    let parse_optional_codepoint = |value: Option<&str>| -> io::Result<Option<char>> {
    ...
    let shifted_key = parse_optional_codepoint(codepoints.next())?;
    let base_layout_key = parse_optional_codepoint(codepoints.next())?;
...
pub(crate) fn parse_csi_special_key_code(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let input_event =
        Event::Key(KeyEvent::new_with_kind_and_state(keycode, modifiers, kind, state));
...
pub(crate) fn parse_csi_rxvt_mouse(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let cb =
        next_parsed::<u8>(&mut split)?.checked_sub(32).ok_or_else(could_not_parse_event_error)?;
...
mod tests {
    ...
    #[test]
    fn test_parse_csi_u_preserves_layout_and_associated_text() {
        assert_eq!(
            parse_csi_u_encoded_key_code(b"\x1B[38:49:49;4;49u").unwrap(),
