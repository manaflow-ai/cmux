...
class ControlClient:
    ...
    def request(self, method: str, params: dict, timeout: float = 120.0) -> dict:
        ...
        while time.monotonic() < deadline:
            ...
            if not line:
                raise RuntimeError(f"control process closed during {method}")
...
class SessionStream:
    ...
    def __init__(self, argv: list[str], token: str, ticket: str):
        ...
        if not result_line:
            ...
            raise RuntimeError(f"stream closed during handshake: {stderr.strip()}")
    ...
    def command(self, cmd: str, timeout: float = 60.0, **params) -> dict:
        ...
        while time.monotonic() < deadline:
            line = self.process.stdout.readline()
            if not line:
                raise RuntimeError(f"stream closed during {cmd}")
            message = json.loads(line)
...
def main() -> int:
    ...
    try:
        ...
        log(
            ...
            + ", ".join(f"{m['display_name']} ({m['subtitle']})" for m in machines)
        )
        target = next(
            (
                m
                for m in machines
                if args.machine_tag in m["display_name"] or args.machine_tag in m.get("subtitle", "")
            ),
            None,
        )
        if target is None:
            log(f"FAIL: no machine matching tag {args.machine_tag!r} in account snapshot")
            return 1
        ...
        if args.send_command:
            log(f"send-command not wired to a surface in this harness; skipping run of {args.send_command!r}")
