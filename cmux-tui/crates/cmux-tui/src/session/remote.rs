...
impl RemoteSession {
    ...
    fn shutdown_error(&self) -> RemoteRequestError {
        match &*self.disconnect_state.lock().unwrap() {
            DisconnectState::ExpectedRemoteShutdown => RemoteRequestError::DaemonShutdown,
            DisconnectState::Remote(_) => RemoteRequestError::RemoteShutdown,
            DisconnectState::Active | DisconnectState::LocalShutdown => {
