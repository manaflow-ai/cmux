...
use cmux_tui_core::server::{
    CLEAR_HISTORY_CAPABILITY, LAYOUT_UNDO_CAPABILITY, PER_SURFACE_CLIENT_SIZING_PROTOCOL_VERSION,
    PROTOCOL_VERSION, VIEWPORT_COLUMN_RESIZE_CAPABILITY, VIEWPORT_SPLITS_CAPABILITY,
};
...
impl HelpText {
    fn resolve(self, catalog: &'static Catalog) -> &'static str {
        match self {
            ...
            Self::Server => catalog.server.help,
...
fn run_command(args: CliArgs) -> i32 {
    ...
    if let Some((capability, unsupported_message)) = required_capability(&request)
        && !probe.as_ref().is_some_and(|probe| probe.identity.supports(capability))
    {
...
mod tests {
    ...
    fn capability_probe_does_not_expose_server_error_text() {
        ...
        assert_eq!(error, crate::localization::catalog().server.identity_failed);
