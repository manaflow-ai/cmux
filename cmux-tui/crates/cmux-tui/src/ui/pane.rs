...
pub fn draw_all(app: &mut App, frame: &mut Frame) -> DrawCursors {
    ...
    app.rendered_terminal_pointer_states.retain(|surface, _| visible_surfaces.contains(surface));
...
fn draw_content(app: &mut App, frame: &mut Frame, area: &PaneArea, focused: bool) -> DrawCursors {
    ...
    app.rendered_terminal_pointer_states.remove(&area.surface);
    ...
    app.rendered_terminal_pointer_states
        .insert(area.surface, (render.mouse_tracking, render.active_screen));
