...
use cmux_tui_core::{
    ...
    SurfaceId, SurfaceKind, TerminalPointerSnapshot, WorkspaceId, ZoomMode,
...
impl RenderedPointerFrame {
    fn route_for_mouse(&self, mouse: &MouseEvent) -> PointerRouteIdentity {
        ...
        for pane in self.panes.iter() {
            ...
            if let Some(hit) = crate::ui::omnibar::hit(rect, x, y, editing) {
...
impl App {
    ...
    fn graphic_placements(&self) -> Vec<GraphicPlacement> {
        ...
        for area in &self.pane_areas {
            ...
            // Clear before reading the frame. An update after this point sets
    ...
    fn advance_pointer_focus_generation(&mut self) {
        ...
        self.deferred_input.retain(|input| !matches!(&input.event, Event::Mouse(_)));
        self.pending_pointer_motion = None;
        self.active_pointer_buttons.clear();
        self.ignored_pty_mouse_buttons.clear();
    }
...
mod tests {
    use super::{
        ...
        SurfaceResizeOwnership, TerminalPointerAdmission, TerminalPointerAdmissionResult,
        TerminalPointerEncoding, WorkspaceRailSelection, action_available_in_mode,
        ...
        sidebar_plugin_status_settles_passive_claim, start_ordered_session, thumb_geometry,
    ...
    use cmux_tui_core::{
        BrowserStatus, Direction, Mux, MuxEvent, Node, PointerSnapshotProbe, Rect, SplitDir,
        SurfaceId, SurfaceKind, SurfaceOptions, ZoomMode, layout_screen,
    };
    use crossterm::event::{
    ...
    fn context_menus_scope_pane_and_sidebar_actions_to_the_clicked_region() {
        ...
        app.hits.push((
            ...
            super::Hit::ScreenEntry { index: 0, id: screen },
        ));
    ...
    fn replayed_discrete_pointer_requeue_keeps_its_original_order() {
        ...
        app.handle_replayed_input(Event::Mouse(pointer), 1, None).unwrap();
