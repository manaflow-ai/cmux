...
fn process_event(
    ...
    let timed_out_surface_lane =
        remote_timed_out && ordering_lane.is_some() && !retry_ambiguous_input;
    let failure = result.err().and_then(|error| {
        (!suppress_mutation_timeout && !retry_ambiguous_input).then(|| PtyOperationFailure {
            ...
            error: if exhausted_ambiguous_input {
                format!(
                    "terminal request timed out after {REMOTE_RELEASE_MAX_ATTEMPTS} attempts; detach and reconnect before sending more input"
                )
            } else {
                error.to_string()
            },
            lane_failed: remote_transport_failed
                || exhausted_ambiguous_input
    ...
    if remote_transport_failed || (exhausted_ambiguous_input && !retired_lane) {
        ...
        if !retired_lane && retry_ambiguous_input && (!state.closed || state.shutdown_release_drain)
        {
            requeue_ambiguous_input(&mut state, event);
        }
        if let Some(lane) = ordering_lane {
            if !retired_lane {
                if !retry_ambiguous_input && state.active_session_generation == session_generation {
