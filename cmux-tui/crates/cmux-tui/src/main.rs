...
/// The Cloud image exposes one Rust executable under three command names:
/// `cmux`, `coderouter`, and `cr`.  Keep the CodeRouter product in its
/// official binary, but make this binary the only front door.  This avoids a
/// second parser in the guest while preserving the complete CodeRouter CLI.
///
/// The dispatcher is intentionally argv based instead of environment based:
/// a normal `cmux-tui` daemon invocation must never unexpectedly launch an
/// agent CLI merely because a model-plane variable exists in its environment.
fn unified_cli_dispatch() -> Option<i32> {
    #[cfg(unix)]
    ...
    let program_name = Path::new(&program)
        ...
        .unwrap_or_default();
    let mut forwarded = args.collect::<Vec<_>>();
    let routed = match program_name {
        "coderouter" | "cr" => true,
        "cmux" => matches!(forwarded.first().and_then(|value| value.to_str()), Some("coderouter" | "cr")),
        _ => matches!(forwarded.first().and_then(|value| value.to_str()), Some("coderouter" | "cr")),
    };
    if !routed {
        return None;
    }

    let invoked_as_front_door = matches!(program_name, "cmux" | "cmux-tui");
    ...
    if machine_mode {
        ...
        // The official CLI uses this mode to load a transient, machine-bound
        // route config. It never reads or writes a Stack session in the VM.
        // SAFETY: this process has not started the daemon or spawned a child.
        unsafe { std::env::set_var("CMUX_CODEROUTER_MACHINE", "1") };
    }
    ...
    let Some(executable) = executable else {
        eprintln!("coderouter: the official CodeRouter binary is not installed");
        eprintln!("Install coderouter from npm, then retry.");
        return Some(127);
    };
    ...
    #[cfg(unix)]
    {
        let error = std::process::Command::new(&executable)
            .args(external_args)
            .exec();
        eprintln!("coderouter: could not start {}: {error}", executable.display());
        Some(127)
    }
    #[cfg(not(unix))]
    {
        match std::process::Command::new(&executable).args(external_args).status() {
    ...
    for directory in std::env::split_paths(&path) {
        for name in ["coderouter", "cr"] {
            ...
            if current.as_ref().is_some_and(|current| {
                candidate.canonicalize().ok().as_ref() == Some(current)
            }) {
                continue;
            }
            // Do not resolve the alias we are already running through when a
            // user invokes the binary as `cr` or `coderouter`.
...
fn main() {
    if let Some(exit_code) = unified_cli_dispatch() {
