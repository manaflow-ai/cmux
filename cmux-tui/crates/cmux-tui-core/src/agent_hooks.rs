...
/// Reserved for journal events generated by the direct socket report path.
/// External hook adapters cannot use this id, so their events cannot pose as
...
fn validate_agent_source(source: &str) -> anyhow::Result<()> {
    anyhow::ensure!(
        ...
        "agent source must contain 1 to {MAX_AGENT_SOURCE_BYTES} lowercase ASCII letters, digits, hyphens, or underscores and must not use a reserved id"
    );
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn socket_adapter_id_is_reserved_for_internal_echoes() {
