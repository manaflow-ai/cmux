...
impl TerminalId {
    /// Protocol-v5 sentinel used by capability-authenticated renderer clients
    /// that do not possess the terminal host's private process identity. A
    /// successful handshake returns the concrete identity in `HostHello`.
