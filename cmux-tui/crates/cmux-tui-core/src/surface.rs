...
pub struct PtyTerminalRuntime {
    ...
    /// Explicit lifecycle authority for this process.
    ...
    fn spawn_with_terminal_id_and_resource_identity_at_cell_pixels(
        ...
        let surface = Arc::new(Surface::Pty(PtySurface {
            ...
            terminal: Arc::new(PtyTerminalRuntime {
                ...
                runtime: Mutex::new(PtyRuntime::Local { writer, master: Some(master) }),
        ...
        let reader_thread = spawn_local_pty_reader(id, {
            ...
            move || {
                ...
                loop {
                    ...
                    // Reserve the capture epoch before read(2). Shutdown can
                    // revoke a blocked reservation, but it cannot place its
                    // final barrier in the read-to-parser gap.
