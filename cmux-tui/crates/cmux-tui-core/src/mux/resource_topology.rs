...
use crate::workspace_registry::{
    ...
    RegistryWorkspace, ResourceCreationPreparation, ResourceCreationRecovery, ResourcePatchCommit,
...
struct ResourceCloseInputs {
    ...
    terminal_public_ids: Vec<TerminalPublicId>,
...
impl Mux {
    ...
    fn terminal_resource_effect_projection_locked(
        ...
        let mut removed_screens = changed_screens
            ...
            .collect::<HashSet<_>>();
        for screen_slot in
            changed_screens.iter().filter(|screen| projected_screen_ids.contains(screen))
        {
            let screen_id =
                live.resource_indexes.screen_ids.get(screen_slot).with_context(|| {
                    format!("terminal scope screen {screen_slot} has no identity")
                })?;
        ...
        for screen_slot in &affected_screens {
            if !projected_screen_ids.contains(screen_slot) || removed_screens.contains(screen_slot)
            {
                continue;
            }
        ...
        for screen_slot in &published_screens {
            ...
            let mut durable =
                durable_screens.get(&screen.public_id).cloned().with_context(|| {
                    format!("terminal scope screen {} is not durable", screen.public_id)
                })?;
            if changed_screen_set.contains(screen_slot) {
                durable = remove_public_panes_from_registry_screen(durable, &removed_public_panes)
                    .with_context(|| {
                        format!("terminal scope screen {} lost its public layout", screen.public_id)
                    })?;
    ...
    pub(super) fn commit_legacy_terminal_close(
        ...
        for cleanup_public_id in &cleanup_public_ids {
            let cleanup_host = registry.terminal_host_id(cleanup_public_id)?.ok_or_else(|| {
                terminal_close_state_error(format!(
                    "terminal {cleanup_public_id} has no durable host"
                ))
            })?;
            anyhow::ensure!(
                cleanup_host == terminal_id,
                "terminal resource changed hosts before close"
            );
        }
        ...
        for cleanup_public_id in &cleanup_public_ids {
            if !changes.iter().any(|change| {
                change["kind"] == "delete"
                    ...
                    && change["id"].as_str() == Some(cleanup_public_id.as_str())
            }) {
                changes.push(json!({
                    "kind":"delete",
                    "sequence":changes.len(),
                    "resource":"terminal",
                    "id":cleanup_public_id,
                }));
            }
        }
        #[cfg(test)]
    ...
    fn commit_resource_close_with(
        ...
        Ok(CommittedResourceClose { commit: close.resource, effects, publish_resource: true })
    }
        ...
        let inputs = match operation {
            ...
            ResourceOperation::TerminalClose => {
                ...
                ResourceCloseInputs {
                    ...
                    terminal_public_ids: vec![public_id],
                    ..Default::default()
                }
            }
    ...
    fn resource_terminal_close_plan_locked(
        ...
        terminal_id: &str,
        terminal_incarnation: Option<&str>,
        public_ids: &[TerminalPublicId],
        state: &State,
    ) -> anyhow::Result<ResourceClosePlan> {
        let terminal_runtime =
            public_ids.iter().find_map(|public_id| state.terminal_catalog.get(public_id).cloned());
        ...
        self.resource_close_plan_from_inputs_locked(
            ResourceCloseInputs {
                ...
                terminal_public_ids: public_ids.to_vec(),
    ...
    fn resource_close_plan_from_inputs_locked(
        ...
        for public_id in &terminal_public_ids {
            surface_ids.extend(terminal_content_placements(self, state, public_id, None));
        }
...
fn remove_public_panes_from_registry_screen(
    ...
    if screen.viewport.columns.is_empty() {
        ...
        let mut columns = std::mem::take(&mut screen.viewport.columns)
            ...
            .filter_map(|mut column| {
                column.layout = remove_public_panes_from_registry_layout(column.layout, removed)?;
    ...
    if screen.zoomed_pane.as_ref().is_some_and(|pane| removed.contains(pane)) {
        screen.zoomed_pane = None;
    }
    Some(screen)
}
