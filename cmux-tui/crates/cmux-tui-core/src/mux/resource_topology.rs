...
impl Mux {
    ...
    pub fn move_tab_to_workspace(
        ...
        anyhow::ensure!(!self.workspaces_are_provider_managed(), "managed workspace creation is not supported by tab moves");
        ...
        let commit = self.commit_resource_mutation_plan(
            &mutation, "tab.move_workspace", &fingerprint, None, None,
            move |state, registry| {
                let source_pane = state.pane_of(surface).context("source tab disappeared")?;
                let (source_wi, source_si) = state.screen_of(source_pane).context("source screen disappeared")?;
                ...
                let tab_id = state.resource_indexes.tab_ids.get(&surface).cloned().context("source tab has no identity")?;
                ...
                let (target_wi, mut target_ws) = if let Some(id) = workspace {
                    let index = state.workspace_index(id).context("destination workspace disappeared")?;
                    anyhow::ensure!(state.workspaces[index].screens.is_empty(), "destination layout changed; retry the move");
                    (index, state.workspaces[index].clone())
                } else {
                    anyhow::ensure!(state.workspaces.len() < WORKSPACE_REGISTRY_LIMIT, "workspace limit reached");
                    (state.workspaces.len(), Workspace {
                        id: mux.next_id(), public_id: WorkspacePublicId::random()?, key: Mux::new_workspace_key()?,
                        name: Mux::default_workspace_name(state), screens: Vec::new(), active_screen: 0,
                    })
                };
                ...
                let old_index = source_tabs.iter().position(|id| *id == surface).context("source tab disappeared")?;
                ...
                } else { state.panes[&source_pane].active_tab };
                source_record.active_tab = source_tabs.get(source_active).map(|id| state.resource_indexes.tab_ids[id].clone());
                let mut changes = Vec::new();
                let mut deltas = Vec::new();
                let mut source_layout = source_screen.layout_snapshot();
                let source_screen_remains = !source_empty || remove_pane_from_layout(&mut source_layout, source_pane);
                if source_empty {
                    ...
                    if source_screen_remains {
                        if source_layout.active_pane == source_pane { source_layout.active_pane = source_layout.root.first_visible_pane(); }
                        if source_layout.zoomed_pane == Some(source_pane) { source_layout.zoomed_pane = None; }
                        let durable = registry_screen_from_layout(state, source_wi, source_si, &source_layout, &after, source_screen.name.clone())?;
                        *after.screens.iter_mut().find(|s| s.public_id == source_screen_id).expect("source screen") = durable.clone();
                        ...
                        let remaining = source_ws.screens.iter().filter(|s| s.id != source_screen_slot)
                            .nth(source_ws.active_screen.min(source_ws.screens.len().saturating_sub(2))).map(|s| s.public_id.clone());
                        changes.push(ResourceChange::UpsertWorkspace {
                            workspace: registry_workspace(state, source_wi, registry.session_id().as_str()),
                            position: source_wi, active_screen: remaining.clone(),
                        });
                        set_active_screen(&mut after, &source_ws_id, remaining);
                        let order = after.screens.iter_mut().filter(|s| s.workspace_id == source_ws_id)
                            .enumerate().map(|(index, s)| { s.position = index; s.public_id.clone() }).collect();
                        changes.push(ResourceChange::SetScreenOrder { workspace_id: source_ws_id.clone(), screen_ids: order });
                    }
                } else {
                    *topology_pane_mut(&mut after, &source_pane_id)? = source_record.clone();
                    changes.push(ResourceChange::UpsertPane(source_record));
                    let order = source_tabs.iter().map(|id| state.resource_indexes.tab_ids[id].clone()).collect::<Vec<_>>();
                    reindex_target_tab_positions(&mut after.tabs, &source_pane_id, &order);
                    changes.push(ResourceChange::SetTabOrder { pane_id: source_pane_id.clone(), tab_ids: order });
                }
                let pane_record = RegistryPane { public_id: target_pane_id.clone(), screen_id: target_screen_id.clone(), name: None, active_tab: Some(tab_id.clone()), creation_ordinal: target_pane };
                let screen_record = RegistryScreen {
                    public_id: target_screen_id.clone(), workspace_id: target_ws.public_id.clone(), position: 0, name: None,
                    layout: RegistryLayoutNode::Leaf { pane: target_pane_id.clone() }, active_pane: target_pane_id.clone(), zoomed_pane: None,
                    auto_layout: Some(vec![target_pane_id.clone()]), viewport: RegistryViewport { base_width: None, columns: Vec::new() },
                };
                ...
                *topology_tab_mut(&mut after, &tab_id)? = moved_tab.clone();
                ...
                if new_workspace { after.active_screens.push((target_ws.public_id.clone(), Some(target_screen_id.clone()))); }
                else { set_active_screen(&mut after, &target_ws.public_id, Some(target_screen_id.clone())); }
                let durable_ws = RegistryWorkspace { id: target_ws.id, public_id: target_ws.public_id.clone(), key: target_ws.key.clone(), name: target_ws.name.clone(), group_key: mux.session.clone() };
                let source_changes = std::mem::take(&mut changes);
                changes.extend([
                    ResourceChange::UpsertWorkspace { workspace: durable_ws.clone(), position: target_wi, active_screen: Some(target_screen_id.clone()) },
                    ResourceChange::UpsertScreen(screen_record.clone()), ResourceChange::UpsertPane(pane_record.clone()), ResourceChange::UpsertTab(moved_tab.clone()),
                    ResourceChange::SetScreenOrder { workspace_id: target_ws.public_id.clone(), screen_ids: vec![target_screen_id.clone()] },
                    ResourceChange::SetTabOrder { pane_id: target_pane_id.clone(), tab_ids: vec![tab_id.clone()] },
                    ResourceChange::SetActiveWorkspace { workspace_id: Some(target_ws.public_id.clone()) },
                ]);
                changes.extend(source_changes);
                if new_workspace {
                    let mut order = state.workspaces.iter().map(|w| w.public_id.clone()).collect::<Vec<_>>();
                ...
                if let ContentPublicId::Terminal(public_id) = &moved_tab.content_id {
                    let host = moved_tab.terminal_id.as_deref().context("terminal has no host id")?;
                    let mut terminal = registry.terminal_record(host)?.context("terminal has no durable host")?;
                    terminal.workspace_key = target_ws.key.clone();
                    changes.push(ResourceChange::UpsertTerminal { public_id: public_id.clone(), terminal });
                }
                deltas.push(upsert(deltas.len(), "tab", tab_id.as_str(), tab_value(&moved_tab, &after)?));
                deltas.push(upsert(deltas.len(), "pane", target_pane_id.as_str(), pane_value_with_flags(&pane_record, true, false)?));
                deltas.push(upsert(deltas.len(), "screen", target_screen_id.as_str(), screen_value(&screen_record, &after, after.active_workspace.as_ref(), Some(&target_screen_id))?));
                deltas.push(workspace_resource_upsert(deltas.len(), registry.session_id().as_str(), &target_ws.public_id, &target_ws.name, target_wi, true));
                if let Some(previous) = state.workspaces.get(state.active_workspace)
                    && previous.public_id != target_ws.public_id {
                    deltas.push(workspace_resource_upsert(deltas.len(), registry.session_id().as_str(), &previous.public_id, &previous.name, state.active_workspace, false));
                }
                if !source_empty {
                    deltas.push(upsert(deltas.len(), "pane", source_pane_id.as_str(), pane_value(state, topology_pane(&after, &source_pane_id)?, &after)?));
                    for id in &source_tabs {
                        let id = &state.resource_indexes.tab_ids[id];
                        deltas.push(upsert(deltas.len(), "tab", id.as_str(), tab_value(topology_tab(&after, id)?, &after)?));
                    }
                }
                for screen in after.screens.iter().filter(|screen| screen.workspace_id == source_ws_id) {
                    deltas.push(upsert(deltas.len(), "screen", screen.public_id.as_str(), screen_value(screen, &after, after.active_workspace.as_ref(), active_screen(&after, &source_ws_id))?));
                }
                deltas.push(upsert(deltas.len(), "workspace", source_ws_id.as_str(), workspace_value(state, &after, &source_ws_id)?));
                let result = json!({"tab":tab_id,"workspace":target_ws.public_id});
                let mut desired = mux.registry_projection(state);
                if new_workspace { desired.push(durable_ws); }
                let key = target_ws.key.clone();
                let target_ws_slot = target_ws.id;
                target_ws.screens.push(Screen {
                    id: target_screen, public_id: target_screen_id, name: None, root: Node::Leaf(target_pane), active_pane: target_pane,
                    zoomed_pane: None, zellij_auto_layout: Some(vec![target_pane]), viewport_splits: Default::default(), viewport_base_width: None,
                    layout_columns: Vec::new(), layout_revision: 0, layout_undo: Default::default(),
                });
                let pane = Pane { id: target_pane, public_id: target_pane_id, name: None, tabs: vec![surface], active_tab: 0, active_at: mux.next_active_at(), focused_at: 0 };
                state.workspaces.reserve(usize::from(new_workspace));
                state.panes.reserve(1);
                let plan = ResourceMutationPlan::new(ResourcePatch { changes }, result.clone(), Value::Array(deltas), move |state| {
                    ...
                    state.panes.get_mut(&source_pane).expect("source pane").active_tab = source_active;
                    if source_empty {
                        collapse_empty_pane(&mux, state, source_pane);
                        if source_screen_remains {
                            overwrite_layout_snapshot(&mut state.workspaces[source_wi].screens[source_si], source_layout);
                        }
                    }
                    if new_workspace { state.push_workspace(target_ws); } else { state.workspaces[target_wi] = target_ws; }
                    ...
                    mux.subscribers.update_surface_session_path(surface, target_ws_slot, target_screen, target_pane);
                });
                Ok(if new_workspace { plan.with_workspace_ledger(ResourceWorkspaceLedger { event_kind: "workspace-added", workspace_key: key, workspaces: desired, legacy_result: result }) } else { plan })
            },
        )?;
        if let Some(surface_handle) = self.surface(surface) {
            let key = self.with_state(|state| state.pane_of(surface).and_then(|pane| state.screen_of(pane)).map(|(wi, _)| state.workspaces[wi].key.clone()));
            if let Some(key) = key { let _ = surface_handle.persist_host_workspace(&key); }
        }
