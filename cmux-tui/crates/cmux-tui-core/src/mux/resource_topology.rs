...
struct CommittedResourceClose {
    ...
    host_cleanup: TerminalHostRecords,
    closed_workspace_terminals: Vec<TerminalPublicId>,
}
...
impl Mux {
        ...
        match preparation {
            ...
            ResourceEffectPreparation::Execute { .. } => {
                ...
                if is_resource_close_operation(operation) {
                    ...
                    return self.finish_resource_close(committed);
                }
    ...
    pub(super) fn commit_legacy_terminal_close(
        ...
        let cleanup_identities = if plan.terminal_runtime.is_none() {
        ...
        self.finish_resource_close(CommittedResourceClose {
            ...
            effects,
            host_cleanup,
            closed_workspace_terminals: Vec::new(),
        })?;
    ...
    fn commit_resource_close_with(
        ...
        } else if plan.terminal_runtime.is_none() {
            plan.terminal_batch.clone()
        } else {
            Vec::new()
        };
        let host_cleanup = self.prepare_terminal_host_cleanup_at_root(
        ...
        Ok(CommittedResourceClose {
            commit: close.resource,
            effects,
            host_cleanup,
            closed_workspace_terminals,
        })
    }
    fn finish_resource_close(
        &self,
        ...
        self.notify_terminal_exit_waiters(closed_workspace_terminals);
        ...
        self.emit_empty_if_current(effects.empty_revision);
        host_termination?;
        remove_terminal_host_cleanup_receipt(&mut commit.result);
        Ok(commit)
    }
