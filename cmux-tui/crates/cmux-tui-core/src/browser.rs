...
#[cfg(test)]
fn authority_capture_attempt_budget() -> Duration {
    // A healthy capture performs seven serialized CDP round trips. The
    // client's 20 ms read poll means 150 ms leaves no scheduler margin.
    crate::test_timeout(Duration::from_millis(300))
}
...
impl BrowserRuntime {
    ...
    pub(crate) fn shutdown_until(&self, deadline: Instant) -> bool {
        let outbound_flushed = if self.source != BrowserSource::Launched {
            if self.is_closed() {
                true
            } else {
                deadline
...
mod tests {
    ...
    #[test]
    fn ordinary_launched_target_close_requires_confirmation() {
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let addr = listener.local_addr().unwrap();
        let server = thread::spawn(move || {
            let (stream, _) = listener.accept().unwrap();
            ...
            write_ws_json(
                &mut ws,
                json!({"id": version["id"], "error": {"code": -32000, "message": "unavailable"}}),
            );
            let discover = read_ws_json(&mut ws);
            assert_eq!(discover["method"], "Target.setDiscoverTargets");
            write_ws_json(&mut ws, json!({"id": discover["id"], "result": {}}));
        ...
        runtime.shutdown();
        let queried = server.join().unwrap();
        assert!(queried, "launched target close skipped the authoritative target query");
        assert!(!terminated, "unconfirmed launched target was discarded during ordinary close");
    }
    ...
    fn latest_navigation_supersedes_an_uncommitted_reload_epoch() {
        ...
        let server = thread::spawn(move || {
            ...
            ws.get_ref().set_read_timeout(Some(BROWSER_TEST_EVENT_TIMEOUT)).unwrap();
