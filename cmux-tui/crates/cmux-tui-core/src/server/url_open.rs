...
impl URLRequests {
    pub(super) fn subscribe(&self, client: u64, terminals: Vec<String>, writer: MessageWriter) -> anyhow::Result<()> {
        anyhow::ensure!(terminals.len() <= 256, "too many URL opener terminals");
        for terminal in &terminals {
            TerminalPublicId::parse(terminal.clone())?;
        }
        let mut state = self.0.lock().unwrap();
        anyhow::ensure!(state.subscribers.len() < CAPACITY || state.subscribers.contains_key(&client), "too many URL opener clients");
        state.subscribers.insert(client, Subscriber { terminals: terminals.into_iter().collect(), writer });
        Ok(())
    }
    ...
    fn prepare(&self, terminal: &str, url: &str) -> Option<(String, mpsc::Receiver<bool>)> {
        let (id, writer, receiver) = {
            let mut state = self.0.lock().unwrap();
            if state.pending.len() >= CAPACITY { return None; }
            ...
            state.pending.insert(id.clone(), Pending {
                client, deadline: Instant::now() + DEADLINE, claimed: false, reply: sender,
            });
            (id, writer, receiver)
        };
        if writer.send_control(&json!({
    ...
    pub(super) fn claim(&self, id: &str) -> bool {
        let mut state = self.0.lock().unwrap();
        let Some(pending) = state.pending.get_mut(id) else { return false; };
        if pending.claimed || Instant::now() >= pending.deadline { return false; }
    ...
    pub(super) fn complete(&self, id: &str, opened: bool) -> bool {
        let mut state = self.0.lock().unwrap();
        let Some(pending) = state.pending.get(id) else { return false; };
        if !pending.claimed || Instant::now() >= pending.deadline { return false; }
        let pending = state.pending.remove(id).unwrap();
        pending.reply.try_send(opened).is_ok()
    }

    fn cancel(&self, id: &str) { self.0.lock().unwrap().pending.remove(id); }
...
fn validate_url(raw: &str) -> bool {
    raw.len() <= 16_384 && !raw.chars().any(|c| c.is_control() || c.is_whitespace())
        && url::Url::parse(raw).is_ok_and(|url| matches!(url.scheme(), "http" | "https") && url.host_str().is_some())
}
...
pub(super) fn start(mux: &Arc<Mux>, client: u64, id: Option<Value>, terminal: String, url: String, writer: &MessageWriter) -> bool {
    let terminal_id = TerminalPublicId::parse(terminal.clone()).ok();
    let valid = mux.control_clients.is_unix(client) && validate_url(&url)
    ...
    let Some((request_id, receiver)) = pending else { return respond(writer, id, false); };
...
fn respond(writer: &MessageWriter, id: Option<Value>, opened: bool) -> bool {
    send_response(writer, Response { id, ok: true, data: Some(json!({"opened": opened})),
        error: None, error_code: None, error_delivery: None })
}
...
mod tests {
    ...
    fn url_open_disconnect_deadline_and_capacity_are_bounded() {
        ...
        for _ in 0..CAPACITY { requests.push(broker.prepare(TERMINAL, "https://example.com").unwrap()); }
        ...
        broker.disconnect(1);
        for (_, receiver) in requests { assert!(receiver.recv().is_err()); }
    ...
    fn url_open_validates_without_rewriting_auth_url_bytes() {
        ...
        for url in ["file:///tmp/file", "javascript:alert(1)", "https://", "https://foo/\ncommand", "https://foo/a b"] {
