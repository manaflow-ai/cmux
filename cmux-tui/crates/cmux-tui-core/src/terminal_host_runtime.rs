...
use crate::terminal_host_protocol::{
    ...
    encode_terminal_exit, read_frame, write_frame,
};
...
mod unix {
    ...
    impl HostShared {
        ...
        fn terminate_and_wait(&self) {
            ...
            if child_reaped {
                ...
                if session_empty {
                    self.child_exit.1.notify_all();
                    let _ = self.publish_exit_if_drained();
                } else {
                    self.abandon_group_escalation();
                }
                return;
    ...
    fn spawn_host_runtime(
        ...
        thread::Builder::new().name("terminal-host-pty".into()).spawn(move || {
            ...
            let _ = reader_host.publish_exit_if_drained();
        })?;
        ...
        crate::process_session::enqueue_reserved_session_leader(
            ...
            move |cleanup_succeeded| {
                if reap_host.child_reaped.load(Ordering::Acquire) {
                    return match reap_host.publish_exit_if_drained() {
                        Ok(()) => crate::process_session::NaturalReapFinish::Complete,
                        Err(error) => {
                            eprintln!(
                ...
                if done {
                    match reap_host.publish_exit_if_drained() {
                        Ok(()) => crate::process_session::NaturalReapFinish::Complete,
                        Err(error) => {
                            eprintln!(
                                "cmux-tui: terminal-host exit persistence failed; retrying: {error:#}"
