...
impl Chrome {
    ...
    pub fn launch_with(options: &ChromeLaunchOptions) -> anyhow::Result<Self> {
        ...
        let stderr = match child.stderr.take() {
            Some(stderr) => stderr,
            None => {
                cleanup_failed_launch(child, &profile_dir, profile_ephemeral);
...
fn cleanup_failed_launch(mut child: Child, profile_dir: &Path, profile_ephemeral: bool) {
    let _ = child.kill();
    let _ = child.wait();
...
mod tests {
    ...
    fn failed_launch_cleanup_kills_child_and_removes_ephemeral_profile() {
        ...
        let child = Command::new(std::env::current_exe().unwrap())
            .args(["--exact", "__cmux_tui_cdp_cleanup_child__"])
            .spawn()
            .unwrap();
        cleanup_failed_launch(child, &profile_dir, true);
