...
Any configured Alt chord is active without the prefix. Default modeless commands are `Alt-t`, `Alt-n`, `Alt-[`, `Alt-]`, `Alt-h/j/k/l`, Alt arrows, `Alt-=`, and `Alt--`. `Alt-n` creates a pane with Zellij's default vertical auto-layout: one full-height left pane, up to four rows on the right, then balanced columns of four through twelve panes.
