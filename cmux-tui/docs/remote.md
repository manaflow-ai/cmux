...
The SSH path probes `~/.local/bin/cmux-tui` on macOS and Linux or `%LOCALAPPDATA%\\cmux\\bin\\cmux-tui.exe` on Windows, then carries framed data over SSH stdio. It keeps a durable headless mux owner and a replaceable remote sidecar as separate processes. Existing terminal panes therefore survive sidecar replacement. If the named tmux-style session already has a live mux socket, the sidecar attaches without creating a second session. Otherwise cmux starts the user-scoped mux owner on demand.
