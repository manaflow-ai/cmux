...
When configured, the machine rail uses the same header, two-line entry, active marker, and selected-row treatment as the built-in workspace list. Press a non-active machine entry to connect to it. Click `+ Connect machine` to open the native SSH host picker, then filter a discovered alias or choose `Add SSH host…` for a process-local target. Provider-owned connection flows may use the same row for a host address or pairing code. Click `+ new machine` to run a provider create action or choose among configured prototype sources. The prototype picker adds only a local catalog row and never invokes Docker or a cloud API.
