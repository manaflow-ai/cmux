...
The hosted workflow builds the candidate product in a separate job and runner. That untrusted job uploads one closed artifact with the candidate binary, available symbol files, and a manifest of all paths, sizes, and SHA-256 hashes. The clean benchmark job rejects symlinks, unsafe paths, extra files, missing files, identity mismatches, size mismatches, and hash mismatches before it imports the candidate product. It then builds the baseline product and all trusted benchmark code. Candidate build scripts never run in the benchmark job.

The workflow has no automatic pull request trigger. An authorized maintainer must start each comparison and explicitly select the exact candidate SHA. The baseline checkout supplies the Zig parser, Rust setup action, benchmark harness, harness tests, evidence verifier, and every native-profile harness. The candidate checkout in the benchmark job supplies read-only source, toolchain, and Ghostty identity data. Only the imported candidate product binary runs there.
