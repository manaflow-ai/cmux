...
impl ForcedPendingConnectProbe {
    pub(crate) fn install_until_poll(max_polls: usize) -> Self {
        ...
        Self::install_with(Some(max_polls))
    }

    fn install_with(max_polls: Option<usize>) -> Self {
    ...
    pub(crate) fn install_with_after_first_poll(action: impl FnOnce() + 'static) -> Self {
        let probe = Self::install_with(None);
...
fn connect_unix_with_poll_checks(
    ...
    if FORCE_PENDING_CONNECT.with(std::cell::Cell::get) {
        ...
        loop {
            ...
            let reached_max_polls = FORCED_CONNECT_MAX_POLLS
                .with(std::cell::Cell::get)
                .is_some_and(|limit| polls >= limit);
            if reached_max_polls {
                return Err(connect_timeout_error(socket_path));
            }
