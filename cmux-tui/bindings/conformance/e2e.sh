...
if [[ -n "${CMUX_TUI_BIN:-}" ]]; then
  MUX_BIN="$CMUX_TUI_BIN"
else
  (cd "$MUX_DIR" && cargo build -p cmux-tui)
  MUX_BIN="$MUX_DIR/target/debug/cmux-tui"
fi
...
run_lang() {
  ...
  local log
  log="$(mktemp "${TMPDIR:-/tmp}/cmux-tui-${lang}.XXXXXX.log")"
  ...
  if CMUX_TUI_SOCKET="$socket" CMUX_MUX_SOCKET="$socket" "$@" run; then
    echo "PASS $lang"
    passed=$((passed + 1))
  else
    echo "FAIL $lang"
...
python_cmd() {
  case "$1" in
    ...
    run) python3 "$ROOT/cmux-tui/bindings/python/e2e.py" ;;
  esac
}
...
typescript_cmd() {
  case "$1" in
    check)
      ...
      (cd "$ROOT/cmux-tui/bindings/typescript" && npm ci --silent --no-audit --no-fund)
      (cd "$ROOT/cmux-tui/bindings/typescript" && npm run build --silent)
      ;;
    run) (cd "$ROOT/cmux-tui/bindings/typescript" && node dist/e2e/e2e.js) ;;
  esac
}
...
go_cmd() {
  case "$1" in
    check)
      ...
      (cd "$ROOT/cmux-tui/bindings/go" && go build ./...)
      ;;
    run) (cd "$ROOT/cmux-tui/bindings/go" && go run ./cmd/e2e) ;;
  esac
}
