...
// MachineStatsChangedEvent is emitted by protocol v12.
type MachineStatsChangedEvent struct {
	Stats RequiredNullable[MachineStats] `json:"-"`
}

func (value MachineStatsChangedEvent) MarshalJSON() ([]byte, error) {
	type wire MachineStatsChangedEvent
	encoded, err := json.Marshal(wire(value))
	if err != nil {
		return nil, err
	}
	...
	if !value.Stats.IsSet() {
		return nil, fmt.Errorf("encode MachineStatsChangedEvent: required nullable field stats is missing")
	}
	if value.Stats.IsNull() {
		...
		encodedField, err := json.Marshal(fieldValue)
		if err != nil {
			return nil, fmt.Errorf("encode MachineStatsChangedEvent.Stats: %w", err)
		}
...
func (value *MachineStatsChangedEvent) UnmarshalJSON(data []byte) error {
	if !isJSONObject(data) {
		return fmt.Errorf("decode MachineStatsChangedEvent: expected object")
	}
	var fields struct {
		Stats RequiredNullable[MachineStats] `json:"stats"`
	}
	if err := json.Unmarshal(data, &fields); err != nil {
		return fmt.Errorf("decode MachineStatsChangedEvent: %w", err)
	}
	type wire MachineStatsChangedEvent
	var decoded wire
	if !fields.Stats.IsSet() {
		return fmt.Errorf("decode MachineStatsChangedEvent: required field stats is missing")
	}
	decoded.Stats = fields.Stats
	*value = MachineStatsChangedEvent(decoded)
	return nil
}

func (MachineStatsChangedEvent) EventName() string { return "machine-stats-changed" }
func (MachineStatsChangedEvent) isDeltaEvent()     {}
func (MachineStatsChangedEvent) isSubscribeEvent() {}
...
func parseEvent(raw map[string]any) Event {
	...
	case "machine-stats-changed":
		var event MachineStatsChangedEvent
		if decodeEvent(raw, &event) {
			return event
		}
