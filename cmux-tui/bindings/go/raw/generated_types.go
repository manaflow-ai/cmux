...
func (value AgentRecord) MarshalJSON() ([]byte, error) {
	...
	if value.Agent.IsAbsent() {
		...
		if err != nil {
			return nil, fmt.Errorf("encode AgentRecord.Agent: %w", err)
		}
...
type ListAgentsResult struct {
	...
	History *[]AgentRecord `json:"history,omitempty"`
}
...
func (value *ListAgentsResult) UnmarshalJSON(data []byte) error {
	...
	var fields struct {
		Agents     *[]AgentRecord                     `json:"agents"`
		HasHistory optionalNonNullJSON[bool]          `json:"has_history"`
		History    optionalNonNullJSON[[]AgentRecord] `json:"history"`
	}
	if err := json.Unmarshal(data, &fields); err != nil {
	...
	if fields.HasHistory.null {
		return fmt.Errorf("decode ListAgentsResult: non-nullable field has_history is null")
	}
	if fields.HasHistory.set {
		decoded.HasHistory = &fields.HasHistory.value
	}
	if fields.History.null {
		return fmt.Errorf("decode ListAgentsResult: non-nullable field history is null")
	}
	if fields.History.set {
		decoded.History = &fields.History.value
	}
