...
type IdentifyResult struct {
	...
	ShutdownCleanup   *ShutdownCleanupStatus      `json:"shutdown_cleanup,omitempty"`
	TerminalRevision  uint64                      `json:"terminal_revision"`
...
func (value *IdentifyResult) UnmarshalJSON(data []byte) error {
	...
	var fields struct {
		...
		RegistryID        *string                                    `json:"registry_id"`
		Session           *string                                    `json:"session"`
		ShutdownCleanup   optionalNonNullJSON[ShutdownCleanupStatus] `json:"shutdown_cleanup"`
		TerminalRevision  *uint64                                    `json:"terminal_revision"`
		Version           *string                                    `json:"version"`
		WorkspaceRevision *uint64                                    `json:"workspace_revision"`
	}
	...
	decoded.Session = *fields.Session
	if fields.ShutdownCleanup.null {
		return fmt.Errorf("decode IdentifyResult: non-nullable field shutdown_cleanup is null")
	}
	if fields.ShutdownCleanup.set {
		decoded.ShutdownCleanup = &fields.ShutdownCleanup.value
	}
	if fields.TerminalRevision == nil {
...
type ShutdownCleanupStatus struct {
	Degraded bool   `json:"degraded"`
	Pending  uint64 `json:"pending"`
	Retrying bool   `json:"retrying"`
}

func (value *ShutdownCleanupStatus) UnmarshalJSON(data []byte) error {
	if !isJSONObject(data) {
		return fmt.Errorf("decode ShutdownCleanupStatus: expected object")
	}
	var fields struct {
		Degraded *bool   `json:"degraded"`
		Pending  *uint64 `json:"pending"`
	...
	if err := json.Unmarshal(data, &fields); err != nil {
		return fmt.Errorf("decode ShutdownCleanupStatus: %w", err)
	}
	type wire ShutdownCleanupStatus
	var decoded wire
	if fields.Degraded == nil {
		return fmt.Errorf("decode ShutdownCleanupStatus: required field degraded is missing or null")
	}
	decoded.Degraded = *fields.Degraded
	if fields.Pending == nil {
		return fmt.Errorf("decode ShutdownCleanupStatus: required field pending is missing or null")
	}
	decoded.Pending = *fields.Pending
	if fields.Retrying == nil {
		return fmt.Errorf("decode ShutdownCleanupStatus: required field retrying is missing or null")
	}
	decoded.Retrying = *fields.Retrying
	*value = ShutdownCleanupStatus(decoded)
	return nil
}
