...
type SetTerminalIdlePolicyResult struct {
	IdleCloseSeconds RequiredNullable[uint64] `json:"-"`
	TerminalID       string                   `json:"terminal_id"`
}

func (value SetTerminalIdlePolicyResult) MarshalJSON() ([]byte, error) {
	type wire SetTerminalIdlePolicyResult
	encoded, err := json.Marshal(wire(value))
	if err != nil {
		return nil, err
	}
	...
	if !value.IdleCloseSeconds.IsSet() {
		return nil, fmt.Errorf("encode SetTerminalIdlePolicyResult: required nullable field idle_close_seconds is missing")
	}
	if value.IdleCloseSeconds.IsNull() {
		object["idle_close_seconds"] = json.RawMessage("null")
	} else {
		fieldValue, _ := value.IdleCloseSeconds.Get()
		encodedField, err := json.Marshal(fieldValue)
		if err != nil {
			return nil, fmt.Errorf("encode SetTerminalIdlePolicyResult.IdleCloseSeconds: %w", err)
		}
...
func (value *SetTerminalIdlePolicyResult) UnmarshalJSON(data []byte) error {
	if !isJSONObject(data) {
		return fmt.Errorf("decode SetTerminalIdlePolicyResult: expected object")
	}
	var fields struct {
		IdleCloseSeconds RequiredNullable[uint64] `json:"idle_close_seconds"`
		TerminalID       *string                  `json:"terminal_id"`
	}
	if err := json.Unmarshal(data, &fields); err != nil {
		return fmt.Errorf("decode SetTerminalIdlePolicyResult: %w", err)
	}
	type wire SetTerminalIdlePolicyResult
	var decoded wire
	if !fields.IdleCloseSeconds.IsSet() {
		return fmt.Errorf("decode SetTerminalIdlePolicyResult: required field idle_close_seconds is missing")
	}
	decoded.IdleCloseSeconds = fields.IdleCloseSeconds
	if fields.TerminalID == nil {
		return fmt.Errorf("decode SetTerminalIdlePolicyResult: required field terminal_id is missing or null")
	}
	decoded.TerminalID = *fields.TerminalID
	*value = SetTerminalIdlePolicyResult(decoded)
	return nil
}
