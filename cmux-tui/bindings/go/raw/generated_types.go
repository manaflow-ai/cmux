...
func (value AgentSource) valid() bool {
	switch value {
	case AgentSourcePlugin, AgentSourceDetected, AgentSourceSocket, AgentSourceHook:
		return true
	default:
		return false
	}
...
type CloudBootstrapResult struct {
	CreatedPath RequiredNullable[JSONValue] `json:"-"`
	Generation  Presence[string]            `json:"-"`
	Occupied    *bool                       `json:"occupied,omitempty"`
	Revision    Presence[string]            `json:"-"`
}

func (value CloudBootstrapResult) MarshalJSON() ([]byte, error) {
	type wire CloudBootstrapResult
	encoded, err := json.Marshal(wire(value))
	if err != nil {
		return nil, err
	}
	...
	if !value.CreatedPath.IsSet() {
		return nil, fmt.Errorf("encode CloudBootstrapResult: required nullable field created_path is missing")
	}
	if value.CreatedPath.IsNull() {
		object["created_path"] = json.RawMessage("null")
	} else {
		fieldValue, _ := value.CreatedPath.Get()
		encodedField, err := json.Marshal(fieldValue)
		if err != nil {
			return nil, fmt.Errorf("encode CloudBootstrapResult.CreatedPath: %w", err)
		}
		object["created_path"] = encodedField
	}
	if value.Generation.IsAbsent() {
		delete(object, "generation")
	} else if value.Generation.IsNull() {
		object["generation"] = json.RawMessage("null")
	} else {
		fieldValue, _ := value.Generation.Get()
		encodedField, err := json.Marshal(fieldValue)
		if err != nil {
			return nil, fmt.Errorf("encode CloudBootstrapResult.Generation: %w", err)
		}
		object["generation"] = encodedField
	}
	if value.Revision.IsAbsent() {
		...
		encodedField, err := json.Marshal(fieldValue)
		if err != nil {
			return nil, fmt.Errorf("encode CloudBootstrapResult.Revision: %w", err)
		}
...
func (value *CloudBootstrapResult) UnmarshalJSON(data []byte) error {
	if !isJSONObject(data) {
		return fmt.Errorf("decode CloudBootstrapResult: expected object")
	}
	var fields struct {
		CreatedPath RequiredNullable[JSONValue] `json:"created_path"`
		Generation  Presence[string]            `json:"generation"`
		Occupied    optionalNonNullJSON[bool]   `json:"occupied"`
		Revision    Presence[string]            `json:"revision"`
	}
	if err := json.Unmarshal(data, &fields); err != nil {
		return fmt.Errorf("decode CloudBootstrapResult: %w", err)
	}
	type wire CloudBootstrapResult
	var decoded wire
	if !fields.CreatedPath.IsSet() {
		return fmt.Errorf("decode CloudBootstrapResult: required field created_path is missing")
	}
	decoded.CreatedPath = fields.CreatedPath
	decoded.Generation = fields.Generation
	if fields.Occupied.null {
		return fmt.Errorf("decode CloudBootstrapResult: non-nullable field occupied is null")
	}
	if fields.Occupied.set {
		decoded.Occupied = &fields.Occupied.value
	}
	decoded.Revision = fields.Revision
	*value = CloudBootstrapResult(decoded)
	return nil
}
...
type ProcessInfoResult struct {
	...
	ForegroundExecutable Presence[string]         `json:"-"`
...
func (value ProcessInfoResult) MarshalJSON() ([]byte, error) {
	...
	if value.ForegroundExecutable.IsAbsent() {
		delete(object, "foreground_executable")
	} else if value.ForegroundExecutable.IsNull() {
		object["foreground_executable"] = json.RawMessage("null")
	} else {
		fieldValue, _ := value.ForegroundExecutable.Get()
		encodedField, err := json.Marshal(fieldValue)
		if err != nil {
			return nil, fmt.Errorf("encode ProcessInfoResult.ForegroundExecutable: %w", err)
		}
		object["foreground_executable"] = encodedField
	}
	if !value.PID.IsSet() {
...
func (value *ProcessInfoResult) UnmarshalJSON(data []byte) error {
	...
	var fields struct {
		...
		ForegroundExecutable Presence[string]         `json:"foreground_executable"`
	...
	decoded.ForegroundExecutable = fields.ForegroundExecutable
	if !fields.PID.IsSet() {
