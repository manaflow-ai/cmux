...
127-operation `cmux.protocol/2` catalog. Journal administration remains a
trusted local CLI-only surface; the facades expose only
`session.journal.subscribe` for the journal. The raw protocol inventory is a
...
The 127 public operations are the API inventory. The public fake-server
