...
Result<Json> Codec<ShutdownRequest>::encode(const ShutdownRequest& value) {
    (void)value;
    Json::Object object;
    return Json(std::move(object));
}

Result<ShutdownRequest> Codec<ShutdownRequest>::decode(const Json& value) {
    auto source = value.as_object();
    if (!source) return std::move(source).error();
...
constexpr std::array<CommandFieldRequirement, 2> kCommand94FieldRequirements{{
    {"surface", 9U, "surface-subscribe-filter"},
    {"tree_events", 7U, ""},
}};
constexpr std::array<CommandMetadata, 102> kCommands{{
    ...
    {"shutdown", "local-admin", 9U, "server-shutdown-v1", false, "", "", std::span<const CommandFieldRequirement>{}},
    {"shutdown-daemon", "local-admin", 9U, "", false, "", "", std::span<const CommandFieldRequirement>(kCommand91FieldRequirements)},
    ...
    {"subscribe", "frontend", 5U, "", true, "subscribe", "", std::span<const CommandFieldRequirement>(kCommand94FieldRequirements)},
...
Result<EmptyResult> Client::shutdown(
    const ShutdownRequest& request, RequestOptions options) {
    auto encoded = encode_value(request);
    if (!encoded) return std::move(encoded).error();
    auto parameters = encoded.value().as_object();
    if (!parameters) return std::move(parameters).error();
    auto response = core_.request("shutdown", *parameters.value(), options.timeout);
    if (!response) return std::move(response).error();
    return decode_value<EmptyResult>(response.value());
}
