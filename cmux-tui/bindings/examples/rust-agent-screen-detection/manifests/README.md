...
Seventeen of these TOML files are unchanged from
...
`src/detect/manifests/`. The Claude, Codex, and GitHub Copilot files include
the upstream fixes present at that commit. `grok.toml` is based on the snapshot
file and carries one cmux correction: idle OSC progress wins over a generic
custom title, while an explicit spinner still wins over retained idle metadata.
