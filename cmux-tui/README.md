...
The default session is `main`. Default sockets use `XDG_RUNTIME_DIR`, then `TMPDIR`, then `/tmp`, under `cmux-tui-<uid>/<session>.sock`; empty values are ignored. On Darwin, paths over 103 filesystem bytes fall back to `/tmp/cmux-tui-<uid>` and are never truncated. Use `--socket <path>` for an explicit path. Detach from an attached TUI with prefix `d`, which is `Ctrl-b d` by default.
