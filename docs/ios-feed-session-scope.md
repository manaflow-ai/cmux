...
- Updated: 2026-09-23. Full-text verification and current Mac authentication are recorded below; earlier results retain their original dates.
...
- Current Mac/iOS development tag: `fdtext`, source `86eabf9ee333924300e9de4ccb6a9890ec7bba42`. Historical `xfd2` installation records remain below.
...
| F02 | Agent identity, relative time, quoted user prompt, assistant output, plans, stop reasons, and failed tool results. Messages offer View full text, opening a selectable reading sheet backed by paged Mac reads. | Built | Full-text row and sheet verified in hosted iPhone UI test V30; other row kinds retain their earlier coverage. |
...
| F20 | Install the current version on Mac and Aziz's physical iPhone, ready to open Feed and try it. | Partial | 2026-09-23: `fdtext` Mac installed and signed in to aziz@manaflow.ai using the saved personal profile. Matching phone package passed exact source/tag/server checks; signed, installed, authenticated, and trusted-paired. A relaunch without injected credentials reconnected successfully (V34). |
...
| F28 | Replace View full text with an inline “… See more” control only when additional text exists beyond the visible preview. | Pending | Requested 2026-09-23. Must account for layout truncation and Mac preview limits. |
| F29 | Preserve the computer picker, computer list, and settings access across every primary tab, including Feed. | Pending | Use the shared root toolbar and computer selection scope. |
| F30 | Search the active tab, including Feed; hide search for future tabs that do not support it. | Pending | Feed needs its own query state, results, and placeholder. |
| F31 | Open an event’s workspace or tab from its row context menu, using its exact owning Mac. | Pending | Hide unavailable destinations and fail closed when the target disappears. |
...
| --- | --- | --- |
| G01 | Resolved for source `86eabf9ee33`: both `fdtext` apps are installed and authenticated; persisted phone reconnect passed. | Repeat delivery after the newly requested presentation changes. Physical-phone Feed interactions remain part of F21. |
...
| G09 | Code confirms reply and triage markers are phone memory only; they do not persist across process restart or sync to another phone. | Current promise is survival across refreshes. Treat durable or cross-device state as a proposed extension unless the user adds it. |
...
| G11 | Mac persistence appends incoming rows but does not append later resolution/expiry mutations; appends are asynchronous and errors are ignored. | Proposed shipping criterion: restart/crash tests and a durable decision record. No new persistence scope approved yet. |
| G12 | Mac history uses one unbounded OS-user JSONL path; phone has no persisted Feed cache or older-history cursor. | Proposed decisions: ownership across accounts/tags, retention/deletion, and offline history expectations. |
