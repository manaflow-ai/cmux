...
| F20 | Install the current version on Mac and Aziz's physical iPhone, ready to open Feed and try it. | Partial | Refreshed Mac installed and signed in to the personal account on 2026-09-16. Matching phone archive signed and retained locally; phone unavailable. Updated saved personal credentials are still rejected by development Stack, so automatic authenticated delivery is not established. See V25–V27. |
...
| --- | --- | --- |
| G01 | Refreshed Mac is signed in through a temporary developer session. Updated saved credentials remain rejected; signed phone archive is retained while the device is unavailable. | When the user physically reconnects/unlocks the iPhone, the agent completes authenticated installation, verifies both accounts and persisted reconnect, and opens Feed. The user runs no setup commands. Resume delivery through a verified authentication path; do not retry the rejected password automatically. |
