...
- M5 browser previews use the capability `browser.preview.v1` and the event topic `browser.preview`. A capable phone replaces per-stream demand through the `browser_preview_demand` parameter on `mobile.events.subscribe`: `preview_surface_ids` requests card-sized frames and `full_surface_ids` requests the larger view-only surface, with full demand winning. The Mac captures only demanded `WKWebView` surfaces, coalesces navigation/title/progress changes to a minimum one-second interval per surface, targets 600 px (cards) or 1,200 px (full view) on the longest edge, and JPEG-compresses toward 60 KB. Unsubscribing the final consumer cancels pending cadence work and releases the captured-frame state.
- M5 injects phone-side agent-chat cards immediately after their bound terminal, preserving the chat/session store's input order when multiple sessions share a terminal. This adjacency is the normal strip order; enabling the attention shelf then applies its existing stable partition, so a waiting chat can move into the leading attention group. The Mac topology DTO remains chat-agnostic.
