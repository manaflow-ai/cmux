...
The `browser.engine` setting has shipped. Working end-to-end on Chromium surfaces: browsing/navigation (omnibar, back/forward/reload via JS shims, URL/title mirroring), native `<select>` dropdown menus, native file-upload pickers, and screenshots (toolbar button + `cmux browser screenshot` automation, via the OWL capture API). Runtime-missing and mid-session crash both fall back gracefully (new surfaces open as WebKit with a notification; a live surface shows a disconnected banner with Reload-to-restart).

DevTools opens in its own window (`mode: .window`). Docked DevTools (`.bottom`/`.right`/`.left`) is a separate shell surface (`kind == .devTools` in the surface tree) that cmux does not composite — it mounts a single `CALayerHost` for the main web-view compositor context only, and no compositor context is delivered for the DevTools surface, so a docked panel renders blank. Compositing docked DevTools in-panel needs the OWL runtime to expose that surface's context.
