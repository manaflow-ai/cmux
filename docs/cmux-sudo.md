...
4. The CLI long-polls `sudo.result` with that request id until the app returns the terminal result. Result polling is accepted only from the same socket peer PID and UID that submitted the request.
