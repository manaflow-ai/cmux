...
3. The app shows the exact command to the user and calls `LAContext.evaluatePolicy(.deviceOwnerAuthentication, localizedReason:)`. Apple documents that this policy uses Touch ID and Apple Watch on macOS when available, then falls back to the user password.
4. The app signs the helper payload with a per-app-session P-256 key and sends it to `/var/run/cmux-sudo-helper.sock`.
5. The privileged helper validates the cmux app signature, verifies the signed payload, resolves the executable without a shell, runs one command, returns captured output and exit status, then exits.
6. The app appends an audit entry whether the request is rejected, denied, or executed.
...
- The helper must validate the connecting process code signature before reading the request.
...
Rotation: the app rotates at 10 MB and keeps five previous files named `sudo-audit.jsonl.1` through `sudo-audit.jsonl.5`. The directory is `0700`; log files are `0600`. Hash chaining is per active file, so rotation preserves history but starts a new chain in the fresh file.
