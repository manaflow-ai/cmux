...
4. The CLI polls `sudo.result` with that request id until the app returns the terminal result. Result polling is accepted only from the same socket peer PID and UID that submitted the request.
...
7. The privileged helper validates the cmux app signature and Team ID, verifies the signed payload, resolves the executable without a shell, runs one command, returns captured output and exit status, then exits.
...
The per-session signing key protects the payload between the app and helper from in-transit modification. It is not a substitute for helper-side code-signature validation because the public key travels with the payload. The helper therefore also pins the connecting app to Manaflow's signing Team ID.
