...
- `DONE` relay authorization (`RemoteRelayCommandPolicy`, GHSA-9vmv-3hjw-j28c): post-authentication commands pass a deny-by-default method allowlist (the remote CLI/agent-hook surface only), workspace/surface/tab ID params must resolve to objects owned by the remote session (unmapped local UUIDs and positional refs are denied), command-bearing params (`initial_command`, `command`, `tmux_start_command`, `pane_start_command`) are denied on every method, `surface.respawn` is denied (plain-SSH respawn falls back to local execution), and local-object-creating methods (`workspace.create`, `window.create`, `workspace.group.create`/`new_workspace`/`delete`) are denied. Denials return a `remote_relay_denied` v2 error and never reach the local socket.
