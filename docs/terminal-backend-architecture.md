...
canonical activity facts plus each presentation's last seen activity sequence.
It is not a mutable global boolean.
...
Each terminal has one ordered input actor. Every request carries `client_id`,
`input_sequence`, `request_id`, and the geometry revision used to produce any
coordinates.

Atomic input operations include:

- a key event and its modifier state
- an IME commit
- one bracketed paste
- mouse press, drag, and release capture
...
events. Explicit automation commands may request a bounded delegated lease.
Lease changes are ordered events. Disconnect revokes all leases held by that
client.
...
- lease ID
- client and presentation IDs
- terminal ID
- rows and columns
- pixel size and backing scale
- geometry revision
- grant and revocation sequences
...
| MULTI-2 | P0 | Input ordering | Concurrent GUI, TUI, and automation input produces the specified total order without split paste, lost release, or duplicate command. | Deterministic integration test. |
...
| COMPAT-1 | P0 | Upgrade behavior | Compatible protocol ranges reconnect; incompatible ranges fail read-only with an actionable error and no state change. | Version-matrix integration test. |
