...
This phase split is not a relay-only IP-privacy boundary. Stock Iroh 1.0 registers a TLS-complete connection with its path manager before cmux can verify a same-account grant, then exchanges public addresses, ports, and local interface addresses. EndpointID TLS proves key possession, not cmux authorization. The pinned cmux noq, Iroh, and FFI forks therefore negotiate QUIC NAT traversal but defer candidate announcement, inbound `REACH_OUT` processing, probes, timers, and direct-path migration on each connection. Every cmux endpoint advertises zero initial bidirectional and unidirectional stream credit; the Mac raises bidirectional credit to one only for the bootstrap control stream. Admission uses an acknowledged, capability-negotiated barrier: the phone's hello offers `natBarrier`, the Mac verifies the peer and returns an admit that acks the offer, the phone authorizes NAT traversal on its exact connection and sends `client-ready` over the control stream, and only then does the Mac complete admission and authorize its own side. The ordering is the point: the deferral forks discard and tombstone any NAT-traversal frame that arrives before local authorization, and the QUIC packets carrying them are still ACKed, so a candidate advertisement that beats the client's authorization is lost permanently and the connection can never leave the relay. The barrier guarantees the Mac's `ADD_ADDRESS` advertisement is emitted only toward an already-authorized phone. A hello without the capability, or an admit without the ack, falls back to unordered authorization (the pre-barrier behavior) so mixed old/new versions still admit. The client separately grants one unidirectional stream to the sole server-event receiver only after that receiver is prepared. One central Mac router owns acceptance, routes terminal lanes to the terminal byte owner, and rejects artifact lanes until a concrete preview consumer registers. Every lane header has a five-second deadline. The Mac's fresh candidate advertisement reaches an already-authorized phone and starts direct-path migration on that same connection. A denial, a role-invalid frame, or a missing `client-ready` on an acked barrier closes the connection without creating a replacement connection; a failed local NAT-traversal authorization is journaled and never fatal, because the relay path keeps carrying the admitted session when traversal cannot. Default upstream behavior remains unchanged unless these endpoint options are enabled.
...
- admission-barrier ordering, proving zero initial stream credit, one bootstrap stream, no server-side NAT activation before the client's `client-ready`, legacy hello/admit fallback, and a reasoned close after denial, timeout, cancellation, an invalid frame, or a missing client-ready frame;
