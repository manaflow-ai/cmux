...
| `Minimal` preset, verified relays only, never n0 defaults or public DNS discovery | `Endpoint::builder(presets::Minimal)` + `RelayMode::Custom` built from `cmux-tui/crates/cmux-tui-iroh/relay-catalog.json`, an embedded copy of the committed server-owned source of truth `config/iroh/managed-relay-catalog.json`; a golden test fails when the copy drifts. No discovery service is configured; no relay URL comes from the environment. |
