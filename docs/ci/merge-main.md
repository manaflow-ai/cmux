...
2. Picks the newest first-parent main commit the branch lacks whose
   **CI fast guards** run passed (`scripts/ci/last_green_base.py`). That
   ...
   request for the workflow's last 100 runs on main, cached by `gh` for a
   minute. Newer commits it passes over are listed with their verdict
...
4. Runs `scripts/ci/guards-local.sh` (the `ci` group, or every group with
   ...
   from main", passing there is "introduced by this branch".

Guard failures never fail the command; the merge stands and the labels say
what to fix. `--strict` exits 3 when the branch introduced one.
...
| `--strict` | Exit 3 when the branch introduced a guard failure. |

The `/catch-up` workflow (`.github/workflows/pr-catch-up.yml`) uses the same
selection for a pull request based on main: it merges the newest green base
commit and falls back to the tip when there is no verdict or no green commit
among the newest 40.
