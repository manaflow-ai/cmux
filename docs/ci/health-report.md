...
The runs endpoint returns newest first, so a page cap truncates the *old* end
of the window. When that happens the header says how much of the window is
actually covered and the run counts read as rates over that covered span. The
header also states the API call count. Nothing is cached, and a rate limit or
API error degrades the report to partial data (announced in a banner) instead
of failing the run.
...
| --- | ---: | --- |
| `CI_HEALTH_MAX_RUN_PAGES` | 12 | Pages of 100 runs per window |
