# Swift package & architecture conventions

These conventions are binding on every new file in `Packages/`, every new file in the app target, and every meaningful rewrite of an existing Swift file. They are summarized in `CLAUDE.md` under "Package & Swift architecture"; the full rules, carve-outs, and worked examples live here. The review bots (Codex, CodeRabbit, Greptile) enforce them on PRs.

The higher-level refactor blueprint, with the per-god-object decomposition and the extraction waves, lives in the cmuxterm-hq control repo under `docs/cmux-refactor-audit/blueprint/`; the enforceable core is reproduced below so it is followable from inside this repo alone.
...
See also the **Test quality policy** in `CLAUDE.md` (no source-shape tests; verify runtime behavior).
