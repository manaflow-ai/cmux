...
The submodule pinned by this branch is `64d7fca66`, the current
`manaflow-ai/ghostty` `main` merge head. It combines nullable CoreText
display-name handling from https://github.com/manaflow-ai/ghostty/pull/133
with the bounded app-mailbox turns from
https://github.com/manaflow-ai/ghostty/pull/169.
The corresponding universal ReleaseFast GhosttyKit archive is published at
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-64d7fca6623148c5a073febba423094bfdeba68c-crashsubdir-cmux-crash-v1
and pinned in `scripts/ghosttykit-checksums.txt`.

### Nullable CoreText display names

CoreText may return `null` from `CTFontCopyDisplayName`. The old wrapper
converted address zero into a non-optional pointer, so a diagnostic path could
crash while reporting a deferred font failure. The wrapper now preserves
nullability, falls back to the family name, and releases every copied Core
Foundation string after copying it into caller-owned storage. The diagnostic
retains the family, traits, variation count, cache state, and null OS result.

- Commits:
  - `5d6586eb9` (test: preserve nullable CoreText display names)
  - `ff362c99c` (fix: guard null CoreText display names)
  - `a4e8dce18` (docs: clarify deferred font name storage)
- Files:
  - `pkg/macos/text/font.zig`
  - `src/font/DeferredFace.zig`
- Conflict note:
  - Future CoreText wrapper changes must preserve nullability and ownership for
    APIs whose names follow Core Foundation's Copy rule.
