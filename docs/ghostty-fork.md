...
The integrated terminal-backend branch pins `cfdc783f0` from
`manaflow-ai/ghostty` branch `task-pr8378-terminal-backend-main-v2`. That
normal merge contains both protected scene/config head `0e750d5d0` and
current fork-main head `f76c132e5`. The combined pin therefore preserves the
scene-only renderer and config kits together with current VT, font, opener,
and Sentry-disabled artifact behavior.
...
The protected scene lineage starts at `bb30526cd`. It advances the previous cmux pin
...
The current fork-main lineage entering the integration is `f76c132e5`, the fork-main merge of
