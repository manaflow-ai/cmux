...
- **Wave C — State engine + reactivity + input controls.** Native-renderer roadmap phase 2: a host-owned state and binding engine. Unlocks TextField/Toggle/Picker/Slider, write-actions beyond `cmux()`, and author `on(event){…}` handlers. The largest lift; gates ~interactivity.
...
- **Interleave native renderer primitives** from `docs/appkit-sidebar-interpreter-surface.md` (List/Section/LazyVStack/Grid/Label/ProgressView/gradients/overlay-background/styles) so the richer data has richer views to render into.
