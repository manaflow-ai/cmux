...
credentials behind a scoped route handle in the same way.
...
The CodeRouter service already solved this shape for model credentials: a worker
...
4. **Keep model and compute authority separate.** A CodeRouter key remains
   model-only. A composite command uses the authenticated Cloud profile to
   request a separate, short-lived machine lease after team policy passes. The
   lease and the model route may share one request and usage receipt, but they
   have separate scopes, revocation, and audit records. Reusing one bearer for
   both planes would let a model credential create or destroy machines.
