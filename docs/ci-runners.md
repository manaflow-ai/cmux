...
| `MACOS_RUNNER_TESTS` | the manual test-debugging lanes: `test-e2e.yml` and `test-depot.yml` | unset (see "Lanes" below) | `blacksmith-6vcpu-macos-26` |
...
- **Manual test debugging** (`test-e2e.yml`, `test-depot.yml`) resolves through
  ...
  Both fallbacks stay on Blacksmith for that reason, and both are macOS 26:
  the macOS 15 pool's queue-to-start p90 was 83 min against 1.0 min on 26,
  measured over 60 dispatches on 2026-09-22/23, and the nightly compilation-cache
  seed both lanes restore is built there with `CMUX_CI_XCODE_APP_PR`.
