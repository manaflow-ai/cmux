...
| `CI_OWNED_LIGHT_RETRY` | unset (off) | `1` lets attempt 2, the full re-run the rescue starts for a job stuck on a full `std` pool, take the `light` pool when the run's whole owned peak is free there; a job stuck or refused there goes to Blacksmith on attempt 3. Order: std, light, Blacksmith |
