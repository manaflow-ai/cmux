...
| `MACOS_RUNNER_PR_COMPILE` | pull-request `macOS compile admission`, and the DerivedData seed it restores (`seed-derived-data.yml`, `nightly.yml` `refresh-test-compilation-cache`), ahead of `MACOS_RUNNER_PR`. The compile is one CPU-bound build every shard waits on, so it belongs on a large machine while the small pull-request jobs can use GitHub-hosted capacity. Its Xcode is still `CMUX_CI_XCODE_APP_PR`, which the persistent producer also reads, so the pool it names must carry that Xcode | unset (follows `MACOS_RUNNER_PR`) | `MACOS_RUNNER_PR`, then `blacksmith-6vcpu-macos-15` |
