...
The built-in Claude action runs `claude --dangerously-skip-permissions {{prompt}}`; cmux shell-quotes `{{prompt}}` before pasting the command. Built-in Codex, OpenCode, and Pi actions launch the provider first and keep the prompt in the TextBox. Once the provider owns the terminal, submit again to send the prompt through Text Entry.
