...
Repository files are untrusted input. Before cmux runs either script for the first time, it shows the exact setup/archive content and asks whether to run once, trust and run, or cancel. Persistent trust is tied to the canonical work-tree root, project config path, and exact script content. Linked worktrees share repository preferences and prompt dismissals, but each checkout requires its own trust decision; changing either script also requires confirmation again.
