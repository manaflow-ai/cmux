...
## `shortcuts.bindings.newSSHWorkspace`

Controls the shortcut for **File > New SSH Workspace...** and the matching command palette action.

The default is unbound so it does not conflict with New Workspace (`cmd+n`) or New Window (`cmd+shift+n`). To bind it in `~/.config/cmux/cmux.json`:

```jsonc
{
  "shortcuts": {
    "bindings": {
      "newSSHWorkspace": "cmd+shift+s"
    }
  }
}
```
