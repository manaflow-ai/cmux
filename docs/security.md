...
The v1 SCA scope is npm only. The workflow runs `bun install --frozen-lockfile --ignore-scripts` first, then tells Snyk to treat `web/package.json` as an npm project and include devDependencies used by the web build pipeline. Lifecycle scripts stay disabled during install because the later SCA step receives `SNYK_TOKEN`. Until Snyk supports `bun.lock` directly, Snyk resolves dependencies from the installed `web/node_modules` snapshot rather than parsing the Bun lockfile itself.
