...
Feed is cmux's inline surface for AI agent decisions. It is a tool provider in
the shared [presentation system](sidebar-system-design.md), mounted in the
right region on `Ctrl-4`. The keyboard-first OpenTUI Feed can also run in the
separate right-sidebar [Dock](dock.md) after you add a Dock control that runs
`cmux feed tui`. It shows three things that need a human response:
