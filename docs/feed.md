...
| Codex        | `~/.codex/hooks.json`                     | PreToolUse / PermissionRequest / PostToolUse / PreCompact / PostCompact / SubagentStart / SubagentStop telemetry |
...
Codex's hook-level `request_user_input`, `update_plan`, and approval prompts stay in Codex's own TUI/app-server path. cmux records Codex's upstream CLI hook events — `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `SubagentStart`, and `SubagentStop` — as non-blocking telemetry only, each under its own hook event name. `PermissionRequest` in particular stays non-blocking because Codex runs `PermissionRequest` hooks before its `Approve for me` auto-review path; blocking in hook mode would make Codex ask for Feed approval before its own reviewer can decide.
