...
| `right-sidebar set <files\|find\|history\|sessions\|vault\|feed\|dock>` | Show the right sidebar, switch mode, and focus it unless `--no-focus` is passed. |
| `right-sidebar files`, `right-sidebar find`, `right-sidebar history`, `right-sidebar sessions`, `right-sidebar vault`, `right-sidebar feed`, `right-sidebar dock` | Short aliases for `right-sidebar set <mode>` with focus. `vault` and `sessions` remain compatibility aliases for History. |
