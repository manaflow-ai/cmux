...
| `vpn` | The private network to your cloud machines (WireGuard): `up`, `down`, `status`, `revoke`, `hosts`. `up` enrolls this Mac through `vm.tunnel_config`, writes the tunnel config, and brings the interface up with `sudo wg-quick`; `status`/`down`/`revoke` use `vm.tunnel_status` / `vm.tunnel_revoke`; `hosts` writes `<machine>.internal` entries from `vm.list` into a cmux-managed `/etc/hosts` block. Machines have no public ports, so run `cmux vpn up` once per boot before `vm open <m>:port/<n>`. |
...
| `vm workspace open <machine> <workspace-id> [--here] [--tabs] [--workspace <local>] [--pane <id\|ref> [--left\|--right\|--up\|--down]] [--json]` | `vm.workspace_open`: the machine workspace's terminals and browsers as a new local workspace, one pane each (what clicking the sidebar row does). `--here`/`--tabs`/`--pane`+side instead project the group into an existing local workspace (`here: true` + the `surface open` destination params; one pane at the destination, the rest as tabs) — the sidebar's "Open All Here" / "Open All in New Tabs" / drop on a pane edge. `--tabs` and a pane side are two placements and are rejected together. The workspace is resolved from the machine's own list (`ws_…` id or name), so an EMPTY workspace answers `opened: 0, empty: true` and opens nothing (D9: open never creates — the row does the same) instead of failing as not found. An explicit `--workspace`/`--pane` that resolves to nothing is `invalid_params`, never a silent fall-through to the selected workspace. |
...
| `vm link <src> <dst>` | Let machine `<src>` talk to machine `<dst>` from inside (`cmux vm exec\|tree\|<resource> <dst> …` on `<src>`): the Mac mints `<dst>`'s route and a single-use invitation (`vm.cmux_remote_info`), pushes the peer file into `<src>` with `vm.exec`, kicks the link, and approves the enrollment (`vm.cmux_remote_approve`). No control-plane credential enters a machine. |
...
- `cmux vm --help` -> `Usage: cmux vm <base|new|ls|tree|status|stats|rename|snapshot|fork|restore|rm|run|route|agent|prompt|exec|push|pull|wait|shell|tui|desktop|open|link|ports|tools|handoff|promote-template|attach|ssh|ssh-info> [args...]`
- `cmux cloud --help` -> `Usage: cmux cloud <base|new|ls|tree|status|stats|rename|snapshot|fork|restore|rm|run|route|agent|prompt|exec|push|pull|wait|shell|tui|desktop|open|link|ports|tools|handoff|promote-template|attach|ssh|ssh-info> [args...]`
