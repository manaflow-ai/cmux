...
| `workspace-font-size <increase\|decrease\|reset>` | Request the GUI font-size action for all terminal panels in a target workspace. `increase` and `decrease` apply a relative 1pt step; `reset` restores the configured size. Accepts `--workspace <id\|ref\|index>` and `--window <id\|ref\|index>`, with `CMUX_WORKSPACE_ID` as the ambient workspace fallback only when no window is explicit. The request does not change focus; human output confirms acceptance because queued work may defer application. The v2 socket method is `workspace.font_size` with `{ "action": "increase"\|"decrease"\|"reset", "workspace_id"?: UUID, "window_id"?: UUID }`. |
