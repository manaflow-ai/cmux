...
| `vm workspace rename <machine> <workspace-id> --name <name> [--json]` | `vm.workspace_rename`: renames the cmux-tui workspace on the machine (the sidebar's Rename…). The daemon persists the name and broadcasts it to every attached client. |
...
| `vm terminal rename <machine> <terminal-id> --name <name> [--json]` | `vm.terminal_rename`: names the terminal's daemon tab view(s) (the sidebar's Rename…); every client shows the name in place of the PTY title, and the daemon persists it. A zero-view pool terminal has no tab to name. |
