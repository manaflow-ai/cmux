...
| `review` | Run a read-only review or inspect receipts without connecting to the cmux socket. `run --intent <task> [--base <ref>] [--reviewer <executable>]` captures staged, unstaged, and non-ignored untracked content with a temporary Git index, then runs two independent Codex diff reviewers and a challenger for each P0–P2 finding. It disables tools, preserves model claims as inferred, and publishes no receipt if a stage fails. The default base is `origin/main`, falling back to `HEAD`. `list`, `show [<id\|latest>]`, and `findings [<id\|latest>] [--all]` read the same validated ledger; default findings hide refuted/suppressed records. All subcommands accept `--repo <path>` and `--json`. Inspect receipts in the native Reviews tab with `right-sidebar set reviews`, or use the palette command Open Reviews as Pane. |
