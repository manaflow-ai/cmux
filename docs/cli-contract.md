...
| `right-sidebar set <files\|find\|vault\|sessions\|feed>` | Show the right sidebar, switch mode, and focus it unless `--no-focus` is passed. |
| `right-sidebar files`, `right-sidebar find`, `right-sidebar vault`, `right-sidebar sessions`, `right-sidebar feed` | Short aliases for `right-sidebar set <mode>` with focus. |
