...
| `new-workspace` | Create a workspace, optionally with cwd, command, description, color, layout, and per-workspace environment variables (`--env KEY=VALUE` repeatable, `--env-file <path>`). See [Workspace environment variables](#workspace-environment-variables). |
