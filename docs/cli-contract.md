...
| `vm open <id> <port> [--print]` | The port form: open the canonical `<id>/browser/port:<port>` resource through `vm.port_open` in the local workspace already showing that machine (or the caller's workspace when none is attached). The pane loads a loopback forward (`http://127.0.0.1:<local port>`) carried over the user-space WireGuard hub, and the command explains both the requested VM port and the local link (for example, VM `8000` via local `64321`), so it works from any app on the Mac without `cmux vpn up`; a machine without a private address gets the control plane's preview URL instead (the same link the pane loads and Copy Preview Link copies), and a machine with neither route is an error, never an empty `url`. JSON includes `remote_port`, `local_url`, and `local_port`; these local fields are JSON `null` when the control-plane preview route is used. `private_url` carries the machine's private address. `--print` only mints and prints the control plane's URL (`vm.open_port`). Same as `vm open <id>:port/<port>`. A provider without a preview capability answers non-retryable `vm_operation_unsupported`. |
