...
`sidebar validate` is evaluation-only and does not mount SwiftUI/AppKit. The
render artifact is distinct from `extension.sidebar.snapshot`, which reports
sidebar data without producing pixels.
