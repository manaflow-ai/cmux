...
| `browser viewport` | Set minimum CSS viewport size. Width and height must be between 0 and 100000 CSS pixels and within the current pane's emulation range. Successful calls return `handled: true`; `changed` reports whether the stored viewport state or magnification changed. |
