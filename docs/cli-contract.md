...
| `vm tab rename <machine> <tab-id> <name> [--json]` | `vm.tab_rename`: renames one exact cmux-tui tab placement. Use the tab id from `vm tree --json`; the daemon revision fence prevents overwriting a concurrent rename. |
...
| `vm terminal rename <machine> <terminal-id> <name> [--json]` | `vm.terminal_rename`: names the terminal's daemon tab view(s) (the sidebar's Rename…); every client shows the name in place of the PTY title, and the daemon persists it. A zero-view pool terminal has no tab to name. |
| `vm base open`, `vm base reset` | Open (creating on first use) or reset the persistent Base machine. `--base` / `--desktop` choose the kind for the create; when omitted, the control plane selects its active Base image default. An existing Base keeps its image. The app's Cloud VM button shows the Set Up Base sheet only when no Base exists yet. `--no-desktop` is an alias for `--base`. |
| `vm shell`, `vm attach` | Open an interactive shell for an existing Freestyle VM. Every cloud open (`vm shell` / `vm new` / `vm fork` / `vm restore` / `vm base open` / `vm base reset`, the Machines panel, the sidebar cloud button) uses one path and lands a PLAIN terminal on the machine (like an ssh session, not the cmux-tui client): `vm.cmux_remote_info`, then `workspace.create` or, for `--workspace`, `workspace.cloud_vm_terminal_ready`, then `workspace.cloud_vm_bind` (records `vm_id`, `base`, and exact `remote_workspace_id`), then `surface.new_terminal {machine, open: true, workspace_id, focus: true, name: "shell"}`. The provider creates a `bash -l` terminal in its cmux-tui session and the catalog projects it into a pane running `attach --terminal` against the headless link. The `OK` line carries `terminal=<term_…>` and a `Reattach: cmux vm open <m>/<ws>/<term>` hint; `--json` adds `terminal_id`, `remote_workspace_id`, `surface_id`. `cmux vm tui <id>` is the only open that runs the full client. Freestyle's scoped SSH proxy is provider-level access only; managed opens use `cmux-remote` and do not silently fall back to SSH or the legacy WebSocket. An unavailable `cmux-remote` link is an explicit error. |
