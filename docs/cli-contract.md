...
| Command | Contract |
| --- | --- |
| `coderouter accounts` | One table of every account the team routes through, both stores: `KIND` (`codex`, `opencode`, `claude`, `anthropic-key`, `bedrock`), account (label, or masked identifier plus label), `STATE` (state, `cooling down Ns (code)`, or `broken (code)` with the repair command in `USAGE`), `USAGE` (sessions and 5h/weekly windows for subscriptions; region for Bedrock), `ID` (8-char prefix). Alias `list`, `ls`. Supports `--team <id>` and `--json` (`{ teamId, accounts: [{ kind, source, id, label, identifier?, state, details }] }`). |
| `coderouter accounts add [kind]` | Adds one account. `kind` is `claude`, `codex`, `opencode`, `anthropic-key`, or `bedrock` (aliases `oauth-token`, `api-key`, `claude-code`, `--kind <k>`). Without a kind: inferred from `CLAUDE_CODE_OAUTH_TOKEN`, then `ANTHROPIC_API_KEY`, then a secret read from stdin (`--stdin` or a pipe; `sk-ant-oat01-` = claude, `sk-ant-` = anthropic-key), else a numbered picker on stderr in a terminal. Bedrock must be explicit because an AWS access-key id alone is not the credential pair. Same secret intake as `claude add`; for `claude` in a terminal the CLI runs `claude setup-token` itself and keeps the printed token. The backend verifies the credential with its provider before storing (422 `credential_rejected` returns a fixed safe reason; `--no-validate` skips), and an identical secret prints `Already added` without a second row. `codex` and `opencode` hand off to the CodeRouter CLI and accept no account options or JSON output. `--label`, `--region`, and `--model` apply only to the corresponding Claude account kinds. |
...
| `coderouter subscriptions add` with optional provider `codex` or `opencode` | Execs `cr add <provider>` (the CodeRouter CLI owns the sign-in flow). Without `cr` on PATH, exits 127 and prints `npx coderouter@latest add <provider>`. It accepts no account options or JSON output and never touches the socket. |
...
- `cmux coderouter --help` -> `Usage: cmux coderouter <accounts|machines|agent> [options]`
