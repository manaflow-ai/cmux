...
| `vm new`, `vm create` | Create a VM. The default kind on this rollout is `base` (shell-only); `--desktop` requests a screen (TigerVNC + openbox + noVNC on 6901) when the deployment advertises a desktop image, while `--base`/`--no-desktop` explicitly request shell-only. The CLI sends the machine **kind** (`desktop`/`base`) and the backend maps it to the image its deployment supports; `--image <id>` is the explicit override and is the only way an image id leaves the client. Supports `--size <4g\|8g\|16g\|24g\|32g\|64g\|<mb>>`; `vm ls --json` → `limits.memoryOptionsMb` is authoritative for the current plan, and the backend uses its default for a parsed but unavailable request. The selected image supplies the matching CPU and initial disk. Also supports `--name <label>` (display label, applied via `vm.rename` after create), `--provider`, `--workspace`, `--detach`, and `-d`. Without `--detach`, opens a plain terminal on the machine through the shared open path (see `vm shell`). The Machines panel's ＋ opens the New Machine sheet (name, size, plan meter) whose Create runs this same command. |
