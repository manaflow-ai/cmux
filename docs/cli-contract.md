...
| `simulator` | Beta namespace for iOS Simulator panes (requires the `simulator.beta.enabled` toggle in Settings → Beta Features; every verb refuses with guidance while it is off): `list` (devices from `simctl`), `open --device <name\|udid> [--workspace <id\|ref\|index>] [--window <id\|ref\|index>] [--focus true\|false]` (open a live simulator display as a pane; `--focus` defaults to `false`), `close [--surface <id\|ref\|index>] [--workspace <id\|ref\|index>]` (close a simulator pane; without `--surface` the workspace must have exactly one). Socket verbs: `simulator.list`, `simulator.open`, `simulator.close`. cmux boots a shut-down device headlessly and owns its shutdown on pane close; an already-booted device is only attached to and never shut down. |
