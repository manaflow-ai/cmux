...
| `vm workspace close <machine> <workspace-id> [--json]` | `vm.workspace_close`: closes the cmux-tui workspace; its terminals keep running and detach into the Terminals pool (the sidebar's "Close Workspace (Keep Terminals)" and the row's ×). |
| `vm workspace rm <machine> <workspace-id> [--json]` (alias `delete`) | `vm.workspace_delete`: kills every terminal viewed in the workspace, then closes it (the sidebar row's "Delete Workspace and Terminals…"). Prints how many terminals were closed. |
