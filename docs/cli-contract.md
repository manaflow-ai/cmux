...
| `right-sidebar set <files\|find\|vault\|artifacts\|sessions\|feed\|dock\|cloud>` | Show the right sidebar, switch mode, and focus it unless `--no-focus` is passed. Artifacts is available after opting in under Settings → Beta Features. |
| `right-sidebar files`, `right-sidebar find`, `right-sidebar vault`, `right-sidebar artifacts`, `right-sidebar sessions`, `right-sidebar feed`, `right-sidebar dock`, `right-sidebar cloud` | Short aliases for `right-sidebar set <mode>` with focus. `artifacts` is the beta artifact catalog; `cloud` (aliases `machines`, `vms`) is the Cloud machines panel; `mode` reports it as `machines`. |
