...
| `workstream` | Namespace for top-level drill-in workstreams (a level above workspace groups): `list`, `create`, `rename`, `delete`, `add`, `remove`, `move`, `enter`, `exit`. `create` accepts `--name` and `--workspaces <id>,<id>`; `add`/`remove` move a workspace into/out of a workstream; `enter`/`exit` drive the sidebar drill-in. `<workstream>` accepts a UUID or a `workstream:N` ref. See [Workstreams](./workstreams.md). |
