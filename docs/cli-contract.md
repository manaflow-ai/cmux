...
| Command | Contract |
| --- | --- |
| `vm ls`, `vm list` | List VMs. `--json` carries each machine's `capabilities` (`snapshot`, `restore`, `fork`, `ports`, `stats`; a missing flag reads as supported): the sidebar and the CLI show only verbs the provider can honor — no Checkpoint/Fork without `snapshot`/`fork`, no port rows / `vm open <m>:port/<n>` without `ports` (the socket answers `Unsupported: opening ports on <m>…`), no stats probe without `stats`. |
