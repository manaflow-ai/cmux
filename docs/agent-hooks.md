...
If relaunch does not resume an agent, run `cmux sessions --agent <name> --json` to inspect the saved record — session ID, cwd, and the saved launch command — and confirm one exists for that workspace and surface. Then run the agent's resume command yourself, outside cmux, to check it still works. `cmux sessions` reads the hook state files directly, so it also works while cmux is not running; read `~/.cmuxterm/<agent>-hook-sessions.json` yourself only when you need a field the command does not print.
