...
Claude Code is handled by the cmux Claude wrapper when Claude Code integration is enabled in Settings. OpenCode is handled by the bundled `opencode` wrapper inside cmux terminals; it installs the session/status and Feed plugins automatically when a live cmux socket is present. Other agents are installed with:
