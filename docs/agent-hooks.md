...
If relaunch does not resume an agent, run `cmux sessions --agent <name> --json` to print the saved session records and verify the agent's resume command still works outside cmux. The command reads the hook state files directly, so it also works while cmux is not running; read `~/.cmuxterm/<agent>-hook-sessions.json` yourself only when you need a field the command does not print.
