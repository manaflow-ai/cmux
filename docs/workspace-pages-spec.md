...
Implemented in the first branch cut:

1. `Workspace` owns an ordered `pages` list plus an active page selection.
2. Page order, titles, active selection, and page-local layouts persist through session restore.
3. The fake titlebar shows a selectable horizontal page strip with a `+` affordance.
4. Page context menus support duplicate, rename, close, and close other pages.
5. Page switching detaches inactive Ghostty and WKWebView-backed panels from the live hierarchy instead of killing PTYs or browser state.
6. Customizable page shortcuts exist in `KeyboardShortcutSettings`, and the default bindings are wired through app-level shortcut handling.
7. The app menu exposes page create, duplicate, rename, close, close others, move left/right, next/previous, and direct page selection actions.
8. Socket v2 page APIs exist for list/current/create/duplicate/select/rename/close/reorder/next/previous/last.
9. CLI page commands exist for `list-pages`, `new-page`, `duplicate-page`, `current-page`, `select-page`, `rename-page`, `close-page`, `reorder-page`, `next-page`, `previous-page`, and `last-page`.
10. `system.identify` includes focused page identity via `page_id`, `page_ref`, `page_index`, and `page_title`.
11. `system.tree` and `cmux tree` render `workspace -> page -> pane -> surface`, while keeping the selected page mirrored into the legacy workspace-level `panes` field for older consumers.
12. Unit coverage exists for page persistence round-trips, shortcut routing, duplicate-page structure preservation, active-page close-neighbor selection, runtime page detach/reattach identity across switches, and the v2 JSON page/tree path for `page.list`, `page.select`, `page.current`, and `system.tree`.
13. A `tests_v2` regression exists for external CLI and socket page parity across create, select, reorder, current, last, and close flows.

Not implemented yet:

1. The deeper model refactor where each page owns its own `bonsplitController` and live panel map directly.
2. Drag-and-drop page reordering and hover-only page-strip chrome.
3. Option-hold page shortcut hint badges in the titlebar strip.
4. Dedicated UI automation for the titlebar page strip create/select/close flow.
5. CI execution and stabilization for the external page API regression still needs to be kept green on this branch.
