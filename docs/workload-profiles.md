...
- semantic operation: profile ID plus explicit integer generation;
...
| `cmux.macos.dev-check` | 1 | `scripts/ci/workloads/macos-dev-check.sh` | Run the canonical tagged `reload.sh` developer build without launch or global CLI-link changes. |
...
- CMUX semantic validator identity;
...
- `semantic_comparison_key`: exact source tree, profile ID/generation, validator, semantic parameters, and runtime-input content identities;
