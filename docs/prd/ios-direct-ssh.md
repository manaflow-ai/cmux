...
**Deferred:** sockets with hashed names (`cmux-tui-hashed-<uid>`, very long session names) are not listed; cmux-tui topology changes made elsewhere show on the next relist (no `subscribe`); cmux-tui "Split Pane"; SSH browser tabs still sit in the switcher's "Mac Browsers" section; whether `new-screen`/`new-tab` move a laptop's active screen/tab is unverified.
