...
| D42 | **cmux-tui "Split Pane".** A screen section offers New Tab, then Split Pane, which runs `split` (right) on the screen's active pane, like tmux's Split Pane on a window's active pane. The label reuses the localized tmux string. |
