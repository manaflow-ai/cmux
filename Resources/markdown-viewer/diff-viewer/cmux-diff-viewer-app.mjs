const cmuxDiffViewerAssets = readJSONScript("cmux-diff-viewer-assets");
const DIFFS_MODULE_URL = moduleURL(cmuxDiffViewerAssets.diffsModuleURL, "./diffs.mjs");
const TREES_MODULE_URL = moduleURL(cmuxDiffViewerAssets.treesModuleURL, "./trees.mjs");
const WORKER_POOL_MODULE_URL = moduleURL(cmuxDiffViewerAssets.workerPoolModuleURL, "./worker-pool/worker-pool.mjs");
const DIFF_WORKER_URL = moduleURL(cmuxDiffViewerAssets.workerModuleURL, "./worker-pool/worker-portable.mjs");
const payload = readJSONScript("cmux-diff-viewer-payload");
...
function moduleURL(value, fallback) {
  const specifier = typeof value === "string" && value.length > 0 ? value : fallback;
...
const registeredThemeNamesByRegistrar = new WeakMap();
let activeFileId = "";
let activeTreePath = "";
let suppressTreeSelectionChange = false;
let itemIdByTreePath = new Map();
let treePathByItemId = new Map();
const diffHubCommonHighlighterLangs = ["cpp", "css", "go", "python", "rust", "sh", "swift", "tsx", "typescript", "zig"];
...
scheduleRender(() => {
  renderDiff().catch((error) => {
    ...
    showStatusError(label("renderFailed"));
  });
});
...
function ensureStatusAttached() {
  if (status.parentNode !== viewerElement) {
    viewerElement.prepend(status);
  }
}
...
function diffHubWorkerPoolDefaults() {
  const mobilePool = globalThis.navigator?.maxTouchPoints > 0 &&
    globalThis.matchMedia?.("(max-width: 767px), (pointer: coarse)").matches === true;
  const cap = mobilePool
    ? { poolSize: 1, totalASTLRUCacheSize: 10 }
    : { poolSize: 3, totalASTLRUCacheSize: 100 };
  const hardwareConcurrency = Number(globalThis.navigator?.hardwareConcurrency ?? 1);
  const usableConcurrency = Number.isFinite(hardwareConcurrency) ? hardwareConcurrency : 1;
  return {
    poolSize: Math.min(Math.max(1, usableConcurrency - 1), cap.poolSize),
    totalASTLRUCacheSize: cap.totalASTLRUCacheSize,
  };
}
...
function workerHighlighterOptions() {
  return {
    ...
    langs: diffHubCommonHighlighterLangs,
...
function setupToolbar() {
  ...
  jumpSelect.addEventListener("click", openJumpToFile);
...
function openJumpToFile() {
  if (!appState.filesVisible) {
  ...
  const firstItemId = codeViewItems[0]?.id ?? diffItems[0]?.id;
  if (firstItemId) {
...
function focusFileTreeSearch() {
  window.requestAnimationFrame(() => {
    const root = fileTree?.getFileTreeContainer?.() ?? fileList;
    const input = root?.querySelector?.("[data-file-tree-search-container] input, input[type='search'], input");
...
function setupPierreFileTree(source, treesModule) {
  ...
  applyPierreFileTreeHostStyles();
}

function applyPierreFileTreeHostStyles() {
  const container = fileTree?.getFileTreeContainer?.();
  if (!container) {
    return;
  }
  container.style.setProperty("--trees-density-override", "0.8");
  container.style.setProperty("--trees-padding-inline-override", "8px");
  container.style.setProperty("--trees-git-renamed-color-override", "light-dark(#007aff, #007aff)");
}
...
function setupJumpSelector(items) {
  ...
  jumpSelect.textContent = label("jumpToFile");
  jumpSelect.dataset.initialized = "true";
  jumpSelect.hidden = items.length === 0;
  updateJumpButtonLabel(activeFileId);
}
...
function appendJumpOptions(items) {
  ...
  updateJumpButtonLabel(activeFileId || items[0]?.id || "");
}

function renameJumpOption(oldId, newId) {
  if (activeFileId !== oldId || jumpSelect.dataset.initialized !== "true") {
    return;
  }
  updateJumpButtonLabel(newId);
}
...
function updateActiveFile(itemId) {
  ...
  updateJumpButtonLabel(itemId);
}

function updateJumpButtonLabel(itemId) {
  if (jumpSelect.dataset.initialized !== "true") {
    return;
  }
  const text = itemId ? itemLabelForId(itemId) : "";
  const title = text || label("jumpToFile");
  jumpSelect.textContent = title;
  jumpSelect.title = title;
  jumpSelect.setAttribute("aria-label", title === label("jumpToFile")
    ? label("jumpToFile")
    : `${label("jumpToFile")}: ${title}`);
}

function itemLabelForId(itemId) {
  const treePath = treePathByItemId.get(itemId);
  if (treePath) {
    return treePath;
  }
  const item = diffItemById.get(itemId);
  if (item?.fileDiff) {
...
function registerGhosttyTheme(registerCustomTheme, theme) {
  let registeredThemeNames = registeredThemeNamesByRegistrar.get(registerCustomTheme);
  if (!registeredThemeNames) {
    registeredThemeNames = new Set();
    registeredThemeNamesByRegistrar.set(registerCustomTheme, registeredThemeNames);
  }
