...
Switch sentry-native from Breakpad backend to `inproc` backend on all Darwin builds (macOS, iOS). The `inproc` backend does not call `thread_get_state()` on other threads, avoiding the Rosetta 2 conflict.
