        ...
        {!query ? (
          <div className="model-picker-rail" role="tablist" aria-label="Harnesses">
            <div className="model-picker-rail-top">
              {installed.map((provider, i) => <button aria-label={provider.label} key={provider.id} className={"rail-btn" + (i === 0 ? " active" : "")}><ProviderIcon provider={provider} /></button>)}
            </div>
            <div className="model-picker-rail-bottom">
              {missing.map((provider) => <button aria-label={provider.label} key={provider.id} className="rail-btn missing"><ProviderIcon provider={provider} /></button>)}
            </div>
          </div>
        ) : null}
        <div className="model-picker-main">
          <div className="model-picker-search"><span>⌕</span><input aria-label="Search models" readOnly value={query} placeholder="Search models..." /></div>
