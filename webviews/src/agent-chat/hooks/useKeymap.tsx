...
export function ShortcutOverlay({ provider, options, running, ctrlJ, onClose }: { provider: string; options: SessionOption[]; running: boolean; ctrlJ: CtrlJMode; onClose: () => void }) {
  ...
  return (
    <div className="shortcut-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <dialog open className="shortcut-panel" aria-label={`${provider} shortcuts`}>
