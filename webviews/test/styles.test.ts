...
test("toolbar and files pane use theme surfaces", () => {
  ...
  expect(styles).toMatch(/#review-tabs\s*\{[^}]*border-bottom: 1px solid color-mix\(in lab, var\(--cmux-diff-fg\) 8%, transparent\)/s);
  expect(styles).toMatch(/#toolbar-main\s*\{[^}]*padding: 4px 6px 5px 10px;/s);
