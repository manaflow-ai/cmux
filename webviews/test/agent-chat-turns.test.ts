import type { Block } from "../src/agent-chat/session";
import { activityRowLabel, groupTurns, summarizeTurnActivity } from "../src/agent-chat/turns";
import { measureVirtualRow, measureVirtualRowFromResize, scrollCompensationDelta, virtualFirstVisibleIndex, virtualRange } from "../src/agent-chat/hooks/useVirtualTurns";

(globalThis as any).location ??= { pathname: "/" };
const { disclosureHeightKeyframes, disclosureShouldRender, disclosureSnapshotStyle } = await import("../src/agent-chat/components/Transcript");
