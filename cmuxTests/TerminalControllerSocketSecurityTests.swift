...
final class TerminalControllerSocketSecurityTests {
    ...
    @Test func testPanelScopedSidebarMutationsRouteMovedSurfaceToCurrentWorkspace() throws {
        let previousAppDelegate = AppDelegate.shared
        let appDelegate = AppDelegate()
        defer { AppDelegate.shared = previousAppDelegate }
        ...
        let panel = moved.panel.id.uuidString
        ...
        TerminalMutationBus.shared.drainForTesting()
        XCTAssertNil(moved.source.statusEntries["codex"])
        XCTAssertEqual(moved.destination.statusEntries["codex"]?.value, "Running")
        XCTAssertEqual(moved.destination.agentPIDs["codex"].map(Int.init), 111)
        ...
        TerminalMutationBus.shared.drainForTesting()
        XCTAssertNil(moved.source.statusEntries["build"])
        XCTAssertEqual(moved.destination.statusEntries["build"]?.value, "compiling")
        ...
        TerminalMutationBus.shared.drainForTesting()
        XCTAssertNil(moved.source.agentPIDs["codex.session"])
        XCTAssertEqual(moved.destination.agentPIDs["codex.session"].map(Int.init), 222)
        ...
        TerminalMutationBus.shared.drainForTesting()
        XCTAssertNil(moved.source.agentLifecycleStatesByPanelId[moved.panel.id]?["codex"])
        XCTAssertEqual(moved.destination.agentLifecycleStatesByPanelId[moved.panel.id]?["codex"], .running)
        ...
        TerminalMutationBus.shared.drainForTesting()
        XCTAssertNil(moved.destination.agentPIDs["codex.session"])
        ...
        TerminalMutationBus.shared.drainForTesting()
        XCTAssertNil(moved.destination.statusEntries["build"])
    }
