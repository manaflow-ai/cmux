...
@Suite struct RemoteTmuxWindowMirrorSizingTests {
    ...
    @Test func singlePaneComposesToItsRenderedGrid() throws {
    ...
    @Test func sideBySideSplitReportsHeaderCorrectedHeight() throws {
    ...
    @Test func sideBySideSplitTakesMaxHeight() throws {
        let node = try #require(RemoteTmuxRawLayoutParser.parse("1a2b,148x85,0,0{74x85,0,0,0,73x85,75,0,1}"))
    ...
    /// header-corrected.
    @Test func stackedSplitSumsHeaderCorrectedHeights() throws {
    ...
    @Test func composingClaimedGeometryReproducesRoot() throws {
        // Nested: { pane4 60x40 | [ pane5 59x20 / pane8 59x19 ] 59x40 }  — the parser's
        // own doc example. (59+1+59? no: 60 + 1 + 59 = 120 across; 20 + 1 + 19 = 40 down.)
    ...
    @Test func notYetLivePanePropagatesNil() throws {
        let node = try #require(
            RemoteTmuxRawLayoutParser.parse("f92f,120x40,0,0{60x40,0,0,4,59x40,61,0[59x20,61,0,5,59x19,61,21,8]}")
        )
