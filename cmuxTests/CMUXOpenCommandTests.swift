...
final class CMUXOpenCommandTests: XCTestCase {
    ...
    func testDiffCommandGeneratesCodeViewAndOpensBrowserSplit() throws {
        ...
        XCTAssertTrue(html.contains("const patchResponsePromise = fetch(payload.patchURL"), html)
        XCTAssertTrue(html.contains("const workerPoolReadyPromise = initializeCodeViewWorkerPool()"), html)
        XCTAssertTrue(html.contains("setupCodeViewIfReady()"), html)
