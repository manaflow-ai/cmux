...
final class CMUXOpenCommandTests: XCTestCase {
    ...
    func testDiffCommandGeneratesCodeViewAndOpensBrowserSplit() throws {
        ...
        let emacsUp = try XCTUnwrap(shortcuts["diffViewerScrollUpEmacs"] as? [String: Any])
        let emacsUpFirst = try XCTUnwrap(emacsUp["first"] as? [String: Any])
        XCTAssertEqual(emacsUpFirst["key"] as? String, "p")
        XCTAssertEqual(emacsUpFirst["control"] as? Bool, true)
