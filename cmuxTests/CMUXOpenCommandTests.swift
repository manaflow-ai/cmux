...
final class CMUXOpenCommandTests: XCTestCase {
    ...
    func testViewCommandUsesFileOpenEndpoint() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("view-open")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let fileURL = rootURL.appendingPathComponent("config.yaml")
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
        try "key: value\n".write(to: fileURL, atomically: true, encoding: .utf8)
        let state = MockSocketServerState()
    ...
    func testViewCommandForwardsExplicitFocusFalse() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("view-focus")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let fileURL = rootURL.appendingPathComponent("notes.txt")
        ...
        let result = runCLI(
            cliPath: cliPath,
            socketPath: socketPath,
            arguments: ["view", "open", fileURL.path, "--focus", "false"]
        )
