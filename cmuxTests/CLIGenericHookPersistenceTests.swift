...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    func testOpenCodeStopSuppressesNotificationFromRawHookPayload() throws {
        ...
        let environment: [String: String] = [
            ...
            "CMUX_AGENT_LAUNCH_EXECUTABLE": "/Users/example/.bun/bin/opencode",
            "CMUX_AGENT_LAUNCH_ARGV_B64": base64NULSeparated(["/Users/example/.bun/bin/opencode"]),
        ...
        let result = runProcess(
            ...
            arguments: ["hooks", "opencode", "stop"],
            environment: environment,
            standardInput: #"{"session_id":"opencode-session-raw-suppress","cwd":"\#(workspace.path)","suppress_notification":true}"#,
            timeout: 5
        )
        ...
        XCTAssertFalse(
            ...
            "Expected raw suppress_notification to suppress legacy notification, saw \(state.commands)"
        )
        XCTAssertTrue(
            state.commands.contains { $0.hasPrefix("set_status opencode ") },
            "Expected OpenCode stop to keep status updates while suppressing only notification, saw \(state.commands)"
        )
    }
