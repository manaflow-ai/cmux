...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    /// `{"decision":"allow|deny","reason":"..."}` on stdout (like Antigravity).
    /// Before the fix, `grok` fell through to `nonClaudePreToolDecision`, which
    /// emits Claude-shaped `permissionDecision` JSON with `decision`
    /// `approve`/`block` — tokens Grok does not recognize, so a sidebar
    /// approve/deny was silently dropped. Guards
    /// https://github.com/manaflow-ai/cmux/issues/6303. Uses the Cursor-style
    /// capitalized `Write` so the event is actionable regardless of the tool
    /// alias change — this isolates the decision-format fix.
    func testGrokFeedResolvedDecisionUsesGrokNativeFormat() throws {
        func runGrokDecision(mode: String) throws -> ProcessRunResult {
            let cliPath = try bundledCLIPath()
            let socketPath = makeSocketPath("grok-feed-decision")
            let listenerFD = try bindUnixSocket(at: socketPath)
            let state = MockSocketServerState()
            let root = FileManager.default.temporaryDirectory
        ...
        for mode in ["once", "always", "all", "bypass"] {
            ...
            XCTAssertFalse(result.timedOut, "\(mode): \(result.stderr)")
            XCTAssertEqual(result.status, 0, "mode \(mode) should exit 0: \(result.stderr)")
            XCTAssertEqual(
                result.stdout,
                "{\"decision\":\"allow\",\"reason\":\"User approved via cmux Feed.\"}\n",
                "mode \(mode) should emit Grok-native allow decision: \(result.stdout)"
            )
        }
        ...
        XCTAssertEqual(
            deny.stdout,
            "{\"decision\":\"deny\",\"reason\":\"User denied permission via cmux Feed.\"}\n",
            "deny should emit Grok-native deny decision: \(deny.stdout)"
        )
    }

    /// Grok-native lowercase tool names (`write`, `run_terminal_cmd`,
    /// `search_replace`, …) must escalate to a blocking Feed approval through
    /// the real CLI — `wait_timeout_seconds > 0` and a `PermissionRequest` wire
    /// event. Before the fix only the Cursor-style capitalized names escalated.
    /// Companion to `testLowercaseGenericFeedToolsStayTelemetryOutsideKiro`
    /// (which proves the negative for non-grok agents). Guards
    /// https://github.com/manaflow-ai/cmux/issues/6303.
    func testGrokLowercaseSideEffectingToolEscalatesToBlockingFeed() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("grok-lowercase-feed-tool")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            XCTAssertEqual(method, "feed.push")
            // Acknowledge without resolving so the CLI returns promptly (no
            // real 120s wait) while still recording the actionable push.
            return self.v2Response(id: id, ok: true, result: ["status": "acknowledged"])
        }
        ...
        XCTAssertEqual(feedPushes.count, 1, "Expected one Grok Feed event, saw \(state.commands)")
        let event = try XCTUnwrap(feedPushes.first?["event"] as? [String: Any])
        let waitTimeout = try XCTUnwrap(feedPushes.first?["wait_timeout_seconds"] as? NSNumber)
        XCTAssertEqual(event["hook_event_name"] as? String, "PermissionRequest")
        XCTAssertEqual(event["_source"] as? String, "grok")
        XCTAssertEqual(event["tool_name"] as? String, "run_terminal_cmd")
        XCTAssertGreaterThan(waitTimeout.doubleValue, 0, "Grok side-effecting tool must block for a Feed decision")
    }
