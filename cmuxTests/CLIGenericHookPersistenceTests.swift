...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    /// https://github.com/manaflow-ai/cmux/issues/5676: when a remote/no-TTY Codex hook inherits a
    /// different live session's CMUX_SURFACE_ID, there is no process binding to correct the leak. The
    /// hook must decline the whole route rather than publishing a resume binding, PID, or lifecycle for
    /// the newcomer onto the owner's pane.
    func testCodexHookWithoutTTYDoesNotRouteOntoDifferentLiveSurfaceOwner() throws {
        let cliPath = try bundledCLIPath()
        ...
        for ownerRuntimeStatus in ["running", "needsInput"] {
            ...
            let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
                ...
                switch method {
                case "surface.list":
                    return self.surfaceListResponse(id: id, surfaceId: ownerSurfaceId)
                case "debug.terminals":
            ...
            XCTAssertFalse(
                ...
                "no-TTY hook for \(ownerRuntimeStatus) owner must not publish newcomer resume binding: \(commands)"
            )
            XCTAssertFalse(
                commands.contains { $0.hasPrefix("set_agent_pid codex.\(newcomerSessionId) ") },
                "no-TTY hook for \(ownerRuntimeStatus) owner must not route newcomer PID: \(commands)"
            )
            XCTAssertFalse(
                commands.contains { $0.hasPrefix("set_agent_lifecycle codex running ") },
                "no-TTY hook for \(ownerRuntimeStatus) owner must not route newcomer lifecycle: \(commands)"
            )

            let storeURL = root.appendingPathComponent("codex-hook-sessions.json")
            let storeJSON = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: storeURL)) as? [String: Any])
            let sessions = try XCTUnwrap(storeJSON["sessions"] as? [String: Any])
            XCTAssertNotNil(sessions[ownerSessionId] as? [String: Any])
            XCTAssertNil(sessions[newcomerSessionId] as? [String: Any])
        }
    }
