...
struct FileSearchAutomationQueryTests {
    ...
    @Test("Focus restoration does not replay the one-shot automation query")
    func focusRestorationPreservesTheUserEditedQuery() throws {
        let store = FileExplorerStore()
        let state = FileExplorerState()
        let searchController = FileSearchAutomationSearchControllerSpy()
        let coordinator = FileExplorerPanelView.Coordinator(
        ...
        store.applyWorkspaceRoot(.local(
            workspaceId: UUID(),
            path: "/tmp/cmux-find-restored-query-test"
        ))
        container.updateHeader(store: store)
        container.updatePresentation(.find)
        ...
        controller.registerFileExplorerHost(container)

        #expect(controller.focusFileSearch(initialQuery: "static needle"))
        let searchField = try #require(Self.findSearchField(in: container))
        #expect(searchField.stringValue == "static needle")
        searchField.stringValue = "user edit"

        _ = window.makeFirstResponder(nil)
        #expect(controller.restoreTargetAfterWindowBecameKey())
        #expect(searchField.stringValue == "user edit")
    }
