...
struct CLICopilotHookFeedTests {
    ...
    @Test func copilotHookInstallWritesToHooksSubdirectory() throws {
        ...
        let cmuxPreToolUseEntry = try #require(
            preservedPreToolUse.first {
                ($0["bash"] as? String)?.contains("hooks feed --source copilot --event preToolUse") == true
                    ...
                    && ($0["timeoutSec"] as? Int) == 125 && ($0["matcher"] as? String) == FeedEventClassifier.copilotPreToolUseApprovalToolMatcher
                    && $0["command"] == nil
                    && $0["hooks"] == nil
            },
        ...
        #expect(
            permissionRequest.count == 1
                && (permissionRequest.first?["bash"] as? String)?.contains("\"behavior\":\"deny\"") == true,
            "Expected existing permissionRequest policy hook to be preserved without cmux permissionRequest hooks, saw \(permissionRequest)"
        )
    }

    @Test func copilotFeedDecisionEmitsCopilotPermissionDecisions() throws {
        func runCopilotDecision(
            mode: String,
        ...
        let (timeout, _) = try runCopilotDecision(mode: "once", status: "timeout")
        #expect(!timeout.timedOut, Comment(rawValue: timeout.stderr))
        #expect(timeout.status == 0, Comment(rawValue: timeout.stderr))
        let timeoutOutput = try #require(Self.jsonObject(timeout.stdout))
