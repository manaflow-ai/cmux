...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    func testSurfaceResumeSetCLIRejectsUnknownFlagWithoutReplacingBinding() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = makeSocketPath("resume-set-unknown")
        let listenerFD = try bindUnixSocket(at: socketPath)
        let state = MockSocketServerState()
        defer {
            Darwin.close(listenerFD)
        ...
        let serverHandled = startMockServer(
            ...
            case "surface.resume.set":
                ...
                return self.v2Response(
                    id: id,
                    ok: true,
                    result: ["resume_binding": ["command": params["command"] as? String ?? ""]]
                )
            case "surface.resume.get":
                let latestSetCommand = state.snapshot().reversed().compactMap { command -> String? in
                ...
                return self.v2Response(
                    id: id,
                    ok: true,
                    result: ["resume_binding": ["command": latestSetCommand ?? approvedCommand]]
                )
            default:
                return self.v2Response(
                    id: id,
        ...
        XCTAssertFalse(before.timedOut, before.stderr)
        XCTAssertEqual(before.status, 0, before.stderr)
        let commandBefore = try XCTUnwrap(
            (jsonPayload(from: before.stdout)["resume_binding"] as? [String: Any])?["command"] as? String
        )
        XCTAssertEqual(commandBefore, approvedCommand)
        ...
        XCTAssertFalse(rejected.timedOut, rejected.stderr)
        XCTAssertNotEqual(rejected.status, 0, rejected.stderr)
        XCTAssertTrue(rejected.stdout.isEmpty, rejected.stdout)
        XCTAssertTrue(rejected.stderr.contains("surface resume set: unknown flag '--bad-flag'"), rejected.stderr)
        XCTAssertTrue(rejected.stderr.contains("Known flags:"), rejected.stderr)
        XCTAssertTrue(rejected.stderr.contains("--workspace"), rejected.stderr)
        XCTAssertTrue(rejected.stderr.contains("--shell"), rejected.stderr)
        ...
        wait(for: [serverHandled], timeout: 5)
        XCTAssertFalse(after.timedOut, after.stderr)
        XCTAssertEqual(after.status, 0, after.stderr)
        let commandAfter = try XCTUnwrap(
            (jsonPayload(from: after.stdout)["resume_binding"] as? [String: Any])?["command"] as? String
        )
        XCTAssertEqual(commandAfter, commandBefore)
        XCTAssertFalse(
            state.snapshot().contains { command in
                self.jsonObject(command)?["method"] as? String == "surface.resume.set"
            },
            "An unknown flag must be rejected before a binding mutation request is sent"
        )
    }
