...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    func testClaudeSessionStartOverridesLeakedEnvSurfaceWithTTYBinding() throws {
        ...
        let request = try XCTUnwrap(
            ...
            "Expected Claude SessionStart to publish a resume binding, saw \(context.state.commands)"
        )
        XCTAssertEqual(
        ...
        XCTAssertTrue(
            ...
            "Claude visible status should also target the TTY surface, saw \(context.state.commands)"
        )
    }

    func testClaudeExplicitSurfaceOverridesMappedSessionAndTTYBinding() throws {
        let context = try makeClaudeHookContext(name: "claude-explicit-surface")
        defer { context.cleanup() }
        ...
        let ttyName = "ttys-claude-explicit-surface"
        let sessionId = "claude-explicit-surface-session"
            ...
            case "surface.list":
                return self.v2Response(
                    ...
                    result: ["surfaces": [
                        ...
                        ["id": explicitSurfaceId, "ref": "surface:2", "focused": false],
                        ["id": ttySurfaceId, "ref": "surface:3", "focused": false],
                    ]]
                )
            case "debug.terminals":
                return self.v2Response(
        ...
        let result = runProcess(
            executablePath: context.cliPath,
            arguments: ["hooks", "claude", "prompt-submit", "--surface", explicitSurfaceId],
            environment: environment,
        ...
        let request = try XCTUnwrap(
            resumeBindingRequests.last,
            "Expected explicit Claude surface to publish a resume binding, saw \(context.state.commands)"
        )
        XCTAssertEqual(
            request["surface_id"] as? String,
            explicitSurfaceId,
            "Explicit --surface must beat both mapped session state and TTY binding; params=\(request)"
        )
        XCTAssertTrue(
            context.state.commands.contains {
                $0.hasPrefix("set_status claude_code Running ")
                    && $0.contains("--panel=\(explicitSurfaceId)")
            },
            "Claude visible status should target the explicit surface, saw \(context.state.commands)"
        )
    }
    ...
    func testClaudeInvalidExplicitSurfaceFallsBackToMappedSession() throws {
        ...
        let ttySurfaceId = "44444444-4444-4444-4444-444444444444"
        let ttyName = "ttys-claude-invalid-explicit-surface"
        let sessionId = "claude-invalid-explicit-surface-session"
        let storeURL = context.root.appendingPathComponent("claude-hook-sessions.json")
        let store: [String: Any] = [
            "version": 1,
            "sessions": [
        ...
        let result = runProcess(
            executablePath: context.cliPath,
            arguments: ["hooks", "claude", "prompt-submit", "--surface", staleExplicitSurfaceId],
            environment: environment,
            standardInput: #"{"session_id":"\#(sessionId)","turn_id":"turn-1","cwd":"\#(context.root.path)","hook_event_name":"UserPromptSubmit"}"#,
            timeout: 5
        )
        ...
        XCTAssertEqual(
            request["surface_id"] as? String,
            mappedSurfaceId,
            "Invalid explicit --surface should fall back to the mapped session, not TTY/default; params=\(request)"
        )
        XCTAssertTrue(
            context.state.commands.contains {
                $0.hasPrefix("set_status claude_code Running ")
            ...
            "Claude visible status should target the mapped surface after invalid explicit fallback, saw \(context.state.commands)"
        )
    }
