...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    func testClaudeSessionEndFeedTelemetryDefaultsMissingRawPayloadMatcher() throws {
        let context = try makeClaudeHookContext(name: "claude-session-end-feed-default-matcher")
        defer { context.cleanup() }

        let sessionId = context.surfaceId
        let start = runClaudeHook(
            context: context,
            arguments: ["hooks", "claude", "session-start"],
        ...
        let commandCountBeforeSessionEnd = context.state.commands.count
        let result = runClaudeHook(
            context: context,
            arguments: ["hooks", "claude", "session-end"],
            standardInput: "not json"
        )

        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK\n")
        let feedEvents = context.state.commands.dropFirst(commandCountBeforeSessionEnd).compactMap { command -> [String: Any]? in
            guard let payload = jsonObject(command),
                  payload["method"] as? String == "feed.push",
                  let params = payload["params"] as? [String: Any],
                  let event = params["event"] as? [String: Any] else {
        ...
        XCTAssertEqual(event["session_id"] as? String, "claude-\(sessionId)")
        XCTAssertEqual(event["hook_event_name"] as? String, "SessionEnd")
        XCTAssertEqual(event["matcher"] as? String, "other")
        XCTAssertEqual(event["_source"] as? String, "claude")
    }
