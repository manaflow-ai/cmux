...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    private func runClaudeNotificationHook(message: String) throws -> MockSocketServerState {
        let context = try makeClaudeHookContext(name: "claude-note-\(message)")
        defer { context.cleanup() }

        let result = runClaudeHook(
            context: context,
            arguments: ["claude-hook", "notification"],
            standardInput: #"{"session_id":"claude-notification-session","cwd":"\#(context.root.path)","hook_event_name":"Notification","message":"\#(message)"}"#
        )
