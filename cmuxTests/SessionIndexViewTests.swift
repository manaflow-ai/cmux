...
final class SessionIndexViewTests: XCTestCase {
    func testLargeTextSelectionPolicyRejectsManyLineSwiftUITextSelection() {
        let text = (0..<50_000)
            .map { "row \($0)" }
            .joined(separator: "\n")
        ...
        XCTAssertEqual(
            LargeTextSelectionPolicy.mode(
                ...
                maximumLineFragments: 2_000
            ),
            .copyOnly
        )
    }

    func testLargeTextSelectionPolicyKeepsBoundedTextSelectable() {
        let text = (0..<100)
            .map { "row \($0)" }
            .joined(separator: "\n")
