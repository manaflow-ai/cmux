...
struct SessionIndexViewTests {
    ...
    func testClaudeLocalCommandCaveatDisplaysReadableTitle() {
        ...
        #expect(
            entry.displayTitle == String(localized: "sessionIndex.localCommand", defaultValue: "Local command")
        )
    }
    ...
    func testClaudeResumeCommandPinsSnapshotConfigDirectory() throws {
        ...
        #expect(
            entry.resumeCommand == posixShWrappedForTest("env CLAUDE_CONFIG_DIR='\(configDir.path)' CMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1 CMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR \"$([ -x \"${CMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$CMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" --resume claude-session-123")
        )
    }
    ...
    func testGrokResumeCommandPreservesSpecifics() {
        ...
        #expect(
            entry.resumeCommand == "'env' 'GROK_HOME=/tmp/grok home' 'grok' '-r' 'grok-session-123' '-m' 'grok-4' '--permission-mode' 'auto' '--sandbox' 'danger-full-access'"
        )
    }
