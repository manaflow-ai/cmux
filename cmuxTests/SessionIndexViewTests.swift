...
struct SessionIndexViewTests {
    @Test func testClaudeLocalCommandCaveatDisplaysReadableTitle() {
        let entry = makeEntry(
        ...
        #expect(entry.displayTitle == String(localized: "sessionIndex.localCommand", defaultValue: "Local command"))
    }

    @Test func testClaudeSlashCommandMarkupDisplaysReadableTitle() {
        let entry = makeEntry(
            title: """
    ...
    @Test func testClaudeSlashCommandMarkupIncludesDistinctMessage() {
        let entry = makeEntry(
            title: """
            <command-name>/plugin</command-name>
    ...
    @Test func testClaudeMetaSlashCommandMarkupDoesNotBecomeTitle() {
        #expect(
    ...
    @Test func testClaudeResumeCommandPinsSnapshotConfigDirectory() throws {
        ...
        #expect(entry.resumeCommand == posixShWrappedForTest("env CLAUDE_CONFIG_DIR='\(configDir.path)' CMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV=1 CMUX_PRESERVE_CLAUDE_AUTH_SELECTION_ENV_KEYS=CLAUDE_CONFIG_DIR \"$([ -x \"${CMUX_CLAUDE_WRAPPER_SHIM:-}\" ] && printf '%s' \"$CMUX_CLAUDE_WRAPPER_SHIM\" || printf claude)\" --resume claude-session-123"))
    }

    @Test func testClaudeResumeCommandUsesSnapshotConfigDirectoryWithNestedProjectsPath() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("cmux-session-index-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func testGrokResumeCommandPreservesSpecifics() {
        let entry = makeEntry(
            agent: .grok,
        ...
        #expect(entry.resumeCommand == "'env' 'GROK_HOME=/tmp/grok home' 'grok' '-r' 'grok-session-123' '-m' 'grok-4' '--permission-mode' 'auto' '--sandbox' 'danger-full-access'")
    }
    ...
    @Test func testCodexResumeCommandReproducesBypassFlagForDisabledSandbox() {
        let entry = makeEntry(
            agent: .codex,
        ...
        #expect(inner == "\(AgentResumeArgv.codexWrapperShellExecutableToken) resume codex-session-123 -c check_for_update_on_startup=false -m gpt-5.5 --dangerously-bypass-approvals-and-sandbox -c model_reasoning_effort=high")
        #expect(
    ...
    @Test func testCodexResumeCommandDropsUnsupportedManagedSandbox() {
        let entry = makeEntry(
            agent: .codex,
        ...
        let command = entry.resumeCommand ?? ""
        let inner = Self.unwrapPortableShellCommand(command)
        #expect(inner == "\(AgentResumeArgv.codexWrapperShellExecutableToken) resume codex-session-managed -c check_for_update_on_startup=false -a on-request")
        #expect(
    ...
    @Test func testCodexResumeCommandPreservesValidSandboxModes() {
        ...
        #expect(Self.unwrapPortableShellCommand(readOnly.resumeCommand ?? "") == "\(AgentResumeArgv.codexWrapperShellExecutableToken) resume codex-ro -c check_for_update_on_startup=false -a untrusted -s read-only")
        ...
        #expect(Self.unwrapPortableShellCommand(dangerFullAccess.resumeCommand ?? "") == "\(AgentResumeArgv.codexWrapperShellExecutableToken) resume codex-dfa -c check_for_update_on_startup=false -m gpt-5.5 -a never -s danger-full-access")
    }
    ...
    @Test func testCurrentDirectorySetterDoesNotPublishEqualValue() {
    ...
    @Test func testDirectoryOrderBackfillUsesLatestModifiedForDuplicateDirectories() {
    ...
    @Test func testDirectoryOrderBackfillUsesPathTieBreakForEqualModifiedDates() {
        preservingSessionIndexDefaults {
            let store = SessionIndexStore()
    ...
    @Test func testAgentOrderBackfillUsesLatestModifiedForDuplicateAgents() {
        preservingSessionIndexDefaults {
            let store = SessionIndexStore()
    ...
    @Test func testAgentOrderBackfillUsesAgentIdTieBreakForEqualModifiedDates() {
        preservingSessionIndexDefaults {
            let store = SessionIndexStore()
            let sameModified = Date(timeIntervalSince1970: 10)
            store.agentOrder = []
    ...
    @Test func testCodexSQLSearchMatchesRolloutTranscriptContent() async throws {
    ...
    @Test func testDirectorySectionOffersShowMoreEvenWhenUnderRowLimit() {
    ...
    @Test func testNoFolderDirectorySectionOffersShowMore() {
        let section = IndexSection(
            key: .directory(nil),
            title: "(no folder)",
            icon: .folder,
    ...
    @Test func testAgentSectionUsesRowCountThresholdForShowMore() {
    ...
    @Test func testPinnedSessionSortsToTopOfSection() {
        withEphemeralStore { store in
            ...
            #expect(section?.entries.map(\.title) == ["c", "a", "b"])
            #expect(section?.pinnedEntryIDs == Set([c.id]))
        }
    }

    @Test func testArchivedSessionHiddenUntilShowArchived() {
        withEphemeralStore { store in
            let a = makeEntry(title: "a", cwd: "/proj", modified: Date(timeIntervalSince1970: 3))
            let b = makeEntry(title: "b", cwd: "/proj", modified: Date(timeIntervalSince1970: 2))
            ...
            #expect(section?.entries.map(\.title) == ["a"])
            #expect(section?.archivedEntryIDs == Set<String>())

            store.showArchived = true

            section = store.sectionsForCurrentGrouping().first { $0.key == .directory("/proj") }
            #expect(section?.entries.map(\.title) == ["a", "b"])
            #expect(section?.archivedEntryIDs == Set([b.id]))
        }
    }

    @Test func testArchivedSortsLastEvenWhenPinned() {
        withEphemeralStore { store in
            let a = makeEntry(title: "a", cwd: "/proj", modified: Date(timeIntervalSince1970: 3))
            let b = makeEntry(title: "b", cwd: "/proj", modified: Date(timeIntervalSince1970: 2))
            store.replaceEntriesForTesting([a, b])
    ...
    @Test func testHasArchivedSessionsReflectsLoadedEntries() {
        withEphemeralStore { store in
            let a = makeEntry(title: "a", cwd: "/proj")
            store.replaceEntriesForTesting([a])

            #expect(!store.hasArchivedSessions)
    ...
    @Test func testPinArchiveStatePersistsAcrossStoreInstances() {
        ...
        #expect(reloaded.isPinned("claude:/p/x"))
        #expect(reloaded.isArchived("codex:/p/y"))
        #expect(reloaded.showArchived)
        #expect(!reloaded.isPinned("claude:/p/z"))
    }

    @Test func testUnpinAndUnarchiveClearState() {
        withEphemeralStore { store in
            let a = makeEntry(title: "a", cwd: "/proj")
            store.replaceEntriesForTesting([a])
            ...
            #expect(store.isArchived(a.id))
            store.toggleArchived(a)
            #expect(!store.isArchived(a.id))
        }
    }

    @Test func testSectionPopoverHostCoordinatorSkipsHiddenRefreshes() {
        ...
        #expect(coordinator.debugRefreshContentCallCount == 0)
    }

    @Test func testSectionPopoverHostCoordinatorRefreshesOnceWhenPresented() {
        ...
        #expect(coordinator.debugIsPopoverShown)
        #expect(coordinator.debugRefreshContentCallCount == 1)
