...
extension SocketListenerAcceptPolicyTests {
    func testHermesAgentResumeCommandPreservesTUIAndHermesHome() {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/hermes repo' 2>/dev/null && 'env' 'HERMES_HOME=/tmp/hermes home' '/opt/homebrew/bin/hermes' '--tui' '--model' 'gpt-5.4' '--resume' 'hermes-session-123'"
        )
    }
    ...
    func testHermesAgentResumeCommandRewritesStaleCodexProvider() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/hermes repo' 2>/dev/null && '/opt/homebrew/bin/hermes' '--provider' 'custom' '--model' 'gpt-5.5' '--resume' 'hermes-session-123'"
        )
    }
    ...
    func testHermesAgentResumeCommandPreservesSubrouterBaseURLs() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/hermes repo' 2>/dev/null && 'env' 'CUSTOM_BASE_URL=http://subrouter-team:31415/v1' 'HERMES_CODEX_BASE_URL=http://subrouter-team:31415/backend-api/codex' 'HERMES_HOME=/tmp/hermes home' '/opt/homebrew/bin/hermes' '--provider' 'anthropic' '--model' 'anthropic/claude-sonnet-4.6' '--resume' 'hermes-session-123'"
        )
    }
