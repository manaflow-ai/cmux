...
@MainActor
@Suite
struct WorkspaceTemplateParameterPromptTests {
    @Test
    func parameterFormHasAVisibleResolvedSize() throws {
        ...
        let handled = WorkspaceTemplateParameterPrompt(
            processEnvironment: [:]
        ).requestParameters(
            for: definition,
        ...
        #expect(grid.frame.width >= 300)
        #expect(grid.frame.height > 0)
        #expect(fields.count == 2)
        #expect(fields.allSatisfy { $0.frame.width >= 300 })
