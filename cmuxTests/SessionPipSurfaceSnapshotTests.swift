...
final class SessionPipSurfaceSnapshotTests: XCTestCase {
    ...
    func testRestoringPipSurfaceCapsWindowsBeforeMergingDetachedSurface() throws {
        let panelId = UUID()
        let homeWorkspaceId = UUID()
        let survivingWindows = (0..<SessionPersistencePolicy.maxWindowsPerSnapshot).map { _ in
            Self.makeWindow(workspaceIds: [UUID()], selectedWorkspaceIndex: 0)
        }
        let unreachableWindow = Self.makeWindow(workspaceIds: [homeWorkspaceId], selectedWorkspaceIndex: 0)
        let snapshot = AppSessionSnapshot(
            version: SessionSnapshotSchema.currentVersion,
            createdAt: Date().timeIntervalSince1970,
            windows: survivingWindows + [unreachableWindow],
            pipSurfaces: [
                SessionPipSurfaceSnapshot(
                    panel: Self.pipTerminalPanelSnapshot(id: panelId),
                    frame: SessionRectSnapshot(x: 100, y: 120, width: 480, height: 320),
        ...
        XCTAssertEqual(restored.windows.count, SessionPersistencePolicy.maxWindowsPerSnapshot)
        let survivingWorkspace = try XCTUnwrap(restored.windows.first?.tabManager.workspaces.first)
        XCTAssertTrue(survivingWorkspace.panels.contains(where: { $0.id == panelId }))
    }
