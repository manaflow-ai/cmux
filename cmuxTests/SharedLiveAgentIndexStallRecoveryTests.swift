...
struct SharedLiveAgentIndexStallRecoveryTests {
    ...
    func completedWorkspaceRefreshRateLimitsPendingHookChange() async {
        ...
        let sharedIndex = SharedLiveAgentIndex(
            indexLoader: {
                ...
                return Self.loadResult(sessionId: "rate-limited-drain-\(invocation)")
            },
    ...
    func twoStalledGenerationsDiscardLateResultsWithoutLaunchingThird() async {
        ...
        releaseFirst.signal()
        #expect(await SharedLiveAgentIndexLoadCoalescingTests.wait(for: firstCompleted))
        #expect(await Self.waitForPhysicalLoadsToFinish(in: sharedIndex))
        #expect(loadState.withLock { $0.maximum } == 2)
        #expect(
            sharedIndex.cachedIndex() == nil,
            "A timed-out generation must not publish its late, stale-at-capture result."
        )
        await timeoutWaiter.cancelAll()
    }

    private static func waitForPhysicalLoadsToFinish(
        ...
        while clock.now < deadline {
            if sharedIndex.refreshGenerationsByID.isEmpty {
                return true
            }
            try? await clock.sleep(for: .milliseconds(10))
        }
        return sharedIndex.refreshGenerationsByID.isEmpty
    }
