...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    ...
    func testFishIntegrationSourcesUserConfigAndReportsShellState() throws {
        ...
        try fileManager.createDirectory(
            at: userFishConfig.deletingLastPathComponent(),
            withIntermediateDirectories: true
        )
        defer { try? fileManager.removeItem(at: root) }
        ...
        let result = try runInteractiveFish(
            ...
            """,
            extraEnvironment: [
                "CMUX_FISH_CONFIG_HOME": userConfigHome.path,
        ...
        XCTAssertTrue(result.stdout.contains("USER_CONFIG=1"), result.stdout)
        XCTAssertTrue(
    ...
    private struct FishProcessRunResult {
        let status: Int32
        let stdout: String
        let stderr: String
        let timedOut: Bool
    ...
    private func requireFishExecutable() throws -> String {
        ...
        guard let fish = candidates.first(where: { FileManager.default.isExecutableFile(atPath: $0) }) else {
    ...
    private func runInteractiveFish(
        ...
        let testIntegrationDir = root.appendingPathComponent("shell-integration", isDirectory: true)
        ...
        var environment: [String: String] = [
            ...
            "USER": NSUserName(),
            "XDG_CONFIG_HOME": testIntegrationDir.path,
    ...
    private func runFishProcess(
        executablePath: String,
        arguments: [String],
        environment: [String: String],
        timeout: TimeInterval
    ) -> FishProcessRunResult {
        let process = Process()
        let stdoutPipe = Pipe()
        let stderrPipe = Pipe()
