...
final class AppDelegateShortcutRoutingTests: XCTestCase {
    ...
    func testCmdShiftReturnFocusedBrowserTogglesSplitZoom() {
        withTemporaryShortcut(action: .toggleSplitZoom) {
            ...
            defer { browserPanel.webView.removeFromSuperview() }
            ...
            guard let event = makeKeyDownEvent(
                ...
                XCTFail("Failed to construct Cmd+Shift+Return event")
                return
            }

            XCTAssertTrue(
                KeyboardShortcutSettings.shortcut(for: .toggleSplitZoom).matches(event: event),
                "The configured pane-zoom shortcut should match Cmd+Shift+Return"
            )

#if DEBUG
            XCTAssertTrue(
                appDelegate.debugHandleShortcutMonitorEvent(event: event),
                "The local shortcut monitor path should recognize Cmd+Shift+Return for a focused browser panel"
            )
            XCTAssertTrue(
                workspace.bonsplitController.isSplitZoomed,
                "The local shortcut monitor path should toggle browser pane zoom"
            )
            ...
            XCTAssertTrue(
                ...
                "Focused browser Return key equivalents should route configured app shortcuts through the same shortcut handler as the local monitor"
            )
            XCTAssertTrue(
                workspace.bonsplitController.isSplitZoomed,
                "Cmd+Shift+Return should toggle pane zoom for the focused browser panel"
            )
        }
    }
