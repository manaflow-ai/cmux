...
final class AppDelegateShortcutRoutingTests: XCTestCase {
    ...
    func testMarkAllNotificationsReadShortcutUsesNotificationStorePrimitive() {
        guard let appDelegate = AppDelegate.shared else {
            XCTFail("Expected AppDelegate.shared")
            return
        }
        ...
        withTemporaryShortcut(action: .markAllNotificationsRead, shortcut: shortcut) {
            guard let event = makeKeyDownEvent(
                key: "r",
                ...
                XCTFail("Failed to construct mark-all-read shortcut event")
                return
            }
    ...
    func testClearAllNotificationsShortcutUsesNotificationStorePrimitive() {
        guard let appDelegate = AppDelegate.shared else {
            XCTFail("Expected AppDelegate.shared")
            return
        }
        ...
        withTemporaryShortcut(action: .clearAllNotifications, shortcut: shortcut) {
            guard let event = makeKeyDownEvent(
                key: "z",
                modifiers: shortcut.modifierFlags,
                keyCode: 6,
                ...
                XCTFail("Failed to construct clear-all-notifications shortcut event")
                return
            }
