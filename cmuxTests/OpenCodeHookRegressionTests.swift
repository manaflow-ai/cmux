...
@Suite(.serialized) struct OpenCodeHookRegressionTests {
    ...
    @Test func testOpenCodeInstallHooksIsIdempotentForLegacySetupAlias() throws {
        ...
        #expect(feedSource.contains("cmux-feed-plugin-marker"))

        let json = try #require(JSONSerialization.jsonObject(with: try Data(contentsOf: configURL), options: []) as? [String: Any])
        let plugin = try #require(json["plugin"] as? [String])
        #expect(plugin == ["other-plugin", "./plugins/cmux-session.js"])
    }
    ...
    @Test func testOpenCodeInstallPreservesFileReferencesWithoutSlashEscaping() throws {
        let cliPath = try bundledCLIPath()
        ...
        let parsed = try #require(JSONSerialization.jsonObject(with: try Data(contentsOf: configURL), options: []) as? [String: Any])
        let agent = try #require(parsed["agent"] as? [String: Any])
        let myagent = try #require(agent["myagent"] as? [String: Any])
        #expect((myagent["prompt"] as? String) == "{file:./AGENTS.md}")
        #expect((parsed["instructions"] as? [String]) == ["./AGENTS.md"])
        let plugin = try #require(parsed["plugin"] as? [String])
        #expect(plugin == ["./plugins/cmux-session.js"])
    }
