...
final class GhosttySurfaceOverlayTests: XCTestCase {
    ...
    func testHiddenTerminalRestoresReviewScrollPositionAfterClipViewReset() {
        let window = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 360, height: 240),
            styleMask: [.titled, .closable],
            backing: .buffered,
        ...
        XCTAssertEqual(
            ...
            500,
            accuracy: 0.01,
            "Switching back to a hidden terminal should restore the review row instead of leaving the clip view at the bottom"
        )
    }
