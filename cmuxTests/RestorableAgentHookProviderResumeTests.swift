...
extension SocketListenerAcceptPolicyTests {
    func testGeminiResumeCommandPreservesSafeFlagsAndDropsSessionSelectors() {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/gemini repo' 2>/dev/null && 'env' 'GEMINI_CLI_HOME=/tmp/gemini home' '/Users/example/.bun/bin/gemini' '--resume' '5839bed1-0a60-4c05-b6d1-2410d7a3741e' '--model' 'gemini-2.5-pro' '--sandbox' 'danger-full-access' '--approval-mode' 'yolo'"
        )
    }
    ...
    func testAntigravityResumeCommandUsesConversationAndDropsStartupSelectors() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/antigravity repo' 2>/dev/null && 'env' 'GEMINI_CLI_HOME=/tmp/gemini home' '/Users/example/.local/bin/agy' '--conversation' 'antigravity-conversation-123' '--sandbox' 'danger-full-access' '--add-dir' '/tmp/extra repo'"
        )
    }
    ...
    func testRovoDevResumeCommandUsesRestoreAndPreservesYolo() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/rovo repo' 2>/dev/null && 'env' 'CMUX_ROVODEV_SESSIONS_DIR=/tmp/rovo sessions' '/opt/homebrew/bin/acli' 'rovodev' 'run' '--restore' 'session with space' '--yolo'"
        )
    }
    ...
    func testCursorResumeCommandDropsCapturedNodeRuntimeFlags() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '~/.cursor' 2>/dev/null && '/usr/local/bin/agent' '--resume' '019dad34-d218-7943-b81a-eddac5c87951' '--model' 'gpt-5.4'"
        )
    }
    ...
    func testAdditionalHookAgentResumeCommandsUseVerifiedCLIResumeFlags() {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/cursor repo' 2>/dev/null && '/Users/example/.local/bin/cursor-agent' '--resume' 'cursor-chat-123' '--model' 'gpt-5.4' '--sandbox' 'enabled'"
        )
        XCTAssertEqual(
            copilot.resumeCommand,
            "cd -- '/tmp/copilot repo' 2>/dev/null && 'env' 'COPILOT_HOME=/tmp/copilot home' '/tmp/cmux-agent-upstreams/copilot-install/bin/copilot' '--resume' 'copilot-session-123' '--model' 'gpt-5.4' '--allow-all-tools'"
        )
        XCTAssertEqual(
            codeBuddy.resumeCommand,
            "cd -- '/tmp/codebuddy repo' 2>/dev/null && 'env' 'CODEBUDDY_CONFIG_DIR=/tmp/codebuddy config' '/Users/example/.npm/bin/codebuddy' '--resume' 'codebuddy-session-123' '--model' 'gpt-5.4' '--permission-mode' 'plan'"
        )
        XCTAssertEqual(
            factory.resumeCommand,
            "cd -- '/tmp/factory repo' 2>/dev/null && '/Users/example/.npm/bin/droid' '--resume' 'factory-session-123' '--append-system-prompt' 'be terse'"
        )
        XCTAssertEqual(
            qoder.resumeCommand,
            "cd -- '/tmp/qoder repo' 2>/dev/null && 'env' 'QODER_CONFIG_DIR=/tmp/qoder config' '/Users/example/.npm/bin/qodercli' '--resume' 'qoder-session-123' '--model' 'gemini-2.5-pro' '--permission-mode' 'plan'"
        )
        XCTAssertEqual(
            kiro.resumeCommand,
            "cd -- '/tmp/kiro repo' 2>/dev/null && 'env' 'KIRO_HOME=/tmp/kiro home' '/Users/example/.cargo/bin/kiro-cli' 'chat' '--resume-id' 'kiro-session-123' '--agent' 'cmux' '--trust-tools' 'fs_read,fs_write'"
        )
        XCTAssertEqual(
            grok.resumeCommand,
            "cd -- '/tmp/grok repo' 2>/dev/null && 'env' 'GROK_HOME=/tmp/grok home' '/Users/example/.grok/bin/grok' '-r' 'grok-session-123' '--model' 'grok-4' '--permission-mode' 'auto'"
        )
        XCTAssertEqual(pi.resumeCommand, "cd -- '/tmp/pi repo' 2>/dev/null && 'env' 'PI_CODING_AGENT_DIR=/tmp/pi home' '/Users/example/.bun/bin/pi' '--session' 'pi-session-123' '--model' 'anthropic/claude-sonnet-4-5' '--thinking' 'high'")
        XCTAssertEqual(
            amp.resumeCommand,
            "cd -- '/tmp/amp repo' 2>/dev/null && 'env' 'AMP_SETTINGS_FILE=/tmp/amp-settings.json' '/Users/example/.local/bin/amp' 'threads' 'continue' '--mode' 'smart' '--effort' 'high' 'T-019e032c-c31a-77a9-ad87-8298ec47029f'"
        )
    }
