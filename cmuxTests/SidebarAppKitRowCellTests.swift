...
struct SidebarAppKitRowCellTests {
    ...
    @Test
    func appKitWorkspaceRenamePresentsThroughOwningWindowRoute() {
        ...
        commands.promptRename(
            resolvePresentingWindow: { workspaceId in
                resolvedWorkspaceId = workspaceId
                return owningWindow
            },
            presentAlert: { alert, input, window in
                presentationCount += 1
                presentedWindow = window
                #expect(alert.accessoryView === input)
                return .alertSecondButtonReturn
            }
        )

        #expect(resolvedWorkspaceId == workspace.id)
        #expect(presentedWindow === owningWindow)
