...
struct CloudTreeCreateAffordanceTests {
    ...
    func emptyMachineShowsWorkspaceCreateRow() throws {
        ...
        #expect(row.searchableTitle == CloudTreeCreateAction.workspace(machine: machine, name: "Big Machine").rowTitle)
    }
    ...
    func selectedWorkspaceGetsTerminalCreateRow() throws {
        ...
        #expect(row.searchableTitle == CloudTreeCreateAction.terminal(machine: machine, workspaceID: main.id, name: main.name).rowTitle)
        #expect(tree.allSatisfy { $0.id != "machine:brave-otter/ws/ws_side/create-terminal" })
    }

    @Test("Every workspaces group ends with the same visible create row")
    func nonEmptyMachineStillShowsWorkspaceCreateRow() throws {
        let group = try #require(rows(workspaces: [workspace("ws_main", "main", index: 0)]).first { $0.structureTag == "workspacesGroup" })
        #expect(group.children.last?.id == "machine:brave-otter/workspaces/create")
        #expect(group.children.last?.searchableTitle == CloudTreeCreateAction.workspace(machine: machine, name: "Big Machine").rowTitle)
    }

    @Test("The header model names only destinations available from the selection")
    func headerModelUsesExplicitVocabulary() {
        let model = CloudTreeCreateMenuModel(selection: .workspace(machine: machine, workspaceID: "ws_main", workspaceName: "main"), machineName: { _ in "Big Machine" })
        #expect(model.actions == [
            .machine,
            .workspace(machine: machine, name: "Big Machine"),
            .terminal(machine: machine, workspaceID: "ws_main", name: "main")
        ])
    }

    @Test("Header create actions invoke the shared node action closures")
    func headerActionsShareMutationPath() {
        var workspaces: [SurfaceMachineID] = []
        var terminals: [(SurfaceMachineID, String?)] = []
        let actions = CloudTreeNodeActions(
            ...
            newTerminal: { terminals.append(($0, $1)) }, openGroup: { _, _, _, _ in },
            openGroupAsWorkspace: { _, _, _ in }, newWorkspace: { workspaces.append($0) },
            closeTerminal: { _ in }, closeWorkspace: { _, _ in }, renameWorkspace: { _, _ in },
            renameTerminal: { _, _ in }, selectLocalWorkspace: { _ in }, copyToPasteboard: { _ in },
            copyPortLink: { _ in }, refresh: {}
        )
        let model = CloudTreeCreateMenuModel(selection: .workspace(machine: machine, workspaceID: "ws_main", workspaceName: "main"), machineName: { _ in "Big Machine" })
        model.actions[1].perform(newMachine: {}, nodeActions: actions)
        model.actions[2].perform(newMachine: {}, nodeActions: actions)
        #expect(workspaces == [machine])
        #expect(terminals.count == 1)
        #expect(terminals.first?.0 == machine)
        #expect(terminals.first?.1 == "ws_main")
    }

    @Test("Creation gates allow local and known machines but deny unknown or expired ones")
    func creationAvailabilityIsExplicit() {
        var expired = fleetRow()
        expired.freeAccess = .expired
        #expect(CloudTreeCreationAvailability.resolve(machine: .local, machines: []) == .allowed)
        #expect(CloudTreeCreationAvailability.resolve(machine: machine, machines: [fleetRow()]) == .allowed)
        #expect(CloudTreeCreationAvailability.resolve(machine: machine, machines: [expired]) == .expired)
        #expect(CloudTreeCreationAvailability.resolve(machine: .cloud("unknown"), machines: [fleetRow()]) == .unknown)
    }
    ...
    func groupCreateMenusUseDisplayName() throws {
        ...
        let outline = try #require(coordinator.outlineView)
        let workspaceTitle = CloudTreeCreateAction.workspace(machine: machine, name: "Big Machine").title
        let terminalTitle = CloudTreeCreateAction.terminal(machine: machine, workspaceID: nil, name: "Big Machine").title
    ...
    @Test("A valid destination survives a rebuild under a collapsed parent")
    func collapsedParentPreservesCreateDestination() async throws {
        var selection: CloudTreeCreateSelection?
        let coordinator = makeCoordinator { selection = $0 }
        let container = CloudTreeContainerView(coordinator: coordinator)
        defer { withExtendedLifetime(container) {} }
        ...
        coordinator.apply(nodes: [try #require(rows(workspaces: [workspace("ws_main", "renamed", index: 0)]).first)])
        await coordinator.pendingSelectionPublication?.value
        #expect(selection == .workspace(machine: machine, workspaceID: main.id, workspaceName: "renamed"))
    }
...
@MainActor
@Suite("Cloud tree operation errors")
struct CloudTreeOperationErrorStateTests {
    @Test("A failure is dismissible without a refresh")
    func failureCanBeDismissedIntentionally() throws {
        let state = CloudTreeOperationErrorState()
        state.report("cmux-tui did not report the new terminal: term_123")
        let failure = try #require(state.failure)
        #expect(failure.message.contains("term_123"))
        state.dismiss(failure.id)
        #expect(state.failure == nil)
    }

    @Test("A stale dismiss action cannot hide a newer failure")
    func staleDismissPreservesCurrentFailure() throws {
        let state = CloudTreeOperationErrorState()
        state.report("first")
        let first = try #require(state.failure)
        state.report("second")
        state.dismiss(first.id)
        #expect(state.failure?.message == "second")
    }
}
