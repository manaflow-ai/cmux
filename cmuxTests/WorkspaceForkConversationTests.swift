...
final class WorkspaceForkConversationTests: XCTestCase {
    ...
    func testForkConversationFocusedWorkspaceActionCreatesSiblingTab() throws {
        let workspace = Workspace()
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let sourcePaneId = try XCTUnwrap(workspace.paneId(forPanelId: sourcePanelId))
        workspace.setRestoredAgentSnapshotForTesting(makeForkableClaudeSnapshot(), panelId: sourcePanelId)

        XCTAssertTrue(workspace.canForkFocusedAgentConversation())
        XCTAssertTrue(workspace.forkFocusedAgentConversation(destination: .newTab))
        XCTAssertEqual(
            workspace.bonsplitController.tabs(inPane: sourcePaneId).count,
            2,
            "The workspace-row fork action should spawn a sibling tab through the shared fork path"
        )
        XCTAssertEqual(
            workspace.bonsplitController.allPaneIds.count,
            1,
            "The workspace-row New Tab destination should not create a split pane"
        )
    }
