...
final class CommandPaletteConfigActionMappingTests: XCTestCase {
    ...
    func testBuiltInActionsMapToTheirExistingPaletteEntries() {
    ...
    func testUnrelatedPaletteEntryHasNoBuiltInConfigMapping() {
        XCTAssertNil(ContentView.commandPaletteBuiltInConfigActionID(for: "palette.openSettings"))
    }

    func testConfiguredBuiltInFocusArgumentIsOptionalAndAutomationDefaultsTrue() {
        XCTAssertEqual(ContentView.commandPaletteOptionalFocusArguments, [
        ...
        XCTAssertNil(ContentView.commandPaletteResolvedFocus(explicit: nil, source: .commandPalette))
        XCTAssertEqual(
    ...
    func testConfiguredActionTargetRequirementsAreDerivedFromActionType() {
        XCTAssertEqual(
            configAction(
                id: "terminal.current",
                target: .currentTerminal
            ).paletteTargetRequirement,
            .terminalPanel
        )
        XCTAssertEqual(
            configAction(id: "terminal.newTab").paletteTargetRequirement,
            .panelInPane
        )
        XCTAssertEqual(
            configAction(
    ...
    func testNewTabVisibilityRequiresFocusedPanelInLivePane() {
        var context = CommandPaletteContextSnapshot()
        XCTAssertFalse(ContentView.commandPalettePanelInPaneIsAvailable(context))
        ...
        context.setBool(CommandPaletteContextKeys.hasFocusedPanel, true)
        XCTAssertTrue(ContentView.commandPalettePanelInPaneIsAvailable(context))
    }

    func testConfigCompositionFiltersOnlyExactLiveIDCollisions() {
        ...
        XCTAssertEqual(composition.actions.map(\.id), ["palette.openSettings.extra"])
        XCTAssertEqual(composition.issues.count, 1)
        XCTAssertEqual(composition.issues.first?.kind, .paletteActionIDCollision)
        XCTAssertEqual(composition.issues.first?.commandName, "palette.openSettings")
        XCTAssertEqual(
            composition.issues.first?.sourcePath,
            "/project-a/.cmux/cmux.json"
        )
    }
}
...
final class CommandPaletteCloudCommandTests: XCTestCase {
    ...
    func testCurrentCloudVMCommandsRequireAnExactManagedCloudWorkspace() {
        ...
        XCTAssertEqual(visibleCloudCommandIds(context), windowLevelCommandIds)
