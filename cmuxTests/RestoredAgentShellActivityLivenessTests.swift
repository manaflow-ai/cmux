...
struct RestoredAgentShellActivityLivenessTests {
    ...
    @Test
    func workspaceKeepsBindingWhenCompletedIndexHasNoMatchingSession() throws {
        let workspace = Workspace()
        defer { workspace.teardownAllPanels() }
        let panelId = try #require(workspace.focusedPanelId)
        ...
        // A completed scan with no matching entry is still only absence of
        // evidence: the hook may have written this session after the scan.
        // Preserve automatic ownership until a scan observes the binding's
        // process and proves that it exited.
        #expect(workspace.surfaceResumeBinding(panelId: panelId)?.allowsAutomaticResume == true)
    }
