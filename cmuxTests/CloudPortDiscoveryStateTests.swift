...
struct CloudPortDiscoveryStateTests {
    ...
    func demandAndCompletion() {
        ...
        #expect(discovery.complete(CloudPortScanResult(ports: [3000]), request: request, at: now, socketPath: "first"))
        #expect(discovery.state == .available)
        let failed = discovery.beginScan()
        #expect(discovery.complete(nil, request: failed, at: now, socketPath: "first"))
        #expect(discovery.state == .stale && discovery.scan?.ports == [3000])
        let retry = discovery.beginScan()
        #expect(discovery.complete(CloudPortScanResult(ports: []), request: retry, at: now, socketPath: "first"))
    ...
    func staleScanCannotPublish() {
        ...
        discovery.reconcile(supportsPreviews: true, isAwake: true, privateAddress: nil)
        #expect(!discovery.complete(CloudPortScanResult(ports: [3000]), request: stale, at: now, socketPath: "old"))
        ...
        #expect(!discovery.complete(CloudPortScanResult(ports: [8000]), request: retired, at: now, socketPath: "new"))
    }
    ...
    func cacheScope() {
        ...
        #expect(discovery.cachedScan(at: now.addingTimeInterval(20), socketPath: "first", force: false)?.ports == [3000])
        #expect(discovery.cachedScan(at: now.addingTimeInterval(31), socketPath: "first", force: false) == nil)
        #expect(discovery.cachedScan(at: now, socketPath: "second", force: false) == nil)
        #expect(discovery.cachedScan(at: now, socketPath: "first", force: true) == nil)
    }
