...
final class BrowserHTTPBasicAuthPromptTests: XCTestCase {
    ...
    func testBasicAuthPromptConfirmReturnsSessionCredential() {
        ...
        alertSpy.beforeResponding = { [weak self] accessoryView in
            ...
            guard fields.count >= 2 else {
                XCTFail("expected 2 editable text fields, got \(fields.count)")
                return
            }
    ...
    func testBasicAuthPromptSanitizesHostAndRealmText() {
        let challenge = makeAuthChallenge(
        ...
        XCTAssertTrue(handled)
        XCTAssertEqual(alertSpy.runModalCallCount, 1)
        XCTAssertEqual(promptMessage?.contains("basic-auth.test"), true)
        XCTAssertEqual(promptMessage?.contains("EnableIT"), true)
    ...
    func testBasicAuthPromptShowsPreviousFailureMessage() {
        ...
        XCTAssertTrue(handled)
        XCTAssertEqual(alertSpy.runModalCallCount, 1)
        XCTAssertEqual(promptMessage?.contains(failureMessage), true)
        XCTAssertEqual(promptMessage?.contains("basic-auth.test"), true)
    }
    ...
    func testProxyBasicAuthChallengeDoesNotPrompt() {
        let challenge = makeAuthChallenge(
            method: NSURLAuthenticationMethodHTTPBasic,
            isProxy: true
        )
