...
struct CLICodexHookTimeoutRegressionTests {
    ...
    @Test func codexInjectedHookScriptPathSurvivesShellEvaluationWhenHomeContainsShellCharacters() throws {
        ...
        for event in lifecycleEvents {
            let command = try injectedCodexHookCommand(
    ...
    private func injectedCodexHookCommand(eventName: String, output: String) throws -> String {
        let eventPrefix = "hooks.\(eventName)="
        let argument = try #require(
            output.split(separator: "\0").map(String.init).first { $0.hasPrefix(eventPrefix) },
            "missing injected \(eventName) argument"
        )
        let commandMarker = try #require(
            argument.range(of: "command='''"),
            "missing command field in injected \(eventName) argument"
        )
        let commandBody = argument[commandMarker.upperBound...]
        let commandEnd = try #require(
            commandBody.range(of: "''',timeout="),
            "missing command terminator in injected \(eventName) argument"
        )
