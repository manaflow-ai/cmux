...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    func testCodexHookMonitorSetsErrorStatusFromCompletedTranscriptWithoutAssistant() throws {
        ...
        XCTAssertTrue(
            state.commands.contains { command in
                command.contains("notify_target \(workspaceId) \(surfaceId) Codex|Error|Codex ended before sending a final response|c=other;p=0;a=codex")
            },
    ...
    func testCodexHookMonitorNotifiesOnRequestUserInput() throws {
        ...
        XCTAssertTrue(
            waitForSocketCommand(state: state, timeout: 5) { command in
                command.contains("notify_target \(workspaceId) \(surfaceId) Codex|Waiting|Which demo path should I use?|c=other;p=0;a=codex")
            },
