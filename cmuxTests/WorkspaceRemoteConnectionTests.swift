...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    func testCodexPromptSubmitRebindsRestoredSessionToCurrentCallerSurface() throws {
        ...
        let hookInput = """
        {"session_id":"\(sessionId)","cwd":"\(root.path)","hook_event_name":"UserPromptSubmit","prompt":"continue"}
        """
        let result = runProcess(
            executablePath: cliPath,
        ...
        XCTAssertTrue(
            state.commands.contains { command in
                command.contains("set_status codex Running") &&
                    command.contains("--tab=\(currentWorkspaceId)")
            },
