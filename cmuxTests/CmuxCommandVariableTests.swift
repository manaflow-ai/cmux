...
@Suite struct CmuxCommandFolderTests {
    ...
    @Test func surfaceTabBarDropsAmbiguousWorkspaceCommandNameFallback() throws {
        ...
        #expect(store.surfaceTabBarButtons.map(\.id) == ["newTerminal"])
    }

    @Test func workspaceCommandActionNameFallbackFailsClosedWhenAmbiguous() {
