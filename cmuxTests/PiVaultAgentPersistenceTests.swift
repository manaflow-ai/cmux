...
final class PiVaultAgentPersistenceTests: XCTestCase {
    ...
    func testBuiltInAntigravityRegistrationLoadsHistoryDisplayAndWorkspace() async throws {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/antigravity repo' 2>/dev/null && 'agy' '--conversation' 'antigravity-conversation-123'"
        )
    }
    ...
    func testBuiltInAntigravityRegistrationIndexesEachHistoryConversation() async throws {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/antigravity repo' 2>/dev/null && 'agy' '--conversation' 'conversation-b'"
        )
    }
    ...
    func testRegisteredAgentJSONLSessionIDDoesNotUseAntigravityConversationID() async throws {
        ...
        let entry = try XCTUnwrap(entries.first)
        XCTAssertEqual(entry.sessionId, "native-session-123")
        XCTAssertEqual(entry.copyResumeCommand, "cd -- '/tmp/acme' 2>/dev/null && 'acme-agent' '--session' 'native-session-123'")
    }
    ...
    func testRegisteredAgentTemplatePreservesCWDArgumentWithWorkingDirectoryPrefix() {
        ...
        XCTAssertEqual(
            command,
            "cd -- '/tmp/acme' 2>/dev/null && 'acme-agent' '--cwd' '/tmp/acme' '--session' 'session-123'"
        )
    }
    ...
    func testGrokVaultLoadsNativeChatHistoryFromEncodedDirectory() async throws {
        ...
        XCTAssertEqual(
            entry.copyResumeCommand,
            "cd -- '/tmp/grok repo' 2>/dev/null && 'env' 'GROK_HOME=\(grokHome.path)' 'grok' '-r' 'grok-session-123' '-m' 'grok-4' '--permission-mode' 'auto' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testGrokVaultLoadsHookObservedShellGrokHome() async throws {
        ...
        XCTAssertEqual(
            entry.copyResumeCommand,
            "cd -- '/tmp/grok observed home' 2>/dev/null && 'env' 'GROK_HOME=\(grokHome.path)' 'grok' '-r' '\(sessionId)' '-m' 'grok-4' '--permission-mode' 'auto' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testGrokVaultLoadsHookObservedShellGrokHomeFromCustomStateDir() async throws {
        ...
        XCTAssertEqual(
            entry.copyResumeCommand,
            "cd -- '/tmp/grok custom state' 2>/dev/null && 'env' 'GROK_HOME=\(grokHome.path)' 'grok' '-r' '\(sessionId)' '-m' 'grok-4'"
        )
    }
    ...
    func testRegisteredGrokSessionDirectoryUsesNativeDirectoryLayout() async throws {
        ...
        XCTAssertEqual(
            entry.copyResumeCommand,
            "cd -- '/tmp/custom grok repo' 2>/dev/null && 'env' 'GROK_HOME=\(tempDir.path)' 'custom-grok' '-r' '\(sessionId)'"
        )
    }
