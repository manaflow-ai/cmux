...
final class PiVaultAgentPersistenceTests: XCTestCase {
    ...
    func testBuiltInAntigravityRegistrationLoadsHistoryDisplayAndWorkspace() async throws {
        ...
        XCTAssertEqual(
            ...
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'agy' '--conversation' 'antigravity-conversation-123'",
    ...
    func testBuiltInAntigravityRegistrationIndexesEachHistoryConversation() async throws {
        ...
        XCTAssertEqual(
            ...
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'agy' '--conversation' 'conversation-b'",
                workingDirectory: "/tmp/antigravity repo"
            )
        )
    }
    ...
    func testRegisteredAgentJSONLSessionIDDoesNotUseAntigravityConversationID() async throws {
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'acme-agent' '--session' 'native-session-123'",
    ...
    func testRegisteredAgentTemplatePreservesCWDArgumentWithWorkingDirectoryPrefix() {
        ...
        XCTAssertEqual(
            command,
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'acme-agent' '--cwd' '/tmp/acme' '--session' 'session-123'",
                workingDirectory: "/tmp/acme"
            )
        )
    }
    ...
    func testGrokVaultLoadsNativeChatHistoryFromEncodedDirectory() async throws {
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'env' 'GROK_HOME=\(grokHome.path)' 'grok' '-r' 'grok-session-123' '-m' 'grok-4' '--permission-mode' 'auto' '--sandbox' 'danger-full-access'",
    ...
    func testGrokVaultLoadsHookObservedShellGrokHome() async throws {
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'env' 'GROK_HOME=\(grokHome.path)' 'grok' '-r' '\(sessionId)' '-m' 'grok-4' '--permission-mode' 'auto' '--sandbox' 'danger-full-access'",
                workingDirectory: "/tmp/grok observed home"
            )
        )
    }
    ...
    func testGrokVaultLoadsHookObservedShellGrokHomeFromCustomStateDir() async throws {
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'env' 'GROK_HOME=\(grokHome.path)' 'grok' '-r' '\(sessionId)' '-m' 'grok-4'",
                workingDirectory: "/tmp/grok custom state"
            )
        )
    }
    ...
    func testRegisteredGrokSessionDirectoryUsesNativeDirectoryLayout() async throws {
        ...
        XCTAssertEqual(
            entry.resumeCommand,
            TerminalStartupWorkingDirectoryPrefix.prefix(
                "'env' 'GROK_HOME=\(tempDir.path)' 'custom-grok' '-r' '\(sessionId)'",
                workingDirectory: "/tmp/custom grok repo"
            )
        )
    }
