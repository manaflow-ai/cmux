...
struct SSHStartupManualReconnectTests {
    ...
    @Test func persistentAttachFailurePreservesReattachIdentityAndConnectionOwner() throws {
        let workspace = Workspace()
        let panel = try #require(workspace.focusedTerminalPanel)
        workspace.configureRemoteConnection(Self.makePersistentRemoteConfiguration(), autoConnect: false)
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to cmux-macmini via shared local proxy 127.0.0.1:64007",
            target: "cmux-macmini"
    ...
    @Test func connectedTransitionReattachesEveryPersistentPlaceholder() throws {
        ...
        let originalSurfaces = [
            first.id: first.surface,
            second.id: second.surface,
        ]
    ...
    @Test func persistentAttachRetriesPastLegacyBudgetWithCappedBackoff() throws {
        ...
        try Self.writeShellFile(at: fakeCLI, lines: [
            ...
            "    count=$((count + 1))",
            "    printf '%s' \"$count\" > \"${CMUX_TEST_ATTEMPT_FILE}\"",
            "    if [ \"$count\" -lt 24 ]; then exit 255; fi",
            "    exit 253",
            "    ;;",
            "  *) exit 0 ;;",
            "esac",
        ...
        let command = SSHPTYAttachStartupCommandBuilder.command(sessionID: "ssh-test-session")
        let result = Self.runProcess(
            executablePath: "/bin/sh",
            arguments: ["-c", command],
            environment: environment,
    ...
    private static func makePersistentRemoteConfiguration() -> WorkspaceRemoteConfiguration {
        WorkspaceRemoteConfiguration(
            destination: "cmux-macmini",
            port: nil,
            identityFile: nil,
