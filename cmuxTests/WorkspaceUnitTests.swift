...
final class StoredShortcutMatchingTests: XCTestCase {
    ...
    @MainActor
    func testNumberedWorkspaceAndSurfaceShortcutsCanSwapModifierFamilies() {
        ...
        XCTAssertEqual(
            presentation?.message,
            "This shortcut conflicts with Select Surface 1…9 (⌃1…9). Swap shortcuts?"
        )
        XCTAssertEqual(presentation?.swapButtonTitle, "Swap")
        XCTAssertTrue(presentation?.canSwap ?? false)
        XCTAssertEqual(presentation?.undoButtonTitle, "Undo")
        ...
        XCTAssertEqual(KeyboardShortcutSettings.shortcut(for: .selectWorkspaceByNumber), surfaceDefault)
        XCTAssertEqual(KeyboardShortcutSettings.shortcut(for: .selectSurfaceByNumber), workspaceDefault)
    }
