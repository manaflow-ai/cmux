...
- Focus events: `focus.panel`, `focus.workspaceLayout`, `focus.firstResponder`, `focus.moveFocus`
- CMUXLayout events: `tab.select`, `tab.close`, `tab.dragStart`, `tab.drop`, `pane.focus`, `pane.drop`, `divider.dragStart`
...
- **Submodule safety:** When modifying a submodule (ghostty, Packages/CMUXLayout, etc.), always push the submodule commit to its remote `main` branch BEFORE committing the updated pointer in the parent repo. Never commit on a detached HEAD or temporary branch — the commit will be orphaned and lost. Verify with: `cd <submodule> && git merge-base --is-ancestor HEAD origin/main`.
