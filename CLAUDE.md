...
- **ObservableObject is banned; use @Observable.** The entire codebase migrated to the Observation framework. Never add `ObservableObject`, `@Published`, `@StateObject`, `@ObservedObject`, `@EnvironmentObject`, `.environmentObject(`, or `objectWillChange`; use `@Observable`, `@State`, `@Bindable`, `@Environment(T.self)`, and `.environment()`. CI enforces this via `python3 scripts/lint_no_observable_object.py` (comments are exempt). File sizes are also ratcheted: every Swift file stays under 1000 lines, enforced by `scripts/swift_file_length_budget.py` against `.github/swift-file-length-budget.tsv`; split files instead of refreshing the budget upward.
