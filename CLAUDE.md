...
Do not launch a background review agent (`$autoreview`, `codex review`, `claude review`, or a judge loop) by default. Second-model review is explicit user opt-in in the current conversation; an implementation request, open PR, CI failure, closeout, or handoff is not that opt-in. GitHub review bots are part of PR bookkeeping: API-created PRs bypass the repository PR template, so an agent-authored PR using the review gate must include `<!-- agent-pr-review-required -->` in its body and post the template's bot-review commands after the latest pushed HEAD, including `@coderabbitai review` and `@greptile-apps review`. After requesting them, let required GitHub checks and review bots run asynchronously, then return to address concrete check failures and actionable findings before merge.
