...
**Every phone install MUST be authenticated before handoff — installed-but-signed-out is a failed install.** A tagged bundle id is a fresh identity with no session, so after EVERY install (first install or rebuild of an existing tag), verify the app is actually past the login screen and paired. The reload scripts do this when left on their defaults; if you installed any other way, or the app lands on login, run `scripts/mobile-dev-launch.sh --tag <tag> --device --device-id <id> --ensure-mac` and confirm the signed-in state before reporting. Never install with raw `devicectl device install app` (it skips sign-in entirely), never pass `--no-sign-in`/`--no-attach` for a dogfood build, and never report a phone build as ready without stating that you verified it authenticated. If sign-in cannot be completed (missing `~/.secrets/cmuxterm-dev.env`, web API down, phone locked), say exactly that in the handoff with the retry command instead of handing off a logged-out app.
