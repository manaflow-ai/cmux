...
Do not launch a background review agent (`$autoreview`, `codex review`, `claude review`, or a judge loop) by default. Second-model review is explicit user opt-in in the current conversation; an implementation request, open PR, CI failure, closeout, or handoff is not that opt-in. Agent-authored/API-created PRs that use the review gate must include `<!-- agent-pr-review-required -->` in the PR body because GitHub's API does not inject the PR template. The trusted gate owns Greptile review requests for each new head; agents should not add duplicate Greptile trigger comments. Let required GitHub checks and review bots run asynchronously, then return to address only concrete check failures and actionable findings before merge.
