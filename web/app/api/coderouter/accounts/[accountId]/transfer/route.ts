...
export function makeCoderouterTransferHandler(
  ...
  return async function POST(request: Request, context: { params: Promise<{ accountId: string }> }): Promise<Response> {
  ...
  const destination = (await dependencies.listTeams(resolved.value.user)).find((team) => team.teamId === destinationTeamId && team.manageAccounts);
  if (!destination || destinationTeamId === resolved.value.team.teamId) return Response.json({ error: "destination_forbidden" }, { status: 403 });
  try {
    const moved = await dependencies.transfer({ accountId, sourceTeamId: resolved.value.team.teamId, destinationTeamId, stackUserId: resolved.value.user.id });
