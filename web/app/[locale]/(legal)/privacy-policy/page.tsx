...
const liveSessionDisclosureByLocale = {
  en: "To show live sessions on your signed-in devices, our account service also processes bounded workspace and session summaries, including titles, identifiers, activity status, agent type, and timestamps. Titles may be set by you or by programs running in a terminal.",
  ja: "サインイン中のデバイスにライブセッションを表示するため、当社のアカウントサービスは、タイトル、識別子、アクティビティ状態、エージェントの種類、タイムスタンプなど、範囲を限定したワークスペースとセッションの概要も処理します。タイトルは、お客様またはターミナルで実行されるプログラムによって設定される場合があります。",
  "zh-CN": "为了在您已登录的设备上显示实时会话，我们的账户服务还会处理有限的工作区和会话摘要，包括标题、标识符、活动状态、代理类型和时间戳。标题可能由您或终端中运行的程序设置。",
  "zh-TW": "為了在您已登入的裝置上顯示即時工作階段，我們的帳戶服務也會處理有限的工作區與工作階段摘要，包括標題、識別碼、活動狀態、代理程式類型及時間戳記。標題可能由您或在終端機中執行的程式設定。",
  ko: "로그인한 기기에 실시간 세션을 표시하기 위해 당사 계정 서비스는 제목, 식별자, 활동 상태, 에이전트 유형 및 타임스탬프를 포함한 제한된 워크스페이스 및 세션 요약도 처리합니다. 제목은 사용자 또는 터미널에서 실행되는 프로그램이 설정할 수 있습니다.",
  de: "Um Live-Sitzungen auf Ihren angemeldeten Geräten anzuzeigen, verarbeitet unser Kontodienst außerdem begrenzte Arbeitsbereichs- und Sitzungszusammenfassungen, darunter Titel, Kennungen, Aktivitätsstatus, Agententyp und Zeitstempel. Titel können von Ihnen oder von Programmen festgelegt werden, die in einem Terminal ausgeführt werden.",
  es: "Para mostrar sesiones activas en los dispositivos donde ha iniciado sesión, nuestro servicio de cuentas también procesa resúmenes limitados de espacios de trabajo y sesiones, incluidos títulos, identificadores, estado de actividad, tipo de agente y marcas de tiempo. Los títulos pueden ser definidos por usted o por programas que se ejecutan en un terminal.",
  fr: "Pour afficher les sessions actives sur vos appareils connectés, notre service de compte traite également des résumés limités des espaces de travail et des sessions, notamment les titres, les identifiants, l’état d’activité, le type d’agent et les horodatages. Les titres peuvent être définis par vous ou par des programmes exécutés dans un terminal.",
  it: "Per mostrare le sessioni attive sui dispositivi su cui hai effettuato l’accesso, il nostro servizio account elabora anche riepiloghi limitati di aree di lavoro e sessioni, inclusi titoli, identificatori, stato dell’attività, tipo di agente e timestamp. I titoli possono essere impostati da te o da programmi in esecuzione in un terminale.",
  da: "For at vise aktive sessioner på dine indloggede enheder behandler vores kontotjeneste også afgrænsede oversigter over arbejdsområder og sessioner, herunder titler, id'er, aktivitetsstatus, agenttype og tidsstempler. Titler kan angives af dig eller af programmer, der kører i en terminal.",
  pl: "Aby wyświetlać aktywne sesje na zalogowanych urządzeniach, nasza usługa konta przetwarza również ograniczone podsumowania obszarów roboczych i sesji, w tym tytuły, identyfikatory, stan aktywności, typ agenta i znaczniki czasu. Tytuły mogą być ustawiane przez użytkownika lub przez programy działające w terminalu.",
  ru: "Чтобы показывать активные сеансы на устройствах, на которых выполнен вход, наша служба учётных записей также обрабатывает ограниченные сводки рабочих пространств и сеансов, включая названия, идентификаторы, состояние активности, тип агента и временные метки. Названия могут задаваться вами или программами, работающими в терминале.",
  bs: "Da bi prikazala aktivne sesije na uređajima na kojima ste prijavljeni, naša usluga računa obrađuje i ograničene sažetke radnih prostora i sesija, uključujući naslove, identifikatore, status aktivnosti, vrstu agenta i vremenske oznake. Naslove možete postaviti vi ili programi koji se izvršavaju u terminalu.",
  ar: "لعرض الجلسات النشطة على أجهزتك المسجّل دخولها، تعالج خدمة الحساب لدينا أيضًا ملخصات محدودة لمساحات العمل والجلسات، بما في ذلك العناوين والمعرّفات وحالة النشاط ونوع الوكيل والطوابع الزمنية. قد تُعيّن العناوين بواسطتك أو بواسطة برامج تعمل في الوحدة الطرفية.",
  no: "For å vise aktive økter på enhetene du er logget inn på, behandler kontotjenesten vår også avgrensede sammendrag av arbeidsområder og økter, inkludert titler, identifikatorer, aktivitetsstatus, agenttype og tidsstempler. Titler kan angis av deg eller av programmer som kjører i en terminal.",
  "pt-BR": "Para mostrar sessões ativas nos dispositivos em que você iniciou sessão, nosso serviço de conta também processa resumos limitados de espaços de trabalho e sessões, incluindo títulos, identificadores, status de atividade, tipo de agente e registros de data e hora. Os títulos podem ser definidos por você ou por programas executados em um terminal.",
  th: "เพื่อแสดงเซสชันที่ใช้งานอยู่บนอุปกรณ์ที่คุณลงชื่อเข้าใช้ บริการบัญชีของเรายังประมวลผลข้อมูลสรุปของพื้นที่ทำงานและเซสชันแบบจำกัด ซึ่งรวมถึงชื่อ ตัวระบุ สถานะกิจกรรม ประเภทเอเจนต์ และการประทับเวลา ชื่ออาจกำหนดโดยคุณหรือโดยโปรแกรมที่ทำงานในเทอร์มินัล",
  tr: "Oturum açtığınız cihazlarda canlı oturumları göstermek için hesap hizmetimiz başlıklar, tanımlayıcılar, etkinlik durumu, aracı türü ve zaman damgaları dahil olmak üzere sınırlı çalışma alanı ve oturum özetlerini de işler. Başlıklar sizin tarafınızdan veya terminalde çalışan programlar tarafından ayarlanabilir.",
  km: "ដើម្បីបង្ហាញសម័យដែលកំពុងដំណើរការនៅលើឧបករណ៍ដែលអ្នកបានចូល សេវាគណនីរបស់យើងក៏ដំណើរការសេចក្តីសង្ខេបកន្លែងធ្វើការ និងសម័យដែលមានដែនកំណត់ រួមទាំងចំណងជើង អត្តសញ្ញាណ ស្ថានភាពសកម្មភាព ប្រភេទភ្នាក់ងារ និងត្រាពេលវេលា។ ចំណងជើងអាចត្រូវបានកំណត់ដោយអ្នក ឬដោយកម្មវិធីដែលដំណើរការនៅក្នុងស្ថានីយ។",
  uk: "Щоб показувати активні сеанси на пристроях, на яких виконано вхід, наша служба облікових записів також обробляє обмежені зведення робочих просторів і сеансів, зокрема назви, ідентифікатори, стан активності, тип агента й часові позначки. Назви можуть задаватися вами або програмами, що працюють у терміналі.",
} satisfies Record<Locale, string>;
