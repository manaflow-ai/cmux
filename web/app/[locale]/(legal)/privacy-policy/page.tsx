...
const liveSessionDisclosureByLocale = {
  en: "To show live sessions on your signed-in devices, our account service also processes bounded workspace and session summaries, including titles, identifiers, activity status, agent type, and timestamps. These summaries exclude terminal transcripts, command text, and terminal output. We remove them when the device stops publishing them, the account is deleted, or stale data is pruned.",
  ja: "サインイン中のデバイスにライブセッションを表示するため、当社のアカウントサービスは、タイトル、識別子、アクティビティ状態、エージェントの種類、タイムスタンプなど、範囲を限定したワークスペースとセッションの概要も処理します。これらの概要にターミナルの記録、コマンドテキスト、ターミナル出力は含まれません。デバイスが公開を停止した場合、アカウントが削除された場合、または古いデータが整理された場合に削除します。",
  "zh-CN": "为了在您已登录的设备上显示实时会话，我们的账户服务还会处理有限的工作区和会话摘要，包括标题、标识符、活动状态、代理类型和时间戳。这些摘要不包含终端记录、命令文本或终端输出。当设备停止发布、账户被删除或陈旧数据被清理时，我们会删除这些摘要。",
  "zh-TW": "為了在您已登入的裝置上顯示即時工作階段，我們的帳戶服務也會處理有限的工作區與工作階段摘要，包括標題、識別碼、活動狀態、代理程式類型及時間戳記。這些摘要不包含終端機記錄、命令文字或終端機輸出。當裝置停止發佈、帳戶遭刪除或過期資料被清理時，我們會刪除這些摘要。",
  ko: "로그인한 기기에 실시간 세션을 표시하기 위해 당사 계정 서비스는 제목, 식별자, 활동 상태, 에이전트 유형 및 타임스탬프를 포함한 제한된 워크스페이스 및 세션 요약도 처리합니다. 이 요약에는 터미널 기록, 명령 텍스트 또는 터미널 출력이 포함되지 않습니다. 기기가 게시를 중단하거나 계정이 삭제되거나 오래된 데이터가 정리되면 해당 요약을 삭제합니다.",
  de: "Um Live-Sitzungen auf Ihren angemeldeten Geräten anzuzeigen, verarbeitet unser Kontodienst außerdem begrenzte Arbeitsbereichs- und Sitzungszusammenfassungen, darunter Titel, Kennungen, Aktivitätsstatus, Agententyp und Zeitstempel. Diese Zusammenfassungen enthalten keine Terminalprotokolle, Befehlstexte oder Terminalausgaben. Wir löschen sie, wenn das Gerät sie nicht mehr veröffentlicht, das Konto gelöscht wird oder veraltete Daten bereinigt werden.",
  es: "Para mostrar sesiones activas en los dispositivos donde ha iniciado sesión, nuestro servicio de cuentas también procesa resúmenes limitados de espacios de trabajo y sesiones, incluidos títulos, identificadores, estado de actividad, tipo de agente y marcas de tiempo. Estos resúmenes no incluyen transcripciones del terminal, texto de comandos ni salida del terminal. Los eliminamos cuando el dispositivo deja de publicarlos, se elimina la cuenta o se depuran datos obsoletos.",
  fr: "Pour afficher les sessions actives sur vos appareils connectés, notre service de compte traite également des résumés limités des espaces de travail et des sessions, notamment les titres, les identifiants, l’état d’activité, le type d’agent et les horodatages. Ces résumés excluent les transcriptions du terminal, le texte des commandes et les sorties du terminal. Nous les supprimons lorsque l’appareil cesse de les publier, lorsque le compte est supprimé ou lorsque les données obsolètes sont purgées.",
  it: "Per mostrare le sessioni attive sui dispositivi su cui hai effettuato l’accesso, il nostro servizio account elabora anche riepiloghi limitati di aree di lavoro e sessioni, inclusi titoli, identificatori, stato dell’attività, tipo di agente e timestamp. Questi riepiloghi non includono trascrizioni del terminale, testo dei comandi o output del terminale. Li eliminiamo quando il dispositivo smette di pubblicarli, l’account viene eliminato o i dati obsoleti vengono rimossi.",
  da: "For at vise aktive sessioner på dine indloggede enheder behandler vores kontotjeneste også afgrænsede oversigter over arbejdsområder og sessioner, herunder titler, id'er, aktivitetsstatus, agenttype og tidsstempler. Oversigterne indeholder ikke terminaludskrifter, kommandotekst eller terminaloutput. Vi sletter dem, når enheden stopper med at offentliggøre dem, kontoen slettes, eller forældede data ryddes op.",
  pl: "Aby wyświetlać aktywne sesje na zalogowanych urządzeniach, nasza usługa konta przetwarza również ograniczone podsumowania obszarów roboczych i sesji, w tym tytuły, identyfikatory, stan aktywności, typ agenta i znaczniki czasu. Podsumowania te nie zawierają transkrypcji terminala, tekstu poleceń ani danych wyjściowych terminala. Usuwamy je, gdy urządzenie przestaje je publikować, konto zostaje usunięte lub nieaktualne dane są czyszczone.",
  ru: "Чтобы показывать активные сеансы на устройствах, на которых выполнен вход, наша служба учётных записей также обрабатывает ограниченные сводки рабочих пространств и сеансов, включая названия, идентификаторы, состояние активности, тип агента и временные метки. Эти сводки не содержат расшифровок терминала, текста команд или вывода терминала. Мы удаляем их, когда устройство прекращает публикацию, удаляется учётная запись или очищаются устаревшие данные.",
  bs: "Da bi prikazala aktivne sesije na uređajima na kojima ste prijavljeni, naša usluga računa obrađuje i ograničene sažetke radnih prostora i sesija, uključujući naslove, identifikatore, status aktivnosti, vrstu agenta i vremenske oznake. Ti sažeci ne sadrže prijepise terminala, tekst naredbi ni izlaz terminala. Brišemo ih kada ih uređaj prestane objavljivati, kada se račun izbriše ili kada se uklone zastarjeli podaci.",
  ar: "لعرض الجلسات النشطة على أجهزتك المسجّل دخولها، تعالج خدمة الحساب لدينا أيضًا ملخصات محدودة لمساحات العمل والجلسات، بما في ذلك العناوين والمعرّفات وحالة النشاط ونوع الوكيل والطوابع الزمنية. لا تتضمن هذه الملخصات نصوص جلسات الطرفية أو نص الأوامر أو مخرجات الطرفية. نحذفها عندما يتوقف الجهاز عن نشرها أو يُحذف الحساب أو تُزال البيانات القديمة.",
  no: "For å vise aktive økter på enhetene du er logget inn på, behandler kontotjenesten vår også avgrensede sammendrag av arbeidsområder og økter, inkludert titler, identifikatorer, aktivitetsstatus, agenttype og tidsstempler. Sammendragene inneholder ikke terminalutskrifter, kommandotekst eller terminaldata. Vi sletter dem når enheten slutter å publisere dem, kontoen slettes eller utdaterte data ryddes opp.",
  "pt-BR": "Para mostrar sessões ativas nos dispositivos em que você iniciou sessão, nosso serviço de conta também processa resumos limitados de espaços de trabalho e sessões, incluindo títulos, identificadores, status de atividade, tipo de agente e registros de data e hora. Esses resumos não incluem transcrições do terminal, texto de comandos nem saída do terminal. Nós os removemos quando o dispositivo deixa de publicá-los, a conta é excluída ou dados obsoletos são eliminados.",
  th: "เพื่อแสดงเซสชันที่ใช้งานอยู่บนอุปกรณ์ที่คุณลงชื่อเข้าใช้ บริการบัญชีของเรายังประมวลผลข้อมูลสรุปของพื้นที่ทำงานและเซสชันแบบจำกัด ซึ่งรวมถึงชื่อ ตัวระบุ สถานะกิจกรรม ประเภทเอเจนต์ และการประทับเวลา ข้อมูลสรุปเหล่านี้ไม่รวมบันทึกการใช้งานเทอร์มินัล ข้อความคำสั่ง หรือผลลัพธ์จากเทอร์มินัล เราจะลบข้อมูลดังกล่าวเมื่ออุปกรณ์หยุดเผยแพร่ บัญชีถูกลบ หรือมีการล้างข้อมูลที่ล้าสมัย",
  tr: "Oturum açtığınız cihazlarda canlı oturumları göstermek için hesap hizmetimiz başlıklar, tanımlayıcılar, etkinlik durumu, aracı türü ve zaman damgaları dahil olmak üzere sınırlı çalışma alanı ve oturum özetlerini de işler. Bu özetler terminal dökümlerini, komut metnini veya terminal çıktısını içermez. Cihaz bunları yayımlamayı bıraktığında, hesap silindiğinde veya eski veriler temizlendiğinde özetleri sileriz.",
  km: "ដើម្បីបង្ហាញសម័យដែលកំពុងដំណើរការនៅលើឧបករណ៍ដែលអ្នកបានចូល សេវាគណនីរបស់យើងក៏ដំណើរការសេចក្តីសង្ខេបកន្លែងធ្វើការ និងសម័យដែលមានដែនកំណត់ រួមទាំងចំណងជើង អត្តសញ្ញាណ ស្ថានភាពសកម្មភាព ប្រភេទភ្នាក់ងារ និងត្រាពេលវេលា។ សេចក្តីសង្ខេបទាំងនេះមិនរួមបញ្ចូលកំណត់ត្រាស្ថានីយ អត្ថបទពាក្យបញ្ជា ឬលទ្ធផលស្ថានីយទេ។ យើងលុបវានៅពេលឧបករណ៍ឈប់ផ្សព្វផ្សាយ គណនីត្រូវបានលុប ឬទិន្នន័យចាស់ត្រូវបានសម្អាត។",
  uk: "Щоб показувати активні сеанси на пристроях, на яких виконано вхід, наша служба облікових записів також обробляє обмежені зведення робочих просторів і сеансів, зокрема назви, ідентифікатори, стан активності, тип агента й часові позначки. Ці зведення не містять журналів термінала, тексту команд або виводу термінала. Ми видаляємо їх, коли пристрій припиняє публікацію, обліковий запис видаляється або застарілі дані очищуються.",
} satisfies Record<Locale, string>;

function irohDisclosureForLocale(locale: string): PrivacyPolicySection {
  const resolvedLocale = locale as Locale;
  const disclosure = irohDisclosures[resolvedLocale] ?? irohDisclosures.en;
  return {
    ...disclosure,
    paragraphs: [
      ...(disclosure.paragraphs ?? []),
      liveSessionDisclosureByLocale[resolvedLocale] ?? liveSessionDisclosureByLocale.en,
    ],
  };
}
