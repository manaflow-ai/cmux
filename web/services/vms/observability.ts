...
function addMemoryUpgradeProperties(requestProperties: PostHogProperties, errorCode: string | undefined, lastError: VmErrorResponseInput | undefined) {
  if (errorCode === "vm_memory_requires_plan") {
    requestProperties.requested_memory_mb = lastError?.details?.requestedMemoryMb ?? null;
    requestProperties.max_memory_mb = lastError?.details?.maxMemoryMb ?? null;
    requestProperties.upgrade_plan = lastError?.details?.upgradePlanId ?? "max";
  }
}
