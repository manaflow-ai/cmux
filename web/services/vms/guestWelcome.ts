 ...
 *
 * Rendering never reads stdin or fetches data. The daemon captures bootstrap
 * output before creating the initial terminal; manual replay is stateless.
 */
...
for index, marks in enumerate(logo):
    ...
    if width >= 58 and index in (1, 3, 4):
        label = {1:brand,3:intro,4:tagline}[index]
...
# This prepares output only. The daemon's durable first-workspace bootstrap
# owns the one-time commit after terminal creation; retries never consume it here.
guest_welcome_bootstrap() {
  ...
  cmux_welcome_identity="\$(cat "\$cmux_welcome_identity_file" 2>/dev/null)" || return 1
  [ -n "\$cmux_welcome_identity" ] || return 1
  [ "\$(cat "\$cmux_welcome_pending_gate" 2>/dev/null)" = "\$cmux_welcome_identity" ] || return 1
  guest_welcome_render
}
...
guest_welcome_command() {
  ...
  while [ "\$#" -gt 0 ]; do
    case "\$1" in
      --bootstrap) cmux_welcome_auto=1; shift ;;
