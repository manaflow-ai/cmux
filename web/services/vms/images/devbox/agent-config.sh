...
cmux_write_agent_configs() {
  ...
  if [ -n "${OPENAI_BASE_URL-}" ] && ! grep -qs '^model_provider = ' "$HOME/.codex/config.toml" \
    && ! grep -qs '^\[model_providers\.cmux\]' "$HOME/.codex/config.toml" \
    && ! grep -qs '^\[history\]' "$HOME/.codex/config.toml"; then
