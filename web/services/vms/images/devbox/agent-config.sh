...
cmux_write_agent_configs() {
  ...
  # TOML allows whitespace around keys, "=", and inside table headers, so the
  # probes accept it; a key the user wrote as model_provider="openai" counts.
  if [ -n "${OPENAI_BASE_URL-}" ] && ! grep -Eqs '^[[:space:]]*(model_provider|"model_provider")[[:space:]]*=' "$HOME/.codex/config.toml" \
    && ! grep -Eqs '^[[:space:]]*\[[[:space:]]*(model_providers|"model_providers")[[:space:]]*\.[[:space:]]*(cmux|"cmux")[[:space:]]*\]' "$HOME/.codex/config.toml" \
    && ! grep -Eqs '^[[:space:]]*\[[[:space:]]*(history|"history")[[:space:]]*\]' "$HOME/.codex/config.toml"; then
