...
# Blaxel's rootfs slimming strips imlib2's loaders down to ico.so; the parked
# copies under /etc/cmux keep tint2 able to draw its PNG launcher icons.
if [ -d /etc/cmux/imlib2-loaders ]; then
  IMLIB2_LOADER_PATH=/etc/cmux/imlib2-loaders
  export IMLIB2_LOADER_PATH
fi
