...
async function authedUserFromStackUser(
  user: StackUserLike,
  options: { readonly requestedTeamId?: string | null; readonly allTeams?: boolean },
): Promise<AuthedUser> {
  ...
  const needsListedTeams = options.allTeams === true || !selectedTeam || (!!requestedTeamId && requestedTeamId !== selectedTeam.id);
