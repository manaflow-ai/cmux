...
# Legacy image compatibility for the account discovery commands. New images
# have /usr/local/libexec/cmux-coderouter and dispatch these through the
# official Rust CodeRouter CLI instead.
guest_coderouter_accounts() {
  cmux_ca_json=false
  for cmux_arg in "\$@"; do
    case "\$cmux_arg" in
...
guest_coderouter_command() {
  # The Rust cmux-tui binary is the single Cloud CLI front door. It dispatches
  # to the official CodeRouter binary, which owns the complete command grammar
  # and Ratatui presentation. The shell shim remains responsible only for the
  # local cmux session grammar.
  if [ -x "\${CMUX_CODEROUTER_BIN:-/usr/local/libexec/cmux-coderouter}" ]; then
    exec "\$CMUX_TUI_BIN" coderouter "\$@"
  fi
  # Compatibility path for an older image while it is being healed. New
  # images always install the marker binary above and never use this table.
