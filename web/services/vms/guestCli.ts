...
//
// Baked into the promoted Freestyle image and retained by the driver as a
// checksum-verified drift repair for older or damaged snapshots. The image and
// the repair path use the same generated bytes and distribution manifest.
...
import { GUEST_BROWSER_OPENER_PATH, guestBrowserInstallCommand, guestBrowserReadyCommand } from "./guestBrowser";
...
/** Offline proof shared by image verification and repair of existing machines. */
export function guestCliReadyCommand(): string {
  const digest = createHash("sha256").update(GUEST_CMUX_SHIM).digest("hex");
  return `test -x '${GUEST_CMUX_SHIM_PATH}' && test "$(sha256sum '${GUEST_CMUX_SHIM_PATH}' 2>/dev/null | cut -d ' ' -f 1)" = '${digest}' && ${guestBrowserReadyCommand} && ${guestCliDistributionCommand(true)}`;
}
