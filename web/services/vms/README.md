...
## Error contract: who is to blame, and what the caller sees

Every VM error goes through `vmErrorResponse` (`routeHelpers.ts`). The response body is the public contract and uses cmux Cloud wording only: `error` (stable code), `message`, `action`, `retryable`, `retryAfterSeconds`, `phase`, `traceId`, and `ui.{title,message,severity,retryable,traceId}`. It never names a vendor, an image id, an env var, or a database (`expectNoCloudVmImplementationLeaks` in `tests/vm-route-auth.test.ts` pins this). Clients act on `retryable`: `true` means back off `retryAfterSeconds` and try again, `false` means stop and show the message with its Reference id.

The operator side lives in `diagnostics`, which never reaches the body. `providerFailureDiagnostics` / `dependencyFailureDiagnostics` fill it with `blame` (the dependency: `freestyle`, `postgres`, `stripe`, `coderouter`, `cmux`), `fault` (`vendor` for a third party we pay, `operator` for something we run; 4xx are `user`), the raw wire answer (`providerCode`, `providerStatus`, `providerPath`, `providerMessage`), the `causeChain`, and one `internalReason` line such as `freestyle openCmuxRemote failed NOT_FOUND 404 /v5/vms/<id>: not found: vm <id>`. `reportVmErrorResponse` puts that line in the Sentry title (`cloud VM <code> [vendor: freestyle] <internalReason>`), tags `vm.fault`, `vm.blame`, `vm.provider_code`, `vm.provider_status`, fingerprints by code + provider + vendor code, and the span carries the same as `cmux.vm.error_*`. The console log line (`cmux.observability.error`) carries the whole context, so a dev-server log already says which vendor answered what.

A machine the vendor no longer has (deleted in its dashboard, reclaimed, expired) is not an outage. `failWhenProviderVmMissing` (`workflows.ts`) wraps every provider call on an existing machine (attach, cmux-remote, exec, stats, open-port, snapshot): a provider 404 is confirmed with one status probe, the row is marked `destroyed` with a `vm.destroyed` usage event (`source: provider_missing`), the model plane is revoked when a revoker is wired, and the caller gets 404 `vm_not_found` with `retryable: false` and `details.reason: "provider_missing"`. Before this, that path answered 502 `vm_cloud_service_unavailable` with `retryable: true`, and attach loops ran forever against a machine that would never return. An unconfirmed 404 (status probe times out, or reports the machine alive) keeps the original retryable error.
