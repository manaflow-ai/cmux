...
function rollbackProviderCreate(
  ...
  return Effect.gen(function* () {
    yield* providers.destroy(provider, handle.providerVmId).pipe(bestEffort("provider_destroy_rollback", { provider, providerVmId: handle.providerVmId }));
...
export function createVm(input: {
  ...
  return Effect.gen(function* () {
    ...
    const materials = yield* measureVmEffect(
      ...
      Effect.tapError((err) =>
        Effect.all([
          refundCredit(billing, repo, create.vm, credit),
        ...
        ].map((step) => step.pipe(bestEffort("model_plane_create_compensation", { vmId: create.vm.id, provider: input.provider }))), { discard: true })
      ),
    );
    ...
    const running = yield* compensated({
      ...
      destroyOrphan: (handle) => rollbackProviderCreate(providers, input.provider, handle),
      revoke: revokeModelPlane(input.modelPlane, create.vm.id),
      ...
      createFailureCode: PROVIDER_CREATE_UNAVAILABLE_FAILURE_CODE,
  ...
  return Effect.gen(function* () {
    ...
    if (nativeFork) {
      ...
      const running = yield* compensated({
        ...
        finalize: (handle) => Effect.gen(function* () {
        ...
        destroyOrphan: (handle) => rollbackProviderCreate(providers, source.provider, handle),
        refund: refundCredit(billing, repo, create.vm, credit),
        markFailed: (failure) =>
          repo.markCreateFailed({ id: create.vm.id, code: failure.code, message: failure.message }),
        createFailureEvent: (failure) =>
...
type CompensatedCreateInput<H> = {
  ...
  readonly finalize: (handle: H) => Effect.Effect<CloudVmRow, VmWorkflowError>;
  ...
  readonly revoke?: Effect.Effect<void, never>;
...
function compensated<H>(
  ...
  const compensate = (
    ...
    Effect.gen(function* () {
      ...
      if (input.revoke) yield* input.revoke;
      yield* input.refund;
  ...
  return Effect.gen(function* () {
    const handle = yield* input.createHandle.pipe(
      Effect.tapError((err) => {
        ...
        return compensate({ ...failure, code: input.createFailureCode ?? failure.code }, failure, input.createFailureEvent);
      }),
    );
    return yield* input.finalize(handle).pipe(
      Effect.tapError((err) =>
        compensate(
          ...
          { code: "database_finalize_failed", message: errorMessage(err) },
