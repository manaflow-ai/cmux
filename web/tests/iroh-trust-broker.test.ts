...
class MemoryRepository implements IrohRepositoryShape {
  ...
  consumeChallengeAndRegister(input: Parameters<IrohRepositoryShape["consumeChallengeAndRegister"]>[0]) {
    ...
    const existing = this.bindings.find((row) =>
      ...
      !row.revokedAt);
    // Mirror the real repository's staleness gate: the (registeredAt, mintSeq)
    // pair of the newest landed challenge, with the strictly monotonic mint
    // sequence breaking same-millisecond ties.
    if (existing && (
      challenge.createdAt < existing.registeredAt
      || (
        challenge.createdAt.getTime() === existing.registeredAt.getTime()
        && challenge.mintSeq <= existing.registeredMintSeq
      )
    )) {
    ...
    if (
      ...
      existing.registeredMintSeq = challenge.mintSeq;
