...
describe("Freestyle platform contract", () => {
  ...
  test("maps a bounded provider VM inventory and reports pagination", async () => {
    ...
    const provider = new FreestyleProvider({
      client: () => client,
      resolveDaemonSource: async () => ({ url: "https://files.cmux.com/x", sha256: "0".repeat(64), commit: "x", builtAt: null, hookUrl: "https://files.cmux.com/hook", hookSha256: "1".repeat(64) }),
    });
