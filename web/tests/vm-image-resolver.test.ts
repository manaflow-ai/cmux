...
// clipboard helper, published DISPLAY, and baked cmux terminfo), one snapshot
// per Freestyle size, listed under both kinds (a desktop image is a superset of
// a base one, so the same snapshot id serves both; the base listing's version
// carries a `-base` suffix). The manifest is the only source of truth for
// images; no env var selects or overrides one, and the plan's memory picks the
// size.
