...
class FakeSocket {
  ...
  static waitForInstance(): Promise<FakeSocket> {
    ...
    return socket ? Promise.resolve(socket) : new Promise(resolve => { FakeSocket.onCreate = resolve; });
  }
  readonly OPEN = 1;
  readyState = 0;
  ...
  sent: string[] = [];
  private sendWaiters = new Map<number, (body: string) => void>();
  protocols: string | string[];
  constructor(_url: string, protocols: string | string[]) {
    this.protocols = protocols;
    FakeSocket.instances.push(this);
    FakeSocket.onCreate?.(this);
    FakeSocket.onCreate = null;
  }
  send(body: string) {
    const index = this.sent.push(body) - 1;
    this.sendWaiters.get(index)?.(body);
    this.sendWaiters.delete(index);
  }
  waitForSend(index: number): Promise<string> {
...
describe("IROH Dashboard v2 controller", () => {
  afterEach(() => { globalThis.fetch = originalFetch; globalThis.WebSocket = originalSocket; FakeSocket.instances = []; FakeSocket.onCreate = null; });
  ...
  test("applies directory frames and sends a revoke mutation", async () => {
    ...
    const pending = controller.start();
    const socket = await FakeSocket.waitForInstance(); socket.open();
    const directoryRequest = JSON.parse(await socket.waitForSend(0));
  ...
  test("uses the cursor for paged directories and sends expected revision for settings", async () => {
    ...
    const pending = controller.start();
    const socket = await FakeSocket.waitForInstance(); socket.open();
    const first = JSON.parse(await socket.waitForSend(0));
    ...
    const second = JSON.parse(await socket.waitForSend(1));
