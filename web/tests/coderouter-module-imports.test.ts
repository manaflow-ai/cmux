...
// Run through the isolated test runner with no module mocks. New files are
// discovered automatically, so an incomplete mock cannot hide a broken import.
...
for (const path of paths) {
  test(`loads ${path} without mocks or credentials`, async () => {
