...
describe("VM Dashboard v2 controller", () => {
  ...
  test("loads team workspaces after the directory and refreshes them on change", async () => {
    ...
    let resolveFirstWorkspaces: (() => void) | undefined;
    const firstWorkspaces = new Promise<void>(resolve => { resolveFirstWorkspaces = resolve; });
    let resolveSecondWorkspaces: (() => void) | undefined;
    const secondWorkspaces = new Promise<void>(resolve => { resolveSecondWorkspaces = resolve; });
    const controller = new V2DashboardController({ origin: "https://cmux-iroh-v2-staging.cmux-presence-worker.workers.dev", environment: "staging", projectId: "p", userId: "u", teamId: "t", getStackToken: async () => "s", onDirectory: () => {}, onWorkspaces: value => { workspaces.push(value); (workspaces.length === 1 ? resolveFirstWorkspaces : resolveSecondWorkspaces)?.(); }, onError: () => {} });
    const pending = controller.start();
    const socket = await FakeSocket.waitForInstance(); socket.open();
    const directoryRequest = JSON.parse(await socket.waitForSent(0));
    socket.message({ schemaId: "dashboard.directory.v1", requestId: directoryRequest.requestId, directory: { teamId: "t", revision: 1, devices: [], relayURLs: [], issuedAt: 1, nextCursor: null, canManageTeam: true, managedDeviceIds: [] } });
    const listRequest = JSON.parse(await socket.waitForSent(1));
    ...
    const workspaceRequest = JSON.parse(await socket.waitForSent(2));
    ...
    await Promise.all([pending, firstWorkspaces]);
