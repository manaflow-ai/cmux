...
const CHECKS: readonly string[] = [
  ...
  // The baked tput caches exist in both homes ble.sh resolves, so the first
  // pane never prints "updating tput cache".
  "test -s /etc/cmux/blesh-cache-seed/blesh/*/term.xterm-256color && test -s /etc/cmux/blesh-cache-seed/blesh/*/term.screen-256color && test -s /etc/cmux/blesh-cache-seed/blesh/*/term.tmux-256color && test -s /etc/cmux/blesh-cache-seed/blesh/*/term.linux && test -s /usr/local/share/blesh/cache.d/0/term.xterm-256color && test -s /usr/local/share/blesh/cache.d/0/term.screen-256color && test -s /usr/local/share/blesh/cache.d/0/term.tmux-256color && test -s /usr/local/share/blesh/cache.d/0/term.linux && echo blesh-seed-ok",
  ...
  "rm -rf /tmp/cmux-agent-config-verify && env HOME=/tmp/cmux-agent-config-verify OPENAI_BASE_URL=https://example.invalid/v1 OPENAI_API_KEY=crt_check CMUX_CODEROUTER_URL=https://example.invalid bash -lc '[ \"$ANTHROPIC_BASE_URL\" = \"https://example.invalid\" ] && [ \"$ANTHROPIC_AUTH_TOKEN\" = \"crt_check\" ]' && grep -q hasCompletedOnboarding /tmp/cmux-agent-config-verify/.claude.json && ! grep -q crt_check /tmp/cmux-agent-config-verify/.claude.json && grep -q 'model_provider = \"cmux\"' /tmp/cmux-agent-config-verify/.codex/config.toml && grep -q 'wire_api = \"responses\"' /tmp/cmux-agent-config-verify/.codex/config.toml && ! grep -q crt_check /tmp/cmux-agent-config-verify/.codex/config.toml &&grep -q \"export OPENAI_API_KEY='crt_check'\" /tmp/cmux-agent-config-verify/.config/cmux/model-plane.env && [ \"$(stat -c %a /tmp/cmux-agent-config-verify/.config/cmux/model-plane.env)\" = \"600\" ] && grep -qF '\"x-coderouter-route-token\": \"$OPENAI_API_KEY\"' /tmp/cmux-agent-config-verify/.pi/agent/models.json && ! grep -q crt_check /tmp/cmux-agent-config-verify/.pi/agent/models.json && test ! -e /tmp/cmux-agent-config-verify/.config/opencode/opencode.json && rm -rf /tmp/cmux-agent-config-verify && test ! -e /root/.claude.json && test ! -e /root/.codex/config.toml && test ! -e /root/.pi/agent/models.json && test ! -e /root/.config/opencode/opencode.json && echo agent-config-ok",
