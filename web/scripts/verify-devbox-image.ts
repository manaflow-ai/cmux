...
const FREESTYLE_BASE_CHECKS: readonly string[] = [
  ...
  "grep -qx 'export TERMINFO=/etc/terminfo' /etc/profile.d/cmux-terminfo.sh && grep -qx 'export TERMINFO_DIRS=/etc/terminfo' /etc/profile.d/cmux-terminfo.sh && echo terminfo-search-path-ok",
  "shadow=$(mktemp -d) && mkdir -p \"$shadow/.terminfo\" && tic -x -o \"$shadow/.terminfo\" /etc/cmux/terminfo.src && sudo -n -u ubuntu env -i HOME=\"$shadow\" USER=ubuntu TERM=xterm-256color PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin bash -lc 'test \"$TERMINFO\" = /etc/terminfo && test \"$(tput setaf 8 | od -An -tx1 | tr -d \" \\n\")\" = 1b5b33383b353b386d && infocmp -x xterm-256color | head -1 | grep -q /etc/terminfo/' && rm -rf \"$shadow\" && echo terminfo-shadow-resistant",
