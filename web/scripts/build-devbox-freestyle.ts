...
import { guestBrowserInstallCommand, guestBrowserReadyCommand } from "../services/vms/guestBrowser";
...
import { guestResourceReporterInstallCommand, guestResourceReporterReadyCommand } from "../services/vms/guestResourceReporter";
import {
...
try {
  ...
  // The guest cmux tools (issue #13070): the in-VM `cmux` shim, the pinned
  // Cloud CLI distribution (`cmux`, `coderouter`, `cr`), the browser openers
  // and the resource reporter unit. Until this epoch the driver installed
  // them on every create and re-checked them on every attach, three to four
  // guest execs on a machine that was still booting. They are static per
  // image epoch, like the daemon pin above, so they are baked with the
  // driver's own generator commands and proven with the driver's own
  // readiness gates, each as its own step: a machine from this snapshot
  // passes ensureGuestCli and ensureResourceReporter (drivers/freestyle.ts)
  // without an upload or an install exec, and the source digest
  // (devboxSourceManifest, schema 3) covers their generated bytes so a change
  // to any of them is a re-promotion. The shim is the driver's own fs upload;
  // the CLI archive is downloaded and sha256-checked by the same python
  // installer the driver runs; the reporter is enabled and running in the
  // snapshot (its posts fail harmlessly on the builder, which has no edge
  // rule, and reach the edge on every machine created from it).
  await vm.fs.writeTextFile(GUEST_CMUX_SHIM_PATH, GUEST_CMUX_SHIM, { mode: 0o755 });
  await step("guest-cli-shim", `${guestCliShimReadyCommand()} && test -x ${GUEST_CMUX_SHIM_PATH} && sh -n ${GUEST_CMUX_SHIM_PATH} && echo guest-cli-shim-ok`);
  await step("guest-cli-distribution", guestCliDistributionCommand());
  await step("guest-cli-distribution-verify", `${guestCliDistributionCommand(true)} && test -x /usr/local/bin/cmux && test -x /usr/local/bin/coderouter && test -x /usr/local/bin/cr && test -x /usr/local/libexec/cmux-coderouter && echo guest-cli-distribution-ok`);
  await step("guest-browser-openers", guestBrowserInstallCommand());
  // The web MIME handler for both accounts where xdg-utils exists (the
  // desktop layer); a base bake has no xdg-mime and the openers still work
  // through BROWSER.
  await step("guest-browser-openers-verify", `${guestBrowserReadyCommand} && if command -v xdg-mime >/dev/null 2>&1; then [ "$(xdg-mime query default x-scheme-handler/https)" = cmux-browser.desktop ] && [ "$(runuser -u ${WORK_USER} -- xdg-mime query default x-scheme-handler/https)" = cmux-browser.desktop ]; fi && echo guest-browser-openers-ok`);
  await step("guest-resource-reporter", guestResourceReporterInstallCommand());
  await step("guest-resource-reporter-verify", `${guestResourceReporterReadyCommand()} && echo guest-resource-reporter-ok`);
