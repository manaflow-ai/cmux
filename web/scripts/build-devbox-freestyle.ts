...
try {
  ...
  await step("opencode-launcher", 'mkdir -p /usr/local/libexec && ln -s "$(readlink -f /usr/local/bin/opencode)" /usr/local/libexec/cmux-opencode-real && rm -f /usr/local/bin/opencode && install -m 755 /etc/cmux/opencode /usr/local/bin/opencode');
  await step(
    "agent-config",
