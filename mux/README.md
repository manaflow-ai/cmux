...
Press prefix-`B` or right-click a pane and choose `New browser tab` to create an `about:blank` browser tab and focus its omnibar. The omnibar passes explicit `://` URLs through, prefixes localhost/loopback addresses with `http://`, prefixes dotted domains with `https://`, and turns other text into a Google search URL. Browser panes share one local Chrome DevTools Protocol connection per mux session. Explicit external attach remains available: cmux first uses `CMUX_MUX_CDP_URL`, then `browser.cdp_url`, and only probes `127.0.0.1` discovery ports such as 9222 when `browser.discover` is set to `true`.

### Browser realism

By default, mux launches your real Google Chrome or another Chrome-family binary in `browser.mode: "headful"` with a visible window. The launched browser uses a persistent profile at `~/Library/Application Support/cmux-mux/chrome-profile/<session>` on macOS, or `$XDG_DATA_HOME/cmux-mux/chrome-profile/<session>` on Linux. Log into Google or other sites once in that visible window; cookies and logins persist across sessions. Launched Chrome also uses `--disable-blink-features=AutomationControlled`, and headless launched runtimes replay a user agent with `HeadlessChrome` rewritten to `Chrome`.

Set `browser.mode: "headless"` to hide the launched Chrome window. Headless mode uses the same persistent profile and stealth settings. Set `browser.ephemeral` to use a temporary profile deleted on shutdown.

Using your actual everyday Chrome profile is possible but caveated. Chrome 136 and newer block `--remote-debugging-port` and `--remote-debugging-pipe` when `--user-data-dir` is unset or points at the default Chrome profile directory, and a running Chrome keeps a SingletonLock on its profile so a second Chrome process defers to it and never exposes mux's debug port. Practical options are: close Chrome, copy your profile directory, and set `browser.user_data_dir` to the copy; fully quit normal Chrome and set `browser.user_data_dir` to the real profile directory while using mux; or use the persistent mux profile and log in once there.

To attach mux to a running Chrome or agent-browser instead of launching one, start Chrome yourself with `--remote-debugging-port=9222 --user-data-dir=<some-dir>`, or run `agent-browser` and then `agent-browser get cdp-url`. Set `browser.cdp_url` or `CMUX_MUX_CDP_URL` to that `ws://` or `http://host:port` endpoint, or enable `browser.discover` to probe `127.0.0.1:9222`. agent-browser exposes a standard CDP endpoint, so mux drives it like any Chrome. This mux build uses a plain CDP socket and does not support TLS `wss://` endpoints without adding TLS support.

External headful Chrome has hard limitations: hidden or occluded windows can screencast black frames and then throttle to zero frames, and viewport overrides may not hold. The omnibar's `⏸ chrome tab hidden` indicator marks a stalled external frame stream.
...
Attach clients stream browser panes over protocol 6. `attach-surface` sends the current browser state and latest PNG frame if one exists, then streams subsequent frames; mouse, hover, wheel, keyboard, text insertion, URL navigation, back, forward, reload, and activate are forwarded over the control socket. Browser command responses mean the command was accepted into the per-surface browser queue, not that the Chrome DevTools operation completed. CDP errors and timeouts are reported later through `browser-state` / `status` events; two consecutive CDP call timeouts mark only that browser surface failed with `browser is not responding`. Older protocol 4/5 servers still show the clear `browser panes are not supported over attach yet` placeholder. Browser device metrics use the rendering client's detected terminal cell pixel size, scaled by the capture budget; with multiple concurrent attach clients, the last `set-cell-pixels` writer wins. `list-workspaces` reports browser tabs with `kind: "browser"`, `browser_source: "external"` or `"launched"` once live, and additive `browser_status` / `browser_error` / `browser_frames_stalled` fields.
...
- Browser input is sent through per-surface workers to CDP `Input.*` commands; CDP screencast frames are acknowledged immediately so Chrome keeps streaming.
