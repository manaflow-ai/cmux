...
Browser panes are local Chrome/Chromium targets controlled with the Chrome DevTools Protocol. They live in the same pane/tab tree as PTY tabs, but their rendering and input path are CDP-based instead of VT-based.
...
Browser panes are local-only as of protocol v5. `attach-surface` returns an error for browser surfaces, attach clients do not receive browser frame streams, and a remote TUI shows a placeholder for browser panes.
