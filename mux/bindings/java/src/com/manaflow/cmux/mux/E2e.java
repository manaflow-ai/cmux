...
public final class E2e {
    public static void main(String[] args) throws Exception {
        String socket = System.getenv("CMUX_MUX_SOCKET");
        if (socket == null || socket.isBlank()) {
            throw new IllegalStateException("CMUX_MUX_SOCKET is required");
        }
        String marker = "CMUX_JAVA_E2E_" + ProcessHandle.current().pid() + "_" + System.nanoTime();
        String later = marker + "_ATTACH";
        try (MuxClient client = MuxClient.builder().socketPath(socket).build()) {
            IdentifyResult identify = client.identify();
            check("cmux-mux".equals(identify.app()), "unexpected app " + identify.app());
            check(identify.protocol() >= 5 && identify.protocol() <= 6, "unsupported protocol " + identify.protocol());
            ...
            try (MuxClient.MuxStream events = client.subscribe()) {
                ...
                try {
                    ...
                    gotDuplicate = true;
                } catch (MuxTimeoutException expectedTimeout) {
                    // no event is the expected result
                }
                check(!gotDuplicate, "same-size resize emitted surface-resized");
            }
            try (MuxClient.MuxStream attach = client.attachSurface(created.surface())) {
                MuxEvent first = attach.next(Duration.ofSeconds(1));
            ...
            } catch (MuxCommandException err) {
                check(!err.serverMessage().isBlank(), "server error string was not preserved");
            }
        }
    }

    private static void waitForMarker(MuxClient client, long surface, String marker) throws Exception {
    ...
    private static SurfaceResizedEvent nextResized(MuxClient.MuxStream stream, long surface, Duration timeout) throws Exception {
        long deadline = System.nanoTime() + timeout.toNanos();
        while (System.nanoTime() < deadline) {
            MuxEvent event = stream.next(Duration.ofNanos(deadline - System.nanoTime()));
            if (event instanceof SurfaceResizedEvent resized && resized.surface() == surface) {
                return resized;
            }
        }
        throw new MuxTimeoutException("surface-resized not observed");
    }

    private static void nextAttachOutput(MuxClient.MuxStream stream, Duration timeout) throws Exception {
        long deadline = System.nanoTime() + timeout.toNanos();
        while (System.nanoTime() < deadline) {
            MuxEvent event = stream.next(Duration.ofNanos(deadline - System.nanoTime()));
        ...
        throw new MuxTimeoutException("attach output not observed");
    }
