...
check_no_bare_github_hosted_runners() {
  ...
  local hits
  # cla-policy-guard.yml is CLA control plane and web-complexity-trusted.yml
  # is the trusted policy workflow: both must run on GitHub-hosted ephemeral
  # runners that no repo variable can redirect. Exempt those files here; the
  # latter is separately protected from pull-request edits by its own gate.
