...
check_persistent_compile_router() {
  ...
  if ! grep -Fq -- '--observe-only' "$CI_FILE"; then
    echo "FAIL: PR CI must remain read-only observation"
    exit 1
  fi
  if grep -Fq 'actions: write' "$CI_FILE"; then
    echo "FAIL: PR-editable CI must not receive Actions write authority"
    exit 1
  fi
