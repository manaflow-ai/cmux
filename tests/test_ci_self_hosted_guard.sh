...
actions: read\ncontents: read\npull-requests: read' ]; then
    echo "FAIL: PR route job permissions must be exactly Actions/content/pull-request read"
    printf 'permissions=%s\n' "$pr_route_permissions"
    exit 1
  fi
  if printf '%s\n' "$pr_route_block" | grep -Eq 'permissions:[[:space:]]*write-all|actions:[[:space:]]*write'; then
    echo "FAIL: PR-editable persistent route job must have no write-all or Actions write authority"
    exit 1
  fi
  ...
  observer_step="$(printf '%s\n' "$pr_route_block" | awk '
    ...
    in_step { print }
  ')"
  if [ -z "$observer_step" ] ||      [ "$(printf '%s\n' "$observer_step" | grep -Fxc '            --observe-only \')" -ne 1 ]; then
    echo "FAIL: PR route observer must invoke persistent_mac_route.py in exact --observe-only mode"
    exit 1
  fi
  if printf '%s\n' "$observer_step" | grep -Eq 'gh[[:space:]]+workflow[[:space:]]+run|actions/runs/.*/cancel'; then
    echo "FAIL: PR route observer must not dispatch or cancel producer work"
    exit 1
  fi
