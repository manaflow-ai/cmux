...
def _base_env(tmp: Path, fakebin: Path) -> dict[str, str]:
    ...
    env["IOS_APPSTORE_EXTENSION_PROVISIONING_PROFILE_NAME"] = "cmux App Store NotificationService"
...
def test_upload_appstore_lane_uses_production_bundle_id(tmp: Path, fakebin: Path) -> None:
    ...
    _check(
        all("CMUX_APP_BUNDLE_IDENTIFIER=com.cmuxterm.app" not in call for call in archive_call),
