...
if [ "${CMUX_MOCK_XCODEBUILD_PROCESS:-0}" = "1" ]; then
  ...
  case "${CMUX_MOCK_XCODEBUILD_MODE:-timeout}" in
    ...
    published-home-sibling-leak) config_home="${CMUX_APP_HOST_HOME}-other" ;;
    xdg-config-leak)
...
if [ "$published_home_alias_status" -ne 0 ]; then
  ...
  echo "FAIL: wrapper must accept the validated published spelling of the app-host home"
  exit 1
fi
...
for regression in sibling-leak published-home-sibling-leak missing-log; do
  case "$regression" in
    sibling-leak|published-home-sibling-leak)
