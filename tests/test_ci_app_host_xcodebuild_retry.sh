...
if [ "${CMUX_MOCK_XCODEBUILD_PROCESS:-0}" = "1" ]; then
  ...
  case "${CMUX_MOCK_XCODEBUILD_MODE:-timeout}" in
    ...
    path-alias) config_home="$CMUX_MOCK_APP_HOST_HOME_ALIAS" ;;
    xdg-config-leak)
  ...
  if [ "${CMUX_MOCK_XCODEBUILD_MODE:-timeout}" = "success" ] \
    || [ "${CMUX_MOCK_XCODEBUILD_MODE:-timeout}" = "path-alias" ] \
...
APP_HOST_LOG_ALIAS_HOME="$APP_HOST_HOME"
if [ "$APP_HOST_LOG_ALIAS_HOME" = "$RESOLVED_APP_HOST_HOME" ]; then
  APP_HOST_LOG_ALIAS_HOME="$TMP_DIR/app-host-log-alias"
  ln -s "$RESOLVED_APP_HOST_HOME" "$APP_HOST_LOG_ALIAS_HOME"
fi
...
# macOS reports the same temporary directory as both /tmp and /private/tmp.
# Exercise that real-path equivalence directly there, and an equivalent symlink
# alias on Linux where the workflow guard also runs.
set +e
PATH="$TMP_DIR:$PATH" \
RUNNER_TEMP="$RUNNER_TEMP_DIR" \
...
CMUX_MOCK_XCODEBUILD_PROCESS=1 \
CMUX_MOCK_XCODEBUILD_MODE=path-alias \
CMUX_MOCK_APP_HOST_HOME_ALIAS="$APP_HOST_LOG_ALIAS_HOME" \
CMUX_APP_HOST_XCODEBUILD_ATTEMPTS=1 \
CMUX_XCODEBUILD_NONINTERACTIVE_IDLE_TIMEOUT_SECONDS=5 \
CMUX_CI_APP_HOST_ISOLATION_REQUIRED=1 \
CMUX_APP_HOST_HOME="$APP_HOST_HOME" \
...
if [ "$path_alias_status" -ne 0 ]; then
  ...
  echo "FAIL: wrapper must accept an equivalent real path for app-host configuration"
  exit 1
fi
