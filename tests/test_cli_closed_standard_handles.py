...
def main() -> int:
    ...
    result = run_with_closed_stderr(cli_path)
    if result.returncode == -6:
        print("FAIL: cmux aborted with SIGABRT when stderr was closed", file=sys.stderr)
        return 1
    if result.returncode < 0:
        print(f"FAIL: cmux terminated by signal {-result.returncode}", file=sys.stderr)
        return 1
    if result.returncode != 1:
        print(f"FAIL: expected CLIError exit code 1, got {result.returncode}", file=sys.stderr)
        return 1

    print("PASS: closed stderr exits cleanly without Objective-C FileHandle abort")
    return 0
