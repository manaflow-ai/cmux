...
in `CLI/CMUXCLI+CommandSuggestions.swift` and the `usage()` help text in
`CLI/cmux.swift`. This test regenerates them from those sources and fails if the
committed files drift -- so adding a CLI command without regenerating turns CI
red instead of silently shipping stale completions. It also guards that every
user-visible command is actually completed, and that each script is shell-syntax
valid.
...
def main() -> int:
    ...
    for shell, filename in SHELLS.items():
        ...
        if committed != fresh:
            failures.append(
                f"{filename} is out of date with CLI command sources. {REGEN_HINT}"
            )
