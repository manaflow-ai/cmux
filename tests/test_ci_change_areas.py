...
def test_macos_compile_admission_precedes_expensive_shards() -> None:
    ...
    restore_script = (ROOT / "scripts/ci/restore-app-host-test-product.sh").read_text()
    assert "app-host-products-archive.sh\" unpack" in restore_script
    assert "app-host-products.tar.gz" not in restore_script
