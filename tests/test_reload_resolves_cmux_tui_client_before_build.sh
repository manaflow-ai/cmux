...
# run before GhosttyKit and xcodebuild, fail fast with the overrides named, run once,
# and stay out of the way when an override supplies the client.
set -euo pipefail
...
exit 3
EOF
chmod +x "$CHECKOUT/scripts/ci/resolve-cmux-tui-client-commit.sh" \
...
echo "PASS: the cmux-tui client commit is resolved once, before the build"
