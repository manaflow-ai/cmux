...
if ! awk '
  ...
  END { exit !(verify_line && thin_line && verify_line < thin_line && saw_script && saw_allowlist) }
' "$WORKFLOW_FILE"; then
  echo "FAIL: nightly must sweep the universal bundle before intentional per-architecture thinning"
  exit 1
fi
