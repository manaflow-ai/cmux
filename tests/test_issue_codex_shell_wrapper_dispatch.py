#!/usr/bin/env python3
"""
Regression: shell integrations should dispatch `codex` through the bundled
wrapper when cmux provides one, even if PATH later prefers another binary.
"""
...
def run_zsh(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        ["zsh", "-d", "-i", "-c", 'PATH="$CMUX_TEST_REAL_BIN:$PATH"; codex zsh-case'],
        env=env,
        capture_output=True,
        text=True,
        timeout=8,
...
def run_zsh_with_alias(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            "zsh",
            "-fic",
            f'alias codex="echo alias"; source "{shell_dir / "cmux-zsh-integration.zsh"}"; '
            'PATH="$CMUX_TEST_REAL_BIN:$PATH"; codex zsh-alias-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-c",
            f'source "{shell_dir / "cmux-bash-integration.bash"}"; PATH="$CMUX_TEST_REAL_BIN:$PATH"; codex bash-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_alias(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-ic",
            f'alias codex="$CMUX_TEST_REAL_BIN/user-codex"; source "{shell_dir / "cmux-bash-integration.bash"}"; '
            'PATH="$CMUX_TEST_REAL_BIN:$PATH"; codex bash-alias-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_function(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-ic",
            f'codex() {{ "$CMUX_TEST_REAL_BIN/user-codex-function" "$@"; }}; '
            f'source "{shell_dir / "cmux-bash-integration.bash"}"; '
            'PATH="$CMUX_TEST_REAL_BIN:$PATH"; codex bash-function-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cmux-codex-shell-wrapper-") as td:
        tmp = Path(td)
...
""",
    ...
    if failures:
        print("FAIL: shell integration did not keep codex on the bundled wrapper")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("PASS: zsh and bash integrations dispatch codex through the bundled wrapper")
    return 0
