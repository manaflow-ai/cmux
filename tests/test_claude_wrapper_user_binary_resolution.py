...
def test_shell_integration_preserves_empty_path_components(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="cmux-shell-path-components-") as td:
        ...
        base_env["CMUX_TEST_PATH_WITH_EMPTY_COMPONENTS"] = f":{first}::{shim_root}:{last}:"
        base_env.pop("CMUX_SOCKET_PATH", None)
        ...
        shell_commands = [
            [
                ...
                "--norc",
                "-c",
                'PATH="$CMUX_TEST_PATH_WITH_EMPTY_COMPONENTS"; source "$CMUX_SHELL_INTEGRATION_DIR/cmux-bash-integration.bash"; printf "%s\\n" "$PATH"',
            ],
            [
                "/bin/zsh",
                "-f",
                "-c",
                'PATH="$CMUX_TEST_PATH_WITH_EMPTY_COMPONENTS"; source "$CMUX_SHELL_INTEGRATION_DIR/cmux-zsh-integration.zsh"; printf "%s\\n" "$PATH"',
            ],
        ]
        for argv in shell_commands:
            result = run_wrapper(argv, base_env)
