...
def test_shell_integration_preserves_empty_path_components(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="cmux-shell-path-components-") as td:
        ...
        test_path = f":{first}::{shim_root}:{last}:"
        base_env["PATH"] = "/usr/bin:/bin"
        base_env["CMUX_TEST_INITIAL_PATH"] = test_path
        base_env.pop("CMUX_SOCKET_PATH", None)
        ...
        shell_commands = [
            [
                ...
                "--norc",
                "-c",
                'PATH="$CMUX_TEST_INITIAL_PATH"; export PATH; source "$CMUX_SHELL_INTEGRATION_DIR/cmux-bash-integration.bash"; printf "%s\\n" "$PATH"',
            ],
            [
                "/bin/zsh",
                "-f",
                "-c",
                'PATH="$CMUX_TEST_INITIAL_PATH"; export PATH; source "$CMUX_SHELL_INTEGRATION_DIR/cmux-zsh-integration.zsh"; printf "%s\\n" "$PATH"',
            ],
        ]
        for argv in shell_commands:
            result = run_wrapper(argv, base_env)
