    ...
    with tempfile.TemporaryDirectory(prefix="cmux-shell-path-components-") as td:
        ...
        shell_commands = [
            [
                ...
                "--norc",
                "-c",
                'PATH="$CMUX_TEST_INPUT_PATH"; source "$CMUX_SHELL_INTEGRATION_DIR/cmux-bash-integration.bash"; printf "%s\\n" "$PATH"',
            ],
            [
                "/bin/zsh",
                "-f",
                "-c",
                'PATH="$CMUX_TEST_INPUT_PATH"; source "$CMUX_SHELL_INTEGRATION_DIR/cmux-zsh-integration.zsh"; printf "%s\\n" "$PATH"',
            ],
        ]
        for argv in shell_commands:
            result = run_wrapper(argv, base_env)
