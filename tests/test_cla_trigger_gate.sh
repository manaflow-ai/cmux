...
workflow_types='types: [opened,closed,edited,reopened,synchronize]'
...
extract_run() {
  ...
  awk -v job="$job" -v marker="$marker" '
    ...
    in_job && index($0, marker) { in_run=0; next }
    in_run && /^      - name:/ { exit }
...
for action in opened edited reopened synchronize; do
...
run_result() {
  local name="$1" expected_status="$2" event_name="$3" comment_body="$4" gate_result="$5" admitted="$6" signer="$7" writer="$8"
  local output status
  set +e
  output="$(
...
run_result lifecycle-success 0 pull_request_target '' success true false success
run_result recheck-success 0 issue_comment recheck success true false success
run_result signing-success 0 issue_comment 'I have read the CLA Document v2.2 and I hereby sign the CLA' success true true success
run_result unauthorized-sign 1 issue_comment 'I have read the CLA Document v2.2 and I hereby sign the CLA' success true false success
run_result writer-failure 1 issue_comment recheck success true false failure
run_result gate-failure 1 issue_comment recheck failure false false skipped
