...
const logPath = process.env.CMUX_TEST_PI_CANCELLATION_LOG;
// The queued prompt hook creates the log, so tolerate a missing file while it starts.
const logText = async () => (await Bun.file(logPath).exists()) ? await Bun.file(logPath).text() : "";
while (!(await logText()).includes("hooks feed")) {
  await new Promise((resolve) => setTimeout(resolve, 10));
}
...
const probeQueues = new Map([["probe-session", { closed: true, tail: Promise.resolve(true) }]]);
mod.surfaceTargetsFor(probeDispatcher).set("probe-session", ["--surface", "old"]);
mod.releaseSessionRuntime(probeDispatcher, probeStates, probeQueues, "probe-session");
if (probeStates.has("probe-session")) throw new Error("session state survived runtime release");
if (probeQueues.has("probe-session")) throw new Error("prompt hook queue survived runtime release");
...
"""
    ...
    # Prompt hooks from separate runtimes run concurrently, so match by session instead of order.
    stale_calls = [line for line in runtime_calls if "pi-runtime-stale" in line]
    healthy_calls = [line for line in runtime_calls if "pi-runtime-healthy" in line]
    if len(runtime_calls) != 2 or len(stale_calls) != 1 or len(healthy_calls) != 1:
...
"""
    ...
    # Each session queues its own prompt hooks, so match by session instead of order.
    stale_calls = [line for line in calls if "pi-session-stale" in line]
    healthy_calls = [line for line in calls if "pi-session-healthy" in line]
    if len(calls) != 2 or len(stale_calls) != 1 or len(healthy_calls) != 1:
