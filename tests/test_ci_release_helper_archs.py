...
set -eu
[[ "$1" == --sdk && "$2" == macosx && "$3" == --show-sdk-build-version ]] || exit 2
printf '15F31d\\n'
''')
...
while (( $# )); do
  case "$1" in
    --manifest-url|--expected-commit) shift; [[ -n "${1:-}" ]] || exit 2 ;;
    --arch) shift; arch="${1:-}" ;;
    *) exit 2 ;;
  esac
  shift
done
case "$arch" in arm64) archs=arm64 ;; universal) archs="arm64 x86_64" ;; *) exit 2 ;; esac
