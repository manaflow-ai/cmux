#!/usr/bin/env python3
"""
Regression tests for OMP HUD panes through cmux's tmux compatibility shim.
"""
    ...
    def handle(self, method: str, params: dict[str, object]) -> dict[str, object]:
        ...
        if method == "surface.read_text":
            if params.get("surface_id") == HUD_SURFACE_ID:
                return {"text": "[OMP#0.15.3] turns:1 | session:23s | last:12s ago\n"}
            return {"text": ""}
...
def run_cli(
    ...
    env["CMUX_OMP_CMUX_BIN"] = cli_path
...
def omp_hud_split_args(cwd: Path) -> list[str]:
    return [
        "__tmux-compat",
        "split-window",
        "-v",
...
def assert_omp_hud_splits_down_with_compact_size(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
    cwd: Path,
    ...
    proc = run_cli(cli_path, socket_path, fake_home, omp_hud_split_args(cwd))
...
def assert_omp_hud_is_visible_to_tmux_pane_formats(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
) -> None:
...
def assert_omp_hud_start_command_visible_to_legacy_tmux_pane_format(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
) -> None:
...
def assert_omp_hud_absolute_width_resize_still_applies(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
    state: FakeCmuxState,
...
def assert_omp_hud_absolute_height_resize_does_not_override_user_layout(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
    state: FakeCmuxState,
...
def assert_omp_hud_feature_probe_is_supported(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
) -> None:
...
def assert_omp_hud_unsupported_feature_probe_fails(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
) -> None:
...
def assert_disabled_omp_hud_does_not_split(
    cli_path: str,
    socket_path: Path,
    fake_home: Path,
    cwd: Path,
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="cmux-omp-hud-split-") as td:
            tmp = Path(td)
            ...
            thread.start()
            try:
                assert_omp_hud_feature_probe_is_supported(cli_path, socket_path, fake_home)
                assert_omp_hud_unsupported_feature_probe_fails(cli_path, socket_path, fake_home)
                assert_omp_hud_splits_down_with_compact_size(
                    cli_path,
                    socket_path,
                    fake_home,
                    cwd,
                ...
                assert_omp_hud_is_visible_to_tmux_pane_formats(
                    cli_path,
                    socket_path,
                    fake_home,
                )
                ...
                assert_omp_hud_start_command_visible_to_legacy_tmux_pane_format(
                    cli_path,
                    socket_path,
                    fake_home,
                )
                assert_omp_hud_absolute_width_resize_still_applies(
                    cli_path,
                    socket_path,
                    fake_home,
                    state,
                )
                assert_omp_hud_absolute_height_resize_does_not_override_user_layout(
                    cli_path,
                    socket_path,
                    fake_home,
                    state,
        ...
        with tempfile.TemporaryDirectory(prefix="cmux-omp-hud-disabled-") as td:
            tmp = Path(td)
            socket_path = tmp / "fake-cmux.sock"
            fake_home = tmp / "home"
            fake_home.mkdir(parents=True, exist_ok=True)
    ...
    print("PASS: OMP HUD tmux splits attach to the bottom and respect disabled config")
    return 0
