...
if ! awk '
  ...
  END {
    exit !(restore_order > 0 && reject_order > restore_order && build_order > reject_order)
  }
' "$RELEASE_FILE"; then
  echo "FAIL: stable releases must restore and size-check the compiler cache before building" >&2
  exit 1
fi
