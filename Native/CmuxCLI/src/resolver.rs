...
pub fn resolve(ctx: &Context, kind: &str, value: Option<&str>) -> Result<Option<String>> {
    ...
    if !matches!(kind.as_str(), "window" | "workspace" | "pane" | "surface") {
        return Err(CliError::new("invalid_kind", format!("Unknown ID kind: {kind}")));
    }
    resolve_kind(ctx, &kind, value.map(str::trim).filter(|v| !v.is_empty()), value.is_some())
}

fn resolve_kind(ctx: &Context, kind: &str, raw: Option<&str>, explicitly_supplied: bool) -> Result<Option<String>> {
    if let Some(raw) = raw {
        if looks_like_uuid(raw) { return Ok(Some(raw.to_string())); }
        if let Some((prefix, index)) = parse_ref(raw) {
            if prefix != kind { return Err(CliError::new("invalid_handle", format!("Invalid {kind} handle: {raw}"))); }
            return lookup(ctx, kind, None, Some(index), Some(raw));
        }
        if let Ok(index) = raw.parse::<i64>() {
            return lookup(ctx, kind, None, Some(index), None);
        }
        return Err(CliError::new("not_found", format!("{kind} not found: {raw}")));
    }
    if explicitly_supplied {
        return Err(CliError::new("invalid_handle", format!("{kind} handle is blank")));
    }
    match kind {
        "window" => {
            let current = ctx.rpc("window.current", json!({}))?;
            Ok(current.get("window_id").or_else(|| current.get("id")).and_then(Value::as_str).map(str::to_owned))
        }
        "workspace" => {
            if ctx.window.is_none() {
                if let Ok(caller) = env::var("CMUX_WORKSPACE_ID") {
                    if looks_like_uuid(caller.trim()) { return Ok(Some(caller.trim().to_string())); }
                }
            }
            let mut params = json!({});
        ...
        "pane" | "surface" => {
            let workspace = resolve_kind(ctx, "workspace", None, false)?.ok_or_else(|| CliError::new("not_found", "No workspace selected"))?;
...
fn lookup(ctx: &Context, kind: &str, workspace: Option<&str>, index: Option<i64>, wanted_ref: Option<&str>) -> Result<Option<String>> {
    let (method, list_key) = match kind { "window" => ("window.list", "windows"), "workspace" => ("workspace.list", "workspaces"), "pane" => ("pane.list", "panes"), "surface" => ("surface.list", "surfaces"), _ => unreachable!() };
    let mut params = json!({});
    ...
    if let Some(wanted_ref) = wanted_ref {
        if let Some(id) = items.iter().find(|item| item.get("ref").and_then(Value::as_str) == Some(wanted_ref)).and_then(item_id) { return Ok(Some(id)); }
        return Err(CliError::new("not_found", format!("{kind} ref not found: {wanted_ref}")));
    }
    if let Some(index) = index {
        if let Some(id) = items.iter().find(|item| item_index(item) == Some(index)).and_then(item_id) { return Ok(Some(id)); }
        return Err(CliError::new("not_found", format!("{kind} index not found")));
    }
    let selected = items.iter().find(|item| item.get("focused").and_then(Value::as_bool) == Some(true)).and_then(item_id).or_else(|| items.first().and_then(item_id));
    selected.map(Some).ok_or_else(|| CliError::new("not_found", format!("No {kind} selected")))
}

fn item_id(item: &Value) -> Option<String> { item.get("id").or_else(|| item.get("uuid")).and_then(Value::as_str).map(str::to_owned) }
