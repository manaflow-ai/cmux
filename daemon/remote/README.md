...
6. Closing the owning workspace sends an authenticated slot-shutdown request, waits for the daemon lock to be released, and removes the relay's shell-state directory. As defense in depth, a disconnected daemon also exits after a previously observed `~/.cmux/relay/<port>.slot` lease disappears.
