...
func TestTmuxListWindowsUsesOneWorkspaceSnapshot(t *testing.T) {
    ...
    if err := tmuxListWindows(&rpcContext{socketPath: recorder.socketPath}, []string{"-F", "#{window_name}"}); err != nil { t.Fatal(err) }
    ...
    if err != nil || info.Size() == 0 { t.Fatalf("missing window output: %v", err) }
}
